...
(* lea *)
  ...
  TranslOp. rewrite normalize_addrmode_32_correct; auto.
(* mullhs *)
  apply SAME. TranslOp. destruct H1. Simplifs.
(* mullhu *)
  apply SAME. TranslOp. destruct H1. Simplifs.
...
Remark not_destroyed_not_in_map:
  forall r l,
    (forall ty, preg_notin r (destroyed_by_setstack ty)) ->
    preg_notin r (flat_map destroyed_by_setstack mreg_type ## l).
Proof.
  induction l as [| r0 l]; simpl; intros.
  - exact I.
  - rewrite preg_notin_charact. intros.
    rewrite in_app in H0. destruct H0.
    specialize H with (mreg_type r0). rewrite preg_notin_charact in H.
    apply H. assumption.
    exploit IHl. apply H. intros. rewrite preg_notin_charact in H1.
    apply H1. assumption.
Qed.

Remark SP_not_destroyed:
  forall ty, preg_notin SP (destroyed_by_setstack ty).
Proof.
  Local Transparent destroyed_by_setstack.
  intros. unfold destroyed_by_setstack. destruct ty; simpl; auto; discriminate.
Qed.

Lemma preg_notin_app:
  forall r l2 l1,
    preg_notin r (l1 ++ l2) ->
    preg_notin r l1 /\ preg_notin r l2.
Proof.
  induction l1 as [|r0 l1]; intros.
  - easy.
  - simpl in H. destruct (l1 ++ l2).
    simpl. destruct l1. split. apply H. apply IHl1. constructor.
    split. split. assumption. apply IHl1. constructor.
    apply IHl1. constructor.
    simpl. destruct l1. split. apply H. apply IHl1. apply H.
    split. split. apply H. apply IHl1. apply H.
    apply IHl1. apply H.
Qed.

Local Hint Resolve SP_not_destroyed not_destroyed_not_in_map: asmgen.
...
Lemma transl_savecallee_correct:
  forall l ofs k c (rs: regset) m1 m2,
    ...
    (forall r : mreg, In r l -> preg_notin (preg_of r) (flat_map destroyed_by_setstack mreg_type ## l)) ->
    l <> nil ->
    exists rs',
      exec_straight ge fn c rs m1 k rs' m2
      /\ forall r, data_preg r = true -> preg_notin r (flat_map destroyed_by_setstack mreg_type ## l) -> rs'#r = rs#r.
Proof. 
  induction l as [ | r l]; simpl; intros until m2; intros TR STOR NIN NNIL.
  - contradiction.
  - set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    ...
    + simpl in EQ. inversion EQ. simpl in STOR. inversion STOR. exists rs1.
      rewrite <- H1. rewrite app_nil_r. split; auto with asmgen.
    + exploit IHl.
      rewrite <- B in STOR; eauto with asmgen.
      eapply store_stack_pregs_swap. rewrite <- B in STOR; auto with asmgen. apply STOR.
      intros. apply B. apply preg_of_data. eapply proj1. eapply preg_notin_app. apply NIN. right. assumption.
      intros. eapply proj2. eapply preg_notin_app. apply NIN. right. assumption.
      discriminate.
      intros [rs' [C D]].
      exists rs'. split.
      eapply exec_straight_trans; eauto. intros. rewrite <- B; eauto with asmgen. apply D; auto.
      eapply proj2. eapply preg_notin_app. apply H0.
      eapply proj1. eapply preg_notin_app. apply H0.
Qed.
