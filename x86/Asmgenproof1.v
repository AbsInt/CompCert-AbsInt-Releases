...
Lemma nextinstr_nf'_inv:
  forall r rs,
  match r with PC => False | CR CF => True | CR _ => False  | _ => True end ->
  (nextinstr_nf' rs)#r = rs#r.
Proof.
  intros. unfold nextinstr_nf'. rewrite nextinstr_inv.
  simpl. repeat rewrite Pregmap.gso; auto;
...
Lemma nextinstr_nf'_inv1:
  forall r rs,
  data_preg r = true -> (nextinstr_nf' rs)#r = rs#r.
Proof.
  intros. apply nextinstr_nf'_inv. destruct r; auto || discriminate.
Qed.


Lemma nextinstr_nf'_set_preg:
  forall rs m v,
  (nextinstr_nf' (rs#(preg_of m) <- v))#PC = Val.offset_ptr rs#PC Ptrofs.one.
Proof.
  intros. unfold nextinstr_nf'.
...
Ltac Simplif :=
  ...
  | [ |- _ = nextinstr_nf _ _] =>
      symmetry; ((rewrite nextinstr_nf_inv by auto with asmgen)
       || (rewrite nextinstr_nf_inv1 by auto with asmgen)); auto
  | [ |- nextinstr_nf' _ _ = _ ] =>
      ((rewrite nextinstr_nf'_inv by auto with asmgen)
       || (rewrite nextinstr_nf'_inv1 by auto with asmgen)); auto
  | [ |- _ = nextinstr_nf' _ _] =>
      symmetry; ((rewrite nextinstr_nf'_inv by auto with asmgen)
       || (rewrite nextinstr_nf'_inv1 by auto with asmgen)); auto
  | [ |- nextinstr _ _ = _ ] =>
      ((rewrite nextinstr_inv by auto with asmgen)
       || (rewrite nextinstr_inv1 by auto with asmgen)); auto
  | [ |- _ = nextinstr _ _ ] =>
      symmetry; ((rewrite nextinstr_inv by auto with asmgen)
       || (rewrite nextinstr_inv1 by auto with asmgen)); auto
...
Ltac Simplif' :=
  ...
  || (rewrite nextinstr_nf'_inv by eauto with asmgen)
  || (rewrite Pregmap.gss)
...
Ltac Splitter' :=
  ...
  | _ => simpl in *; try Simplifs; auto with asmgen
  end.
...
Lemma transl_op_correct:
  ...
  eval_operation ge (rs#RSP) op (get_pairs preg_rpair_of ## args rs) m = Some v ->
  exists rs',
     exec_straight ge fn c rs m k rs' m
...
- (* move *)
  destruct res; inv EQ. destruct r; inv EQ1.
  exploit mk_mov_correct; eauto. intros [rs2 [A [B C]]].
  econstructor; split; eauto. Splitter.
- (* intconst *)
  destruct (Int.eq_dec n Int.zero); [subst n|]; TranslOpSplitter.
- (* longconst *)
  destruct (Int64.eq_dec n Int64.zero); [subst n|]; TranslOpSplitter.
-(* floatconst *)
  destruct (Float.eq_dec n Float.zero); [subst n|]; TranslOpSplitter.
- (* singleconst *)
  destruct (Float32.eq_dec n Float32.zero); [subst n|]; TranslOpSplitter.
(* cast8signed *)
- exploit mk_intconv_correct; eauto.
  intros. simpl. reflexivity. intros [rs2 [A [B C]]].
  exists rs2; split; eauto. Splitter.
- (* cast8unsigned *)
  exploit mk_intconv_correct; eauto.
  intros. simpl. reflexivity. intros [rs2 [A [B C]]].
  exists rs2; split; eauto. Splitter.
- (* mull' *)
  ...
  repeat Splitter'. Simpl.
  destruct (rs RAX), (rs x0); auto. simpl.
  rewrite Int64.mul'_mulhu, Int64.hi_ofwords; auto.
  destruct (rs RAX), (rs x0); auto. simpl.
  rewrite Int64.mul'_mulhu, Int64.lo_ofwords; auto.
  intuition Simplifs.
- (* div *)
  ...
  change (rs1 RCX) with (rs RCX); rewrite C; auto.
  rewrite D; auto. reflexivity. auto. Splitter. congruence.
  simpl; intros. destruct H5. unfold rs1; Simplifs.
- (* divu *)
  ...
  rewrite D. reflexivity. auto. auto. Splitter. congruence.
  simpl; intros. destruct H5. unfold rs1; Simplifs.
- (* mod *)
...
- (* modu *)
  ...
  simpl; intros. destruct H5. unfold rs1; Simplifs.
- (* shrximm *)
  exploit mk_shrximm_correct; eauto. intros [rs1 [A [B C]]].
  econstructor; split; eauto. Splitter.
- (* lea *)
  exploit transl_addressing_mode_32_correct; eauto.
  erewrite <- get_pairs_singles; eauto. intros EA.
  econstructor; split; eauto. apply exec_straight_one; simpl; eauto.
  split. rewrite nextinstr_inv; auto with asmgen. rewrite Pregmap.gss. rewrite normalize_addrmode_32_correct; auto.
  intuition Simplifs.
- (* negl *)
  destruct Archi.ptr64; monadInv H. TranslOpSplitter.
  econstructor; split. eapply exec_straight_three; simpl; eauto.
  repeat Splitter'; Simpl.
  destruct (rs RDX), (rs RAX); auto. simpl.
  unfold Val.cmpu. simpl.
  destruct (Int.eq i0 Int.zero) eqn:?; simpl.
  rewrite Int64.decompose_neg, Int64.hi_ofwords.
  rewrite ! (Int.same_if_eq _ _ Heqb). rewrite ! Int.add_zero.
  unfold Int.sub_borrow. rewrite Int.unsigned_zero. simpl.
  rewrite Int.sub_zero_l. auto.
  rewrite Int64.decompose_neg, Int64.hi_ofwords.
  rewrite  ! Int.add_zero.
  unfold Int.sub_borrow. rewrite zlt_true. rewrite Int.neg_add_distr.
  rewrite Int.sub_add_opp. auto.
  generalize (Int.unsigned_range i0). unfold Int.eq in Heqb. rewrite Int.unsigned_zero in Heqb.
  destruct (zeq (Int.unsigned i0) 0); inv Heqb. lia.
  destruct (rs RDX), (rs RAX); auto. simpl.
...
- (* subl *)
  ...
  econstructor; split. eapply exec_straight_two; simpl; eauto.
  repeat Splitter'; Simpl.
  apply Val.subl_hiword_lessdef.
  apply Val.subl_loword_lessdef.
  intuition Simplifs.
- (* divl *)
...
- (* modlu *)
  ...
  simpl; intros. destruct H5. unfold rs1; Simplifs.
- (* shrxlimm *)
  exploit mk_shrxlimm_correct; eauto. intros [rs1 [A [B C]]].
  exists rs1. Splitter. eexact A.
- (* leal *)
  exploit transl_addressing_mode_64_correct; eauto.
  erewrite <- get_pairs_singles; eauto. intros EA.
  generalize (normalize_addrmode_64_correct x0 rs). destruct (normalize_addrmode_64 x0) as [am' [delta|]]; intros EV.
  econstructor; split. eapply exec_straight_two.
  ...
  intros; Simplifs.
  econstructor; split.
  eapply exec_straight_one; simpl; eauto. split.
...
- (* condition *)
  ...
  destruct (eval_condition cond rs ## (preg_of ## x0) m).
  destruct Q as [Q _]. rewrite Q. auto.
  simpl; auto.
  intros. transitivity (rs2 r0); auto.
- (* selection *)
  exploit transl_sel_correct; eauto. intros (rs' & A & B & C).
