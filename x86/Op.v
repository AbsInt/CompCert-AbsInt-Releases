...
Definition type_of_addressing_typ := type_of_addressing_gen Tptr.
...
Lemma type_shift_stack_operation {Reg}:
  forall delta op (args: list Reg) (res: Reg), type_of_operation (shift_stack_operation delta op) args res = type_of_operation op args res.
Proof.
  intros. destruct op; auto;
  simpl; unfold type_of_operation; cbn;
  do 4 f_equal; destruct a; auto.
Qed.
...
Definition aindexed_addr32 (p: ptrofs) :=
  Aindexed (Int.signed (Ptrofs.to_int p)).

Definition aindexed_addr64 (p: ptrofs) :=
  Aindexed (Int64.signed (Ptrofs.to_int64 p)).

Definition aindexed_addr (p: ptrofs) :=
  if Archi.ptr64 then aindexed_addr64 p else aindexed_addr32 p.

Definition symbol_op (s: ident) :=
  Oindirectsymbol s.
...
Proof.
  ...
  unfold aindexed_addr, eval_addressing.
  destruct Archi.ptr64 eqn:?; simpl; rewrite Heqb0; rewrite Ptrofs.add_zero_l.
  rewrite Int64.repr_signed, Ptrofs.of_int64_to_int64; auto.
  rewrite Int.repr_signed, Ptrofs.of_int_to_int; auto.
Qed.
...
Proof.
  intros. unfold symbol_op. simpl. reflexivity.
Qed.
