...
Module CC.
  Definition reg_cc (r: mreg): RegCC :=
    ...
    | AX | CX | DX => RCCaller
    | BX | BP => RCCallee
    (**r callee-save in ELF 64 bits *)
    | SI | DI => if negb Archi.ptr64 || Archi.win64 then RCCallee else RCCaller
    | R8 | R9 | R10 | R11 => RCCaller
    | R12 | R13 | R14 | R15 => RCCallee
    | X0 | X1 | X2 | X3 | X4 | X5 => RCCaller
    | X6 | X7 | X8 | X9 | X10 | X11 | X12 | X13 | X14 | X15 => if Archi.win64 then RCCallee else RCCaller
    | FP0 => RCCaller
    end.
End CC.
...
Proof.
  intros. unfold is_auto_save. destruct r; simpl; try reflexivity.
  all: destruct Archi.win64; reflexivity.
Qed.
...
- (* WIN 64 bits *)
  assert (A: forall r, In r int_param_regs_win64 -> is_caller_save r = true) by (unfold is_caller_save, reg_cc; rewrite SF; decide_goal).
  assert (B: forall r, In r float_param_regs_win64 -> is_caller_save r = true) by (unfold is_caller_save, reg_cc; decide_goal).
...
- (* ELF 64 bits *)
  assert (A: forall r, In r int_param_regs_elf64 -> is_caller_save r = true) by (unfold is_caller_save, reg_cc; rewrite SF, W64; decide_goal).
  assert (B: forall r, In r float_param_regs_elf64 -> is_caller_save r = true) by (unfold is_caller_save, reg_cc; rewrite W64; decide_goal).
