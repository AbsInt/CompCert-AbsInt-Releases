...
let expand_builtin_inline name args res =
  ...
  | "__builtin_dtob", [BA(FR a1)],
     ...
     emit (Psubl_ri (RSP, _8));
     emit (Pcfi_adjust _8);
     emit (Pmovsd_mf ((linear_addr RSP _0z), a1));
     emit (Pmovl_rm (rl, (linear_addr RSP _0z)));
     emit (Pmovl_rm (rh, (linear_addr RSP _4z)));
     emit (Paddl_ri (RSP, _8));
     emit (Pcfi_adjust (Integers.Int.neg _8))
  | "__builtin_dtob", [BA(FR a1)],
                          BR(IR res) ->
     assert (Archi.ptr64);
     emit (Psubq_ri (RSP, _16));
     emit (Pcfi_adjust _16);
     emit (Pmovsd_mf ((linear_addr RSP _0z), a1));
     emit (Pmovq_rm (res, (linear_addr RSP _0z)));
     emit (Paddq_ri (RSP, _16));
     emit (Pcfi_adjust (Integers.Int.neg _16))
