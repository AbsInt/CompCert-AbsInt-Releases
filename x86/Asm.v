...
(** Manipulations over the [PC] register: continuing with the next
  ...
  [nextinstr_nf'] is a vaiant of [nextinstr] that sets all condition flags
  except for [CR CF] to [Vundef] in addition to incrementing the [PC]. *)
...
Definition nextinstr_nf' (rs: regset) : regset :=
  nextinstr (undef_regs (CR ZF :: CR PF :: CR SF :: CR OF :: nil) rs).
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pnegl rd =>
      Next (nextinstr_nf' (rs#rd <- (Val.neg rs#rd)
                             #(CR CF) <- (Val.cmpu (Mem.valid_pointer m) Cne (rs#rd) Vzero))) m
  | Pnegq rd =>
      Next (nextinstr_nf (rs#rd <- (Val.negl rs#rd))) m
  | Paddl_ri rd n =>
      Next (nextinstr_nf (rs#rd <- (Val.add rs#rd (Vint n)))) m
  | Paddl_rr rd a =>
      Next (nextinstr_nf' (rs#rd <- (Val.add rs#rd rs#a)
                         #(CR CF) <- (Val.add_carry rs#rd rs#a Vzero))) m
  | Padcl_rr rd a =>
      Next (nextinstr_nf (rs#rd <- (Val.add (Val.add rs#rd rs#a) rs#(CR CF)))) m
  | Padcl_ri rd n =>
      Next (nextinstr_nf (rs#rd <- (Val.add (Val.add rs#rd (Vint n)) rs#(CR CF)))) m
  | Paddq_ri rd n =>
      Next (nextinstr_nf (rs#rd <- (Val.addl rs#rd (Vlong n)))) m
  | Psbbl_rr rd r1 =>
      Next (nextinstr_nf (rs#rd <- (Val.sub (Val.sub rs#rd rs#r1) rs#(CR CF)))) m
  | Psubl_rr rd r1 =>
      Next (nextinstr_nf' (rs#rd <- (Val.sub rs#rd rs#r1)
                         #(CR CF) <- (Val.sub_borrow rs#rd rs#r1 Vzero))) m
  | Psubq_rr rd r1 =>
  ...
  | Pnegd rd =>
      Next (nextinstr_nf (rs#rd <- (Val.negf rs#rd))) m
  | Pabsd rd =>
