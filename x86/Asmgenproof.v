...
Lemma transl_restorecallee_label:
  forall l ofs k c,
  transl_restorecallee ofs l k = OK c ->
  tail_nolabel k c.
Proof.
  induction l as [ | h l]; simpl; intros; TailNoLabel.
  eapply tail_nolabel_trans. eapply loadind_label. TailNoLabel. TailNoLabel.
Qed.
...
Proof.
  induction l as [ | h l]; simpl; intros; TailNoLabel.
  eapply tail_nolabel_trans. eapply storeind_label. TailNoLabel. TailNoLabel.
Qed.
...
- (* Msavecallee *)
  ...
  intros. simpl in TR. destruct l. monadInv TR.
  rewrite (sp_val _ _ _ AG) in A.
  exploit transl_savecallee_correct; eauto.
  intros. rewrite preg_notin_charact. intros.
  red; intros. red in H0. eapply H0. apply H1. rewrite (preg_of_injective r mr); auto.
  discriminate.
  intros [rs2 [C D]].
  exists rs2; split. eauto. split.
  eapply agree_undef_regs. apply AG. auto with asmgen.
  rewrite D; auto.
  apply not_destroyed_not_in_map.
  intros. destruct ty; simpl; intuition congruence.
