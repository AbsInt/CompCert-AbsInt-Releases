...
Definition destroyed_by_builtin (ef: external_function): list mreg :=
  match ef with
  | EF_memcpy sz al =>
      if Ptrofs.ltu sz (Ptrofs.repr 33) then CX :: X7 :: nil else CX :: SI :: DI :: nil
...
Definition mregs_for_operation (op: operation): list (option (rpair mreg)) * option (rpair mreg) :=
  match op with
  | Omulhs => (Some (One AX) :: None :: nil, Some (One DX))
  | Omulhu => (Some (One AX) :: None :: nil, Some (One DX))
  | Odiv => (Some (One AX) :: Some (One CX) :: nil, Some (One AX))
  | Odivu => (Some (One AX) :: Some (One CX) :: nil, Some (One AX))
  | Omod => (Some (One AX) :: Some (One CX) :: nil, Some (One DX))
  | Omodu => (Some (One AX) :: Some (One CX) :: nil, Some (One DX))
  | Oshl => (None :: Some (One CX) :: nil, None)
  | Oshr => (None :: Some (One CX) :: nil, None)
  | Oshru => (None :: Some (One CX) :: nil, None)
  | Oshrximm _ => (Some (One AX) :: nil, Some (One AX))
  | Omull' => (Some (One AX) :: None :: nil, Some (Two DX AX))
  | Omullhs => (Some (One AX) :: None :: nil, Some (One DX))
  | Omullhu => (Some (One AX) :: None :: nil, Some (One DX))
  | Odivl => (Some (One AX) :: Some (One CX) :: nil, Some (One AX))
  | Odivlu => (Some (One AX) :: Some (One CX) :: nil, Some (One AX))
  | Omodl => (Some (One AX) :: Some (One CX) :: nil, Some (One DX))
  | Omodlu => (Some (One AX) :: Some (One CX) :: nil, Some (One DX))
  | Oshll => (None :: Some (One CX) :: nil, None)
  | Oshrl => (None :: Some (One CX) :: nil, None)
  | Oshrlu => (None :: Some (One CX) :: nil, None)
  | Oshrxlimm _ => (Some (One AX) :: nil, Some (One AX))
  | Oaddl => if  Archi.ptr64 then (nil, None) else (Some (Two DX AX) :: Some (Two CX BX) :: nil, Some (Two DX AX))
  | Osubl => if Archi.ptr64 then (nil, None) else (Some (Two DX AX) :: Some (Two CX BX) :: nil, Some (Two DX AX))
  | Onegl => if Archi.ptr64 then (nil, None) else (Some (Two DX AX) :: nil, Some (Two DX AX))
  | _ => (nil, None)
  end.
...
Definition mregs_for_builtin (ef: external_function): list (option mreg) * list (option mreg) :=
  match ef with
  | EF_memcpy sz al =>
     if Ptrofs.ltu sz (Ptrofs.repr 33) then (Some AX :: Some DX :: nil, nil) else (Some DI :: Some SI :: nil, nil)
  | EF_builtin name sg =>
     if string_dec name "__builtin_va_start" then
       (Some DX :: nil, nil)
     else
       (nil, nil)
  | _ => (nil, nil)
