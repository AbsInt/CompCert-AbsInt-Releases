...
Proof.
  ...
  - inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
    inv SEL. inv SEM. exists (minf v1 v0); split.
    apply eval_Eop with (v1 :: v0 :: nil).
    do 3 (constructor; auto). simpl. reflexivity.
    unfold maxf. auto.
  - inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
    inv SEL. inv SEM. exists (maxf v1 v0); split.
    apply eval_Eop with (v1 :: v0 :: nil).
    do 3 (constructor; auto). simpl. reflexivity.
    unfold maxf. auto.
Qed.
...
Proof.
  ...
  destruct bf; try discriminate.
  -  (* BI_negl *)
    inv ARGS; try discriminate. inv H0; inv SEM.
    simpl in *. destruct Archi.ptr64 eqn:?; inv SEL.
    econstructor; split. EvalOp. reflexivity. auto.
  - (* BI_addl *)
  ...
  - (* BI_mull *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; inv SEM.
    simpl in *. destruct Archi.ptr64; inv SEL.
    econstructor; split. EvalOp. reflexivity. auto.
Qed.

Theorem eval_lowlong32 :
  forall le a b v,
  Archi.ptr64 = false ->
  lowlong32 a = Some b ->
  eval_expr ge sp e m le a v ->
...
Proof.
  ...
  unfold lowlong32; case (lowlong32_match a); intros; inv H1; InvEval.
  - TrivialExists. rewrite Val.loword_addl. destruct v1, v0; auto.
    simpl. rewrite Int.add_zero. auto.
  - TrivialExists. rewrite Val.loword_subl; auto.
  - TrivialExists. rewrite Val.loword_negl; auto.
Qed.

Theorem eval_lowlong64:
  forall le a b v,
  Archi.ptr64 = true ->
  lowlong64 a = Some b ->
  eval_expr ge sp e m le a v ->
...
Theorem eval_lowlong:
  ...
  exists v', eval_expr ge sp e m le b v' /\ Val.lessdef (Val.lowordoflong v) v'.
Proof.
  intros until v.  unfold lowlong. destruct Archi.ptr64 eqn:?; intros.
  eapply eval_lowlong64; eauto. eapply eval_lowlong32; eauto.
Qed.
