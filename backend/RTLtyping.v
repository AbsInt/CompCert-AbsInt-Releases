...
Inductive wt_instr : instruction -> Prop :=
  ...
  | wt_Itailcall:
      ...
      match ros with inl r => env r = Tptr | inr s => True end ->
      map env args = proj_sig_args sig ->
      tailcall_rettype_match sig.(sig_res) funct.(fn_sig).(sig_res) ->
...
Definition type_instr (e: S.typenv) (i: instruction) : res S.typenv :=
  ...
  | Itailcall sig ros args =>
      do e1 <- type_ros e ros;
      do e2 <- S.set_list e1 args (proj_sig_args sig);
      if tailcall_matching_rettype sig.(sig_res) f.(fn_sig).(sig_res) then
...
- (* tailcall *)
  destruct (tailcall_matching_rettype (sig_res s) (sig_res (fn_sig f))); try discriminate.
...
- (* call *)
  ...
  eauto with ty.
- (* tailcall *)
  destruct (tailcall_matching_rettype (sig_res s) (sig_res (fn_sig f))) eqn:?; try discriminate.
  destruct (tailcall_is_possible s) eqn:TCIP; inv EQ2.
  constructor.
  eapply type_ros_sound; eauto with ty.
  eapply S.set_list_sound; eauto with ty.
  apply tailcall_matching_rettype_correct; auto.
...
- (* tailcall *)
  ...
  replace (tailcall_matching_rettype (sig_res sig) (sig_res (fn_sig f))) with true; auto.
  ...
  symmetry. apply (tailcall_matching_rettype_correct). auto.
- (* builtin *)
...
Lemma wt_regset_list2:
  forall env rs,
  wt_regset env rs ->
  forall rl al,
  List.map proj_argtype al = (List.map env rl) ->
  Val.has_argtype_list (rs##rl) al.
Proof.
  induction rl; simpl.
  intros. destruct al; auto. simpl in H0. inv H0.
  intros. destruct al; auto. inv H0.
  split. simpl in H0. inv H0.
  destruct a0 eqn:?; simpl in *; try rewrite H2; apply H.
  apply IHrl. inv H0. reflexivity.
Qed.
...
Inductive wt_stackframes: list stackframe -> signature -> Prop :=
  | wt_stackframes_nil: forall sg,
      tailcall_rettype_match (sg.(sig_res)) Tint ->
...
Inductive wt_state: state -> Prop :=
  ...
  | wt_state_call:
      ...
      wt_fundef f ->
      Val.has_argtype_list args (sig_args (funsig f)) ->
...
Remark wt_stackframes_change_sig:
  forall s sg1 sg2,
  tailcall_matching_rettype sg2.(sig_res) sg1.(sig_res) = true -> wt_stackframes s sg1 -> wt_stackframes s sg2.
Proof.
  intros. inv H0.
- constructor.
  assert (forall r1 r2 r3, tailcall_rettype_match r1 r2 -> tailcall_rettype_match r2 r3 -> tailcall_rettype_match r1 r3) by congruence.
  eapply H0. eapply tailcall_matching_rettype_correct. eapply H. auto.
- econstructor; eauto. rewrite H3. symmetry.
  apply tailcall_rettype_match_proj_sig. apply tailcall_matching_rettype_correct. auto.
Qed.
...
Proof.
  ...
  inv WTI.  apply (wt_regset_list2 env); auto.
  (* Itailcall *)
  assert (wt_fundef fd).
    destruct ros; simpl in H0.
    pattern fd. apply Genv.find_funct_prop with fundef unit p (rs#r).
  ...
  simpl in *. inv H5.
  econstructor; eauto.
  inv H1. apply wt_init_regs; auto. rewrite wt_params0.
  apply Val.has_proj_argtype_list. auto.
  (* external function *)
  econstructor; eauto.
...
Proof.
  ...
  intros. simpl. unfold tailcall_rettype_match, simpl_rettype_match. auto.
