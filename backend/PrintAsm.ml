...
module Printer(Target:TARGET) =
  struct
    ...
    (*- E_COMPCERT_CODE_PrintAsm_splitlong_001 *)
    (*- #Justify_Derived "Functional decomposition" *)
    let splitlong b =
    ...
    (*- E_COMPCERT_CODE_PrintAsm_emit_constants_001 *)
    (*- #Justify_Derived "Functional decomposition" *)
    ...
    (*- E_COMPCERT_CODE_PrintAsm_get_section_names_001 *)
    (*- #Justify_Derived "Functional decomposition" *)
    ...
    (*- #Link_to E_COMPCERT_TR_Function_PRINT_ASM_005 *)
    ...
    (*- #Link_to E_COMPCERT_TR_Function_PRINT_ASM_004 *)
    ...
    let print_globdef oc (name,gdef) =
      ...
      | Gfun (External ef) ->
        begin match C2C.find_weak_alias name with
        | Some id  when Configuration.alias_supported ->
          fprintf oc "	.weak	%a\n" Target.symbol name;
          fprintf oc "	.set	%a, %a\n" Target.symbol name Target.symbol id
        (*- #Link_to E_COMPCERT_TR_Function_PRINT_ASM_004 *)
        | _ -> ()
        end
...
let print_program oc sourcename p =
  let module Target = (val (sel_target ()):TARGET) in
  let module Printer = Printer(Target) in
  Fileinfo.init sourcename;

  (*- #Link_to E_COMPCERT_TR_Function_PRINT_ASM_001 *)
  ...
  PrintAsmaux.print_compiler_ident oc;
