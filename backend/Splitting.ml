...
(*- E_COMPCERT_CODE_Splitting_001 *)
(*- #Justify_Derived "Module Splitting.ml is part of CompCert's register allocator. Computed results are later checked by a formally verified validator." *)
...
let types_ref : ptype PTree.t ref = ref PTree.empty

let ren_reg env map r =
  let r' =
    try reg_for (RMap.find r map)
    with Not_found ->
      XTL.new_reg()
  in
  types_ref := PTree.set r' (env r) !types_ref;
  r'

let ren_regs env map rl =
  List.map (ren_reg env map) rl

let ren_ros env map ros =
  sum_left_map (ren_reg env map) ros

(* Rename in an instruction *)
let ren_instr f env maps pc i =
  let after = PMap.get pc maps in
  let before = transfer f pc after in
  match i with
  | Inop s -> Inop s
  | Iop(op, args, res, s) ->
    Iop(op, ren_regs env before args, ren_reg env after res, s)
  | Iload(chunk, addr, args, dst, s) ->
      Iload(chunk, addr, ren_regs env before args, ren_reg env after dst, s)
  | Istore(chunk, addr, args, src, s) ->
      Istore(chunk, addr, ren_regs env before args, ren_reg env before src, s)
  | Icall(sg, ros, args, res, s) ->
      Icall(sg, ren_ros env before ros, ren_regs env before args, ren_reg env after res, s)
  | Itailcall(sg, ros, args) ->
      Itailcall(sg, ren_ros env before ros, ren_regs env before args)
  | Ibuiltin(ef, args, res, s) ->
      Ibuiltin(ef, List.map (AST.map_builtin_arg (ren_reg env before)) args,
                   AST.map_builtin_res (ren_reg env after) res, s)
  | Icond(cond, args, s1, s2) ->
      Icond(cond, ren_regs env before args, s1, s2)
  | Ijumptable(arg, tbl) ->
      Ijumptable(ren_reg env before arg, tbl)
  | Ireturn optarg ->
      Ireturn(option_map (ren_reg env before) optarg)


(* Rename live ranges in a function *)

let rename_function f (env: RTLtyping.regenv) : RTL.coq_function * RTLtyping.regenv =
  Hashtbl.clear live_range_cache;
  types_ref := PTree.empty;
  let maps =
  ...
  let f' =
    { fn_sig = f.fn_sig;
      fn_params = ren_regs env before_entrypoint f.fn_params;
      fn_stacksize = f.fn_stacksize;
      fn_code = PTree.map (ren_instr f env maps) f.fn_code;
      fn_entrypoint = f.fn_entrypoint } in
  let new_env pc = PMap.get pc (Ptyp Tint, !types_ref) in
  (f',new_env)
