...
Definition mull' (v w: aval) :=
  match v, w with
  | I i1, I i2 => L (Int64.mul' i1 i2)
  | _, _ => ntop2 v w
  end.
...
Proof.
  intros. inv H; auto with va; inv H0; auto with va.
Qed.
...
Definition bitsoffloat (x: aval) :=
  match x with
  | F f => L (Float.to_bits f)
  | _ => ntop1 x
  end.

Lemma bitsoffloat_sound:
  forall v x, vmatch v x -> vmatch (Val.bitsoffloat v) (bitsoffloat x).
Proof.
  intros. unfold bitsoffloat; inv H; simpl; auto with va.
Qed.
...
Definition to_optbool (c: aval) :=
  match c with
  | I n => if Int.eq n Int.one then Just true
          else if Int.eq n Int.zero then Just false
          else Bnone
  | IU n => if Int.eq n Int.one then Maybe true
           else if Int.eq n Int.zero then Maybe false
           else Bnone
  | _ => Btop
  end.

Definition select_bool (c x y: aval) (ty: typ) :=
  select (to_optbool c) x y ty.

Lemma select_bool_sound:
  forall c v w ac x y ty,
  vmatch c ac -> vmatch v x -> vmatch w y ->
  vmatch (Val.select_bool c v w ty) (select_bool ac x y ty).
Proof.
  unfold Val.select_bool, select_bool; intros.
  apply select_sound; auto.
  unfold to_optbool, Val.to_optbool.
  destruct ac; inv H; auto with va; destruct (Int.eq n Int.one), (Int.eq n Int.zero); auto with va.
Qed.
...
Global Hint Resolve cnot_sound symbol_address_sound
       ...
       mul_sound mulhs_sound mulhu_sound mull'_sound
       ...
       bitsoffloat_sound longofwords_sound lowordoflong_sound hiwordoflong_sound
