...
(*- E_COMPCERT_FTR_Function_Locations_slot_0_001 *)
...
(* a.d. TODO update documentation for ptype. *)
...
(*- E_COMPCERT_FTR_Function_Locations_typesize_001 *)
...
Definition ptypesize (pt: ptype) : Z := typesize (proj_ptype_typ pt).

Lemma proj_ptype_typ_size:
  forall (ty: ptype), typesize (proj_ptype_typ ty) = ptypesize ty.
Proof.
  reflexivity.
Qed.

Lemma inj_typ_ptype_size:
  forall (ty: typ), typesize ty = ptypesize (inj_typ_ptype ty).
Proof.
  unfold ptypesize. destruct ty; reflexivity. 
Qed.
...
Lemma ptypesize_pos (ty: ptype): ptypesize ty > 0.
Proof.
  unfold ptypesize.
  destruct_ptype ty; cbn; unfold Tptr; 
...
Inductive ploc : Type :=
  | PR (r: mreg)
  | PS (sl: slot) (pos: Z) (ty: ptype).

(*- E_COMPCERT_FTR_Function_Locations_loc_0_001 *)
(*- #Link_to E_COMPCERT_TOR_Function_SEM_ASM_005 *)
Inductive loc : Type :=
  | R (r: mreg)
...
Definition inj_loc_ploc (l: loc): ploc :=
  match l with
  | R r => PR r
  | S sl pos ty => PS sl pos (inj_typ_ptype ty)
  end.

Definition proj_ploc_loc (l: ploc): loc :=
  match l with
  | PR r => R r
  | PS sl pos ty => S sl pos (proj_ptype_typ ty)
  end.

Module PLoc.
  Definition type (l: ploc) : ptype :=
    match l with
    | PR r => inj_typ_ptype (mreg_type r)
    | PS sl pos ty => ty
    end.

  Lemma eq: forall (p q: ploc), {p = q} + {p <> q}.
  Proof.
    decide equality.
    apply mreg_eq.
    apply ptype_eq.
...
- They are different but overlapping.
  ...
  by the following [Ploc.diff] predicate.
*)
  Definition diff (l1 l2: ploc) : Prop :=
    match l1, l2 with
    | PR r1, PR r2 =>
        r1 <> r2
    | PS s1 d1 t1, PS s2 d2 t2 =>
        s1 <> s2 \/ d1 + ptypesize t1 <= d2 \/ d2 + ptypesize t2 <= d1
    | _, _ =>
        True
    end.
  ...
  Proof.
    ...
    red; intros. destruct H; auto. generalize (ptypesize_pos ty); lia.
  Qed.
  ...
  Definition diff_dec (l1 l2: ploc) : { diff l1 l2 } + { ~diff l1 l2 }.
  Proof.
  ...
  - destruct (slot_eq sl sl0).
    destruct (zle (pos + ptypesize ty) pos0).
    left; auto.
    destruct (zle (pos0 + ptypesize ty0) pos).
    left; auto.
    right; red; intros [P | [P | P]]. congruence. lia. lia.
    left; auto.
  Defined.

(* (** We now redefine some standard notions over lists, using the [Ploc.diff]
  predicate instead of standard disequality [<>].

  [Ploc.notin l ll] holds if the location [l] is different from all locations
  in the list [ll]. *)

  Fixpoint notin (l: ploc) (ll: list ploc) {struct ll} : Prop :=
    match ll with
    | nil => True
  ...
  Lemma notin_iff:
    forall l ll, notin l ll <-> (forall l', In l' ll -> diff l l').
  Proof.
    induction ll; simpl.
    tauto.
  ...
  Lemma notin_dec (l: ploc) (ll: list ploc) : {notin l ll} + {~notin l ll}.
  Proof.
    induction ll; simpl.
    left; auto.
...
(** [Ploc.disjoint l1 l2] is true if the locations in list [l1]
  are different from all locations in list [l2]. *)

  Definition disjoint (l1 l2: list ploc) : Prop :=
...
(** [Ploc.norepet ll] holds if the locations in list [ll] are pairwise
  different. *)

  Inductive norepet : list ploc -> Prop :=
  | norepet_nil:
      norepet nil
  | norepet_cons:
      forall hd tl, notin hd tl -> norepet tl -> norepet (hd :: tl).

  Lemma norepet_dec (ll: list ploc) : {norepet ll} + {~norepet ll}.
  Proof.
    induction ll.
    left; constructor.
...
(** [Ploc.no_overlap l1 l2] holds if elements of [l1] never overlap partially
  with elements of [l2]. *)

  Definition no_overlap (l1 l2 : list ploc) :=
   forall r, In r l1 -> forall s, In s l2 ->  r = s \/ diff r s. *)
...
Module Locmap.
  ...
  Proof.
    ...
    - apply IHll.
      ...
      + destruct a.
        * auto.
        * unfold Val.load_result. now destruct (chunk_of_type (ty)).
      + destruct (Loc.diff_dec a l); auto.
    }
    induction ll; simpl; intros.
    - contradiction.      
    - destruct H.
      + apply P. subst a. apply gss_typed. destruct (Loc.type l); exact I.
      + auto.
  Qed.
...
Module Indexedtype <: INDEXED_TYPE.
  Definition t := typ.
  ...
  Proof.
    destruct x, y; cbn; try congruence.
    all: destruct Archi.ptr64; cbn; congruence.
  Qed.
  Definition eq := typ_eq.
End Indexedtype.

Module Orderedtype := OrderedIndexed(Indexedtype).
...
Module OrderedLoc <: OrderedType.
  ...
  Definition lt (x y: t) :=
    ...
    | S sl1 ofs1 ty1, S sl2 ofs2 ty2 =>
        ...
        (ofs1 < ofs2 \/ (ofs1 = ofs2 /\ Orderedtype.lt ty1 ty2)))
    end.
  ...
  Proof.
    ...
    right.  split. auto.
    intuition try lia.
    right; split. congruence. eapply Orderedtype.lt_trans; eauto.
  Qed.
  ...
  Proof.
    ...
    destruct H0. eelim Orderedtype.lt_not_eq; eauto. red; auto.
  Qed.
  ...
  - destruct (OrderedSlot.compare sl sl0).
    ...
    + destruct (OrderedZ.compare pos pos0).
      * apply LT. red. auto.
      * destruct (Orderedtype.compare ty ty0).
        apply LT. red; auto.
...
(** Connection between the ordering defined here and the [Loc.diff] predicate. *)
  ...
  Proof.
    ...
    - assert (RANGE: forall ty, 1 <= typesize ty <= 2).
      { intros; unfold typesize, typesize. destruct ty0; cbn; lia. }
      destruct H.
      + destruct H. left. apply not_eq_sym. apply OrderedSlot.lt_not_eq; auto.
        ...
        destruct H0.
        right.
        unfold Orderedtype.lt in H1. destruct ty'; compute in H1; cbn; (lia || congruence || idtac).
      + destruct H. left. apply OrderedSlot.lt_not_eq; auto.
        destruct H. right.
        destruct H0. left; lia.
        destruct H0. exfalso. destruct ty'; compute in H1; try congruence.
  Qed.
  ...
  Proof.
    ...
    - destruct (OrderedSlot.compare sl sl'); auto.
      ...
      assert (EITHER: typesize ty' = 1 /\ Orderedtype.lt ty' (Tany64) \/ typesize ty' = 2).
      { destruct ty'; compute; auto. }
