...
type graph = {
  ...
  rest_registers: mreg array array;
...
let class_of_type ty = Machregsaux.class_of_type ty
...
let init costs =
  let int_preferred_regs, int_rest_regs = int_alloc_register () in
  let int_preferred_regs = remove_reserved int_preferred_regs
  and float_preferred_regs = remove_reserved float_caller_save_regs
  and int_rest_regs = remove_reserved int_rest_regs
  and float_rest_regs = remove_reserved float_callee_save_regs in
  {
    preferred_registers =
      [| Array.of_list int_preferred_regs;
         ...
         [||] |];
    rest_registers =
      [| Array.of_list int_rest_regs;
         Array.of_list float_rest_regs;
         [||] |];
    num_available_registers =
      [| List.length int_preferred_regs + List.length int_rest_regs;
         List.length float_preferred_regs + List.length float_rest_regs;
         0 |];
    start_points =
      [| 0; 0; 0 |];
    allocatable_registers =
      int_preferred_regs @ int_rest_regs @ float_preferred_regs @ float_rest_regs;
    stats_of_reg = costs;
...
(* Assign a color (i.e. a hardware register or a stack location)
   ...
   hardware registers of the correct type; second the rest; third the stack.
   ...
   Callee-save registers and stack locations  are ``expensive'' resources,
   so we try to minimize their number by picking the smallest available
   callee-save register or stack location.  In contrast, caller-save
...
let find_reg g conflicts regclass =
  ...
  and rest_reg = g.rest_registers.(regclass)
  ...
  | None ->
      ...
      | Some _ as res ->
          ...
          res
      | None ->
          find rest_reg 0 (Array.length rest_reg)
