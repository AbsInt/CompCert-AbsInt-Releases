...
type node =
  { ident: int;                         (*r unique identifier *)
    ...
    mutable squeeze: int;		(*r Number of candidates blocked by neighbors *)
    ...
    mutable color: ploc option;          (*r chosen color *)
...
let name_of_loc = function
  ...
  | PS (Local, ofs, ty) ->
      sprintf "L%c%s" (PrintXTL.short_name_of_ptype ty) (Z.to_string ofs)
  | PS (Incoming, ofs, ty) ->
      sprintf "I%c%s" (PrintXTL.short_name_of_ptype ty) (Z.to_string ofs)
  | PS (Outgoing, ofs, ty) ->
      sprintf "O%c%s" (PrintXTL.short_name_of_ptype ty) (Z.to_string ofs)

let name_of_node n =
  match n.var with
  | V(r, ty) -> sprintf "x%ld (%s)" (P.to_int32 r) (PrintAST.name_of_ptype ty)
  | L l -> name_of_loc l
...
module DLinkNode = struct
  ...
  let make state =
    let rec empty =
      { ident = 0; typ = Ptyp Tint; var = V(P.one, Ptyp Tint); regclass = 0;
...
let newNodeOfReg g r ty =
  ...
  { ident = g.nextIdent; typ = ty;
    var = V(r, ty);
    regclass = if st.cost >= 0 then class_of_ptype ty else no_spill_class;
    accesses = st.usedefs;
...
let initialNodePartition g =
  let part_node n =
    match n.nstate with
    | Initial ->
        assert(Array.length g.num_available_registers > n.regclass);
...
let _degreeInvariant g n =
  let c = ref 0 in
  iterAdjacent (fun m -> c := !c + worst n.regclass m.regclass) n;
  if !c <> n.squeeze then
    (printf "Degree: %d\n%!" !c;
...
let decrementDegree g regclass n =
  let k = g.num_available_registers.(n.regclass) in
  let d = n.squeeze in
  let d' = d - (worst n.regclass regclass) in
  n.squeeze <- d';
  if d >= k && d' < k then begin
    enableMoves g n;
...
let canCoalesceBriggs g u v =
  ...
  assert(Array.length g.num_available_registers > u.regclass);
...
let canCoalesce g u v =
  ...
  (* We only allow coalescing of registers of the same class.
     This is used in the TriCore backend to stop address and data registers from being coalesced.
     For other backends it should never happen that two registers of different classes share an
     interference edge and are thus not considered for coalescing. *)
  if not (classes_alias u.regclass v.regclass)
  then false
  else begin
    if u.nstate = Colored
...
(* We check for the following:
    A - - - x0
            |
            B
   A, B are machine registers and alias.
   The move cannot be coalesced and is constrained *)
...
let coalesce g =
  let check_alias r n =
    ...
    match x.color with
    | Some (PR r') -> if regs_alias r r' then raise Exit
    | _ -> ()
  in
...
let rec reuse_slot conflicts n mvlist =
  ...
  | mv :: rem ->
      let attempt_reuse n' =
        match n'.color with
        | Some(PS(Local, _, _) as l)
             when List.for_all (PLoc.diff_dec l) conflicts -> Some l
...
let compare_slots s1 s2 =
  match s1, s2 with
  | PS(_, ofs1, _), PS(_, ofs2, _) -> Z.compare ofs1 ofs2
  | _, _ -> assert false

let align a b = (a + b - 1) land (-b)   (* assuming b is a power of 2 *)

let find_slot conflicts typ =
  let sz = Z.to_int (Locations.ptypesize typ) in
  let al = Z.to_int (Locations.typealign (proj_ptype_typ typ)) in
  let rec find curr = function
  | [] ->
      PS(Local, Z.of_uint curr, typ)
  | PS(Local, ofs, typ') :: l ->
...
let record_reg_conflict cnf n =
  ...
  | Some (PR r) -> Regset.add r cnf
  | _ -> cnf

let record_slot_conflict cnf n =
  match (getCanonicalName n).color with
  | Some (PS _ as l) -> l :: cnf
  | _ -> cnf
...
let location_of_var g v =
  ...
  | V(r, ty) ->
      ...
      let idx = class_of_ptype ty in
      if idx < Array.length dummy_regs
      then PR (dummy_regs.(idx))
      else assert false
