...
Lemma find_last_reg_in :
  forall r l,
  find_last_reg l = Some r ->
  In r l.
Proof.
  induction l. intros. inv H.
  simpl. destruct l; auto. intros.
  inv H. left. auto.
Qed.
...
Proof.
  ...
  rewrite E. apply find_last_reg_in. auto.
Qed.
...
Proof.
  ...
  destruct (regs_valnums n vl) eqn:?; inv H3; auto.
  erewrite (regs_valnums_sound n valu); eauto.
...
Proof.
  unfold reduce; intros.
  destruct (reduce_rec A f n 4%nat op vl) as [[op1 rl1] | ] eqn:?; inv H.
  eapply reduce_rec_lessdef_sound; eauto. congruence.
  destruct (regs_valnums n vl) eqn:?; inv H3; try discriminate; eauto.
  exists r. split; auto. rewrite <- H0 in H1.
  erewrite (regs_valnums_sound n valu); eauto.
Qed.
...
Lemma reduce_reg_sound:
  forall n valu sp m rs r r',
  numbering_holds valu ge sp rs m n ->
  reduce_reg n r = r' ->
  rs#r = rs#r'.
Proof.
  unfold reduce_reg.
  intros.
  destruct (valnum_reg n r) eqn:?.
  destruct (reg_valnum n0 v) eqn:?; try congruence.
  exploit valnum_reg_holds; eauto.
  intros (valu' & N & EQ & AG & P & Q).
  exploit reg_valnum_sound; eauto. congruence.
Qed.

Lemma reduce_regs_sound:
  forall n valu sp m rs args args',
  numbering_holds valu ge sp rs m n ->
  reduce_regs n args = args' ->
  rs##args = rs##args'.
Proof.
  unfold reduce_regs.
  intros.
  destruct (valnum_regs n args) eqn:?.
  destruct (regs_valnums n0 l) eqn:?; try congruence.
  exploit valnum_regs_holds; eauto.
  intros (valu' & N & EQ & AG & P & Q).
  exploit regs_valnums_sound; eauto. congruence.
Qed.

Lemma reduce_builtin_arg_sound:
  forall n valu sp rs arg arg' m v,
  numbering_holds valu ge sp rs m n ->
  reduce_builtin_arg n arg = arg' ->
  eval_builtin_arg ge (fun r => rs#r) sp m arg v ->
  eval_builtin_arg ge (fun r => rs#r) sp m arg' v.
Proof.
   unfold reduce_builtin_arg; intros.
   destruct arg; auto; rewrite <- H0; auto.
   inv H1. erewrite reduce_reg_sound; eauto.
   econstructor.
Qed.

Lemma reduce_builtin_args_sound:
  forall n valu sp rs args args' m vl,
  numbering_holds valu ge sp rs m n ->
  reduce_builtin_args n args = args' ->
  eval_builtin_args ge (fun r => rs#r) sp m args vl ->
  eval_builtin_args ge (fun r => rs#r) sp m args' vl.
Proof.
  induction args; intros. inv H1; simpl; constructor.
  inv H1. constructor. eapply reduce_builtin_arg_sound; eauto.
  eapply IHargs; eauto.
Qed.
...
-  destruct (valnum_regs approx!!pc args) as [n1 vl] eqn:?.
   ...
   destruct ( find_rhs_non_trivial n1 op vl) eqn:TRIV.
+ (* replaced by move *)
  unfold find_rhs_non_trivial in TRIV. destruct (find_rhs n1 (Op op vl)) eqn:RHS, (is_trivial_op op); inv TRIV.
...
- (* Istore *)
  ...
  assert (ADDR': eval_addressing tge sp addr' rs'##args' = Some a').
  { rewrite <- A. apply eval_addressing_preserved. exact symbols_preserved. }
  exploit reduce_reg_sound; eauto. intros.
  exploit Mem.storev_extends. eauto. eauto. eauto.
  rewrite H2. eauto.
  intros [m'' [X Y]].
  left; econstructor; split.
...
** (* the builtin function succeeds *)
  exploit reduce_regs_sound; eauto. intros. rewrite H1 in SEM.
  ...
  unfold transfer; rewrite H, IK.
  eapply add_builtin_holds with (res := BR res); eauto using kill_cheap_computations_hold, eval_builtin_args_trivial.
  inv H2. rewrite <- H4 in  SEM. auto.
  apply set_reg_lessdef; auto. inv H2. auto.
** (* the builtin function fails *)
  ...
  congruence.
+ (* other function *)
  exploit reduce_regs_sound; eauto. intros.
  left; econstructor; split.
  eapply exec_Icall; eauto.
...
- (* Ibuiltin *)
  destruct SAT as [valu NH].
  exploit reduce_builtin_args_sound; eauto.
  intros vars'.
  ...
  intros (v' & m1' & P & Q & R & S).
  assert (DEFAULT:
    (fn_code (transf_function' f cu approx)) ! pc = Some(Ibuiltin ef (reduce_builtin_args approx # pc args) res pc') ->
