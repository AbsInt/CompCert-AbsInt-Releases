...
Definition undef_callee_may_modify_regs (ls: locset) : locset :=
  fun (l: loc) =>
    match l with
    | R r => if is_modifiable_by_callee r then Vundef else ls (R r) 
    | S Outgoing ofs ty => Vundef
...
Definition external_call_regs (caller ls: locset) : locset :=
  restore_auto_save_regs caller (undef_callee_may_modify_regs ls).
...
Definition tailcall_regs (caller ls: locset) : locset := pre_return_regs caller ls.
