...
(** [pre_return_regs caller ls] returns the location set before
  a return instruction, as a function of the location set [caller]
  of the caller before the call to enter this function and of the location
  set [ls] of the callee before the return instruction.
...
- Auto-save machine registers have the same values as in the callee.
...
Definition pre_return_regs (caller ls: locset) : locset :=
  fun (l: loc) =>
    match l with
    | R r => if is_callee_save r then caller (R r) else ls (R r)
    | S Outgoing ofs ty => Vundef
    | S sl ofs ty => caller (S sl ofs ty)
    end.

(** [restore_auto_save_regs caller ls] modifies the location set [ls]
  by restoring the values of auto-save machine registers to those of the 
  location set [caller]. 
*)

Definition restore_auto_save_regs (caller ls: locset) : locset :=
  fun (l: loc) =>
    match l with
    | R r => if is_auto_save r then caller (R r) else ls (R r) 
    | _ => ls l
    end.
  
(** [undef_caller_save_regs ls] modifies the location set [ls]
  by setting the values of all caller-save locations to Vundef.
- Callee-save machine registers are preserved.
- Caller-save machine registers are set to Vundef.
- Auto-save machine registers are preserved
- Local and Incoming stack slots are preserved.
- Outgoing stack slots are set to Vundef to reflect the fact that they
  may have been changed by the callee. *)
Definition undef_caller_save_regs (ls: locset) : locset :=
  fun (l: loc) =>
    match l with
    | R r => if is_caller_save r then Vundef else ls (R r)
    | S Outgoing ofs ty => Vundef
...
(** [return_regs caller callee] returns the location set after
  ...
  It combines [restore_auto_save_regs] and [pre_return_regs].
- Callee-save machine registers have the same values as in the caller
  before the call.
- Caller-save machine registers have the same values as in the callee.
- Auto-save machine registers have the same values as in the caller.
- Local and Incoming stack slots have the same values as in the caller.
- Outgoing stack slots are set to Vundef to  reflect the fact that they
  may have been changed by the callee.
*)
...
(** [tailcall_regs caller ls] returns the location set before a tail-call, 
    which is equivalent to the location set before a return instruction. *)
Definition tailcall_regs := pre_return_regs.
                                                
(** [external_call_regs caller ls] models the effect of calling
    an external function: caller and callee-save registers and outgoing locations
    can change unpredictably, hence we set them to [Vundef]. Auto-save registers are preserved. *)
Definition external_call_regs (caller ls: locset) : locset :=
  restore_auto_save_regs caller (undef_caller_save_regs ls).
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Ltailcall: forall s f sp sig ros bb rs m fd rs' m',
      rs' = tailcall_regs (parent_locset s) rs ->
      find_function ros rs' = Some fd ->
      funsig fd = sig ->
  ...
  | exec_Ljumptable: forall s f sp arg tbl bb rs m n pc rs',
      ...
      step (Block s f sp (Ljumptable arg tbl :: bb) rs m)
        E0 (State s f sp pc rs' m)
  | exec_Lreturn: forall s f sp bb rs m rs' m',
  ...
  | exec_function_external: forall s ef t args res rs m rs' m',
      ...
      rs' = Locmap.setpair (loc_result (ef_sig ef)) res (external_call_regs (parent_locset s) rs) ->
