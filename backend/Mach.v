...
Inductive instruction: Type :=
  ...
  | Mrestorecallee: Z -> list mreg -> instruction
  | Msavecallee: Z -> list mreg -> instruction
...
Fixpoint store_stack_regs (m: mem) (sp: val) (ofs: Z) (l: list mreg) (rs: regset) :=
  match l with
  | r :: l =>
      let ty := mreg_type r in
      let sz := AST.typesize ty in
      let ofs1 := align ofs sz in
      match Mem.storev (chunk_of_type ty) m (Val.offset_ptr sp (Ptrofs.repr ofs1)) (rs r) with
      | Some m' => store_stack_regs m' sp (ofs1 + sz) l rs
      | _ => None
      end
  | nil => Some m
  end.

Fixpoint load_stack_regs (m: mem) (sp: val) (ofs: Z) (l: list mreg) (rs: regset) :=
  match l with
  | r :: l =>
      let ty := mreg_type r in
      let sz := AST.typesize ty in
      ...
      | Some v => load_stack_regs m sp (ofs1 + sz) l (rs#r <- v)
      | _ => None
      end
  | nil => Some rs
  end.
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Mrestorecallee:
      ...
      load_stack_regs m sp ofs l rs = Some rs' ->
      step (State s f sp (Mrestorecallee ofs l :: c) rs m)
        E0 (State s f sp c rs' m)
  | exec_Msavecallee:
      forall s f sp l ofs c rs m m' rs',
      store_stack_regs m sp ofs l rs = Some m' ->
      (forall r, In r l -> ~ In r (flat_map destroyed_by_setstack (map mreg_type l))) ->
      rs' = undef_regs (flat_map destroyed_by_setstack (map mreg_type l)) rs ->
