...
Definition pre_return_regs (caller callee: regset) : regset :=
  fun r =>
    if is_callee_save r then caller r else callee r.

Definition undef_caller_save_regs (rs: regset) : regset :=
  fun r => if is_caller_save r then Vundef else rs r.

Definition restore_auto_save_regs (caller callee: regset) : regset :=
  fun r => if is_auto_save r then caller r else callee r.
...
Definition external_call_regs (caller rs: regset) : regset :=
  restore_auto_save_regs caller (undef_caller_save_regs rs).
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Mcall:
      ...
      step (State s fb sp (Mcall sig ros :: c) rs m)
        E0 (Callstate (Stackframe fb sp (Vptr fb ra) rs c :: s)
                       f' rs m)
  | exec_Mtailcall:
      forall s fb stk soff sig ros c rs m f f' rs' m',
  ...
  | exec_Mjumptable:
      ...
      step (State s fb sp (Mjumptable arg tbl :: c) rs m)
        E0 (State s fb sp c' rs' m)
  | exec_Mreturn:
      forall s fb stk soff c rs m f rs' m',
