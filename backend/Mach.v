...
- [Msetstack r ofs ty] is a memory store at offset [ofs * 4] relative
  to the stack pointer.
- [Mgetparam ofs ty r] is a memory load at offset [ofs * 4] relative
  to the pointer found at offset [f.(fn_link_ofs)] from the stack pointer.
...
caller function at offset [f.(fn_link_ofs)]. If [f.(fn_retaddr_saved)] is true,
a return address is saved at offset [f.(fn_retaddr_ofs)]. This will be used
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_Mtailcall:
      ...
      (f.(fn_retaddr_saved) = true -> load_stack m (Vptr stk soff) Tptr f.(fn_retaddr_ofs) = Some (parent_ra s)) ->
  ...
  | exec_function_internal:
      ...
      (if f.(fn_retaddr_saved)
       then store_stack m2 sp Tptr f.(fn_retaddr_ofs) (parent_ra s) = Some m3
       else m3 = m2) ->
