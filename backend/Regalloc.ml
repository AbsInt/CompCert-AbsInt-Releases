...
let vreg_pair tyenv r =
  let ty = tyenv r in
  if Archi.splitlong && ty = Ptyp Tlong then
    Vsplitlong (V (r, Ptyp Tint), V ((twin_reg r), Ptyp Tint))
  else
    Vr (V (r, ty))

let vregs tyenv rl = List.map (vreg tyenv) rl

let vregs_pair tyenv rl = List.map (vreg_pair tyenv) rl

let constrain_reg_pair v c =
  match c with
  | None -> v
  | Some (One mr) -> Vr (L (PR mr))
  | Some (Two (mr1, mr2)) -> Vsplitlong (L (PR mr1), L (PR mr2))

let rec constrain_reg_pairs vl cl =
  match vl, cl with
  | [], _ -> []
  | v1 :: vl', [] -> vl
  | v1 :: vl', Some (One mr1) :: cl' -> Vr (L(PR mr1)) :: constrain_reg_pairs vl' cl'
  | v1 :: vl', Some (Two (mr1, mr2)) :: cl' ->  Vsplitlong (L (PR mr1), L (PR mr2)) :: constrain_reg_pairs vl' cl'
  | v1 :: vl', None :: cl' -> v1 :: constrain_reg_pairs vl' cl'
...
let rec new_temps = function
  | [] -> []
  | c :: cl -> (new_temp (default_ptype_of_class c)) :: (new_temps cl)
...
let parmove_regs2locs tyenv srcs dsts k =
  ...
  let rec expand srcs' dsts' rl ll =
    ...
    | r :: rl, Two(l1, l2) :: ll ->
        assert (tyenv r = Ptyp Tlong);
        if Archi.splitlong then
          expand (V(r, Ptyp Tint) :: V(twin_reg r, Ptyp Tint) :: srcs')
                 (L l1 :: L l2 :: dsts')
                 rl ll
        else
          Xop(Ohighlong, [Vr (V (r, Ptyp Tlong))], Vr (L l1)) ::
          Xop(Olowlong,  [Vr (V(r, Ptyp Tlong))], Vr (L l2)) ::
    ...
    | Two(l1, l2) :: ll, r :: rl ->
        ...
        if Archi.splitlong then
          expand (L l1 :: L l2 :: srcs')
                 (V(r, Ptyp Tint) :: V(twin_reg r, Ptyp Tint) :: dsts')
                 ll rl
        else
          Xop(Omakelong, [Vr (L l1); Vr (L l2)], Vr (V(r, Ptyp Tlong))) ::
...
let rec convert_builtin_arg tyenv = function
  | BA r ->
      let ty = tyenv r in
      if Archi.splitlong && ty = Ptyp Tlong
      then BA_splitlong(BA(V(r, Ptyp Tint)), BA(V(twin_reg r, Ptyp Tint)))
      else BA(V(r, ty))
  | BA_int n -> BA_int n
  | BA_long n -> BA_long n
...
let convert_builtin_res tyenv = function
  | BR r ->
      let ty = tyenv r in
      if Archi.splitlong && ty = Ptyp Tlong
      then BR_splitlong(BR(V(r, Ptyp Tint)), BR(V(twin_reg r, Ptyp Tint)))
      else BR(V(r, ty))
  | BR_none -> BR_none
...
let block_of_RTL_instr funsig tyenv = function
  ...
  | RTL.Iop(Omove, [arg], res, s) ->
      if Archi.splitlong && tyenv arg = Ptyp Tlong then
        [Xmove(V(arg, Ptyp Tint), V(res, Ptyp Tint));
         Xmove(V(twin_reg arg, Ptyp Tint), V(twin_reg res, Ptyp Tint));
         Xbranch s]
      else
        [Xmove(vreg tyenv arg, vreg tyenv res); Xbranch s]
  | RTL.Iop(Omakelong, [arg1; arg2], res, s) when Archi.splitlong ->
      [Xmove(V(arg1, Ptyp Tint), V(res, Ptyp Tint));
       Xmove(V(arg2, Ptyp Tint), V(twin_reg res, Ptyp Tint));
       Xbranch s]
  | RTL.Iop(Olowlong, [arg], res, s) when Archi.splitlong ->
      [Xmove(V(twin_reg arg, Ptyp Tint), V(res, Ptyp Tint)); Xbranch s]
  | RTL.Iop(Ohighlong, [arg], res, s) when Archi.splitlong ->
      [Xmove(V(arg, Ptyp Tint), V(res, Ptyp Tint)); Xbranch s]
  | RTL.Iop(op, args, res, s) ->
        let (cargs, cres) = mregs_for_operation op in
        let args1 = vregs_pair tyenv args and res1 = vreg_pair tyenv res in
        let args2 = constrain_reg_pairs args1 cargs and res2 = constrain_reg_pair res1 cres in
        let (args3, res3) =
          ...
          | Some(arg, args2') ->
            if arg = res2 || not (List.mem res2 args2') then
              ...
              let ty = tyenv res in
              assert (ty <> Ptyp Tlong || negb Archi.splitlong);
              let t = Vr (new_temp ty) in
              (t :: args2', t) in
        movelist (vars_of_var_pairs args1) (vars_of_var_pairs args3)
          (Xop(op, args3, res3) :: movelist (vars_of_var_pair res3) (vars_of_var_pair res1)
             [Xbranch s])
      ...
      if Archi.splitlong && chunk = Mint64 then begin
        ...
        | Some addr' ->
            [Xload(Mint32, addr, vregs tyenv args,
                   V((if Archi.big_endian then dst else twin_reg dst), Ptyp Tint));
             Xload(Mint32, addr', vregs tyenv args,
                   V((if Archi.big_endian then twin_reg dst else dst), Ptyp Tint));
             Xbranch s]
      end else
      ...
      if Archi.splitlong && chunk = Mint64 then begin
        ...
        | Some addr' ->
            [Xstore(Mint32, addr, vregs tyenv args,
                   V((if Archi.big_endian then src else twin_reg src), Ptyp Tint));
             Xstore(Mint32, addr', vregs tyenv args,
                   V((if Archi.big_endian then twin_reg src else src), Ptyp Tint));
             Xbranch s]
      end else
        [Xstore(chunk, addr, vregs tyenv args, vreg tyenv src); Xbranch s]
  | RTL.Icall(sg, ros, args, res, s) ->
      let args' = loc_arguments sg
      and res' = [map_rpair (fun r -> PR r) (loc_result sg)] in
      let args'' = List.map (map_rpair inj_loc_ploc) args' in
  ...
  | RTL.Ireturn (Some arg) ->
      let args' = [map_rpair (fun r -> PR r) (loc_result funsig)] in
...
let function_of_RTL_function f tyenv =
  ...
  let b_entrypoint =
    parmove_locs2regs tyenv
              (List.map (map_rpair inj_loc_ploc) (loc_parameters f.RTL.fn_sig))
              f.RTL.fn_params
...
let vset_pair_mem r after =
  match r with
  | Vr v -> VSet.mem v after
  | Vsplitlong (v1, v2) -> VSet.mem v1 after || VSet.mem v2 after
...
let live_before instr after =
  ...
  | Xop(op, args, res) ->
      if vset_pair_mem res after
      then vset_addlist (vars_of_var_pairs args) (vset_removelist (vars_of_var_pair res) after)
      else after
...
let spill_costs f =
  ...
  let charge_instr = function
    ...
    | Xop(op, args, res) ->
        charge_list 10 1 (vars_of_var_pairs args); charge_list 10 1 (vars_of_var_pair res)
    | Xload(chunk, addr, args, dst) ->
...
let add_interfs_destroyed g live mregs =
  List.iter
    (fun mr -> VSet.iter (IRC.add_interf g (L (PR mr))) live)
    mregs
    ...
    (fun v ->
       ...
       List.iter
         (fun mr ->
            ...
            then IRC.add_interf g (L (PR mr)) v)
         all_mregs)
    live
...
let add_interfs_mreg_list g v rl =
  List.iter (fun r -> IRC.add_interf g (L (PR r)) v) rl

let add_interfs_list_mreg g vl mr =
  List.iter (fun v -> IRC.add_interf g v (L (PR mr))) vl

let rec add_interfs_pairwise g = function
  | [] -> ()
  | v1 :: vl -> add_interfs_list g v1 vl; add_interfs_pairwise g vl

let add_interfs_pair g res live =
  match res with
  | Vr v -> add_interfs_def g v live
  | Vsplitlong (v1, v2) -> add_interfs_def g v1 live; add_interfs_def g v2 live

let add_interfs_parallel_move_constraints g srcs dsts =
  let class_of_var v =
    match v with
    | L l -> class_of_loc l
    | _ -> class_of_ptype (typeof v)
      in
...
let add_interfs_instr g instr live =
  ...
  | Xparmove(srcs, dsts, tmps) ->
      ...
      Array.iter2 (fun v l -> add_interfs_list g v l) tmps (parallel_move_interfs_tmps srcs dsts tmps);
      ...
      if List.exists (function (L(Locations.PS(Incoming, _, _))) -> true | _ -> false) srcs
      then begin
  ...
  | Xop(op, args, res) ->
      ...
      | None ->
          add_interfs_pair g res live
      | Some(arg1, argl) ->
          let res = match res with | Vr v1 -> v1 | _ -> assert false in
          let arg1 = match arg1 with | Vr v1 -> v1 | _ -> assert false in
          (* Treat as "res := arg1; res := op(res, argl)" *)
          add_interfs_def g res live;
          IRC.add_pref g arg1 res;
          add_interfs_move g arg1 res
            (vset_addlist (res :: (vars_of_var_pairs argl)) (VSet.remove res live))
      end;
      add_interfs_destroyed g (vset_removelist (vars_of_var_pair res) live) (destroyed_by_op op)
  | Xload(chunk, addr, args, dst) ->
  ...
  | Xbuiltin(ef, args, res) ->
      ...
      | EF_inline_asm(txt, sg, clob), _, _ ->
          ...
          List.iter (fun c ->
            ...
            | Some (Two (mr1, mr2)) ->
                add_interfs_list_mreg g vargs mr1;
                add_interfs_list_mreg g vargs mr2;
                add_interfs_list_mreg g vres mr1;
                add_interfs_list_mreg g vres mr2)
            clob
      | _ -> ()
      end
  | Xbranch s ->
...
let is_reg alloc v =
  match alloc v with PR _ -> true | Locations.PS _ -> false

let add_tospill alloc v ts =
  match alloc v with PR _ -> ts | Locations.PS _ -> VSet.add v ts
...
let addres_tospill alloc res ts =
  match res with
  | Vr v -> add_tospill alloc v ts
  | Vsplitlong (v1, v2) -> addlist_tospill alloc [v1; v2] ts
...
let tospill_instr alloc instr ts =
  ...
  | Xparmove(srcs, dsts, tmps) ->
      ...
      ts
  | Xop(op, args, res) ->
      addlist_tospill alloc (vars_of_var_pairs args) (addres_tospill alloc res ts)
  | Xload(chunk, addr, args, dst) ->
...
let reload_var_pair tospill eqs = function
  | Vr v ->
    let (t, c1, eqs1) = reload_var tospill eqs v in
    (Vr t, c1, eqs1)
  | Vsplitlong (hi, lo) ->
    let (hi', c1, eqs1) = reload_var tospill eqs hi in
    let (lo', c2, eqs2) = reload_var tospill eqs1 lo in
    (Vsplitlong (hi', lo'), c1 @ c2, eqs2)

let rec reload_var_pairs tospill eqs = function
  | [] -> ([], [], eqs)
  | a1 :: al ->
    let (t1, c1, eqs1) = reload_var_pair tospill eqs a1 in
    let (tl, cl, eqs2) = reload_var_pairs tospill eqs1 al in
    (t1 :: tl, c1 @ cl, eqs2)
...
let save_op_res tospill eqs = function
  | Vr v ->
    let (v, c, eqs) = save_var tospill eqs v in
    (Vr v, c, eqs)
  | Vsplitlong (hi, lo) ->
...
let spill_instr tospill eqs instr =
  ...
  | Xop(op, args, res) ->
      begin match is_two_address op args with
      | None ->
          let (args', c1, eqs1) = reload_var_pairs tospill eqs args in
          let (res', c2, eqs2) = save_op_res tospill eqs1 res in
          (c1 @ Xop(op, args', res') :: c2, eqs2)
      | Some(arg1, argl) ->
          let res = match res with Vr v -> v | _ -> assert false in
          let arg1 = match arg1 with Vr v -> v | _ -> assert false in
          begin match VSet.mem res tospill, VSet.mem arg1 tospill with
          | false, false ->
              let (argl', c1, eqs1) = reload_var_pairs tospill eqs argl in
              (c1 @ [Xop(op, (Vr arg1) :: argl', Vr res)], kill res eqs1)
          | true, false ->
              let tmp = new_temp (typeof res) in
              let (argl', c1, eqs1) = reload_var_pairs tospill eqs argl in
              (c1 @ [Xmove(arg1, tmp); Xop(op, (Vr tmp) :: argl', Vr tmp); Xspill(tmp, res)],
               add res tmp (kill res eqs1))
          | false, true ->
              let eqs1 = add arg1 res (kill res eqs) in
              let (argl', c1, eqs2) = reload_var_pairs tospill eqs1 argl in
              (* PR#113, PR#122: [Xreload] here causes [res] to become
                 ...
                 below used instead. *)
              (Xmove(arg1, res) :: c1 @ [Xop(op, Vr res :: argl', Vr res)],
               kill res eqs2)
          | true, true ->
              let tmp = new_temp (typeof res) in
              let eqs1 = add arg1 tmp eqs in
              let (argl', c1, eqs2) = reload_var_pairs tospill eqs1 argl in
              (Xreload(arg1, tmp) :: c1 @ [Xop(op, Vr tmp :: argl', Vr tmp); Xspill(tmp, res)],
...
let mreg_of alloc v = match alloc v with PR mr -> mr | Locations.PS _ -> raise Bad_LTL
...
let locs_alias l1 l2 =
  match l1, l2 with
  | Locations.PS _, Locations.PS _ -> l1 = l2
  | PR r1, PR r2 -> regs_alias r1 r2
  | _, _ -> false
...
let make_move src dst k =
  ...
  | PR rsrc, Locations.PS(sl, ofs, ty) ->
      LTL.Lsetstack(expand_mreg rsrc, sl, ofs, proj_ptype_typ ty) :: k
  | Locations.PS(sl, ofs, ty), PR rdst ->
      LTL.Lgetstack(sl, ofs, proj_ptype_typ ty, expand_mreg rdst) :: k
  | Locations.PS _, Locations.PS _ ->
      if src = dst then k else raise Bad_LTL

type parmove_status = To_move | Being_moved | Moved

let make_parmove (srcs: Locations.ploc list)  dsts tmps k =
  ...
  let add_move s d =
    ...
    | PR rs, Locations.PS(sl, ofs, ty) ->
        code := LTL.Lsetstack(expand_mreg rs, sl, ofs, proj_ptype_typ ty) :: !code
    | Locations.PS(sl, ofs, ty), PR rd ->
        code := LTL.Lgetstack(sl, ofs, proj_ptype_typ ty, expand_mreg rd) :: !code
    | Locations.PS(sls, ofss, tys), Locations.PS(sld, ofsd, tyd) ->
        let tmp = temp_for (class_of_ptype tys) in
        (* code will be reversed at the end *)
        code := LTL.Lsetstack(expand_mreg tmp, sld, ofsd, proj_ptype_typ tyd) ::
                LTL.Lgetstack(sls, ofss, proj_ptype_typ tys, expand_mreg tmp) :: !code
    in
  let rec move_one i =
      ...
      for j = 0 to n - 1 do
        if locs_alias src.(j) dst.(i) then
          ...
          | To_move ->
              move_one j
          | Being_moved ->
              let tmp = PR (temp_for (class_of_loc src.(j))) in
              add_move src.(j) tmp;
              src.(j) <- tmp
          | Moved ->
              ()
...
let mreg_rpair_of_var_pair alloc res =
  match res with
  | Vr v -> mreg_rpair_of alloc v
  | Vsplitlong (v1, v2) ->
    let v1 = match mreg_rpair_of alloc v1 with One v1 -> v1 | _ -> assert false
    and v2 = match mreg_rpair_of alloc v2 with One v2 -> v2 | _ -> assert false in
    Two (v1, v2)

let mreg_rpairs_of_var_pairs alloc al =
  List.map (mreg_rpair_of_var_pair alloc) al
...
let transl_instr alloc instr k =
  ...
  | Xop(op, args, res) ->
      let rargs = mreg_rpairs_of_var_pairs alloc args
      and rres  = mreg_rpair_of_var_pair alloc res in
  ...
  | Xbuiltin(ef, args, res) ->
      ...
      LTL.Lbuiltin(ef, List.map (map_builtin_arg (map_rpair proj_ploc_loc)) (List.map (AST.map_builtin_arg alloc_pair) args),
...
(*- E_COMPCERT_CODE_Regalloc_regalloc_001 *)
let regalloc f env =
  init_trace();
  reset_temps();
  if !option_dalloctrace then begin
        fprintf !pp "-------------- Original RTL\n\n";
        PrintTypedRTL.print_function !pp (fresh_atom ()) (RTLtyping.TF (f,env))
      end;
  let (f1, env') = Splitting.rename_function f env in
  if !option_dalloctrace then begin
      fprintf !pp "-------------- Renamed RTL\n\n";
      PrintTypedRTL.print_function !pp (fresh_atom ()) (RTLtyping.TF (f1, env'))
      end;
     let f2 = function_of_RTL_function f1 env' in
     if !option_dalloctrace then begin
        fprintf !pp "-------------- XTL from RTL\n\n";
      ...
      with
      | Type_error_at pc ->
          (*- #Link_to E_COMPCERT_TR_Robustness_REGALLOC_002 *)
          Errors.Error [Errors.MSG "ill-typed XTL code at PC ";
                 Errors.POS pc]
      | Bad_LTL ->
         (*- #Link_to E_COMPCERT_TR_Robustness_REGALLOC_003 *)
