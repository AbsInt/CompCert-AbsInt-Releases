...
let vreg_pair tyenv r =
  ...
  if Archi.splitlong && ty = Tlong then
    Vsplitlong (V (r, Tint), V ((twin_reg r), Tint))
  else
    Vr (V (r, ty))
...
let constrain_reg_pair v c =
  match c with
  | None -> v
  | Some (One mr) -> Vr (L (R mr))
  | Some (Two (mr1, mr2)) -> Vsplitlong (L (R mr1), L (R mr2))

let rec constrain_reg_pairs vl cl =
  match vl, cl with
  | [], _ -> []
  | v1 :: vl', [] -> vl
  | v1 :: vl', Some (One mr1) :: cl' -> Vr (L(R mr1)) :: constrain_reg_pairs vl' cl'
  | v1 :: vl', Some (Two (mr1, mr2)) :: cl' ->  Vsplitlong (L (R mr1), L (R mr2)) :: constrain_reg_pairs vl' cl'
  ...
  let rec expand srcs' dsts' rl ll =
    ...
    | r :: rl, Two(l1, l2) :: ll ->
        ...
        if Archi.splitlong then
          ...
          Xop(Ohighlong, [Vr (V (r, Tlong))], Vr (L l1)) ::
          Xop(Olowlong,  [Vr (V(r, Tlong))], Vr (L l2)) ::
  ...
  let rec expand srcs' dsts' ll rl =
    ...
    | Two(l1, l2) :: ll, r :: rl ->
        ...
        if Archi.splitlong then
          ...
          Xop(Omakelong, [Vr (L l1); Vr (L l2)], Vr (V(r, Tlong))) ::
        ...
        let (args3, res3) =
          ...
          | Some(arg, args2') ->
            if arg = res2 || not (List.mem res2 args2') then
              ...
              let ty = tyenv res in
              assert (ty <> Tlong || negb Archi.splitlong);
...
let regalloc f =
  ...
  | Errors.Error msg ->
      (*- #Link_to E_COMPCERT_TR_Robustness_REGALLOC_004 *)
