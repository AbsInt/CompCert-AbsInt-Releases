...
Lemma wt_pre_return_regs:
  forall caller callee,
  wt_locset caller -> wt_locset callee -> wt_locset (pre_return_regs caller callee).
Proof.
...
Lemma wt_tailcall_regs:
  forall caller callee,
  wt_locset caller -> wt_locset callee -> wt_locset (tailcall_regs caller callee).
Proof.
  apply wt_pre_return_regs.
Qed.

Lemma wt_undef_callee_may_modify_regs:
  forall ls, wt_locset ls -> wt_locset (undef_callee_may_modify_regs ls).
Proof.
  intros; red; intros. unfold undef_callee_may_modify_regs.
  destruct l.
- destruct (is_modifiable_by_callee r); simpl; auto.
- destruct sl; auto; red; auto. 
Qed.

Lemma wt_restore_auto_save_regs:
  forall caller callee,
  wt_locset caller -> wt_locset callee -> wt_locset (restore_auto_save_regs caller callee).
Proof.
...
Lemma wt_external_call_regs:
  forall s ls, wt_locset (parent_locset s) -> wt_locset ls -> wt_locset (external_call_regs (parent_locset s) ls).
Proof.
...
Inductive wt_state: state -> Prop :=
  ...
  | wt_return_state: forall s rs m
        (WTSTK: wt_callstack s)
        (WTRS: wt_locset rs)
        (AGCS: agree_preserved_across_call rs (parent_locset s))
...
- (* getstack *)
  ...
  econstructor; eauto.
  eapply wt_setpair1; eauto. eapply Val.has_subtype; [eauto|apply WTRS].
...
- (* tailcall *)
  ...
  econstructor; eauto.
  eapply wt_find_function; eauto.
  apply wt_pre_return_regs; auto. apply wt_parent_locset; auto.
...
- (* return *)
  simpl in *. InvBooleans.
  econstructor; eauto.
  + apply wt_restore_auto_save_regs. apply wt_parent_locset; auto.
    apply wt_pre_return_regs; auto. apply wt_parent_locset; auto.
...
- (* external function *)
  ...
  + apply wt_setpair2. 
    ...
    apply wt_restore_auto_save_regs. apply wt_parent_locset; auto.
    apply wt_undef_callee_may_modify_regs; auto.
  + red; simpl; intros. rewrite locmap_get_set_loc_result_preserved_across_call; auto.
    unfold external_call_regs, restore_auto_save_regs, undef_callee_may_modify_regs.
    destruct l; simpl in H.
    * reg_cc_all r; rewrite HrAuto, ? HrModify.
      apply AGCS. simpl. assumption. reflexivity.
    * destruct sl; auto; congruence.
...
Lemma wt_returnstate_agree:
  forall s rs m,
  wt_state (Returnstate s rs m) ->
  agree_preserved_across_call rs (parent_locset s) /\ outgoing_undef rs.
Proof.
  intros. inv H; auto.
Qed.
