...
(** * Correspondence between load/store_stack_regs and load/store_stack_pregs *)

Fixpoint store_stack_pregs (m: mem) (sp: val) (ofs: Z) (l: list mreg) (rs: Asm.regset) :=
  match l with
  | r :: l =>
      let ty := mreg_type r in
      let sz := AST.typesize ty in
      let ofs1 := align ofs sz in
      match Mem.storev (chunk_of_type ty) m (Val.offset_ptr sp (Ptrofs.repr ofs1)) (rs (preg_of r)) with
      | Some m' => store_stack_pregs m' sp (ofs1 + sz) l rs
      | _ => None
      end
  | nil => Some m
  end.

Fixpoint load_stack_pregs (m: mem) (sp: val) (ofs: Z) (l: list mreg) (rs: Asm.regset) :=
  match l with
  | r :: l =>
      let ty := mreg_type r in
      let sz := AST.typesize ty in
      let ofs1 := align ofs sz in
      match Mem.loadv (chunk_of_type ty) m (Val.offset_ptr sp (Ptrofs.repr ofs1)) with
      | Some v => load_stack_pregs m sp (ofs1 + sz) l (rs#(preg_of r) <- v)
      | _ => None
      end
  | nil => Some rs
  end.

Lemma store_stack_regs_extends:
  forall l m1 m1' m2 sp ofs rs rs',
    agree rs sp rs' ->
    Mem.extends m1 m1' ->
    store_stack_regs m1 sp ofs l rs = Some m2 ->
    exists m2',
      store_stack_pregs m1' sp ofs l rs' = Some m2'
      /\ Mem.extends m2 m2'.
Proof.
  induction l as [ | r l]; intros until rs'; intros AG MEXT STOR.
  - exists m1'. inversion STOR. rewrite <- H0. split; auto.
  - unfold store_stack_regs in STOR. unfold store_stack_pregs.
    set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    remember (Mem.storev (chunk_of_type ty) m1 (Val.offset_ptr sp (Ptrofs.repr ofs1)) (rs r)).
    destruct o;[|discriminate].
    exploit Mem.storev_extends. apply MEXT.
    symmetry. apply Heqo.
    constructor. eapply preg_val. apply AG.
    intros [m' [STOR' MEXT']].
    rewrite STOR'. eapply IHl; eauto.
Qed.

Lemma load_stack_regs_extends:
  forall l rs1 sp rs1' m m' ofs rs2,
    agree rs1 sp rs1' ->
    Mem.extends m m' ->
    load_stack_regs m sp ofs l rs1 = Some rs2 ->
  exists rs2',
    load_stack_pregs m' sp ofs l rs1' = Some rs2'
    /\ agree rs2 sp rs2'.
Proof.
  induction l as [| r l]; intros until rs2; intros AG MEXT LOAD.
  - exists rs1'. inversion LOAD. rewrite <- H0. easy.
  - unfold load_stack_regs in LOAD. unfold load_stack_pregs.
    set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    remember (Mem.loadv (chunk_of_type ty) m (Val.offset_ptr sp (Ptrofs.repr ofs1))).
    destruct o;[|discriminate].
    exploit Mem.loadv_extends; eauto. intros [v' [A B]].
    rewrite A. eapply IHl; eauto. apply agree_set_mreg_parallel; eauto.
Qed.

Lemma store_stack_pregs_swap:
  forall l m1 (rs : regset) ofs (rs' : regset) m2 sp,
    store_stack_pregs m1 sp ofs l rs = Some m2 ->
    (forall r, In r l -> rs'#(preg_of r) = rs#(preg_of r)) ->
    store_stack_pregs m1 sp ofs l rs' = Some m2.
Proof.
  induction l as [| r l]; intros.
  - assumption.
  - unfold store_stack_pregs in *.
    set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    remember (Mem.storev (chunk_of_type ty) m1 (Val.offset_ptr sp (Ptrofs.repr ofs1)) (rs (preg_of r))).
    rewrite H0. destruct o;[|discriminate]. rewrite <- Heqo. eapply IHl; eauto.
    intros. apply H0. apply in_cons; auto. simpl.
    left. reflexivity.
Qed.

Lemma load_stack_pregs_mregs:
  forall l r m ofs rs1 rs2 sp,
    data_preg r = false \/ r = SP ->
    load_stack_pregs m sp ofs l rs1 = Some rs2 ->
    rs2#r = rs1#r.
Proof.
    induction l as [|r l]; intros.
  - inversion H0. reflexivity.
  - unfold load_stack_pregs in *.
    set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    remember (Mem.loadv (chunk_of_type ty) m (Val.offset_ptr sp (Ptrofs.repr ofs1))).
    destruct o;[|discriminate].
    exploit IHl; eauto.
    intros. rewrite H1. apply Pregmap.gso.
    red. intros. rewrite H2 in H. rewrite preg_of_data in H.
    destruct H. discriminate. destruct r; discriminate.
Qed.

Lemma load_stack_pregs_swap:
  forall l m ofs (rs1 : regset) rs2 rs1' sp,
    load_stack_pregs m sp ofs l rs1 = Some rs2 ->
    (forall r, data_preg r = true -> rs1'#r = rs1#r) ->
    exists rs2',
      load_stack_pregs m sp ofs l rs1' = Some rs2'
...
Proof.
  induction l as [|r l]; intros.
  - exists rs1'. inversion H. rewrite <- H2. easy.
  - unfold load_stack_pregs in *.
    set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    remember (Mem.loadv (chunk_of_type ty) m (Val.offset_ptr sp (Ptrofs.repr ofs1))).
    destruct o;[|discriminate].
    exploit IHl. apply H. instantiate (1:= rs1' # (preg_of r) <- v).
    intros. destruct (preg_eq (preg_of r) r0).
    rewrite e. rewrite Pregmap.gss. symmetry. apply Pregmap.gss.
    rewrite Pregmap.gso; eauto. rewrite Pregmap.gso; eauto.
    intros [rs2' [A [B C]]]. exists rs2'.
    split. assumption. split. assumption.
    intros. rewrite C; eauto. rewrite Pregmap.gso; auto.
    red; intros. rewrite H2 in H1. rewrite preg_of_data in H1. contradiction.
Qed.
