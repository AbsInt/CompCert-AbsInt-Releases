(* *********************************************************************)
(*                                                                     *)
(*              The Compcert verified compiler                         *)
(*                                                                     *)
(*          Xavier Leroy, INRIA Paris-Rocquencourt                     *)
(*                                                                     *)
(*  Copyright Institut National de Recherche en Informatique et en     *)
(*  Automatique.  All rights reserved.  This file is distributed       *)
(*  under the terms of the INRIA Non-Commercial License Agreement.     *)
(*                                                                     *)
(* *********************************************************************)

(** Pretty-printer for XTL *)

open Printf
open Camlcoq
open Datatypes
open Maps
open AST
open PrintAST
open PrintOp
open XTL

(*- E_COMPCERT_CODE_PrintXTL_mreg_001 *)
(*- #Justify_Derived "Utility function" *)
let mreg pp r =
  match Machregsnames.name_of_register r with
  | Some s -> fprintf pp "%s" s
  | None -> fprintf pp "<unknown machreg>"
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_short_name_of_type_001 *)
(*- #Justify_Derived "Utility function" *)
let short_name_of_type = function
  | Tint -> 'i'
  | Tfloat -> 'f'
  | Tlong -> 'l'
  | Tsingle -> 's'
  | Tany32 -> 'w'
  | Tany64 -> 'd'
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_short_name_of_ptype_001 *)
(*- #Justify_Derived "Utility function" *)
let short_name_of_ptype = function
  | Pptr -> 'a'
  | Ptyp t -> short_name_of_type t
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_loc_001 *)
(*- #Justify_Derived "Utility function" *)
let loc pp = function
  | Locations.PR r -> mreg pp r
  | Locations.PS(Locations.Local, ofs, ty) ->
      fprintf pp "L%c%s" (short_name_of_ptype ty) (Z.to_string ofs)
  | Locations.PS(Locations.Incoming, ofs, ty) ->
      fprintf pp "I%c%s" (short_name_of_ptype ty) (Z.to_string ofs)
  | Locations.PS(Locations.Outgoing, ofs, ty) ->
      fprintf pp "O%c%s" (short_name_of_ptype ty) (Z.to_string ofs)
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_current_alloc_001 *)
(*- #Justify_Derived "Variable for global state" *)
let current_alloc = ref (None: (var -> Locations.ploc) option)
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_current_liveness_001 *)
(*- #Justify_Derived "Variable for global state" *)
let current_liveness = ref (None: VSet.t PMap.t option)
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_reg_001 *)
(*- #Justify_Derived "Utility function" *)
let reg pp r ty =
  match !current_alloc with
  | None -> fprintf pp "x%d:%s" (P.to_int r) (PrintAST.name_of_ptype ty)
  | Some alloc -> fprintf pp "x%d:%s{%a}" (P.to_int r) (PrintAST.name_of_ptype ty) loc (alloc (V(r, ty)))
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_var_001 *)
(*- #Justify_Derived "Utility function" *)
let var pp = function
  | V(r, ty) -> reg pp r ty
  | L l -> loc pp l
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_vars_001 *)
(*- #Justify_Derived "Utility function" *)
let rec vars pp = function
  | [] -> ()
  | [r] -> var pp r
  | r1::rl -> fprintf pp "%a, %a" var r1 vars rl
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_ros_001 *)
(*- #Justify_Derived "Utility function" *)
let ros pp = function
  | Coq_inl r -> var pp r
  | Coq_inr s -> fprintf pp "\"%s\"" (extern_atom s)
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_liveset_001 *)
(*- #Justify_Derived "Utility function" *)
let liveset pp lv =
  fprintf pp "{";
  VSet.iter (function V(r, ty) -> fprintf pp " x%d" (P.to_int r)
                    | L l -> ())
    lv;
  fprintf pp " }"
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_print_succ_001 *)
(*- #Justify_Derived "Utility function" *)
let print_succ pp s dfl =
  let s = P.to_int s in
  if s <> dfl then fprintf pp "goto %d" s
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_var_pair_001 *)
(*- #Justify_Derived "Utility function" *)
let var_pair pp = function
  | Vr v -> var pp v
  | Vsplitlong (v1, v2) -> fprintf pp "(%a, %a)" var v1 var v2
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_print_instruction_001 *)
(*- #Justify_Derived "Utility function" *)
let print_instruction pp succ = function
  | Xmove(src, dst) ->
      fprintf pp "%a = %a" var dst var src
  | Xreload(src, dst) ->
      fprintf pp "%a =r %a" var dst var src
  | Xspill(src, dst) ->
      fprintf pp "%a =s %a" var dst var src
  | Xparmove(srcs, dsts, tmps) ->
      fprintf pp "(%a) = (%a) using %a" vars dsts vars srcs vars (Array.to_list tmps)
  | Xop(op, args, res) ->
      fprintf pp "%a = %a" var_pair res (print_operation var_pair) (op, args)
  | Xload(chunk, addr, args, dst) ->
      fprintf pp "%a = %s[%a]"
         var dst (name_of_chunk chunk) (print_addressing var) (addr, args)
  | Xstore(chunk, addr, args, src) ->
      fprintf pp "%s[%a] = %a"
         (name_of_chunk chunk) (print_addressing var) (addr, args) var src
  | Xcall(sg, fn, args, res) ->
      fprintf pp "%a = call %a(%a)" vars res ros fn vars args
  | Xtailcall(sg, fn, args) ->
      fprintf pp "tailcall %a(%a)" ros fn vars args
  | Xbuiltin(ef, args, res) ->
      fprintf pp "%a = %s(%a)"
        (print_builtin_res var) res
        (name_of_external ef)
        (print_builtin_args var) args
  | Xbranch s ->
      print_succ pp s succ
  | Xcond(cond, args, s1, s2) ->
      fprintf pp "if (%a) goto %d else goto %d"
        (print_condition var) (cond, args)
        (P.to_int s1) (P.to_int s2)
  | Xjumptable(arg, tbl) ->
      let tbl = Array.of_list tbl in
      fprintf pp "jumptable (%a)" var arg;
      for i = 0 to Array.length tbl - 1 do
        fprintf pp "\n\t\tcase %d: goto %d" i (P.to_int tbl.(i))
      done
  | Xreturn args ->
      fprintf pp "return %a" vars args
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_print_instructions_001 *)
(*- #Justify_Derived "Utility function" *)
let rec print_instructions pp succ = function
  | [] -> ()
  | [i] -> print_instruction pp succ i
  | i :: il ->
      print_instruction pp succ i;
      fprintf pp "; ";
      print_instructions pp succ il
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_print_block_001 *)
(*- #Justify_Derived "Utility function" *)
let print_block pp (pc, blk) =
  fprintf pp "%5d:\t" pc;
  print_instructions pp (pc - 1) blk;
  fprintf pp "\n";
  match !current_liveness with
  | None -> ()
  | Some liveness -> fprintf pp "%a\n" liveset (PMap.get (P.of_int pc) liveness)
(*- #End *)

(*- E_COMPCERT_CODE_PrintXTL_print_function_001 *)
(*- #Justify_Derived "Utility function" *)
let print_function pp ?alloc ?live f =
  current_alloc := alloc;
  current_liveness := live;
  fprintf pp "f() {\n";
  let instrs =
    List.sort
      (fun (pc1, _) (pc2, _) -> compare pc2 pc1)
      (List.map
        (fun (pc, i) -> (P.to_int pc, i))
        (PTree.elements f.fn_code)) in
  print_succ pp f.fn_entrypoint
    (match instrs with (pc1, _) :: _ -> pc1 | [] -> -1);
  List.iter (print_block pp) instrs;
  fprintf pp "}\n\n";
  current_alloc := None;
  current_liveness := None
(*- #End *)

