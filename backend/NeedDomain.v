...
Definition lowordoflong (x: nval) :=
  match x with
  | Nothing => Nothing
...
Lemma lowordoflong_sound: forall v w x,
  vagree v w (lowordoflong x) ->
  vagree (Val.lowordoflong v) (Val.lowordoflong w) x.
Proof.
  ...
  unfold lowordoflong, Val.lowordoflong; intros. destruct x; simpl in *.
- auto.
- InvAgree.
- InvAgree.
...
Definition hiwordoflong (x: nval) :=
  match x with
  | Nothing => Nothing
...
Lemma hiwordoflong_sound: forall v w x,
  vagree v w (hiwordoflong x) ->
  vagree (Val.hiwordoflong v) (Val.hiwordoflong w) x.
Proof.
  ...
  { red; intros. rewrite ! Int64.bits_hiword by auto.
    ...
    assumption. }
  unfold hiwordoflong, Val.hiwordoflong; intros. destruct x; simpl in *.
- auto.
- InvAgree.
- InvAgree.
- InvAgree. apply Val.lessdef_same. f_equal. apply iagree_mone. auto.
...
Lemma select_bool_sound:
  forall v1 v2 v3 w1 w2 w3 ty x,
  vagree v1 w1 All -> vagree v2 w2 x -> vagree v3 w3 x ->
  vagree (Val.select_bool v1 v2 v3 ty) (Val.select_bool w1 w2 w3 ty) x.
Proof.
  unfold Val.select_bool; intros.
  inv H. apply select_sound; auto. apply vagree_lessdef.
  auto.
Qed.
