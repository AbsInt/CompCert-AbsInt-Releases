...
(** [contains_callee_saves j sp pos rl ls] is a memory assertion that holds
  ...
  callee-save registers [pl] as given by the location map [ls],
...
Proof.
  ...
  destruct (is_caller_save r); auto.
Qed.
...
- rewrite H0; auto. unfold mreg_within_bounds in H. unfold preserved_across_call, mreg_preserved_across_call.
  destruct (is_caller_save r) eqn:Er; [|reflexivity]. 
  contradiction H. intros. apply reg_callee_not_caller in H1; congruence.
- rewrite <- agree_incoming0 by auto. apply H0. reflexivity. 
Qed.
...
Proof.
  ...
  - set (sz := compute_size p) in *.
    ...
    destruct p; simpl; try split; apply load_result_inject; eauto.
    intros [m0 [STORE SEP']].
...
Proof.
  ...
  split. red; unfold LTL.pre_return_regs; intros.
    destruct (is_callee_save r) eqn:C; auto.
...
- (* external function *)
  ...
  apply agree_regs_restore_auto_save_regs. 
  apply agree_regs_undef_caller_save_regs.
  apply agree_regs_inject_incr with j; auto.
  inv STACKS. simpl. unfold agree_regs. intros. econstructor. simpl.
  apply agree_regs_inject_incr with j; auto.
  auto.
