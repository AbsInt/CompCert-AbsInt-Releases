...
Proof.
  ...
  - set (ty := mreg_type r) in *.
    ...
    apply range_drop_left with (mid := pos1) in RAN; [ | lia ].
    apply range_split with (mid := pos1 + sz) in RAN; [ | lia].
    unfold sz at 1 in RAN. rewrite <- size_type_chunk in RAN.
    apply range_contains in RAN; auto.
    exploit store_rule; eauto. apply load_result_inject; eauto.
    intros [m0 [STO SEP]].
    rewrite Ptrofs.add_zero_l, Ptrofs.unsigned_repr, STO.
    exploit IHl; eauto.
    rewrite sep_swap in SEP. apply SEP.
    intros (m' & A & B & C).
    exists m'. split; auto.
    split. apply sep_swap in B. apply sep_assoc. assumption.
    intros. apply C. eapply Mem.perm_store_1; eauto.
    apply sep_proj1 in RAN. simpl in RAN. apply proj1 in RAN. assumption.
Qed.
...
Remark destroyed_by_setstack_map_function_entry:
  forall l,
    incl (flat_map destroyed_by_setstack l) destroyed_at_function_entry.
Proof.
  induction l as [| ty l].
  - apply incl_nil_l.
  - simpl. apply incl_app. apply destroyed_by_setstack_function_entry.
    apply IHl.
Qed.
...
Remark callee_save_not_destroyed:
  forall l r,
    ...
    ~ In r (flat_map destroyed_by_setstack mreg_type ## l).
Proof.
  intros.
...
Proof.
  ...
  assert(AG': agree_regs j ls1 rs1).
  { apply agree_regs_undef_regs. apply agree_regs_call_regs. assumption. }
  unfold save_callee_save. remember (fe_used_callee_save fe) as l. destruct l.
  - exists rs1, m.
    split. apply star_refl.
    split. replace (used_callee_save b) with (nil : list mreg) by easy.
    simpl. apply sep_pure. split. auto. apply (sep_proj2 _ _ _ SEP).
    split; auto.
  - exploit (store_callee_saves_correct j sp ls1).
    intros. apply undef_regs_type. apply TY.
    apply b.(used_callee_save_prop).
    eexact SEP.
    apply AG'.
    clear SEP. intros [m' [STOR [SEP PERMS]]].
    econstructor. exists m'.
    split. apply star_one. econstructor. rewrite Heql. exact STOR.
    intros.
    apply callee_save_not_destroyed. eapply used_callee_save_prop. rewrite Heql in H. apply H.
    rewrite Heql.
    reflexivity.
    split. rewrite (contains_callee_saves_exten j sp ls0 ls1). assumption.
    ...
    assert (existsb is_callee_save destroyed_at_function_entry = true)
      by (apply existsb_exists; exists r; auto).
    congruence.
    split. assumption.
    red; intros.
    destruct (In_dec mreg_eq r (flat_map destroyed_by_setstack mreg_type ## (fe_used_callee_save fe))).
    unfold ls1. rewrite LTL_undef_regs_same. constructor. eapply destroyed_by_setstack_map_function_entry; eauto.
...
Lemma load_callee_saves_correct:
  ...
  exists rs',
    load_stack_regs m (Vptr sp Ptrofs.zero) ofs l rs = Some rs'
...
Proof.
  ...
  induction l as [ | r l]; simpl; intros until rs; intros CS AG BND.
  - exists rs.
  ...
  - set (ty := mreg_type r) in *.
    ...
    exploit contains_get_stack. apply CS. unfold load_stack. simpl. rewrite Ptrofs.add_zero_l.
    intros [v [LD INJ]].
    rewrite LD.
    exploit (IHl (ofs1 + sz) (rs#r <- v)); eauto.
    apply (sep_proj2 _ _ _ CS).
    red; intros. rewrite Regmap.gso; intuition.
    intros [rs' [A [B [C D]]]].
    exists rs'.
    split. assumption. split.
    intros. destruct H. destruct (In_dec mreg_eq r0 l).
    apply B. assumption. rewrite C;[|assumption]. rewrite <- H. rewrite Regmap.gss. assumption.
    apply B. assumption.
    intros.
    split. intuition. rewrite C. rewrite Regmap.gso. reflexivity. intuition. assumption.
    assumption.
Qed.
...
Proof.
  ...
  remember (fe_used_callee_save fe) as l. destruct l.
  - exists rs.
    split. apply star_refl.
    split. intros. apply H0. intuition. apply H2 in H1.
    replace (used_callee_save b) with (nil : list mreg) in H1 by easy. contradiction.
    intros. reflexivity.
  - unfold frame_contents, frame_contents_1 in H.
    apply mconj_proj1 in H. rewrite ! sep_assoc in H. apply sep_pick5 in H.
    exploit load_callee_saves_correct.
    eexact H.
    eexact H0.
    intros. unfold mreg_within_bounds; auto.
    intros [rs' [LOAD [INJ [UNU AG]]]].
    exists rs'.
    split.
    apply star_one. econstructor. rewrite Heql. apply LOAD.
    split; intros.
    destruct (In_dec mreg_eq r (used_callee_save b)). apply INJ; auto.
    rewrite UNU by auto. apply H0. unfold mreg_within_bounds; tauto.
    apply UNU. red; intros. apply (used_callee_save_prop b) in H2. congruence.
Qed.
...
Proof.
  intros. unfold save_callee_save. destruct (fe_used_callee_save fe); reflexivity.
Qed.
...
Proof.
  intros. unfold restore_callee_save. destruct (fe_used_callee_save fe); reflexivity.
Qed.
...
- rewrite transl_code_eq.
  ...
  destruct s; simpl; auto.
  simpl. rewrite find_label_restore_callee_save. auto.
...
Proof.
  ...
  destruct s; auto with coqlib.
  unfold restore_callee_save. destruct (fe_used_callee_save fe); auto with coqlib.
  unfold restore_callee_save. destruct (fe_used_callee_save fe); auto with coqlib.
Qed.
...
Proof.
  ...
  destruct (fe_used_callee_save (make_env (function_bounds f))).
  apply is_tail_transl_code. assumption.
  apply is_tail_cons. apply is_tail_transl_code. assumption.
Qed.
