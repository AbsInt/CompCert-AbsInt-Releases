...
Definition contains_rpair (sp: block) (pos: Z) (p: (rpair mreg)) (ls:locset) (spec: mreg -> val -> Prop): massert :=
  match p with
  | One r => contains (chunk_of_type (mreg_type r)) sp pos (spec r)
  | Two r1 r2 => let rhi := if Archi.big_endian then r2 else r1 in
                let rlo := if Archi.big_endian then r1 else r2 in
                contains (chunk_of_type (mreg_type rlo)) sp pos (spec rlo)
                ** contains (chunk_of_type (mreg_type rhi)) sp (pos + AST.typesize (mreg_type rlo)) (spec rhi)
  end.

(** [contains_callee_saves j sp pos pl ls] is a memory assertion that holds
  if block [sp], starting at offset [pos], contains the values of the
  callee-save registers pairs [pl] as given by the location map [ls],
  up to the memory injection [j].  The memory layout of the registers in [pl]
  is the same as that implemented by [save_callee_save_rec]. *)

Fixpoint contains_callee_saves (j: meminj) (sp: block) (pos: Z) (pl: list (rpair mreg)) (ls: locset) : massert :=
  match pl with
  | nil => pure True
  | p :: pl => let sz := compute_size p in
              let pos1 := align pos sz in
              contains_rpair sp pos1 p ls (fun r v => Val.inject j (ls (R r)) v)
              ** contains_callee_saves j sp (pos1 + sz) pl ls
  end.
...
Proof.
  induction rl as [ | p1 rl]; simpl; intros.
- reflexivity.
- destruct p1; unfold contains_rpair. apply sepconj_morph_1; auto. apply contains_imp. eauto.
  repeat apply sepconj_morph_1; auto; apply contains_imp; eauto.
Qed.

Lemma contains_callee_saves_exten:
  forall j sp ls ls' pl pos,
  (forall r, In r (regs_of_rpairs pl) -> (ls' (R r)) = (ls (R r))) ->
  massert_eqv (contains_callee_saves j sp pos pl ls)
              (contains_callee_saves j sp pos pl ls').
Proof.
  induction pl as [ | p1 pl]; simpl; intros.
- reflexivity.
- destruct p1.
  + apply sepconj_morph_2; eauto. simpl. rewrite H; simpl; auto. eapply IHpl. intros. apply H. apply in_app; auto.
  + apply sepconj_morph_2; eauto. simpl. rewrite H; simpl; [|destruct Archi.big_endian; intuition].
    apply sepconj_morph_2; eauto. rewrite H; simpl; simpl;[auto|destruct Archi.big_endian; intuition].
    eapply IHpl. intros. apply H. apply in_app; auto.
Qed.

Lemma contains_callee_saves_app:
  forall l1 l2 j sp ls pos m P,
    m |= contains_callee_saves j sp pos l1 ls **
      contains_callee_saves j sp (size_callee_save_area_rec l1 pos) l2 ls ** P <->
    m |= contains_callee_saves j sp pos (l1 ++ l2) ls ** P.
Proof.
  Local Opaque contains sepconj.
  induction l1; intros.
  - split; intros.
    apply sep_drop in H. auto.
    simpl in *. apply sep_pure; auto.
  - split; intros.
    + rewrite <- app_comm_cons. simpl.
      simpl in H. rewrite sep_assoc, sep_swap.
      rewrite <- IHl1; eauto.
      rewrite sep_swap3, sep_swap23, <- sep_assoc. apply H.
    + simpl in *. rewrite sep_assoc, sep_swap in H.
      rewrite !sep_assoc, sep_swap, sep_swap23. rewrite IHl1; auto.
Qed.
 ...
 ** (if tf.(fn_retaddr_saved) then hasvalue Mptr sp fe.(fe_ofs_retaddr) retaddr else pure True)
 ** contains_callee_saves j sp fe.(fe_ofs_callee_save) (flatten_group_list fe.(fe_used_callee_save)) ls0.
...
Proof.
  ...
  erewrite  <- contains_callee_saves_exten with (ls := ls0) (ls' := ls0'); cycle 1.
  subst fe. intros. eapply H1, callee_save_correspond. simpl in H3. eapply H3.
  assumption.
Qed.
...
Record agree_locs (ls ls0: locset) : Prop :=
  mk_agree_locs {
    ...
    (** Unused callee-save registers have the same value as in the caller. *)
    agree_unused_reg:
...
Lemma agree_regpair:
  forall j ls rs p,
    agree_regs j ls rs ->
    Val.inject j (Locmap.getpair (map_rpair R p) ls) (get_pair p rs).
Proof.
  intros. destruct p. apply agree_reg; auto.
  simpl. apply Val.combine_inject; apply agree_reg; auto.
Qed.

Lemma agree_regpairlist:
  forall j ls rs rl,
    agree_regs j ls rs -> Val.inject_list j (rpairlist ls rl) (get_pairs rl rs).
Proof.
  induction rl; simpl; intros.
  auto. constructor; auto using agree_regpair.
Qed.

Hint Resolve agree_reg agree_reglist agree_regpair: stacking.
...
Lemma agree_regs_undef_callee_may_modify_regs:
  forall j ls rs,
  agree_regs j ls rs ->
  agree_regs j (LTL.undef_callee_may_modify_regs ls) (Mach.undef_callee_may_modify_regs rs).
Proof.
  intros; red; intros.
  unfold LTL.undef_callee_may_modify_regs, Mach.undef_callee_may_modify_regs.
  destruct (is_modifiable_by_callee r); auto.
Qed.

Lemma agree_regs_restore_auto_save_regs:
  forall j caller1 caller2 ls rs,
  agree_regs j ls rs ->
  agree_regs j caller1 caller2 ->
  agree_regs j (LTL.restore_auto_save_regs caller1 ls) (restore_auto_save_regs caller2 rs).
Proof.
  unfold agree_regs. intros.
  unfold LTL.restore_auto_save_regs, restore_auto_save_regs.
...
Lemma agree_locs_set_pair1:
  forall ls ls0 p v,
  agree_locs ls ls0 ->
  forall_rpair (mreg_within_bounds b) p ->
  agree_locs (Locmap.setpair p v ls) ls0.
Proof.
  destruct p; simpl; [intros| (intros v H [H1 H2])]; auto using agree_locs_set_reg.
Qed.

Lemma non_callee_save_reg_within_bounds:
  forall r,
...
Lemma agree_locs_set_res:
  forall ls0 res v ls,
  agree_locs ls ls0 ->
  (forall r, In r (regs_of_rpairs (params_of_builtin_res res)) -> mreg_within_bounds b r) ->
  agree_locs (Locmap.setres res v ls) ls0.
Proof.
  induction res; simpl; intros.
- rewrite app_nil_r in H0. eapply agree_locs_set_pair1; eauto using regs_in_rpair_forall .
- auto.
- apply IHres2; [apply IHres1; auto|]; intros; apply H0; auto; rewrite regs_of_rpairs_app; auto using in_or_app.
Qed.
...
Proof.
  ...
  intros. apply non_callee_save_reg_within_bounds. auto.
Qed.

Lemma agree_locs_undef_locs:
  forall ls0 regs ls,
  agree_locs ls ls0 ->
  existsb (fun r => is_callee_save r) regs = false ->
  agree_locs (LTL.undef_regs regs ls) ls0.
Proof.
  intros. eapply agree_locs_undef_locs_1; eauto.
  intros. destruct (is_callee_save r) eqn:CS; auto.
  assert (existsb (fun r => is_callee_save r) regs = true).
  { apply existsb_exists. exists r. rewrite CS; auto. }
  congruence.
Qed.
...
Lemma agree_locs_return:
  forall ls ls0 ls',
  agree_locs ls ls0 ->
  agree_preserved_across_call ls' ls ->
  agree_locs ls' ls0.
Proof.
  intros. red in H0. inv H; constructor; auto; intros.
- rewrite H0; auto. simpl. unfold mreg_within_bounds in H.
  reg_cc_all r; auto. contradiction H. congruence.
- rewrite <- agree_incoming0 by auto. apply H0. simpl. reflexivity.
Qed.

(** ** Properties of destroyed registers. *)

Section DESTROYED_REGS.

Definition no_callee_saves (l: list mreg) : Prop :=
  existsb (fun r => is_callee_save r) l = false.
...
Remark destroyed_by_setstackpair_function_entry:
  forall p, incl (destroyed_by_setstack_pair p) destroyed_at_function_entry.
Proof.
Local Transparent destroyed_by_setstack destroyed_at_function_entry.
  unfold incl; destruct p; simpl.
  destruct (mreg_type r); tauto.
  destruct (mreg_type rhi), (mreg_type rlo); simpl; tauto.
Qed.
...
Remark ptrofs_add_repr:
  forall a b, Ptrofs.add (Ptrofs.repr a) (Ptrofs.repr b) = Ptrofs.repr (a + b).
Proof.
    intros. apply Ptrofs.eqm_samerepr. apply Ptrofs.eqm_add; apply Ptrofs.eqm_sym; apply Ptrofs.eqm_unsigned_repr.
Qed.
...
Lemma contains_rpair_save_callee_pair:
  forall (spec: mreg -> val -> Prop) spec1 m sp ofs P ls p rs,
    m |= contains_rpair sp ofs p ls spec1 ** P ->
    forall_rpair (fun r => spec r (Val.load_result (chunk_of_type (mreg_type r)) (rs r))) p ->
    exists m',
      save_callee_pair m (Vptr sp Ptrofs.zero) ofs p rs = Some m'
    /\ m' |= contains_rpair sp ofs p ls spec ** P.
Proof.
  Local Opaque chunk_of_type.
  intros. destruct p; simpl in *.
  - eapply contains_set_stack; eauto.
  - set (hireg := if Archi.big_endian then rlo else rhi) in *.
    ...
    exploit (contains_set_stack (spec loreg)).
    eapply H. instantiate (1:= (rs loreg)). unfold loreg. destruct Archi.big_endian; eauto.
    intros (m' & A & B). clear H.
    rewrite sep_swap in B.
    exploit (contains_set_stack (spec hireg)).
    eapply B. instantiate (1:= (rs hireg)). unfold hireg. destruct Archi.big_endian; eauto.
    intros (m'' & C & D). rewrite sep_swap in D.
    exists m''. rewrite A. split. rewrite ptrofs_add_repr. apply C.
    rewrite sep_assoc. apply D.
Qed.

Lemma range_contains_rpair:
  forall b ofs p P ls m,
  m |= range b ofs (ofs + compute_size p) ** P ->
  (compute_size p | ofs) ->
  pair_wf p ->
  m |= contains_rpair b ofs p ls (fun r v => True) ** P.
Proof.
  Local Opaque chunk_of_type mreg_type.
  intros. destruct p; simpl in *.
  - assert (align_chunk (chunk_of_type (mreg_type r)) | ofs).
    { eapply Z.divide_transitive; eauto.
      try rewrite <- size_type_chunk; auto with divide; eauto using align_size_chunk_divides.
    }
    rewrite <- size_type_chunk in *.  eapply range_contains; eauto.
  - set (hireg := if Archi.big_endian then rlo else rhi) in *.
    set (loreg := if Archi.big_endian then rhi else rlo) in *.
    destruct H1.
    assert (mreg_type loreg = mreg_type hireg). { unfold loreg, hireg. destruct Archi.big_endian; auto. }
    assert (mreg_type loreg = mreg_type rlo). { unfold loreg. destruct Archi.big_endian; auto. }
    assert (align_chunk (chunk_of_type (mreg_type loreg)) | ofs).
    { eapply Z.divide_transitive; eauto. rewrite H1, H4.
      apply Z.divide_add_r; rewrite <- size_type_chunk; eauto using align_size_chunk_divides.
    }
    rewrite <- !size_type_chunk, H1, <- H3, H4 in *.
    rewrite Z.add_assoc in H.
    apply range_split with (mid := ofs + AST.typesize (mreg_type rlo)) in H.
    rewrite <- !size_type_chunk in H.
    exploit range_contains; eauto. clear H.
    intros. apply sep_swap in H. exploit range_contains; eauto.
    eapply Z.divide_add_r; eauto using align_size_chunk_divides.
    rewrite sep_swap. rewrite sep_assoc. auto.
    rewrite !size_type_chunk. assert (AST.typesize (mreg_type rlo) > 0) by apply AST.typesize_pos.
    lia.
Qed.
...
Hypothesis ls_temp_undef:
  forall p r, In r (destroyed_by_setstack_pair p) -> ls (R r) = Vundef.

Hypothesis wt_ls: forall r, Val.has_type (ls (R r)) (mreg_type r).

Lemma store_stack_regs_correct:
  forall l pos m rs P,
  (forall r, In r (regs_of_rpairs l) -> is_callee_save r = true) ->
  m |= range sp pos (size_callee_save_area_rec l pos) ** P ->
  agree_regs j ls rs ->
  (forall p, In p l -> pair_wf p) ->
  exists m',
    store_stack_rpairs m (Vptr sp Ptrofs.zero) pos l rs = Some m'
  /\ m' |= contains_callee_saves j sp pos l ls ** P
  /\ (forall ofs k p, Mem.perm m sp ofs k p -> Mem.perm m' sp ofs k p).
Proof.
  Local Opaque mreg_type.
  induction l as [| p l]; simpl; intros until P; intros CS SEP AG WF.
  - exists m. simpl. intuition. rewrite sep_pure; split; auto. eapply sep_drop; eauto.
  - set (sz := compute_size p) in *.
    set (pos1 := align pos sz) in *.
    assert (SZPOS: sz > 0) by (apply compute_size_pos).
    ...
    assert (AL1: (sz | pos1)) by (apply align_divides; apply compute_size_pos).
    apply range_drop_left with (mid := pos1) in SEP; [ | lia ].
    apply range_split with (mid := pos1 + sz) in SEP; [ | lia].
    unfold sz at 1 in SEP. eapply range_contains_rpair with (ls := ls) in SEP; eauto.
    exploit (contains_rpair_save_callee_pair (fun r v' => Val.inject j (ls (R r)) v')); eauto.
    destruct p; simpl; try split; apply Val.load_result_inject'; eauto.
    intros [m0 [STORE SEP']].
    rewrite STORE. exploit IHl; eauto with datatypes.
    rewrite sep_swap in SEP'. apply SEP'.
    intros (m' & A & B & C).
    exists m'. split; auto.
    split. apply sep_swap in B. apply sep_assoc. assumption.
    intros. apply C. unfold save_callee_pair, store_stack in STORE; simpl in STORE.
    destruct p; [| destruct (Archi.big_endian); Destructor; try discriminate]; repeat (eapply Mem.perm_store_1; eauto).
    destruct (zle (Ptrofs.unsigned (Ptrofs.add Ptrofs.zero (Ptrofs.repr pos1)) + size_chunk (chunk_of_type (mreg_type r))) Ptrofs.modulus); try discriminate; eauto.
Qed.

Lemma undef_list_agree:
  forall l ls rs j,
    agree_regs j ls rs ->
    (forall r1, In r1 l -> (ls (R r1)) = Vundef) ->
    agree_regs j ls (undef_regs l rs).
Proof.
  induction l; intros; simpl; auto.
  intro. destruct (mreg_eq r a).
  - subst r. rewrite H0; auto with datatypes.
  - rewrite Regmap.gso; auto. apply IHl; auto with datatypes.
Qed.

Remark destroyed_by_setstack_map_function_entry:
  forall l,
    incl (flat_map destroyed_by_setstack_pair l) destroyed_at_function_entry.
Proof.
  induction l as [| p l].
  - apply incl_nil_l.
  - simpl. apply incl_app. apply destroyed_by_setstackpair_function_entry. apply IHl.
Qed.

Remark callee_save_not_destroyed:
  forall l r,
    is_callee_save r = true ->
    ~ In r (flat_map destroyed_by_setstack_pair l).
Proof.
  intros.
  red. intros. apply destroyed_by_setstack_map_function_entry in H0.
  assert (exists x, In x destroyed_at_function_entry /\ is_callee_save x = true).
  { exists r. rewrite H; auto. } rewrite <- existsb_exists in H1.
  contradict H1. rewrite not_true_iff_false. apply destroyed_at_function_entry_caller_save.
Qed.

Lemma save_callee_save_rec_correct:
  forall k (l: list mreg_group) pos rs m P,
  (forall r, In r (regs_of_rpairs (flatten_group_list l)) -> is_callee_save r = true) ->
  (forall p, In p (flatten_group_list l) -> pair_wf p) ->
  m |= range sp pos (size_callee_save_area_rec (flatten_group_list l) pos) ** P ->
  agree_regs j ls rs ->
  exists rs', exists m',
     star step tge
        (State cs fb (Vptr sp Ptrofs.zero) (save_callee_save_rec l pos k) rs m)
     E0 (State cs fb (Vptr sp Ptrofs.zero) k rs' m')
  /\ m' |= contains_callee_saves j sp pos (flatten_group_list l) ls ** P
  /\ (forall ofs k p, Mem.perm m sp ofs k p -> Mem.perm m' sp ofs k p)
  /\ agree_regs j ls rs'.
Proof.
Local Opaque mreg_type.
  induction l as [ | r l]; simpl; intros until P; intros CS WF SEP AG.
- exists rs, m.
...
- destruct r as [r | rl].
  + simpl in SEP.
    ...
    assert (SZREC: pos1 + sz <= size_callee_save_area_rec (flatten_group_list l) (pos1 + sz)) by (apply size_callee_save_area_rec_incr).
    assert (POS1: pos <= pos1) by (apply align_le; auto).
    ...
    simpl in SEP. apply range_drop_left with (mid := pos1) in SEP; [ | lia ].
    apply range_split with (mid := pos1 + sz) in SEP; [ | lia ].
    ...
    eexact SEP.
    apply Val.load_result_inject'; [auto|apply wt_ls].
    clear SEP; intros (m1 & STORE & SEP).
    set (rs1 := undef_regs (destroyed_by_setstack_pair (One r)) rs).
    assert (AG1: agree_regs j ls rs1).
    { red; intros. unfold rs1. destruct (In_dec mreg_eq r0 (destroyed_by_setstack_pair (One r))).
      erewrite ls_temp_undef by eauto. auto.
      rewrite undef_regs_other by auto. apply AG. }
    rewrite sep_swap in SEP. simpl in CS.
    exploit (IHl (pos1 + sz) rs1 m1); eauto.
    auto with datatypes.
    intros (rs2 & m2 & A & B & C & D).
    exists rs2, m2.
    split. eapply star_left; eauto. constructor. exact STORE. auto. traceEq.
    split. simpl. rewrite sep_assoc, sep_swap. exact B.
    split. intros. apply C. unfold store_stack in STORE; simpl in STORE. eapply Mem.perm_store_1; eauto.
    destruct (zle (Ptrofs.unsigned (Ptrofs.add Ptrofs.zero (Ptrofs.repr pos1)) + size_chunk (chunk_of_type ty)) Ptrofs.modulus); try discriminate; eauto.
    auto.
  + rewrite size_callee_save_area_rec_app in SEP.
    set (pos1 := size_callee_save_area_rec rl pos) in *.
    set (rs1 := undef_regs (flat_map destroyed_by_setstack_pair rl) rs) in *.
    assert (POS: pos <= pos1 <= size_callee_save_area_rec (flatten_group_list l) pos1) by eauto using size_callee_save_area_rec_incr.
    rewrite regs_of_rpairs_app in CS. eapply range_split in SEP; eauto.
    subst pos1. exploit (store_stack_regs_correct rl); eauto with datatypes.
    intros (m1 & STORE & SEP1 & PERM).
    rewrite sep_swap in SEP1. clear SEP.
    exploit (IHl (size_callee_save_area_rec rl pos) rs1 m1); eauto with datatypes.
    subst rs1. apply undef_list_agree; eauto.
    intros. eapply in_flat_map in H. destruct H as (x & A & B). eapply ls_temp_undef; eauto.
    intros (rs2 & m2 & A & B & C & D).
    exists rs2, m2.
    split; eauto. eapply star_left; eauto. constructor; eauto.
    intros. eapply callee_save_not_destroyed; eauto. auto with datatypes.
    eauto. split; eauto.
    rewrite <- contains_callee_saves_app; eauto. eapply sep_swap in B; eauto.
Qed.
...
Lemma save_callee_save_correct:
  ...
  m |= range sp fe.(fe_ofs_callee_save) (size_callee_save_area fe fe.(fe_ofs_callee_save)) ** P ->
  ...
  exists rs', exists m',
     ...
     E0 (State cs fb (Vptr sp Ptrofs.zero) k rs' m')
  /\ m' |= contains_callee_saves j sp fe.(fe_ofs_callee_save) (flatten_group_list fe.(fe_used_callee_save)) ls0 ** P
  /\ (forall ofs k p, Mem.perm m sp ofs k p -> Mem.perm m' sp ofs k p)
  /\ agree_regs j ls1 rs'.
Proof.
  Local Opaque split_used_callee_save.
  intros until P; intros SEP TY AGCS AG; intros ls1 rs1.
  exploit (save_callee_save_rec_correct j cs fb sp ls1).
- intros. unfold ls1. apply LTL_undef_regs_same. eapply destroyed_by_setstackpair_function_entry; eauto.
- intros. unfold ls1. apply undef_regs_type. apply TY.
- intros. eapply b.(used_callee_save_prop). eapply callee_save_correspond; eauto.
- simpl. intros. eapply callee_save_wellformed; eauto.
- eapply SEP.
- instantiate (1 := rs1). apply agree_regs_undef_regs. apply agree_regs_call_regs. auto.
- clear SEP. intros (rs' & m' & EXEC & SEP & PERMS & AG').
  exists rs', m'.
  split. subst fe. unfold save_callee_save. simpl. eexact EXEC.
  split. rewrite (contains_callee_saves_exten j sp ls0 ls1). exact SEP.
  intros. assert (is_callee_save r = true) by (eapply b.(used_callee_save_prop), callee_save_correspond, H).
    unfold ls1. rewrite LTL_undef_regs_others. unfold call_regs.
    apply AGCS; auto.
    red; intros.
    assert (existsb (fun r => is_callee_save r) destroyed_at_function_entry = false)
       by  (apply destroyed_at_function_entry_caller_save).
    assert (existsb (fun r => is_callee_save r) destroyed_at_function_entry = true).
    { apply existsb_exists. exists r. rewrite H0; auto. }
    congruence.
...
Lemma function_prologue_correct:
  ...
  /\ (if tf.(fn_retaddr_saved)
      then store_stack m3' (Vptr sp' Ptrofs.zero) Tptr tf.(fn_retaddr_ofs) ra = Some m4'
      else m4' = m3')
  /\ star step tge
...
Local Opaque b fe.
  ...
  apply (range_contains Mptr) in SEP; [|tauto].
  assert (HRA: exists m4', 
                 (if tf.(fn_retaddr_saved) then store_stack m3' (Vptr sp' Ptrofs.zero) Tptr (Ptrofs.repr fe.(fe_ofs_retaddr)) ra = Some m4' else m4' = m3')
               /\ m4' |= (if tf.(fn_retaddr_saved) then contains (chunk_of_type Tptr) sp' (fe_ofs_retaddr fe) (fun v' : val => v' = ra) else pure True) **
  ...
  { eapply frame_mconj. eexact SEPCONJ.
    ...
    { intros. apply PERMS.
      ...
      - subst m4'. eauto using Mem.perm_store_1 with mem. }
...
Lemma contains_rpair_restore_callee_save:
  forall spec m sp ofs p (rs: regset) ls,
  m |= contains_rpair sp ofs p ls spec ->
  pair_wf p ->
  exists (rs':regset),
    restore_callee_pair m (Vptr sp Ptrofs.zero) ofs p rs = Some rs'
    /\ forall_rpair (fun r => spec r (rs' r)) p
    /\ forall r, ~ In r (regs_of_rpair p) -> rs r = rs' r.
Proof.
  intros. destruct p; unfold contains_rpair in H.
  - exploit contains_get_stack; eauto.
    intros (v & LOAD & SPEC). exists (rs # r <- v).
    split. simpl. rewrite LOAD. auto.
    split. simpl. rewrite Regmap.gss. auto.
    simpl. intros. rewrite Regmap.gso; auto.
  - unfold restore_callee_pair.
    set (hireg := if Archi.big_endian then rlo else rhi) in *.
    set (loreg := if Archi.big_endian then rhi else rlo) in *.
    exploit contains_get_stack; eauto. eapply sep_drop. eapply sep_comm. eapply H.
    intros (v1 & LOAD1 & SPEC1).
    exploit contains_get_stack; eauto. eapply sep_drop. eapply H.
    intros (v2 & LOAD2 & SPEC2). exists (rs # hireg <- v2) # loreg <- v1.
    rewrite LOAD1, ptrofs_add_repr, LOAD2.
    split. auto.
    split. simpl. unfold pair_wf in H0.
      destruct Archi.big_endian; rewrite Regmap.gss; rewrite Regmap.gso; try rewrite Regmap.gss; intuition auto.
    simpl. intros. rewrite !Regmap.gso; auto; (unfold loreg, hireg; destruct Archi.big_endian; auto).
Qed.
...
Lemma load_callee_saves_correct:
  forall l ofs rs,
  m |= contains_callee_saves j sp ofs l ls0 ->
  agree_unused ls0 rs ->
  (forall p, In p l -> forall_rpair (mreg_within_bounds b) p) ->
  (forall p, In p l -> pair_wf p) ->
  exists rs',
    load_stack_rpairs m (Vptr sp Ptrofs.zero) ofs l rs = Some rs'
  /\ (forall r, In r (regs_of_rpairs l) -> Val.inject j (ls0 (R r)) (rs' r))
  /\ (forall r, ~(In r (regs_of_rpairs l)) -> rs' r = rs r)
  /\ agree_unused ls0 rs'.
Proof.
  Local Opaque mreg_type.
  Local Opaque Mem.loadv.
  induction l as [ | p l]; simpl; intros until rs; intros CS AG BND WF.
  - exists rs.
    repeat (split; try (rewrite RES); auto).
  - set (sz := compute_size p) in *.
    set (ofs1 := align ofs sz) in *.
    assert (BOUND: forall_rpair (mreg_within_bounds b) p) by auto.
    exploit contains_rpair_restore_callee_save; eauto. eapply sep_proj1; eauto.
    intros (rs' & LOAD & SPEC & RS').
    exploit (IHl (ofs1 + sz) rs'); eauto. eapply CS.
    red; intros. rewrite <- RS'. auto. red; intros. apply H; auto. eapply forall_regs_in_rpair; eauto.
    intros (rs'' & A & B & C & D). rewrite LOAD.
    exists rs''.
    split. eauto.
    split. intros. apply in_app in H.
    destruct (In_dec mreg_eq r (regs_of_rpairs l)).
    apply B; auto. destruct H; try contradiction.
    rewrite C; auto. eapply forall_regs_in_rpair in H; eauto. apply H.
    split. intros. rewrite C, RS'; eauto with datatypes.
    exact D.
Qed.

Lemma restore_callee_save_rec_correct:
  forall l ofs rs k,
  m |= contains_callee_saves j sp ofs (flatten_group_list l) ls0 ->
  agree_unused ls0 rs ->
  (forall p, In p (flatten_group_list l) -> forall_rpair (mreg_within_bounds b) p) ->
  (forall p, In p (flatten_group_list l) -> pair_wf p) ->
  exists rs',
    star step tge
      (State cs fb (Vptr sp Ptrofs.zero) (restore_callee_save_rec l ofs k) rs m)
   E0 (State cs fb (Vptr sp Ptrofs.zero) k rs' m)
  /\ (forall r, In r (regs_of_rpairs (flatten_group_list l)) -> Val.inject j (ls0 (R r)) (rs' r))
  /\ (forall r, ~(In r (regs_of_rpairs (flatten_group_list l))) -> rs' r = rs r)
  /\ agree_unused ls0 rs'.
Proof.
Local Opaque mreg_type.
...
- (* inductive case *)
  ...
  + set (ty := mreg_type r) in *.
    ...
    assert (BOUND: mreg_within_bounds b r) by (specialize H1 with (One r); simpl in H1; auto).
    exploit contains_get_stack.
      eapply sep_proj1; eassumption.
    intros (v & LOAD & SPEC).
    exploit (IHl (ofs1 + sz) (set_pair (One r) v rs)); auto with datatypes.
      eapply sep_proj2; eassumption.
      red; intros. simpl. rewrite Regmap.gso. auto. intuition congruence.
    intros (rs' & A & B & C & D).
    exists rs'.
    split. eapply star_step; eauto.
      econstructor. exact LOAD. traceEq.
    simpl. split. intros.
      destruct (In_dec mreg_eq r0 (regs_of_rpairs (flatten_group_list l))). auto.
      assert (r = r0) by tauto. subst r0.
      rewrite C by auto. simpl. rewrite Regmap.gss. exact SPEC.
    split. intros.
      rewrite C by tauto. apply Regmap.gso. intuition auto.
    exact D.
  + eapply conj with (A := True) in H; auto. rewrite <- sep_pure, sep_comm in H. rewrite <- contains_callee_saves_app in H.
    exploit (load_callee_saves_correct l0); eauto with datatypes.
    eapply sep_proj1; eauto.
    intros (rs1 & LOAD & A & B & C). set (ofs1 := size_callee_save_area_rec l0 ofs) in *.
    exploit (IHl ofs1 rs1); eauto with datatypes. eapply H.
    intros (rs2 & D & E & F & G).
    exists rs2.
    split. eapply star_step; eauto.
    econstructor; eauto. eauto. split. intros.
      destruct (In_dec mreg_eq r (regs_of_rpairs (flatten_group_list l))). auto.
      rewrite regs_of_rpairs_app, in_app in H3. destruct H3; intuition. rewrite F; auto.
    split; auto.
      intros. rewrite regs_of_rpairs_app, in_app in H3. rewrite F; auto.
Qed.
...
Lemma restore_callee_save_correct:
  ...
  exists rs',
    ...
    E0 (State cs fb (Vptr sp Ptrofs.zero) k rs' m)
  /\ (forall r, is_callee_save r = true -> Val.inject j (ls0 (R r)) (rs' r))
  /\ (forall r, is_callee_save r = false -> rs' r = rs r).
Proof.
  intros.
  ...
  unfold mreg_within_bounds. intros.
  destruct p; simpl;[|split]; intros; rewrite <- callee_save_correspond;
    subst fe; simpl in *; rewrite in_regs_of_rpairs; econstructor; eauto; split; eauto; simpl; eauto.
  intros. eapply callee_save_wellformed; eauto. eapply H1.
  intros (rs' & A & B & C & D).
  exists rs'.
  split. eexact A.
  split; intros.
  destruct (In_dec mreg_eq r (used_callee_save b)).
  apply B. subst fe. simpl. rewrite callee_save_correspond; auto.
  rewrite C. apply H0. unfold mreg_within_bounds; tauto. rewrite <- callee_save_correspond in n. subst fe. simpl. eauto.
  apply C. red; intros. contradict H1. rewrite (b.(used_callee_save_prop)). congruence.
  rewrite <- callee_save_correspond. subst fe; simpl in *; eauto.
Qed.
...
Lemma function_epilogue_correct:
  ...
  /\ (tf.(fn_retaddr_saved) = true -> load_stack m' (Vptr sp' Ptrofs.zero) Tptr tf.(fn_retaddr_ofs) = Some ra)
  /\ Mem.free m' sp' 0 tf.(fn_stacksize) = Some m1'
  /\ star step tge
       (State cs fb (Vptr sp' Ptrofs.zero) (restore_callee_save fe k) rs m')
    E0 (State cs fb (Vptr sp' Ptrofs.zero) k rs1 m')
  /\ agree_regs j (LTL.pre_return_regs ls0 ls) rs1
  /\ agree_callee_save (LTL.pre_return_regs ls0 ls) ls0
...
Proof.
  ...
  assert (tf.(fn_retaddr_saved) = true -> load_stack m' (Vptr sp' Ptrofs.zero) Tptr (Ptrofs.repr fe.(fe_ofs_retaddr)) = Some ra).
  { intros. rewrite H in SEP. 
    exploit (hasvalue_get_stack Tptr). rewrite chunk_of_Tptr. eapply sep_pick4; eexact SEP. trivial. }
  clear SEP.
  (* Conclusions *)
  rewrite unfold_transf_function.
  cbn [ fn_link_ofs fn_retaddr_ofs fn_stacksize ].
  rewrite <- unfold_transf_function.
  exists rs', m1'.
  split. assumption.
  split. assumption.
  split. assumption.
  split. eassumption.
  split. red; unfold LTL.pre_return_regs; intros.
    destruct (is_callee_save r) eqn:C.
    apply CS; auto.
    rewrite NCS by auto. apply AGR.
  split. red; unfold LTL.pre_return_regs; intros.
    destruct l; simpl in H0. rewrite H0; auto.
...
Fixpoint stack_contents (j: meminj) (cs: list Linear.stackframe) (cs': list Mach.stackframe) : massert :=
  match cs, cs' with
  | nil, nil => pure True
  | Linear.Stackframe f _ ls c :: cs, Mach.Stackframe fb (Vptr sp' _) ra rs c' :: cs' =>
      match transf_function f with
      | OK tf => frame_contents f tf j sp' ls (parent_locset cs) (parent_sp cs') (parent_ra cs')
...
Inductive match_stacks (j: meminj):
  ...
  | match_stacks_cons: forall f sp ls c cs fb sp' ra rs c' cs' sg trf
      ...
      match_stacks j
                   ...
                   (Stackframe fb (Vptr sp' Ptrofs.zero) ra rs c' :: cs')
                   sg.
...
Local Opaque sepconj.
  ...
  destruct sp0; auto.
  destruct (transf_function f) as [ tf | ]; auto.
...
Proof.
  induction 2; intros.
- constructor; auto.
- econstructor; eauto using agree_regs_inject_incr.
Qed.
...
+ elim (H1 _ H).
+ simpl in SEP.
  destruct (transf_function f) as [ tf | ]; [| congruence]. 
  unfold parent_sp.
...
- destruct (transl_external_argument l) as (v & A & B). eapply in_regs_of_rpairs. exists (One l); simpl; eauto.
  exists v; split; auto. constructor; auto.
- destruct (transl_external_argument l1) as (v1 & A1 & B1). eapply in_regs_of_rpairs. exists (Two l1 l2); simpl; auto.
  destruct (transl_external_argument l2) as (v2 & A2 & B2). eapply in_regs_of_rpairs. exists (Two l1 l2); simpl; auto.
  exists (Val.combine v1 v2); split.
  constructor; auto.
  apply Val.combine_inject; auto.
Qed.
...
Hypothesis AGR: agree_regs j ls rs.
Hypothesis SEP: m' |= frame_contents f tf j sp' ls ls0 parent retaddr ** minjection j m ** globalenv_inject ge j.

Lemma transl_builtin_arg_correct:
  forall a v,
  eval_builtin_arg ge (fun p => Locmap.getpair p ls) (Vptr sp Ptrofs.zero) m a v ->
  (forall p, In p (params_of_builtin_arg a) -> loc_pair_valid f p = true) ->
  (forall sl ofs ty, In (S sl ofs ty) (regs_of_rpairs (params_of_builtin_arg a)) -> slot_within_bounds b sl ofs ty) ->
  exists v',
     eval_builtin_arg ge (fun p => get_pair p rs) (Vptr sp' Ptrofs.zero) m' (transl_builtin_arg fe a) v'
  /\ Val.inject j v v'.
Proof.
...
- assert (loc_pair_valid f x = true) by auto.
  destruct x as [l| hi lo].
  + destruct l as [r | [] ofs ty]; try discriminate.
    * exists (rs r); simpl. split. constructor. auto.
    * rewrite app_nil_r in BOUNDS. simpl in BOUNDS.
      exploit frame_get_local; eauto. intros (v & A & B).
      exists v; split; auto. constructor; auto.
  + destruct hi as [hi|]; destruct lo as [lo|]; try discriminate.
    exists (get_pair (Two hi lo) rs). simpl. split. constructor. apply Val.combine_inject; auto.
...
- econstructor; split; eauto with barg.
- rewrite regs_of_rpairs_app in BOUNDS.
...
Lemma transl_builtin_args_correct:
  forall al vl,
  eval_builtin_args ge (fun p => Locmap.getpair p ls) (Vptr sp Ptrofs.zero) m al vl ->
  (forall p, In p (params_of_builtin_args al) -> loc_pair_valid f p = true) ->
  (forall sl ofs ty, In (S sl ofs ty) (regs_of_rpairs (params_of_builtin_args al)) -> slot_within_bounds b sl ofs ty) ->
  exists vl',
     eval_builtin_args ge (fun p => get_pair p rs) (Vptr sp' Ptrofs.zero) m' (List.map (transl_builtin_arg fe) al) vl'
  /\ Val.inject_list j vl vl'.
Proof.
  induction 1; simpl; intros VALID BOUNDS.
- exists (@nil val); split; constructor.
- rewrite regs_of_rpairs_app in BOUNDS.
  exploit transl_builtin_arg_correct; eauto using in_or_app.
  intros (v1' & A & B).
...
Inductive match_states: Linear.state -> Mach.state -> Prop :=
  | match_states_intro:
      forall cs f sp c ls m cs' fb sp' rs m' j tf
        ...
        (TAIL: is_tail c (Linear.fn_code f))
        (SEP: m' |= frame_contents f tf j sp' ls (parent_locset cs) (parent_sp cs') (parent_ra cs')
                 ** stack_contents j cs cs'
                 ** minjection j m
...
+ (* Lgetstack, local *)
  ...
  econstructor; eauto using frame_set_regpair with coqlib.
  apply agree_regs_set_pair; auto.
  apply agree_locs_set_pair1; auto.
+ (* Lgetstack, incoming *)
  ...
  apply sep_proj2 in SEP. simpl in SEP. rewrite TRF in SEP. rewrite sep_assoc in SEP. eexact SEP.
  eapply ARGS; eauto.
  ...
  econstructor; eauto using frame_set_regpair with coqlib.
  econstructor; eauto.
  apply agree_regs_set_pair. apply agree_regs_set_reg. auto. auto.
  erewrite agree_incoming by eauto. exact B.
  apply agree_locs_set_pair1; auto. apply agree_locs_undef_locs; auto.
+ (* Lgetstack, outgoing *)
  exploit frame_get_outgoing; eauto. intros (v & A & B).
  econstructor; split.
  apply plus_one. apply exec_Mgetstack. exact A.
...
- (* Lsetstack *)
  ...
  assert (A: exists m'',
              store_stack m' (Vptr sp' Ptrofs.zero) ty (Ptrofs.repr ofs') (get_pair src rs0) = Some m''
           /\ m'' |= frame_contents f tf j sp' (Locmap.set (S sl ofs ty) (Locmap.getpair (map_rpair R src) rs)
  ...
  { unfold ofs'; destruct sl; try discriminate.
    eapply frame_set_local; eauto using agree_regpair.
    eapply frame_set_outgoing; eauto using agree_regpair. }
...
- (* Lop *)
  assert (exists v',
          eval_operation ge (Vptr sp' Ptrofs.zero) (transl_op (make_env (function_bounds f)) op) (get_pairs args rs0) m' = Some v'
       /\ Val.inject j v v').
  ...
  eapply agree_regpairlist; eauto.
  ...
  apply agree_locs_set_pair1; auto. apply agree_locs_undef_locs. auto. apply destroyed_by_op_caller_save.
  apply frame_set_regpair. apply frame_undef_regs. exact SEP.
...
- (* Lload *)
  ...
  eexact E. eauto.
  econstructor; eauto using frame_set_regpair with coqlib.
  apply agree_regs_set_pair. rewrite transl_destroyed_by_load, simplify_load_destroyed. apply agree_regs_undef_regs; auto. auto.
  apply agree_locs_set_pair1. apply agree_locs_undef_locs. auto. apply destroyed_by_load_caller_save. auto.
...
- (* Lstore *)
  ...
  destruct H1 as [a' [A B]].
  rewrite sep_swap3 in SEP.
  exploit storev_parallel_rule. eexact SEP. eauto. eauto. apply agree_regpair. apply AGREGS.
...
- (* Lcall *)
  ...
  simpl. rewrite TRANSL. rewrite sep_assoc. exact SEP.
...
- (* Lcond, true *)
  ...
  eapply eval_condition_inject with (m1 := m). eapply agree_regpairlist; eauto. apply sep_pick3 in SEP; exact SEP. auto.
...
- (* Lreturn *)
  ...
  apply agree_regs_restore_auto_save_regs. assumption.
  inv STACKS. simpl. unfold agree_regs. intros. econstructor. auto.
...
- (* external function *)
  ...
  eapply match_stacks_change_meminj; eauto.
  apply agree_regs_set_pair; auto.
  unfold LTL.external_call_regs, external_call_regs.
  apply agree_regs_inject_incr with j; auto.
  apply agree_regs_restore_auto_save_regs; auto. apply agree_regs_undef_callee_may_modify_regs; auto.
  inv STACKS. simpl. unfold agree_regs. intros. econstructor. simpl. auto.
  apply stack_contents_change_meminj with j; auto.
  rewrite sep_comm, sep_assoc; auto.

- (* return *)
  inv STACKS. exploit wt_returnstate_agree; eauto. intros [AGCS OUTU].
  simpl in AGCS. simpl in SEP. rewrite TRF, sep_assoc in SEP.
  econstructor; split.
  apply plus_one. apply exec_return.
  econstructor; eauto.
  apply agree_locs_return with rs0; auto.
  apply frame_contents_exten with rs0 (parent_locset s); auto.
  intros; apply Val.lessdef_same; apply AGCS; simpl; congruence.
...
Proof.
  ...
  rewrite symbols_preserved. eauto. eassumption.
