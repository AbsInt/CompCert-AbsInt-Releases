...
Definition loc_constrained_half (l: loc) (env: regenv) (e: eqs) (ty: typ) : bool :=
  ...
  EqSet2.fold (fun q b => typ_eq (proj_ptype_typ (env (ereg q))) ty && (IndexedEqKind.eq Low (ekind q) || IndexedEqKind.eq High (ekind q)) && b) e' true.

Definition loc_pair_constrained (hi lo: loc) (env: regenv) (e: eqs) : bool :=
...
Definition mreg_pair_constrained (hi lo: mreg) (env: regenv) (e: eqs) : bool :=
  ...
  then loc_pair_constrained (R hi) (R lo) env e
  else false.
...
Definition loc_type_compat (env: regenv) (l: loc) (e: eqs) : bool :=
  EqSet2.for_all_between
    (fun q => subtype (sel_type (ekind q) (proj_ptype_typ (env (ereg q)))) (Loc.type l))
...
Definition long_type_compat (env: regenv) (l: loc) (e: eqs) : bool :=
  EqSet2.for_all_between
    (fun q => subtype (proj_ptype_typ (env (ereg q))) Tlong)
    (select_loc_l l) (select_loc_h l) (eqs2 e).
...
Definition pair_type_compat (env: regenv) (l: loc) (e: eqs) : bool :=
  EqSet2.for_all_between
    ...
    (select_loc_l l) (select_loc_h l) (eqs2 e)
  || EqSet2.for_all_between
    (fun q => subtype (proj_ptype_typ (env (ereg q))) Tfloat)
    (select_loc_l l) (select_loc_h l) (eqs2 e).

Definition half_long_type_compat (env: regenv) (l: loc) (e: eqs) : bool :=
   EqSet2.for_all_between
    (fun q => if IndexedEqKind.eq Full (ekind q) then subtype (proj_ptype_typ (env (ereg q))) Tint else subtype (proj_ptype_typ (env (ereg q))) Tlong)
    (select_loc_l l) (select_loc_h l) (eqs2 e).
...
Definition add_equations_mreg_pair (env: regenv) (r: reg) (p: rpair mreg) (e: eqs) :=
  ...
  | Two hi lo => assertion ((typ_eq (proj_ptype_typ (env r)) Tlong && negb Archi.ptr64)|| typ_eq (proj_ptype_typ (env r)) Tfloat);
...
Fixpoint add_equations_builtin_arg
  ...
  | BA r, BA_splitlong (BA (One lhi)) (BA (One llo)) =>
      assertion (typ_eq (proj_ptype_typ (env r)) Tlong);
...
(** [only_preserved_regs e] returns true if only locations that are preserved across a function call
  are constrained in [e].  In other words, [e] contains no equations
  involving a caller-save register or [Outgoing] stack slot. *)

Definition only_preserved_regs (e: eqs) : bool :=
  EqSet.for_all
   (fun eq => preserved_across_call (eloc eq))
    e.
...
(** [ros_compatible_tailcall ros] returns true if [ros] is a function
  name or not a callee-save register.  This is used to check [Itailcall]
  instructions. Function pointers for tail-calls must not reside in
  callee-save registers because they will be clobbered by the epilogue
  before the jump is executed. *)
...
Fixpoint track_moves (env: regenv) (mv: moves) (e: eqs) : option eqs :=
  ...
  | MV (Two (R srchi) (R srclo)) (Two (R dsthi) (R dstlo)) :: mv =>
      ...
      assertion (mreg_pair_constrained dsthi dstlo env e1);
...
Definition transfer_aux (f: RTL.function) (env: regenv)
  ...
  | BScall sg ros args res mv1 ros' mv2 s =>
      ...
      assertion (only_preserved_regs e2);
...
Parameter regalloc: RTL.function -> regenv -> res LTL.function.

(** Register allocation followed by validation. *)

Definition transf_function env (f: RTL.function) : res LTL.function :=
      match regalloc f env with
      | Error m => Error m
      | OK tfn => do x <- check_function f tfn env; OK tfn
      end.

Definition transf_fundef (fd: typed_fundef) : res LTL.fundef :=
  match fd with
| Internal f => match f with TF g env wt => do f' <- transf_function env g; OK (Internal f') end
| External ef => OK (External ef)
end.

Definition transf_program (p: RTLtyping.program) : res LTL.program :=
