...
(*- E_COMPCERT_CODE_XTL_var_pair_001 *)
(*- #Justify_Derived "Type definition" *)
...
let typeof = function V(_, ty) -> ty | L l -> PLoc.coq_type l
(*- #End *)
...
let is_stack_reg = function
  | L(PR r) -> Machregs.is_stack_reg r
  | _      -> false
(*- #End *)
...
(*- #Justify_Derived "Utility function" *)
let new_reg() =
  let r = !next_temp in next_temp := P.succ !next_temp; r (* Would it not be better to replace the second !next_temp with r? *)
(*- #End *)
...
(*- E_COMPCERT_CODE_XTL_vars_of_var_pair_001 *)
(*- #Justify_Derived "Utility function" *)
let vars_of_var_pair = function
  | Vr v -> [v]
  | Vsplitlong (v1, v2) -> [v1; v2]
(*- #End *)

(*- E_COMPCERT_CODE_XTL_vars_of_var_pairs_001 *)
(*- #Justify_Derived "Utility function" *)
let vars_of_var_pairs al =
  List.fold_right (fun a l -> (vars_of_var_pair a) @ l) al []
(*- #End *)
...
(*- E_COMPCERT_CODE_XTL_set_var_pair_type_001 *)
(*- #Justify_Derived "Utility function" *)
let set_var_pair_type v ty =
  match v with
  | Vr v -> set_var_type v ty
  | Vsplitlong (v1, v2) ->
    if ty <> Ptyp Tlong then
      raise Type_error
    else begin
      set_var_type v1 (Ptyp Tint);
      set_var_type v2 (Ptyp Tint)
    end
(*- #End *)
...
(*- E_COMPCERT_CODE_XTL_set_vars_pair_type_001 *)
(*- #Justify_Derived "Utility function" *)
let rec set_vars_pair_type vl tyl =
  match vl, tyl with
  | [], [] -> ()
  | v1 :: vl, ty1 :: tyl -> set_var_pair_type v1 ty1; set_vars_pair_type vl tyl
  | _, _ -> raise Type_error
(*- #End *)
...
let rec type_builtin_arg a ty =
  ...
  | BA_splitlong(a1, a2) -> type_builtin_arg a1 (Ptyp Tint); type_builtin_arg a2 (Ptyp Tint)
  | BA_addptr(a1, a2) -> type_builtin_arg a1 Pptr; type_builtin_arg a2 Pptr
  | _ -> ()
(*- #End *)
...
let rec type_builtin_res a ty =
  ...
  | BR_splitlong(a1, a2) -> type_builtin_res a1 (Ptyp Tint); type_builtin_res a2 (Ptyp Tint)
  | _ -> ()
(*- #End *)
...
let type_instr = function
  ...
  | Xparmove(srcs, dsts, tmps) ->
      ...
      (* This is only used for debugging but destroys cyclic dependencies *)
      (* Array.iteri (fun idx var -> set_var_type var (AllocInterface.default_type_of_class idx)) tmps *)
  | Xop(op, args, res) ->
      begin match type_of_operation op args res with
      | Errors.Error _ -> raise Type_error
      | Errors.OK ((targs, tres), _) ->
        set_vars_pair_type args (List.map Subtyping.S.proj_lo targs);
        set_var_pair_type res (Subtyping.S.proj_lo tres)
      end
  | Xload(chunk, addr, args, dst) ->
      set_vars_type args (List.map Subtyping.S.proj_lo (type_of_addressing addr));
      set_var_type dst (inj_typ_ptype (type_of_chunk chunk))
  | Xstore(chunk, addr, args, src) ->
      set_vars_type args (List.map Subtyping.S.proj_lo (type_of_addressing addr));
      set_var_type src (inj_typ_ptype (type_of_chunk chunk))
  | Xcall(sg, Coq_inl v, args, res) ->
      set_var_type v (Ptyp Tint)
  ...
  | Xbuiltin(ef, args, res) ->
      let sg = ef_sig ef in
      type_builtin_args args (proj_sig_args_ptype sg);
      type_builtin_res res (inj_typ_ptype (proj_sig_res sg))
  | Xbranch s ->
      ()
  | Xcond(cond, args, s1, s2) ->
      set_vars_type args (List.map Subtyping.S.proj_lo (type_of_condition cond))
  | Xjumptable(arg, tbl) ->
      set_var_type arg (Ptyp Tint)
  | Xreturn args ->
      ()
(*- #End *)
