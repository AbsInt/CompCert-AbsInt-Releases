...
- preserved across function calls:
  callee-save registers, auto-save registers, [Local] and [Incoming] stack slots;
- possibly modified by a function call & not needed to be preserved by callee before modifying:
  caller-save registers, [Outgoing] stack slots.
...
Definition callee_save (l: loc) : bool :=
  match l with
...
Definition agree_callee_save (ls1 ls2: Locmap.t) : Prop :=
  forall l, callee_save l = true -> ls1 l = ls2 l.

Definition preserved_across_call (l: loc) : bool :=
  match l with
  | R r => is_preserved_across_call r
  | S Outgoing _ _ => false
  | S _ _ _ => true
  end.

Definition agree_preserved_across_call (ls1 ls2: Locmap.t) : Prop :=
  forall l, preserved_across_call l = true -> ls1 l = ls2 l.
...
Lemma locmap_get_set_loc_result:
  forall sg v rs l,
  match l with R r => is_preserved_across_call r = true | S _ _ _ => True end ->
  Locmap.setpair (loc_result sg) v rs l = rs l.
Proof.
  intros. apply Locmap.gpo. 
  assert (X: forall r, is_preserved_across_call r = false -> Loc.diff l (R r)).
  { intros. destruct l; simpl. congruence. auto. }
  generalize (loc_result_caller_save sg). 
  destruct (loc_result sg); simpl; intros. 
  - auto using is_caller_not_preserved.
  - destruct H0 as [??]. auto using is_caller_not_preserved.
Qed.

Lemma locmap_get_set_loc_result_preserved_across_call:
  forall sg v rs l,
  preserved_across_call l = true ->
  Locmap.setpair (loc_result sg) v rs l = rs l.
Proof.
