...
(*- E_COMPCERT_FTR_Function_Conventions_mreg_preserved_across_call_001 *)
(*- #Link_to E_COMPCERT_TOR_Function_SEM_ASM_005 *)
Definition mreg_preserved_across_call (r: mreg): bool := negb (is_caller_save r).
(*- #End *)

Definition preserved_across_call (l: loc): bool :=
  match l with
  | R r => mreg_preserved_across_call r
...
Lemma locmap_get_set_loc_result:
  forall sg v rs l,
  match l with R r => is_caller_save r = false | S _ _ _ => True end ->
  Locmap.setpair (loc_result sg) v rs l = rs l.
Proof.
  intros. apply Locmap.gpo. 
  assert (X: forall r, is_caller_save r = true -> Loc.diff l (R r)).
...
Proof.
  ...
  red in H; destruct l; auto. apply reg_callee_not_caller; auto.
Qed.
...
Proof.
  intros. apply locmap_get_set_loc_result. 
  unfold preserved_across_call in H. destruct l; auto.
  unfold mreg_preserved_across_call in H.
  destruct (is_caller_save r); simpl in H; congruence.
Qed.
