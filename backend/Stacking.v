...
(** [save_callee_save fe k] adds before [k] the instruction that
  store in the frame the values of callee-save registers. *)

Definition save_callee_save (fe: frame_env) (k: Mach.code) :=
  match fe.(fe_used_callee_save) with
  | nil => k
  | _ => Msavecallee fe.(fe_ofs_callee_save) fe.(fe_used_callee_save) :: k
  end.

(** [restore_callee_save fe k] adds before [k] the instruction that
...
Definition restore_callee_save (fe: frame_env) (k: Mach.code) :=
  match fe.(fe_used_callee_save) with
  | nil => k
  | _ => Mrestorecallee fe.(fe_ofs_callee_save) fe.(fe_used_callee_save) :: k
  end.
