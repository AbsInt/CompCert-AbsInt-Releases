...
(** [save_callee_save_rec gl ofs k] adds before [k] the instructions that
  store in the frame the values of callee-save registers in [gl] starting 
  at offset [ofs]. In the case of a group of multiple registers, the alignment
  of the offset is ensured in the operational semantics of Msavecallee. *)

Fixpoint save_callee_save_rec (gl: list mreg_group) (ofs: Z) (k: Mach.code) :=
  match gl with
  | nil => k
  | Single r :: gl =>
      ...
      Msetstack (One r) (Ptrofs.repr ofs1) ty :: save_callee_save_rec gl (ofs1 + sz) k
  | Multiple l :: gl =>
      Msavecallee ofs l :: save_callee_save_rec gl (size_callee_save_area_rec l ofs) k
  end.
...
(** [restore_callee_save_rec gl ofs k] adds before [k] the instructions that
  re-load from the frame the values of callee-save registers in [gl] starting 
  at offset [ofs]. In the case of a group of multiple registers, the alignment
  of the offset is ensured in the operational semantics of Mrestorecallee. *)

Fixpoint restore_callee_save_rec (gl: list mreg_group) (ofs: Z) (k: Mach.code) :=
  match gl with
  | nil => k
  | Single r :: gl =>
      let ty := mreg_type r in
      let sz := AST.typesize ty in
      let ofs1 := align ofs sz in
      Mgetstack (Ptrofs.repr ofs1) ty (One r) :: restore_callee_save_rec gl (ofs1 + sz) k
  | Multiple l :: gl =>
      Mrestorecallee ofs l :: restore_callee_save_rec gl (size_callee_save_area_rec l ofs) k
  end.
