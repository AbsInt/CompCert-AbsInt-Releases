...
Fixpoint type_stmt (tret: xtype) (e: S.typenv) (s: stmt) : res S.typenv :=
  ...
  | Sreturn opta =>
      match opta with
      | None => OK e
      | Some a => type_expr e a (proj_xtype_typ tret)
(*
          if xtype_eq tret Tvoid
          then Error (msg "inconsistent return")
          else type_expr e a (proj_xtype_typ tret)
*)
      end
  | Slabel lbl s1 =>
      type_stmt tret e s1
...
Definition wt_opt_assign (optid: option ident) (ty: xtype) : Prop :=
  match optid with
  | Some id => proj_xtype_typ ty = env id
  | _ => True
  end.
...
Inductive wt_state: state -> Prop :=
  ...
  | wt_return_state: forall v k m tret
        (WT_RES: Val.has_type v (proj_xtype_typ tret))
