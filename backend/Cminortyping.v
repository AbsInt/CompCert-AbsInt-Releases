...
Inductive wt_state: state -> Prop :=
  ...
  | wt_call_state: forall f args k m
        (WT_FD: wt_fundef f)
        (WT_ARGS: Val.has_argtype_list args (funsig f).(sig_args))
...
Lemma wt_eval_exprlist: forall env sp e m al vl,
  ...
  forall tl,
  list_forall2 (wt_expr env) al (List.map proj_argtype tl) ->
  wt_env env e ->
  Val.has_argtype_list vl tl.
Proof.
  induction 1; intros tl WT ENV; inv WT; simpl.
- destruct tl; auto; inv H. 
- destruct tl;  inv H3.
  split; eauto. destruct a; simpl in *; eapply wt_eval_expr; eauto.
Qed.
...
- inv WT_FD. inversion H1; subst. econstructor; eauto.
  constructor; auto.
  apply wt_env_set_locals. apply wt_env_set_params.
  rewrite H2; auto. apply Val.has_proj_argtype_list. auto.
