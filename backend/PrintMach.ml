...
let print_instruction pp i =
  ...
  | Mgetstack(ofs, ty, res) ->
      fprintf pp "\t%a = stack(%Ld, %s)\n"
              reg_pair res (camlint64_of_ptrofs ofs) (name_of_type ty)
  | Msetstack(arg, ofs, ty) ->
      fprintf pp "\tstack(%Ld, %s) = %a\n"
              (camlint64_of_ptrofs ofs) (name_of_type ty) reg_pair arg
  | Mrestorecallee(ofs, l) ->
     fprintf pp "\trestore_callee %s %a\n" (Z.to_string ofs) reg_pairs l
  | Msavecallee(ofs, l) ->
     fprintf pp "\tsave_callee %s %a\n" (Z.to_string ofs) reg_pairs l
