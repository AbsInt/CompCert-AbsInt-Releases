...
let reg_pair = PrintAST.print_rpair reg

let rec reg_pairs pp = function
  | [] -> ()
  | [p] -> reg_pair pp p
  | p :: pl -> fprintf pp "%a, %a" reg_pair p reg_pairs pl
...
let print_instruction pp i =
  ...
  | Mgetstack(ofs, ty, res) ->
      fprintf pp "\t%a = stack(%ld, %s)\n"
              reg_pair res (camlint_of_coqint ofs) (name_of_type ty)
  | Msetstack(arg, ofs, ty) ->
      fprintf pp "\tstack(%ld, %s) = %a\n"
              (camlint_of_coqint ofs) (name_of_type ty) reg_pair arg
  | Mrestorecallee(ofs, l) ->
     fprintf pp "\trestore_callee %ld %a\n" (camlint_of_coqint ofs) reg_pairs l
  | Msavecallee(ofs, l) ->
     fprintf pp "\tsave_callee %ld %a\n" (camlint_of_coqint ofs) reg_pairs l
  | Mgetparam(ofs, ty, res) ->
      fprintf pp "\t%a = param(%ld, %s)\n"
              reg_pair res (camlint_of_coqint ofs) (name_of_type ty)
  | Mop(op, args, res) ->
      fprintf pp "\t%a = %a\n"
         reg_pair res (PrintOp.print_operation reg_pair) (op, args)
  | Mload(chunk, addr, args, dst) ->
      fprintf pp "\t%a = %s[%a]\n"
         reg_pair dst (name_of_chunk chunk)
  ...
  | Mbuiltin(ef, args, res) ->
      fprintf pp "\t%a = %s(%a)\n"
        (print_builtin_res reg_pair) res
        (name_of_external ef)
        (print_builtin_args reg_pair) args
  | Mlabel lbl ->
  ...
  | Mcond(cond, args, lbl) ->
      fprintf pp "\tif (%a) goto %d\n"
        (PrintOp.print_condition reg_pair) (cond, args)
        (P.to_int lbl)
  | Mjumptable(arg, tbl) ->
