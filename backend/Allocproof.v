...
Proof.
  intros; red; intros. unfold pre_return_regs.
  unfold callee_save in H.
  destruct l.
  - rewrite H; auto.
  - destruct sl; congruence.
Qed.

Lemma restore_auto_save_regs_agree_preserved_across_call:
  forall caller callee,
...
Proof.
  ...
  - reg_cc_all r; rewrite HrAuto; auto.
  - apply H; auto.
Qed.
...
Proof.
  ...
  destruct l eqn:El; simpl in H0.
  - reg_cc_all r; rewrite HrAuto, ? HrModify; auto.
  - destruct sl; auto || congruence.
Qed.

Lemma only_preserved_regs_sound:
  forall e q,
  only_preserved_regs e = true ->
  EqSet.In q e ->
  preserved_across_call (eloc q) = true.
Proof.
  unfold only_preserved_regs; intros.
...
- (* One register *)
  ...
  exploit reg_loc_unconstrained_sound; eauto. intros [A B].
  rewrite Regmap.gso; auto.
  exploit only_preserved_regs_sound; eauto. intros.
...
Lemma pre_return_regs_arg_values:
  forall sg ls1 ls2,
  tailcall_is_possible sg = true ->
  map (fun p => Locmap.getpair p (pre_return_regs ls1 ls2)) (loc_arguments sg)
...
Proof.
  ...
  reg_cc_all r; rewrite HrCallee; auto.
Qed.

Lemma find_function_tailcall:
  forall tge ros ls1 ls2,
  ros_compatible_tailcall ros = true ->
  find_function tge ros (pre_return_regs ls1 ls2) = find_function tge ros ls2.
Proof.
  unfold ros_compatible_tailcall, find_function; intros.
  destruct ros as [r|id]; auto. simpl.
  destruct (is_callee_save r); discriminate || auto.
Qed.
...
- set (sg := RTL.funsig fd) in *.
  ...
  + rewrite <- eq2. eapply match_states_call; eauto.
    ...
    * rewrite SIG. rewrite pre_return_regs_arg_values; auto. eapply add_equations_args_lessdef; eauto.
      inv WTI.
      unfold proj_sig_args.
      rewrite proj_xtype_ptype_typ_list.
      eapply Val.has_ptype_sub_list'; eauto with ty.
...
+ (* without an argument *)
  ...
  apply restore_auto_save_regs_agree_preserved_across_call.
  apply pre_return_regs_agree_callee_save.
  constructor.
+ (* with an argument *)
  exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  ...
  destruct (loc_result (RTL.fn_sig f)); simpl; intros.
  * reg_cc_all r; rewrite HrAuto, HrCallee. reflexivity.
  * destruct H12 as [??].
    reg_cc_all rhi; reg_cc_all rlo. rewrite HrAuto, HrAuto0, HrCallee, HrCallee0. reflexivity.
  * apply restore_auto_save_regs_agree_preserved_across_call.
    apply pre_return_regs_agree_callee_save.
  * rewrite <- H11.
    eapply Val.has_ptype_sub'; eauto with ty.
