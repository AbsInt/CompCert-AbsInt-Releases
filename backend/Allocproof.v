...
Inductive match_states: RTL.state -> LTL.state -> Prop :=
  ...
  | match_states_call:
      forall s f args m ts tf ls m'
        ...
        (MEM: Mem.extends m m')
        (WTARGS: Val.has_argtype_list args (sig_args (funsig tf))),
...
Lemma match_stackframes_change_sig:
  ...
  tailcall_matching_rettype sg'.(sig_res) sg.(sig_res) = true ->
  match_stackframes s ts sg'.
Proof.
  intros. inv H.
  constructor.
  unfold tailcall_matching_rettype, tailcall_rettype_match, simpl_rettype_match in *.
  rewrite <- H1. apply (tailcall_matching_rettype_correct); auto.
  econstructor; eauto.
  rewrite WTRES. unfold proj_sig_res.
  destruct (tailcall_matching_rettype_correct (sig_res sg') (sig_res sg)).
  symmetry. apply tailcall_rettype_match_proj_sig. apply H.  apply H0.
  intros. rewrite (loc_result_exten sg' sg) in H. eauto.
...
- set (sg := RTL.funsig fd) in *.
  ...
  simpl. red; auto.
  inv WTI. rewrite SIG. apply (wt_regset_list2 env); auto.
...
- set (sg := RTL.funsig fd) in *.
  ...
  destruct (tailcall_matching_rettype_correct (sig_res sg) (sig_res (RTL.fn_sig f))).
  generalize (H1 Heqb0). intros. 
  destruct (transf_function_inv _ _ FUN); auto. rewrite <- H15. apply H3. auto.
  ...
  rewrite SIG. inv WTI. apply (wt_regset_list2 env); auto.
...
- monadInv FUN. simpl in *.
  ...
  { apply wt_init_regs. inv H0. rewrite wt_params. rewrite H9. apply Val.has_proj_argtype_list. auto. }
...
Proof.
  ...
  simpl. unfold tailcall_rettype_match. reflexivity.
