...
Lemma can_eval_safe_arg:
  forall (rs: locset) sp m (a: builtin_arg (rpair loc)),
  safe_builtin_arg a -> exists v, eval_builtin_arg tge (fun p => Locmap.getpair p rs) sp m a v.
Proof.
...
- (* tailcall *)
  exploit find_function_translated; eauto. intros (tf' & A & B).
  exploit match_parent_locset; eauto. intros PLS.
  econstructor; split.
  apply plus_one.
...
- (* return *)
  econstructor; split.
  apply plus_one.  constructor; eauto. inv TRF; eauto. traceEq.
  rewrite (match_parent_locset _ _ STACKS). constructor; auto.
- (* internal function *)
