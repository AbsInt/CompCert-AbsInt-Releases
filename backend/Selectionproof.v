...
Require Import AST Linking Errors Integers Floats.
...
Lemma eval_sel_bswap32:
  forall le a v v1,
  eval_expr tge sp e m le a v ->
  standard_builtin_sem BI_i32_bswap (v :: nil) = Some v1 ->
  exists v', eval_expr tge sp e m le (sel_bswap32 a) v' /\ Val.lessdef v1 v'.
Proof.
  intros. unfold sel_bswap32.
  destruct (platform_standard_builtin BI_i32_bswap (a ::: Enil)) eqn:?.
  eapply (eval_platform_standard_builtin tge sp e m BI_i32_bswap); eauto.
  econstructor. eexact H. econstructor.
  econstructor; split. econstructor. econstructor. eexact H. econstructor.
...
Proof.
  Local Opaque platform_standard_builtin.
  intros until le; intros SEL ARGS SEM.
  destruct bf as [bf|bf]; simpl in SEL.
  - assert (BASE: platform_standard_builtin bf args = Some a -> exists v', eval_expr tge sp e m le a v' /\ Val.lessdef v v').
    intros. eapply eval_platform_standard_builtin; eauto.
    destruct bf; try apply (BASE SEL).
    + (* select *)
    ...
    + (* BI_negl *)
      inv ARGS; try discriminate. inv H0; try discriminate.
      inv SEL.
      simpl in SEM; inv SEM. apply eval_negl; auto.
    + (* BI_addl *)
      inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
      inv SEL.
      simpl in SEM; inv SEM. apply eval_addl; auto.
    + (* BI_subl *)
      inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
      inv SEL.
      simpl in SEM; inv SEM. apply eval_subl; auto.
    + (* BI_mull *)
      inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
      inv SEL. inv SEM.
      exploit eval_mul'; try eapply HF'. eexact H. eexact H1. intros (v2 & A2 & B2).
      exists v2; split; eauto.
    + (* BI_i64_bswap *)
      inv ARGS; try discriminate. inv H0; [|discriminate].
      destruct Archi.splitlong eqn:?; eauto. clear BASE. inv SEL.
      exploit eval_lowlong. eexact H.
      intros (v2 & A2 & B2).
      exploit eval_sel_bswap32. eexact A2. reflexivity. simpl.
      intros (v3 & A3 & B3).
      exploit eval_highlong. eexact H.
      intros (v4 & A4 & B4).
      exploit eval_sel_bswap32. eexact A4. reflexivity. simpl.
      intros (v5 & A5 & B5).
      inv SEM.
      econstructor; split. EvalOp. try reflexivity.
      destruct v1; auto. inv B2. inv B3. inv B4. inv B5. simpl.
      apply Val.lessdef_same. f_equal.
      assert (forall x, 0 <= decode_int (rev (encode_int 4%nat (Int.unsigned x))) <= Int.max_unsigned).
      {  intros. unfold decode_int, encode_int, rev_if_be.
         generalize (int_of_bytes_range (rev (bytes_of_int 4 (Int.unsigned x)))).
         rewrite List.rev_length, length_bytes_of_int.
         change (two_p (Z.of_nat 4 * 8)) with (Int.max_unsigned + 1).
         intros. destruct Archi.big_endian; [rewrite rev_involutive|]; lia.
      }
      unfold decode_int, encode_int in *.
      rewrite bytes_of_int64.
      rewrite Int64.ofwords_add. f_equal. f_equal.
      destruct Archi.big_endian eqn:?.
      * unfold rev_if_be in *. rewrite Heqb0 in *. rewrite ! rev_involutive.
        rewrite rev_app_distr. rewrite ! Int.unsigned_repr by (eapply H0; auto).
        rewrite int_of_bytes_append.  rewrite List.rev_length, length_bytes_of_int.
        change (Z.of_nat 4 * 8) with 32. lia.
      * unfold rev_if_be in *; rewrite Heqb0 in *.
        rewrite rev_app_distr. rewrite ! Int.unsigned_repr by (apply H0; auto).
        rewrite int_of_bytes_append.  rewrite List.rev_length, length_bytes_of_int.
        change (Z.of_nat 4 * 8) with 32. lia.
    + (* BI_isnanf *)
      inv ARGS; try discriminate. inv H0; try discriminate.
      inv SEL. inv SEM.
      exploit (eval_compfs tge sp e m Cne); eauto.
      intros (v & (E & L)).
      exists v. split; auto.
      destruct v1; simpl; auto. unfold Val.cmpfs in *.
      simpl in *. rewrite Float32.cmp_ne_nan in L. destruct (Binary.is_nan 24 128 f); auto.
    + (* BI_isnan *)
      inv ARGS; try discriminate. inv H0; try discriminate.
      inv SEL. inv SEM.
      exploit (eval_compf tge sp e m Cne); eauto.
      intros (v & (E & L)).
      exists v. split; auto.
      destruct v1; simpl; auto. unfold Val.cmpf in *.
      simpl in *. rewrite Float.cmp_ne_nan in L. destruct (Binary.is_nan 53 1024 f); auto.
    + (* BI_isinff *)
      inv ARGS; try discriminate. inv H0; try discriminate.
      inv SEL. inv SEM.
      exploit eval_absfs. eexact H.
      intros (v2 & (E2 & L2)).
      exploit eval_compfs. eexact E2.
      instantiate (2 := Eop (Osingleconst (Binary.B754_infinity 24 128 false)) Enil).
      econstructor. econstructor. reflexivity.
      intros (v3 & (E3 & L3)).
      exists v3. split; eauto.
      destruct v1; auto.
      inv L2. simpl in L3.
    ...
    + (* BI_isinf *)
      inv ARGS; try discriminate. inv H0; try discriminate.
      inv SEL. inv SEM.
      exploit eval_absf. eexact H.
      intros (v2 & (E2 & L2)).
      exploit eval_compf. eexact E2.
      instantiate (2 := Eop (Ofloatconst (Binary.B754_infinity 53 1024 false)) Enil).
      econstructor. econstructor. reflexivity.
      intros (v3 & (E3 & L3)).
      exists v3. split; eauto.
      destruct v1; auto.
