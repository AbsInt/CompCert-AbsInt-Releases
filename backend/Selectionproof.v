...
Inductive match_states: nat -> Cminor.state -> CminorSel.state -> Prop :=
...
Lemma sel_step_correct:
  ...
  forall i1 T1, match_states i1 S1 T1 -> wt_state S1 ->
  (exists i2 T2, plus step tge T1 t T2 /\ match_states i2 S2 T2)
  \/ (exists i2, i2 < i1 /\ t = E0 /\ match_states i2 S2 T1)%nat
  \/ (exists i2 T2 n, step tge T1 t T2 /\ eventually n S2 (fun S3 => match_states i2 S3 T2)).
Proof.
  induction 1; intros i1 T1 ME WTS; inv ME; try (monadInv TS).
- (* skip seq *)
  inv MC. left; do 2 econstructor; split. apply plus_one; econstructor. econstructor; eauto.
  inv H.
- (* skip block *)
  inv MC. left; do 2 econstructor; split. apply plus_one; econstructor. econstructor; eauto.
  inv H.
...
+ (* turned into Sbuiltin *)
  intros EQ. subst fd.
  right; left; exists 0%nat; split. lia. split; auto. econstructor; eauto.
- (* Stailcall *)
...
- (* Sbuiltin *)
  ...
  left; do 2econstructor; split. eexact P. econstructor; eauto.
- (* Seq *)
  left; do 2 econstructor; split.
...
+ inv WTS. inv WT_FN. assert (env0 = env) by congruence. subst env0. inv WT_STMT.
  ...
  right; right. do 2 econstructor; exists n.
  split. eexact A. eapply eventually_implies. eexact B.
  intros S3 EQ; subst S3. econstructor; eauto.
+ exploit sel_expr_correct; eauto. intros [v' [A B]].
  assert (Val.bool_of_val v' b). inv B. auto. inv H0.
  left; econstructor; exists (State f' (if b then x else x0) k' sp e' m'); split.
  apply plus_one; econstructor; eauto. eapply eval_condexpr_of_expr; eauto.
  econstructor; eauto. destruct b; auto.
- (* Sloop *)
  left; do 2 econstructor; split. apply plus_one; constructor. econstructor; eauto.
  constructor; auto. simpl; rewrite EQ; auto.
- (* Sblock *)
  left; do 2 econstructor; split. apply plus_one; constructor. econstructor; eauto. constructor; auto.
- (* Sexit seq *)
  inv MC. left; do 2 econstructor; split. apply plus_one; constructor. econstructor; eauto.
  inv H.
- (* Sexit0 block *)
  inv MC. left; do 2 econstructor; split. apply plus_one; constructor. econstructor; eauto.
  inv H.
...
- (* external call turned into a Sbuiltin *)
  exploit sel_builtin_correct; eauto. intros (e2' & m2' & P & Q & R).
  left; do 2 econstructor; split. eexact P. econstructor; eauto.
- (* return *)
  inv MC.
  left; do 2 econstructor; split.
...
- (* return of an external call turned into a Sbuiltin *)
  right; left; exists 1%nat; split. lia. split. auto. econstructor; eauto.
Qed.

Lemma sel_initial_states:
  forall S, Cminor.initial_state prog S ->
  exists i R, initial_state tprog R /\ match_states i S R.
Proof.
  destruct 1.
...
Lemma sel_final_states:
  forall i S R r,
  match_states i S R -> Cminor.final_state S r -> final_state R r.
Proof.
...
Proof.
  set (MS := fun idx S T => match_states idx S T /\ wt_state S).
  apply forward_simulation_eventually  with (order := lt) (match_states := MS).
- apply lt_wf.
- intros S INIT. exploit sel_initial_states; eauto. intros (i & T & P & Q).
  assert (W: wt_state S). { eapply wt_initial_state. eexact wt_prog. auto. }
  unfold MS; eauto 7.
- intros i S T r (M & W) FIN.
  eapply sel_final_states; eauto.
- intros S1 t S2 A i1 T1 (M & W1).
  assert (W2: wt_state S2) by (eapply subject_reduction; eauto using wt_prog).
  exploit sel_step_correct; eauto.
  intros [(i2 & T2 & D & E) | [(i2 & D & E & F) | (i2 & T2 & n & D & E)]].
+ exists O, i2, T2; split. auto. apply eventually_now; unfold MS; auto.
+ exists O, i2, T1; split. subst t; auto using star_refl. apply eventually_now; unfold MS; auto.
+ exists n, i2, T2; split. auto using plus_one.
