...
Function transfer_builtin (app: VA.t) (ef: external_function)
  ...
  | EF_memcpy sz al, dst::src::nil =>
      if nmem_contains nm (aaddr_arg app dst) (Ptrofs.unsigned sz) then
        transfer_builtin_args
           (kill_builtin_res res ne,
            nmem_add (nmem_remove nm (aaddr_arg app dst) (Ptrofs.unsigned sz)) (aaddr_arg app src) (Ptrofs.unsigned sz))
           args
      else (ne, nm)
...
Definition transf_instr (dm: defmap) (approx: PMap.t VA.t) (an: PMap.t NA.t)
  ...
  | Ibuiltin (EF_memcpy sz al) (dst :: src :: nil) res s =>
      if nmem_contains (snd an!!pc) (aaddr_arg approx!!pc dst) (Ptrofs.unsigned sz)
      then instr
      else Inop s
