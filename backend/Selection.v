...
Definition sel_bswap32 e :=
  match SelectOp.platform_standard_builtin BI_i32_bswap (e ::: Enil) with
  | Some e => e
  | None => Ebuiltin (EF_builtin "__builtin_bswap32" [Xint ---> Xint]%asttyp) (e ::: Enil)
  end.
...
Function sel_known_builtin (bf: builtin_function) (args: exprlist) :=
  ...
  | BI_standard BI_isnanf, e ::: Enil => Some (compfs Cne e e)
  | BI_standard BI_isnan, e ::: Enil => Some (compf Cne e e)
  | BI_standard BI_isinf, e ::: Enil =>
        let inf := Eop (Ofloatconst (Binary.B754_infinity 53 1024 false)) Enil in
        let abs := SelectOp.absf e in
        Some (compf Ceq abs inf)
  | BI_standard BI_isinff, e ::: Enil =>
        let inf := Eop (Osingleconst (Binary.B754_infinity 24 128 false)) Enil in
        let abs := SelectOp.absfs e in
        Some (compfs Ceq abs inf)
  | BI_standard BI_isfinitef, e ::: Enil =>
        let inf := Eop (Osingleconst (Binary.B754_infinity 24 128 false)) Enil in
        let abs := SelectOp.absfs e in
        Some (compfs Clt abs inf)
  | BI_standard BI_isfinite, e ::: Enil =>
        let inf := Eop (Ofloatconst (Binary.B754_infinity 53 1024 false)) Enil in
        let abs := SelectOp.absf e in
        Some (compf Clt abs inf)
  | BI_standard BI_addl, e1 ::: e2 ::: Enil =>
      Some (addl e1 e2)
  | BI_standard BI_subl, e1 ::: e2 ::: Enil =>
      Some (subl e1 e2)
  | BI_standard BI_negl, e ::: Enil => Some (negl e)
  | BI_standard BI_mull, e1 ::: e2 ::: Enil =>
      Some (mul' e1 e2)
  | BI_standard BI_i64_bswap, e ::: Enil =>
      if Archi.splitlong then
        let lo := sel_bswap32 (highlong e) in
        let hi := sel_bswap32 (lowlong e) in
        Some (Eop Omakelong (hi ::: lo ::: Enil))
      else
        SelectOp.platform_standard_builtin BI_i64_bswap args
  | BI_standard b, _=>
      SelectOp.platform_standard_builtin b args
  end.
...
Definition get_helpers (defmap: PTree.t globdef) : res helper_functions :=
  ...
  do i64_ftos <- lookup_helper globs "__compcert_i64_ftos" sig_s_l ;
  do i64_ftou <- lookup_helper globs "__compcert_i64_ftou" sig_s_l ;
  ...
  OK (mk_helper_functions
     i64_ftos i64_ftou i64_dtos i64_dtou i64_stod i64_utod i64_stof i64_utof
