...
- [bound_outgoing] number of stack slots required for arguments to function calls.
- [bound_local] number of stack slots required for values of spilled registers.
- [used_callee_save] callee-save registers used by the function.
- [bound_stack_data] number of stack slots required for stack-allocated local variables.
  *)
...
Inductive mreg_group := Single (r: mreg) | Multiple (l: list (rpair mreg)).

Fixpoint flatten_group_list (l: list mreg_group) :=
  match l with
  | Single r :: l => One r :: (flatten_group_list l)
  | Multiple rl :: l => rl ++ (flatten_group_list l)
  | nil => nil
  end.

Lemma flatten_group_app:
  forall l1 l2, flatten_group_list (l1 ++ l2) = flatten_group_list l1 ++ flatten_group_list l2.
Proof.
  induction l1; intros; simpl; auto.
  destruct a; simpl; try rewrite <- app_assoc; congruence.
Qed.
...
Record frame_env : Type := mk_frame_env {
  ...
  fe_stack_data: Z;
  fe_used_callee_save: list mreg_group
}.
...
Definition size_callee_save_area (fe: frame_env) (ofs: Z) : Z :=
  size_callee_save_area_rec (flatten_group_list (fe_used_callee_save fe)) ofs.
...
Lemma size_callee_save_area_rec_app:
  forall l1 l2 ofs,
    let sz1 := size_callee_save_area_rec l1 ofs in
    let sz2 := size_callee_save_area_rec l2 sz1 in
    size_callee_save_area_rec (l1 ++ l2) ofs = sz2.
Proof.
  induction l1; intros; simpl. auto. apply IHl1.
Qed.

Definition default_callee_save (rl: list mreg) := List.map Single rl.

Lemma default_callee_save_correspond:
  forall r l, In r (regs_of_rpairs (flatten_group_list (default_callee_save l))) <-> In r l.
Proof.
  intros r l. induction l; intros; simpl; tauto.
Qed.

Lemma default_callee_save_wellformed:
  forall p l, In p (flatten_group_list (default_callee_save l)) -> pair_wf p.
Proof.
