...
Proof.
  ...
  intros. unfold Mach.undef_caller_save_regs, Asm.undef_caller_save_regs. 
...
(* Note that we add all *_label lemmas with the general conclusion "tail_nolabel k c"
   to the hint database labels_gen. This speeds up the TailNoLabel tactic considerably
   because it tries all of these lemmas on all goals which results in a combinatorical explosion.
   This results only in a handful more subgoals. An alternative would be to use Hint Extern to
   restrict their application to when their assumptions are known but it is more work in the end.
 *)
Create HintDb labels_gen.
...
Lemma exec_straight_four:
  ...
  exec_instr ge fn i2 rs2 m2 = Next rs3 m3 ->
  exec_instr ge fn i3 rs3 m3 = Next rs4 m4 ->
  exec_instr ge fn i4 rs4 m4 = Next rs5 m5 ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
  rs4#PC = Val.offset_ptr rs3#PC Ptrofs.one ->
...
Lemma exec_straight_five:
  forall i1 i2 i3 i4 i5 c rs1 m1 rs2 m2 rs3 m3 rs4 m4 rs5 m5 rs6 m6,
  exec_instr ge fn i1 rs1 m1 = Next rs2 m2 ->
  exec_instr ge fn i2 rs2 m2 = Next rs3 m3 ->
  exec_instr ge fn i3 rs3 m3 = Next rs4 m4 ->
  exec_instr ge fn i4 rs4 m4 = Next rs5 m5 ->
  exec_instr ge fn i5 rs5 m5 = Next rs6 m6 ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
  rs4#PC = Val.offset_ptr rs3#PC Ptrofs.one ->
  rs5#PC = Val.offset_ptr rs4#PC Ptrofs.one ->
  rs6#PC = Val.offset_ptr rs5#PC Ptrofs.one ->
  exec_straight (i1 :: i2 :: i3 :: i4 :: i5 :: c) rs1 m1 c rs6 m6.
Proof.
  intros. apply exec_straight_step with rs2 m2; auto.
