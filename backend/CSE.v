...
Fixpoint find_last_reg (l: list reg) : option reg :=
  match l with
  | nil => None
  | r :: nil => Some r
  | _ :: l => find_last_reg l
...
Definition reg_valnum (n: numbering) (vn: valnum) : option reg :=
  find_last_reg (PMap.get vn n.(num_val)).
...
Definition reduce (op: A) (rl: list reg) (vl: list valnum) : A * list reg :=
  ...
  | None     => match regs_valnums n vl with
               | None => (op, rl)
               | Some rl' => (op, rl')
               end
  | Some res => res
...
Definition find_rhs_non_trivial (n: numbering) (op: operation) (vl: list valnum) : option reg :=
  if is_trivial_op op then
    None
  else
    match find_rhs n (Op op vl) with
    | Some r => Some r
    | _ => None
    end.

Definition reduce_reg (n: numbering) (r: reg) : reg :=
  let (n1, v) := valnum_reg n r in
  match reg_valnum n1 v with
  | None => r
  | Some r => r
  end.

Definition reduce_regs (n: numbering) (l: list reg) : list reg :=
  let (n1, vl) := valnum_regs n l in
  match regs_valnums n1 vl with
  | None => l
  | Some l => l
  end.

Definition reduce_builtin_arg (n: numbering) (arg: builtin_arg reg) : builtin_arg reg :=
  match arg with
  | BA r => BA (reduce_reg n r)
  | _ => arg
  end.

Definition reduce_builtin_args (n: numbering) (args: list (builtin_arg reg)) : list (builtin_arg reg) :=
  List.map (reduce_builtin_arg n) args.
...
Definition transf_instr (dm: defmap) (n: numbering) (instr: instruction) :=
  ...
  | Iop op args res s =>
      ...
      match find_rhs_non_trivial n1 op vl with
      | Some r =>
  ...
  | Istore chunk addr args src s =>
      let (n1, vl) := valnum_regs n args in
      let (addr', args') := reduce _ combine_addr n1 addr args vl in
      let src' := reduce_reg n1 src in
      Istore chunk addr' args' src' s
  | Icall sg ros args res s =>
      match is_known_runtime_function dm ros with
      | None =>
          let args' := reduce_regs n args in
          Icall sg ros args' res s
      | Some bf =>
  ...
  | Ibuiltin (EF_builtin name sg) args (BR res) s =>
      ...
      | Some bf =>
          ...
          match find_rhs n1 (Builtin bf args') with
          | Some r => Iop Omove (r :: nil) res s
          | None   => let args' := reduce_builtin_args n args in
                     Ibuiltin (EF_builtin name sg) args' (BR res) s
          end
      | None =>
          let args' := reduce_builtin_args n args in
          Ibuiltin (EF_builtin name sg) args' (BR res) s
      end
