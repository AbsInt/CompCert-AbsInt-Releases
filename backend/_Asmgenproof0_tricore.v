...
Lemma ireg_of_eq:
  forall r r', ireg_of r = OK r' -> preg_of r = r'.
Proof.
  unfold ireg_of; intros. destruct (preg_of r); inv H; auto.
Qed.

Lemma freg_of_eq:
  forall r r', freg_of r = OK r' -> preg_of r = r'.
Proof.
...
Lemma agree_undef_caller_save_regs:
  forall ms sp rs,
  agree ms sp rs ->
  agree (Mach.undef_caller_save_regs ms) sp (undef_non_preserved_regs rs).
Proof.
  intros. inv H. constructor; auto.
  intros. unfold Mach.undef_caller_save_regs, undef_non_preserved_regs. 
  rewrite proj_sumbool_is_false. 2: apply preg_of_not_SP. rewrite orb_false_l.
  destruct (is_caller_save r) eqn:Er; auto.
  destruct in_dec as [Hin|]; auto.
  exfalso. apply n. apply in_map. apply filter_In. split. apply all_mregs_complete.
  unfold mreg_preserved_across_call. rewrite Er; reflexivity.
Qed.
...
Inductive exec_straight: code -> regset -> mem -> 
                         code -> regset -> mem -> list regset -> Prop :=
  | exec_straight_one:
      forall i1 c rs1 m1 rs2 m2 pcl,
      exec_instr ge fn i1 rs1 m1 pcl = Next rs2 m2 pcl ->
      rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
      exec_straight (i1 :: c) rs1 m1 c rs2 m2 pcl
  | exec_straight_step:
      forall i c rs1 m1 rs2 m2 c' rs3 m3 pcl,
      exec_instr ge fn i rs1 m1 pcl = Next rs2 m2 pcl ->
      rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
      exec_straight c rs2 m2 c' rs3 m3 pcl ->
      exec_straight (i :: c) rs1 m1 c' rs3 m3 pcl.

Lemma exec_straight_trans:
  forall c1 rs1 m1 c2 rs2 m2 c3 rs3 m3 pcl,
  exec_straight c1 rs1 m1 c2 rs2 m2 pcl ->
  exec_straight c2 rs2 m2 c3 rs3 m3 pcl ->
  exec_straight c1 rs1 m1 c3 rs3 m3 pcl.
Proof.
  induction 1; intros.
...
Lemma exec_straight_two:
  forall i1 i2 c rs1 m1 rs2 m2 rs3 m3 pcl,
  exec_instr ge fn i1 rs1 m1 pcl = Next rs2 m2 pcl ->
  exec_instr ge fn i2 rs2 m2 pcl = Next rs3 m3 pcl ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
  exec_straight (i1 :: i2 :: c) rs1 m1 c rs3 m3 pcl.
Proof.
...
Lemma exec_straight_three:
  forall i1 i2 i3 c rs1 m1 rs2 m2 rs3 m3 rs4 m4 pcl,
  exec_instr ge fn i1 rs1 m1 pcl = Next rs2 m2 pcl ->
  exec_instr ge fn i2 rs2 m2 pcl = Next rs3 m3 pcl ->
  exec_instr ge fn i3 rs3 m3 pcl = Next rs4 m4 pcl ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
  rs4#PC = Val.offset_ptr rs3#PC Ptrofs.one ->
  exec_straight (i1 :: i2 :: i3 :: c) rs1 m1 c rs4 m4 pcl.
Proof.
  intros. apply exec_straight_step with rs2 m2; auto.
...
Lemma exec_straight_four:
  forall i1 i2 i3 i4 c rs1 m1 rs2 m2 rs3 m3 rs4 m4 rs5 m5 pcl,
  exec_instr ge fn i1 rs1 m1 pcl = Next rs2 m2 pcl ->
  exec_instr ge fn i2 rs2 m2 pcl = Next rs3 m3 pcl ->
  exec_instr ge fn i3 rs3 m3 pcl = Next rs4 m4 pcl ->
  exec_instr ge fn i4 rs4 m4 pcl = Next rs5 m5 pcl ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
  rs4#PC = Val.offset_ptr rs3#PC Ptrofs.one ->
  rs5#PC = Val.offset_ptr rs4#PC Ptrofs.one ->
  exec_straight (i1 :: i2 :: i3 :: i4 :: c) rs1 m1 c rs5 m5 pcl.
Proof.
  intros. apply exec_straight_step with rs2 m2; auto.
  eapply exec_straight_three; eauto.
Qed.

Lemma exec_straight_five:
  forall i1 i2 i3 i4 i5 c rs1 m1 rs2 m2 rs3 m3 rs4 m4 rs5 m5 rs6 m6 pcl,
  exec_instr ge fn i1 rs1 m1 pcl = Next rs2 m2 pcl ->
  exec_instr ge fn i2 rs2 m2 pcl = Next rs3 m3 pcl ->
  exec_instr ge fn i3 rs3 m3 pcl = Next rs4 m4 pcl ->
  exec_instr ge fn i4 rs4 m4 pcl = Next rs5 m5 pcl ->
  exec_instr ge fn i5 rs5 m5 pcl = Next rs6 m6 pcl ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  rs3#PC = Val.offset_ptr rs2#PC Ptrofs.one ->
  rs4#PC = Val.offset_ptr rs3#PC Ptrofs.one ->
  rs5#PC = Val.offset_ptr rs4#PC Ptrofs.one ->
  rs6#PC = Val.offset_ptr rs5#PC Ptrofs.one ->
  exec_straight (i1 :: i2 :: i3 :: i4 :: i5 :: c) rs1 m1 c rs6 m6 pcl.
Proof.
  intros. apply exec_straight_step with rs2 m2; auto.
  eapply exec_straight_four; eauto.
Qed.
...
Lemma exec_straight_steps_1:
  forall c rs m c' rs' m' pcl,
  exec_straight c rs m c' rs' m' pcl ->
  list_length_z (fn_code fn) <= Ptrofs.max_unsigned ->
  forall b ofs,
  rs#PC = Vptr b ofs ->
  Genv.find_funct_ptr ge b = Some (Internal fn) ->
  code_tail (Ptrofs.unsigned ofs) (fn_code fn) c ->
  plus step ge (State rs m pcl) E0 (State rs' m' pcl).
Proof.
  induction 1; intros.
  apply plus_one.
  econstructor; eauto.
...
Inductive exec_straight_opt: code -> regset -> mem -> code -> regset -> mem -> list regset -> Prop :=
  | exec_straight_opt_refl: forall c rs m pcl,
      exec_straight_opt c rs m c rs m pcl
  | exec_straight_opt_intro: forall c1 rs1 m1 c2 rs2 m2 pcl,
      exec_straight c1 rs1 m1 c2 rs2 m2 pcl ->
      exec_straight_opt c1 rs1 m1 c2 rs2 m2 pcl.

Lemma exec_straight_opt_left:
  forall c3 rs3 m3 c1 rs1 m1 c2 rs2 m2 pcl,
  exec_straight c1 rs1 m1 c2 rs2 m2 pcl ->
  exec_straight_opt c2 rs2 m2 c3 rs3 m3 pcl ->
  exec_straight c1 rs1 m1 c3 rs3 m3 pcl.
Proof.
...
Lemma exec_straight_opt_step:
  ...
  exec_instr ge fn i rs1 m1 pcl = Next rs2 m2 pcl ->
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  exec_straight_opt c rs2 m2 c' rs3 m3 pcl ->
  exec_straight (i :: c) rs1 m1 c' rs3 m3 pcl.
Proof.
  intros. inv H1. 
- apply exec_straight_one; auto.
...
Lemma exec_straight_opt_step_opt:
  ...
  rs2#PC = Val.offset_ptr rs1#PC Ptrofs.one ->
  exec_straight_opt c rs2 m2 c' rs3 m3 pcl ->
  exec_straight_opt (i :: c) rs1 m1 c' rs3 m3 pcl.
Proof.
...
Inductive match_stack: list Mach.stackframe -> Prop :=
  | match_stack_nil:
      match_stack nil
  | match_stack_cons: forall fb sp ra rs c s f tf tc,
      Genv.find_funct_ptr ge fb = Some (Internal f) ->
      transl_code_at_pc ge ra fb f c false tf tc ->
      sp <> Vundef ->
      match_stack s ->
      match_stack (Stackframe fb sp ra rs c :: s).
...
(** * Properties of save-/restore_callee_pair and store-/load_stack_rpairs *)

Lemma restore_callee_pair_extends:
  forall m m' sp ofs p (rs1 rs2 rs1': Mach.regset),
  Mem.extends m m' ->
  restore_callee_pair m sp ofs p rs1 = Some rs2 ->
  (forall (r: mreg), Val.lessdef (rs1 r) (rs1' r)) ->
  exists (rs2': Mach.regset), restore_callee_pair m' sp ofs p rs1' = Some rs2' /\ forall (r: mreg), Val.lessdef (rs2 r) (rs2' r).
Proof.
  destruct p; simpl; unfold load_stack; intros.
  - Destructor; try discriminate. exploit Mem.loadv_extends; eauto.
    intros (v2 & A & B). rewrite A. econstructor; split; eauto.
    intros. inv H0. destruct (mreg_eq r0 r). subst r0. rewrite !Regmap.gss; auto.
  ...
  - set (hireg := if Archi.big_endian then rlo else rhi) in *.
    ...
    exploit Mem.loadv_extends; eauto. intros (v1 & A & B).
    rewrite A. clear a0. exploit Mem.loadv_extends; eauto. intros (v2 & C & D).
    rewrite C. econstructor. split; eauto.
    inv H0. intros. destruct (mreg_eq r loreg). subst r. rewrite !Regmap.gss; auto.
    rewrite !Regmap.gso with (j := loreg); auto. destruct (mreg_eq r hireg). subst r.
    rewrite !Regmap.gss; auto. rewrite !Regmap.gso; auto.
Qed.

Lemma load_stack_rpairs_extends:
  forall l m m' (rs1 rs2 rs1': Mach.regset) sp ofs,
  Mem.extends m m' ->
  load_stack_rpairs m sp ofs l rs1 = Some rs2 ->
  (forall (r: mreg), Val.lessdef (rs1 r) (rs1' r)) ->
  exists rs2', load_stack_rpairs m' sp ofs l rs1' = Some rs2' /\ forall (r: mreg), Val.lessdef (rs2 r) (rs2' r).
Proof.
  induction l; simpl; intros.
  - exists rs1'. inv H0. split; auto.
  - destruct (restore_callee_pair m _ _ _ _) eqn: E1; try discriminate.
    exploit restore_callee_pair_extends; eauto. intros (rs3' & A & B).
    rewrite A. eapply IHl; eauto.
Qed.

Lemma store_stack_rpairs_swap:
  forall l m1 (rs : Mach.regset) ofs (rs' : Mach.regset) m2 sp,
    store_stack_rpairs m1 sp ofs l rs = Some m2 ->
    (forall r, In r (regs_of_rpairs l) -> rs'#r = rs#r) ->
    store_stack_rpairs m1 sp ofs l rs' = Some m2.
Proof.
  induction l as [|p l]; simpl; unfold save_callee_pair; intros; auto.
  assert (A: forall r, In r (regs_of_rpair p) -> rs' r = rs r) by (intros; auto with datatypes).
  assert (B: forall r, In r (regs_of_rpairs l) -> rs' r = rs r) by (intros; auto with datatypes).
  destruct p.
  - rewrite A; simpl; auto. destruct (store_stack _); try discriminate; eauto.
  - rewrite !A; try (destruct Archi.big_endian; simpl; auto; fail).
    destruct (store_stack _); try discriminate; eauto.
    destruct (store_stack _); try discriminate; eauto.
Qed.
