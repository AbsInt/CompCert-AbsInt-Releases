...
(*- E_COMPCERT_CODE_Inliningaux_001 *)
(*- #Justify_Derived "Module Inliningaux.ml is part of CompCert's inlining optimization. The heuristic decides whether a function should be inlined, but the actual translation is performed in the Inlining pass." *)
...
let static_called_once id io =
  if !Clflags.option_finline_functions_called_once then
    C2C.atom_is_static id && call_count id io <= 1 && not (address_taken id io) && not (C2C.atom_is_used id)
  else
    false
