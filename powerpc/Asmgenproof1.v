...
Proof.
  induction l as [|r l]; simpl; intros until rs2; intros ARCH LOAD MAP.
  - inversion MAP. inversion LOAD. simpl. reflexivity.
  - monadInv MAP.
    assert (TY: (mreg_type r) = Tany32).
    { unfold mreg_type. rewrite ARCH. destruct r; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
...
Proof.
  induction l as [|r l]; simpl; intros until m2; intros ARCH STOR MAP.
...
Proof. 
  ...
  - set (ty := mreg_type r) in *.
    ...
    + exploit IHl. rewrite <- B in STOR; eauto with asmgen.
      eapply store_stack_pregs_swap. rewrite <- B in STOR; auto with asmgen. apply STOR.
      auto with asmgen. discriminate.
      intros [rs' [C D]].
      exists rs'. split.
