...
Lemma ireg_of_not_GPR0''':
  forall m r, ireg_of m = OK r -> GPR0 <> r.
Proof.
  intros. destruct m; inv H; congruence.
Qed.
Global Hint Resolve ireg_of_not_GPR0''': asmgen.
...
Lemma exec_lmw_correct:
  forall l l' l'' rs1 sp ofs m rs2 rs1',
    Archi.ppc64 = false ->
    ...
    /\ agree rs2 sp rs2'.
Proof.
  induction l as [|r l]; simpl; intros until rs1'; intros ARCH LOAD ERR MAP AG.
  - exists rs1'. inv ERR. inv MAP. inv LOAD. split; auto.
  - monadInv ERR. simpl in MAP. monadInv MAP.
    assert (TY: (mreg_type x) = Tany32).
    { unfold mreg_type. rewrite ARCH. destruct x; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
...
Lemma exec_stmw_correct:
  forall l l' l'' (rs1: Mach.regset) (rs1': regset) sp ofs m1 m2 m1',
    Archi.ppc64 = false ->
    ...
    /\ Mem.extends m2 m2'.
Proof.
  induction l as [|r l]; simpl; intros until m1'; intros ARCH STORE ERR MAP AG MEXT.
...
(* geu n*)
  ...
  destruct (transl_cond_correct_1 cond (r :: nil) (Pmfcrbit x CRbit_1 :: k) rs m c EQ0) as [rs1 [EX1 [RES1 AG1]]].
  rewrite Val.cmpu_bool_ge_gt; auto.
  econstructor; split. eapply exec_straight_trans. eexact EX1.
  eapply exec_straight_one; simpl; reflexivity. simpl in RES1.
  rewrite RES1.
  intuition Simpl.
...
Opaque Int.eq.
  ...
  - (* Omove *)
  destruct res, r; simpl; inv H.
  destruct (preg_of r0) eqn:RES; destruct (preg_of r) eqn:ARG; inv H1;
  TranslOpSplitter. destruct r0; inv H1; TranslOpSplitter.
- (* Ointconst *)
...
+ (* small data *)
  econstructor; split. eapply exec_straight_two; simpl; reflexivity.
  split.  Simpl. rewrite (Val.add_commut (rs x)).
  rewrite small_data_area_addressing by auto. auto. rewrite add_zero_symbol_address; auto.
  intros; Simpl.
+ (* relative data *)
...
- (* Olongconst *)
  ...
  exists rs'; auto with asmgen.
- (* Oaddl *)
  destruct Archi.splitlong; monadInv H; ArgsInv; try TranslOpSimpl.
...
- (* Osubl *)
  destruct Archi.splitlong; monadInv H; ArgsInv; try TranslOpSimpl.
  assert (x <> x0) by (destruct x, x0; discriminate).
  generalize Val.subl'_hiword_lessdef. generalize Val.subl_loword_lessdef. intros.
  destruct (ireg_eq x0 x1 || ireg_eq x0 x3) eqn:?; inv EQ3.
  + econstructor; split. eapply exec_straight_three; simpl; eauto.
    repeat Splitter'.
    intuition Simpl.
...
- (* Ofloatoflong *)
  ...
  (* Ofsel *)
- econstructor; split. apply exec_straight_one; simpl; reflexivity.
  split. Simpl. unfold Val.cmpf_bool. destruct (rs x); auto. simpl.
  destruct (Float.cmp Cge f Float.zero); auto.
  destruct (rs x0); auto.
  destruct (rs x1); auto.
  intros. Simpl.
  (* Ointoffloat *)
...
-
  ...
  { intros. apply freg_of_eq in H. apply important_data_preg_1. rewrite <- H.
    auto with asmgen. }
  destruct res; inv EQ0. destruct (preg_of r1) eqn:RES; monadInv H0.
  + ArgsInv. exploit transl_select_op_correct; eauto. intros [rs' [E [LD INV]]].
    exists rs'; split; eauto. ErrorSingle.
  + ArgsInv. exploit transl_fselect_op_correct; eauto. intros [rs' [E [LD INV]]].
    exists rs'; split; eauto. ErrorSingle.
Qed.
