...
Theorem eval_bsel:
  forall c e1 e2 v1 v2 v3 v le,
    eval_expr ge sp e m le e1 v1 ->
    eval_expr ge sp e m le e2 v2 ->
    eval_expr ge sp e m le c v3 ->
    platform_builtin_sem BI_bsel (v3 :: v1 :: v2 :: nil) = Some v ->
    exists v', eval_expr ge sp e m le (bsel c e1 e2) v' /\ Val.lessdef v v'.
Proof.
  assert (EC: forall c le el i, eval_expr ge sp e m le (Eop (Ocmp c) el) (Vint i) ->
                    i = Int.zero \/ i = Int.one).
  { intros. inv H. simpl in *. inv H5.
    destruct (eval_condition c vl m). destruct b; inv H0; auto.
    inv H0.
  }
  assert (TB: unary_constructor_sound to_bool (fun v => Val.cmpu (Mem.valid_pointer m) Cne v Vzero)).
  { red;intros. unfold to_bool.
    exploit (eval_compu Cne le a x (Eop (Ointconst Int.zero) Enil) Vzero); eauto. repeat econstructor.
  }
  intros until le.  unfold bsel.
  case (bsel_match e1 e2); intros; inv H2.
  - exists (Val.select (Val.cmp_bool Ceq v3 (Vzero)) v2 v1 Tint).
    split. econstructor. repeat (econstructor; eauto). reflexivity.
    destruct v1, v2, v3; auto. simpl.
    unfold Builtins1.bsel, isel.
    destruct (Int.eq i1 Int.zero). simpl.
    exploit EC. apply H0. intros. destruct H2; rewrite H2; constructor.
    exploit EC. apply H. intros. destruct H2; rewrite H2; constructor.
  - exploit TB. exact H0. intros (v2' & E2' & L2).
    exists (Val.select (Val.cmp_bool Ceq v3 (Vzero)) v2' v1 Tint).
    split. econstructor. repeat (econstructor; eauto). reflexivity.
    destruct v1, v2, v3; auto. simpl.
    unfold Builtins1.bsel, isel, Val.cmpu in *.
    destruct (Int.eq i1 Int.zero). simpl.
    destruct (Int.eq i0 Int.zero) eqn:?; simpl in L2; rewrite Heqb in L2; inv L2; auto.
    exploit EC. apply H. intros. destruct H2; rewrite H2; constructor.
  - exploit TB. exact H. intros (v1' & E1' & L1).
    exists (Val.select (Val.cmp_bool Ceq v3 (Vzero)) v2 v1' Tint).
    split. econstructor. repeat (econstructor; eauto). reflexivity.
    destruct v1, v2, v3; auto. simpl.
    unfold Builtins1.bsel, isel, Val.cmpu in *.
    destruct (Int.eq i1 Int.zero). simpl.
    exploit EC. apply H0. intros. destruct H2; rewrite H2; constructor.
    destruct (Int.eq i Int.zero) eqn:?; simpl in L1; rewrite Heqb in L1; inv L1; auto.
  - exists (Val.cmpu (Mem.valid_pointer m) Cne (Val.select (Val.cmp_bool Ceq v3 (Vzero)) v2 v1 Tint) Vzero).
    split. econstructor. repeat (econstructor; eauto). reflexivity.
    destruct v1, v2, v3; auto. simpl.
    unfold Builtins1.bsel, isel.
    destruct (Int.eq i1 Int.zero). simpl.
...
Proof.
  ...
  - (* BI_isel *)
    ...
    inv SEL. inv SEM.
    exists (Val.select (Val.cmp_bool Ceq v1 (Vzero)) v2 v0 Tint); split.
    apply eval_Eop with (v2 :: v0 :: v1 :: nil).
    do 4 (constructor; auto).
    simpl. reflexivity.
    destruct v0, v1, v2; simpl; auto. unfold isel.
    destruct (Int.eq i0 Int.zero); auto.
  - (* BI_isel64 *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate. inv H3; try discriminate.
    simpl in *. destruct Archi.ppc64; try discriminate.
    inv SEL. inv SEM.
    exists (Val.select (Val.cmp_bool Ceq v1 (Vzero)) v2 v0 Tlong); split.
    apply eval_Eop with (v2 :: v0 :: v1 :: nil).
    do 4 (constructor; auto).
    simpl. reflexivity.
    destruct v0, v1, v2; simpl; auto. unfold isel.
  ...
  - (* BI_bsel *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate. inv H3; try discriminate.
    inv SEL. inv SEM.
    eapply eval_bsel; eauto.
  - (* BI_fsel *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate. inv H3; try discriminate.
    inv SEL. inv SEM.
    exists (Val.select (Val.cmpf_bool Cge v1 (Vfloat Float.zero)) v0 v2 Tfloat).
    split; auto.   apply eval_Eop with (v1 :: v0 :: v2 :: nil).
    do 4 (constructor; auto). reflexivity.
    destruct v1; auto. destruct v0; auto. destruct v2; auto.
    simpl. destruct (Float.cmp Cge f Float.zero); auto.
  - (* BI_mulhw *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
    inv SEL. TrivialExists.
  - (* BI_mulhwu *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate.
    inv SEL. TrivialExists.
  - (* BI_mulhd *)
...
Proof.
  ...
  destruct bf; try discriminate.
  - (* BI_negl *)
    inv ARGS; try discriminate. inv H0; inv SEL; inv SEM.
    TrivialExists.
  - (* BI_addl *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2 ; inv SEL; inv SEM.
    TrivialExists.
  - (* BI_subl *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2 ; inv SEL; inv SEM.
    TrivialExists.
...
Theorem eval_lowlong :
  forall le a b v,
  lowlong a = Some b ->
  eval_expr ge sp e m le a v ->
  exists v', eval_expr ge sp e m le b v' /\ Val.lessdef (Val.lowordoflong v) v'.
Proof.
  intros until v.
  assert (forall v a, eval_expr ge sp e m le a v -> eval_expr ge sp e m le (Eop Olowlong (a ::: Enil)) (Val.lowordoflong v)).
  {  intros. econstructor. econstructor; [|econstructor]. eexact H. auto. }
  unfold lowlong; case (lowlong_match a); intros; inv H0; InvEval; inv H0.
  - TrivialExists. rewrite Val.loword_addl. auto.
  - TrivialExists. rewrite Val.loword_subl; auto.
  - TrivialExists. rewrite Val.loword_negl; auto.
Qed.

Theorem eval_highlong :
  forall le a b v,
  highlong a = Some b ->
  eval_expr ge sp e m le a v ->
  exists v', eval_expr ge sp e m le b v' /\ Val.lessdef (Val.hiwordoflong v) v'.
Proof.
  intros. discriminate.
Qed.
