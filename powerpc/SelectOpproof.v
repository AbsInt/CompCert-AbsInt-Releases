...
Proof.
  ...
  - (* BI_bsel *)
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; try discriminate. inv H3; try discriminate.
    inv SEL. inv SEM.
    exists ((Val.zero_ext 8 (Val.select (Val.cmp_bool Ceq v1 Vzero) v2 v0 Tint))); split.
    simpl. unfold cast8unsigned, andimm. simpl.
    replace (Int.eq (Int.repr 255) Int.zero) with false by reflexivity.
    replace (Int.eq (Int.repr 255) Int.mone) with false by reflexivity.
    apply eval_Eop with ((Val.select (Val.cmp_bool Ceq v1 Vzero) v2 v0 Tint) :: nil).
    constructor.
    apply eval_Eop with (v2 :: v0 :: v1 :: nil).
    do 4(constructor; auto).
    simpl. reflexivity.
    constructor. simpl. rewrite Val.zero_ext_and by lia. simpl; reflexivity.
    unfold isel. unfold Val.select.
    destruct v1, v0, v2; auto. simpl.
