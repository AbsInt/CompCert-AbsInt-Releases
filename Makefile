...
ifeq ($(CLIGHTGEN_ONLY),true)
EXTRACTION=extraction/extractionClight.v
EXTRACTION_DEPS=extraction/extractionClight.v $(ARCH)/extractionMachdepClight.v
else
EXTRACTION=extraction/extraction.v
EXTRACTION_DEPS=extraction/extraction.v $(ARCH)/extractionMachdep.v
endif
...
lib/IEEE754_extra.vo : COQCOPTS += -w -deprecated-since-9.0
...
VLIB=Axioms.v Coqlib.v Intv.v Maps.v Ordered.v \
  Iteration.v Zbits.v Integers.v Archi.v IEEE754_extra.v Floats.v \
  Wfsimpl.v Decidableplus.v BoolEqual.v
ifneq ($(CLIGHTGEN_ONLY),true)
VLIB+=\
	 Parmov.v UnionFind.v FSetAVLplus.v Heaps.v Lattice.v \
	 Postorder.v IntvSets.v
endif
...
COMMON=Errors.v AST.v Linking.v \
  ...
  Values.v Smallstep.v Behaviors.v Determinism.v \
  Builtins0.v Builtins1.v Builtins.v Subtyping.v
ifneq ($(CLIGHTGEN_ONLY),true)
COMMON+=Separation.v Switch.v Unityping.v
endif


# Back-end modules (in backend/, $(ARCH)/)

BACKEND=Machregs.v Op.v Locations.v Conventions0.v Conventions1.v
ifneq ($(CLIGHTGEN_ONLY),true)
BACKEND+=\
  Cminor.v Cminortyping.v CminorSel.v \
  ...
  BaseExtraction.v BaseExtractionproof.v BaseExtractionspec.v \
  CSEdomain.v CombineOp.v CSE.v CombineOpproof.v CSEproof.v \
  NeedDomain.v NeedOp.v Deadcode.v Deadcodeproof.v \
  Unusedglob.v Unusedglobproof.v \
  BaseMerge.v BaseMergeproof.v \
  Conventions.v LTL.v \
  PointerCasts.v PointerCastsproof.v \
  PointerCasts1.v PointerCastsproof1.v \
  SimplPCasts.v SimplPCastsproof.v\
  SimplPCasts1.v SimplPCastsproof1.v \
  ...
  Asm.v Asmgen.v Asmgenproof1.v Asmgenproof0.v Asmgenproof.v
endif

# C front-end modules (in cfrontend/)

CFRONTEND=Ctypes.v Cop.v Csyntax.v Csem.v Ctyping.v Cstrategy.v Cexec.v \
  Initializers.v SimplExpr.v Clight.v SimplLocals.v
ifneq ($(CLIGHTGEN_ONLY),true)
CFRONTEND+=\
	SimplExprspec.v SimplExprproof.v ClightBigstep.v SimplLocalsproof.v \
	Initializersproof.v Cshmgen.v Cshmgenproof.v \
...
extraction/STAMP: $(FILES:.v=.vo) $(EXTRACTION_DEPS)
...
backend/Asmgenproof0.v: backend/_Asmgenproof0_tricore.v backend/_Asmgenproof0_generic.v Makefile.config
ifeq ($(ARCH),tricore)
	cp backend/_Asmgenproof0_tricore.v backend/Asmgenproof0.v
else
	cp backend/_Asmgenproof0_generic.v backend/Asmgenproof0.v
endif
