...
Proof.
  generalize Int.eq_dec Int64.eq_dec Ptrofs.eq_dec Float.eq_dec Float32.eq_dec typ_eq ident_eq eq_condition; intros.
  decide equality.
Defined.
...
Definition eval_operation
  ...
  | Osel ty, v1::v2::v3::nil =>
      let cond :=
        if Val.eq v1 Vone then Some true else
        if Val.eq v1 Vzero then Some false else None in
      Some (Val.select cond v2 v3 ty)
  | _, _ => None
  end.
...
Proof with (try exact I; try reflexivity; auto using Val.Vptr_has_type).
  ...
  (* sel *)
  - destruct (Val.eq v0 Vone)... apply Val.normalize_type.
    destruct (Val.eq v0 Vzero)... apply Val.normalize_type.
Qed.
...
Proof.
  ...
  destruct c;  try congruence; try reflexivity.
Qed.
...
Proof.
  ...
  - apply Val.select_inject; auto.
    destruct (Val.eq v Vone), (Val.eq v Vzero); auto;
    subst v; inv H4; right; try reflexivity.
Qed.
