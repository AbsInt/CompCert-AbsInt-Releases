...
Fixpoint transl_restorecallee ofs l (k : code) :=
  match l with
  | nil => OK k
  | r :: l => let ty := mreg_type r in
            let sz := AST.typesize ty in
            let ofs1 := align ofs sz in
            do c <- (transl_restorecallee (ofs1 + sz) l k); (loadind SP (Ptrofs.repr ofs1) ty r c)
  end.

Fixpoint transl_savecallee ofs l (k : code) :=
  match l with
  | nil => OK k
  | r :: l => let ty := mreg_type r in
            let sz := AST.typesize ty in
            let ofs1 := align ofs sz in
            do c <- (transl_savecallee (ofs1 + sz) l k); (storeind r SP (Ptrofs.repr ofs1) ty c)
  end.
