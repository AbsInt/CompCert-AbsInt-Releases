...
let offset_in_range ofs =
  I32.eq (Asmgen.high_s_20 ofs) _0l
...
let memcpy_aux sz src osrc dst odst =
  if Ptrofs.eq sz _1p then begin
    ...
    let osrc' = I32.add osrc _2l in
    let odst' = I32.add odst _2l in
    emit (Pmovh_mr (Reg1, ADimm (src,osrc)));
    emit (Pmovh_rm (ADimm (dst, odst), Reg1));
    emit (Pmovb_mr (Reg1, ADimm (src, osrc')));
    emit (Pmovb_rm (ADimm (dst, odst'), Reg1))
...
let memcpy_small_arg sz arg tmp =
  ...
  | BA_addrstack ofs ->
    ...
    let ofs' = I32.add ofs (Ptrofs.to_int sz) in
...
let expand_builtin_memcpy_small sz src dst =
  ...
  if Ptrofs.cmpu Cgt sz _8p then begin
    memcpy_aux sz rsrc osrc rdst odst;
    memcpy_aux (Ptrofs.sub sz _4p) rsrc (I32.add osrc _4l) rdst (I32.add odst _4l);
    memcpy_aux (Ptrofs.sub sz _8p) rsrc (I32.add osrc _8l) rdst (I32.add odst _8l)
  end else if Ptrofs.cmpu Cgt sz _4p then begin
    memcpy_aux sz rsrc osrc rdst odst;
    memcpy_aux (Ptrofs.sub sz _4p) rsrc (I32.add osrc _4l) rdst (I32.add odst _4l)
  end else
    memcpy_aux sz rsrc osrc rdst odst
...
let expand_builtin_vload chunk args dst =
  match args with
  | [BA (GPR addr)] ->
    expand_builtin_vload_common chunk addr Ptrofs.zero dst
  | [BA_addrstack ofs] ->
    expand_builtin_vload_common chunk Reg0 ofs dst
  | [BA_addptr(BA(GPR addr), BA_int ofs)] ->
    expand_builtin_vload_common chunk addr (Ptrofs.of_int ofs) dst
  | _ ->
    assert false

let vararg_start_ofs : I32.int option ref = ref None
...
let expand_builtin_va_start r =
  ...
  | None -> assert false
  | Some ofs ->
    expand_loadimm Reg1 ofs;
    emit (Padd (Reg1,Reg0));
    emit (Pmov_rm (ADimm (r, _0l), Reg1))

let expand_builtin_vstore_common chunk base ofs src =
  match chunk, src with
  | (Mbool | Mint8signed | Mint8unsigned), BA (GPR src) ->
    expand_accessind (fun a -> Pmovb_rm (a, src)) base ofs
  | (Mint16signed | Mint16unsigned), BA (GPR src) ->
...
let expand_builtin_vstore chunk args =
  ...
  | [BA_addptr(BA(GPR addr), BA_int ofs); src] ->
    expand_builtin_vstore_common chunk addr (Ptrofs.of_int ofs) src
  | _ ->
    assert false
...
let expand_ctz arg res =
  ...
  emit (Pjmp_l lbl1);
  emit (Plabel lbl2);
  emit (Pmov_ri (res, _32l));
  emit (Plabel lbl3)


(** Store the value in Reg1 in byte swapped order on the stack. *)
let expand_store_reg1_swapped () =
  emit (Pmovb_rm (ADimm (Reg0,_m5l), Reg1));
  emit (Psril (Reg1, _8l));
  emit (Pmovb_rm (ADimm (Reg0,_m6l), Reg1));
  emit (Psril (Reg1, _8l));
  emit (Pmovb_rm (ADimm (Reg0,_m7l), Reg1));
  emit (Psril (Reg1, _8l));
  emit (Pmovb_rm (ADimm (Reg0,_m8l), Reg1))
...
let expand_builtin_inline name args res =
  ...
  | ("__builtin_bswap" | "__builtin_bswap32"), [BA (GPR a)], BR (GPR res) ->
    ...
    emit (Pmov_mr (res, ADimm (Reg0, _m8l)))
  | ("__builtin_bswap16"), [BA (GPR a)], BR (GPR res) ->
    emit (Pmov_rr (Reg1, a));
    emit (Pmovb_rm (ADimm (Reg0,_m7l), Reg1));
    emit (Psril (Reg1, _8l));
    emit (Pmovb_rm (ADimm (Reg0,_m8l), Reg1));
    emit (Pmovh_mr (res, ADimm (Reg0, _m8l)));
    expand_zero_ext res _16l
...
let va_stack_offset sg =
  ...
  let args = (proj_sig_args sg) @ [AST.Tint] in
  (* We calculate the offset by using the loc_arguments_rec function with the
     fixed arguments from the signature and one additional artifiical argument that
     will be placed on the stack, then the offset of that argument is the start
     offset of the varargs *)
  let args = Conventions1.loc_arguments_rec args fixed _0 _0 in
...
let expand_instruction instr =
  ...
  | Pallocframe (sz, ofs) ->
    ...
    let sz' = I32.neg  sz in
    expand_addimm Reg0 sz';
    ...
    if sg.sig_cc.cc_vararg <> None then begin
      ...
      let va_ofs = I32.add sz (I32.repr ofs) in
      vararg_start_ofs := Some va_ofs
    end else
      vararg_start_ofs := None
  | Pfreeframe (sz, ofs) ->
    let sz = I32.repr sz in
    if I32.eq (Asmgen.high_s_18 sz) _0l then
      emit (Paddi (Reg0,sz))
    else
  ...
  | Pcmov (rc, rd, r1, r2 ) ->
    ...
    emit (Pbnz (rc, lbl_false));
    emit (Pmov_rr (rd, r1));
    emit (Pjmp_l lbl_after);
    emit (Plabel lbl_false);
    emit (Pmov_rr (rd, r2));
    emit (Plabel lbl_after);
  | Pff2i rd ->
  ...
  | Pjmp (r, sg) ->
    ...
    emit (Pjmp (r, sg))
  | Pjmp_s (id, sg) ->
    emit (Ploadsymbol (Reg1, id, Ptrofs.zero));
    emit (Psril (Reg1, _2l));
    emit (Pjmp (Reg1, sg))
  | Pjmp_p (r, sg) ->
    emit (Pmov_rr (Reg1, r));
...
let expand_function id fn =
  try
    ...
    expand id (* sp= *) (fst (preg_to_dwarf (GPR Reg0))) preg_to_dwarf expand_instruction fn.fn_code;
