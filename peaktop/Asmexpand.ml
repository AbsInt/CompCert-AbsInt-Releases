...
let expand_builtin_memcpy_small sz al src dst =
  ...
  let rsrc = memcpy_small_arg src tsrc in
  let rdst = memcpy_small_arg dst tdst in
  ...
  let rec copy sz =
    if sz >= 4 then begin
      ...
      emit (Pmovh_mr (Reg1, ADregpostincr rsrc));
      emit (Pmovh_rm (ADregpostincr rdst, Reg1));
      copy (sz - 2)
    end
    else if sz >= 1 then begin
      emit (Pmovb_mr (Reg1, ADregpostincr rsrc));
      emit (Pmovb_rm (ADregpostincr rdst, Reg1));
      copy (sz - 1)
    end
  in copy  sz
...
let memcpy_big_arg arg tmp =
  match arg with
  | BA (GPR r) ->
    if r <> tmp then emit (Pmov_rr (tmp, r));
    tmp
  | BA_addrstack ofs ->
    emit (Pmov_rr (tmp, Reg0));
    expand_addimm tmp ofs;
...
let expand_builtin_memcpy_big sz al src dst =
  ...
  let rsrc = memcpy_big_arg src tsrc in
  let rdst = memcpy_big_arg dst tdst in
