...
Inductive instruction: Type :=
  ...
  | Pmov_fucr (rd: reg)                  (**r move UCR to reg *)
  | Pmov_tucr (rd: reg)                  (**r move reg to UCR *)
  | Pcmov  (rcond rd r1 r2: reg)         (**r conditional move *)
  ...
  | Pfsqr (rd: reg)                     (**r floating point square root *)
  | Pfabs (rd: reg)                      (**r floating point absolute value *)
  | Pfneg (rd: reg)                      (**r floating point negate *)
  | Pff2i (rd: reg)                     (**r single to signed int32 conversion *)
  | Pff2iu (rd: reg)                    (**r single to unsigned int32 conversion *)
  | Pfi2f (rd: reg)                     (**r signed int32 to single conversion *)
  | Pfiu2f (rd: reg)                    (**r unsigned int32 to single conversion *)
  ...
  | Psb (rd: reg) (imm: int)              (* set bit *)
  | Prb (rd: reg) (imm: int)              (* reset bit *)
.
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pmov_fucr rd =>
    Next (nextinstr (rs#rd <- (rs#UCR))) m
  | Pmov_tucr r =>
    Next (nextinstr (rs#UCR <- (rs#r))) m
  | Pcmov c rd r1 r2 =>
    let v :=
      if Val.eq rs#c Vone then rs#r1 else
      if Val.eq rs#c Vzero then rs#r2 else Vundef in
  ...
  | Pff2i rd =>
    Next (nextinstr (rs#rd <- (Val.maketotal (Val.intofsingle rs#rd)))#TMP <- Vundef) m
  | Pff2iu rd =>
    Next (nextinstr (rs#rd <- (Val.maketotal (Val.intuofsingle rs#rd)))#TMP <- Vundef) m
  | Pfi2f rd =>
    Next (nextinstr (rs#rd <- (Val.maketotal (Val.singleofint rs#rd)))) m
  | Pfiu2f rd =>
    Next (nextinstr (rs#rd <- (Val.maketotal (Val.singleofintu rs#rd)))) m
  (** Branches *)
  | Pjmp_l l =>
    goto_label f l rs m
  | Pjmp r sg =>
