...
Proof.
  ...
  - case (addimm_match a); intros; InvEval; simpl.
    ...
    + subst. rewrite Val.add_commut. rewrite Val.sub_add_opp. rewrite Val.add_commut.
...
Proof.
  ...
  - destruct (use_fused_integer_arithmetic tt); subst; econstructor; split; EvalOp.
  - destruct (use_fused_integer_arithmetic tt); subst; econstructor; split; EvalOp.
    rewrite Val.add_commut; eauto.
  - subst.
...
Theorem eval_subimm:
  forall n, unary_constructor_sound (subimm n) (fun v => Val.sub v (Vint n)).
Proof.
  ...
  - subst n. intros. exists x; split; auto.
    ...
    destruct Archi.ptr64; auto. rewrite Ptrofs.sub_zero_l; auto.
  - destruct (subimm_match a); intros; InvEval; simpl.
   + TrivialExists.
   + subst. rewrite Int.add_commut. rewrite Val.sub_add_opp.
     rewrite Val.sub_add_opp. rewrite Val.add_assoc.
     simpl. rewrite <- Int.neg_add_distr. rewrite <- Val.sub_add_opp.
     TrivialExists.
   + subst. rewrite Val.sub_add_opp. rewrite Val.add_assoc.
     rewrite <- Val.sub_add_opp. TrivialExists.
   + TrivialExists.
Qed.
...
Proof.
  ...
  - subst. rewrite Val.sub_add_l. rewrite Val.sub_add_r.
    ...
    simpl. rewrite Int.add_commut. rewrite <- Int.sub_add_opp.
    apply eval_addimm; EvalOp.
...
Proof.
  ...
  predSpec Int.eq Int.eq_spec n Int.zero.
  intros; subst. exists x; split; auto. destruct x; simpl; auto.
  ...
  - destruct (Int.ltu (Int.add n n1) Int.iwordsize) eqn:?.
    + exists (Val.shl v1 (Vint (Int.add n n1))); split. EvalOp.
      ...
      rewrite Int.add_commut. rewrite Int.shl_shl; auto.
...
Proof.
  ...
  - destruct (Int.ltu (Int.add n n1) Int.iwordsize) eqn:?.
    ...
    subst. destruct v1; simpl; auto. rewrite Heqb.
    destruct (Int.ltu n1 Int.iwordsize) eqn:?; simpl; auto.
    rewrite LT. rewrite Int.add_commut. rewrite Int.shru_shru; auto.
    rewrite Int.add_commut; auto. subst. TrivialExists.
  - TrivialExists.
  - intros; TrivialExists.
Qed.
...
Proof.
  ...
  - destruct (Int.ltu (Int.add n n1) Int.iwordsize) eqn:?.
    ...
    destruct (Int.ltu n1 Int.iwordsize) eqn:?; simpl; auto.
    rewrite LT.
    rewrite Int.add_commut. rewrite Int.shr_shr; auto.
    rewrite Int.add_commut; auto.
    subst. TrivialExists.
  - TrivialExists.
  - intros; TrivialExists.
...
Proof.
  ...
  - destruct l as [ | k l ].
    + exploit (eval_shlimm i (x :: le) (Eletvar 0) x). constructor; auto.
      intros [v1 [A1 B1]].
      exploit (eval_shlimm j (x :: le) (Eletvar 0) x). constructor; auto.
      intros [v2 [A2 B2]].
      ...
      apply Val.lessdef_trans with (Val.add v1 v2); auto.
...
Proof.
  ...
  - subst. rewrite Val.mul_add_distr_l.
    exploit eval_mulimm_base; eauto. instantiate (1 := n).
    intros [v' [A1 B1]].
    exploit (eval_addimm (Int.mul n n2) le (mulimm_base n t2) v'). auto.
    intros [v'' [A2 B2]].
    exists v''; split; auto. eapply Val.lessdef_trans.
...
Proof.
  ...
  - predSpec Int.eq Int.eq_spec n Int.mone.
    ...
    + subst. rewrite Val.and_assoc. simpl. rewrite Int.and_commut. TrivialExists.
    + TrivialExists.
Qed.
...
Proof.
  intros. subst z. rewrite Int.sub_add_l. rewrite Int.sub_idem.
...
- (* shlimm - shruimm *)
  predSpec Int.eq Int.eq_spec (Int.add n1 n2) Int.iwordsize; simpl.
...
Proof.
  assert (DN: forall v, Val.lessdef (Val.notint (Val.notint v)) v).
  ...
  - subst x. TrivialExists. simpl.
    destruct v1; auto. simpl. rewrite Int.not_involutive; auto.
  - subst x. TrivialExists. simpl. rewrite Val.not_xor.
...
Proof.
  ...
  simpl. destruct (eval_condition cond vl m); unfold Val.select; auto.
  destruct b; auto.
Qed.
...
-  InvEval.
   replace v with (if Archi.ptr64 then Val.addl v1 (Vint n) else Val.add v1 (Vint n)).
...
Proof.
  ...
  - (* BI_madd *)
    ...
    inv H0; try discriminate. inv H2; try discriminate. inv H3; try discriminate.
    inv SEL. inv SEM.
    exists (Val.add (Val.mul v1 v0) v2); split.
    apply eval_Eop with (v1 :: v0 :: v2 :: nil).
    do 4 (constructor; auto). simpl. reflexivity.
    destruct v1, v0, v2; auto.
  - (* BI_msub *)
    inv ARGS; try discriminate.
    inv H0; try discriminate. inv H2; try discriminate. inv H3; try discriminate.
    inv SEL. inv SEM.
    exists (Val.sub (Val.mul v1 v0) v2); split.
    apply eval_Eop with (v1 :: v0 :: v2 :: nil).
    do 4 (constructor; auto). simpl. reflexivity.
    destruct v1, v0, v2; auto.
Qed.
