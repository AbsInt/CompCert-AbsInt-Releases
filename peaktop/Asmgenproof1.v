...
Proof.
  ...
  - (* osel *)
    destruct (reg_eq x1 x2) eqn:?; inv EQ4.
    + econstructor. split.
      eapply exec_straight_one; reflexivity.
      split; Simpl.
      destruct (Val.eq (rs x0) Vone); simpl; auto using Val.lessdef_normalize.
      destruct (Val.eq (rs x0) Vzero); simpl; auto using Val.lessdef_normalize.
      intros; Simpl.
    + econstructor. split.
      eapply exec_straight_one; reflexivity.
      split; Simpl.
...
Proof.
  ...
  { rewrite <- H1. destruct chunk; auto; simpl; symmetry. }
