...
(* Printing of instructions *)
    ...
    let print_instruction oc = function
      ...
      | Ploadsymbol (rd, id, ofs) ->
        fprintf oc "	MOV_WU	%a, hi18(%a+%a)\n" reg rd symbol id ptrofs ofs;
        fprintf oc "	SL_W	%a, 14\n" reg rd;
        fprintf oc "	ADD_WU	%a, lo14(%a+%a)\n" reg rd symbol id ptrofs ofs;
      | Pnop ->
        assert false (* should be removed in Asmexpand *)
      | Pbtbl (r1, tbl) ->
        fprintf oc "	MOV_W	%a, %a\n" reg Reg1 reg r1;
        ...
        begin match ef with
          | EF_annot (kind, txt, targs) ->
            begin match (P.to_int kind) with
              | 1 ->
                let annot = annot_text preg_annot "Reg0" txt args in
                fprintf oc "%s annotation: %S\n" comment annot
              | 2 -> let lbl = new_label () in
                fprintf oc "%a:\n" label lbl;
                add_ais_annot lbl preg_annot "Reg0" txt args
            | _ -> assert false
            end
          ...
          | EF_inline_asm(txt, sg, clob) ->
            ...
            print_inline_asm (fun oc ty r -> preg_asm oc r) oc txt sg args res;
