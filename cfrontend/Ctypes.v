...
Parameter supports_double : unit -> bool.
...
Definition default_argument_conversion (ty: type) : type :=
  match ty with
  | Tint (I8 | I16 | IBool) _ _ => Tint I32 Signed noattr
  | Tfloat _ _          => if (supports_double tt) then Tfloat F64 noattr else Tfloat F32 noattr
  | Tarray t sz a       => Tpointer t noattr
  | Tfunction _ _ _     => Tpointer ty noattr
  | _                   => remove_attributes ty
  end.
...
Definition rettype_of_type (t: type) : AST.rettype :=
  ...
  | Tfloat F32 _ => AST.Tsingle
  | Tfloat F64 _ => AST.Tfloat
  | Tpointer _ _ => AST.Trptr
  | Tarray _ _ _ | Tfunction _ _ _ | Tstruct _ _ | Tunion _ _ => AST.Tvoid
  end.

Definition argtype_of_type (t: type) : AST.argtype :=
  match t with
  | Tpointer _ _ | Tarray _ _ _ | Tfunction _ _ _ | Tstruct _ _ | Tunion _ _ => AST.Taptr
  | _ => Targ (typ_of_type t)
  end.

Fixpoint typlist_of_typelist (tl: typelist) : list AST.argtype :=
  match tl with
  | Tnil => nil
  | Tcons hd tl => argtype_of_type hd :: typlist_of_typelist tl
  end.
