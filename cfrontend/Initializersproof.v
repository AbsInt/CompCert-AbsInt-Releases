...
Variable ge: genv.
Variable oe: offset_env.
Variable ce_oe_match: precompute_offsets ge = oe.

Definition coe : composite_offset_env := {|
  coe_env := ge;
  coe_offsets := oe;
|}.
...
Proof.
  ...
  (* rval *)
  assert (constval coe l = OK vres) by (destruct (access_mode ty); congruence).
  ...
  (* field struct *)
  rewrite H0 in EQ.
  destruct (lookup_struct_offset_sound coe ce_oe_match _ _ _ EQ) as [x2 [EQ0 EQ3]].
  destruct x0; monadInv EQ2.
  simpl in EQ0, EQ3, H2. rewrite H1 in EQ0. inv EQ0.
  ...
  (* field union *)
  rewrite H0 in EQ.
  destruct (lookup_union_offset_sound coe ce_oe_match _ _ _ EQ) as [x2 [EQ0 EQ3]].
  destruct x0; monadInv EQ2.
  simpl in EQ0, EQ3, H2. rewrite H1 in EQ0. inv EQ0.
  exploit constval_rvalue; eauto. intro MV.
  split. congruence.
...
Lemma constval_simple:
  forall a v, constval coe a = OK v -> simple a.
Proof.
...
Lemma transl_init_single_sound:
  ...
  transl_init_single coe ty a = OK data ->
...
Fixpoint initialized_fields_of_struct (ms: members) (pos: Z) : res (list (offset * type)) :=
  match ms with
  | nil =>
      OK nil
  | m :: ms' =>
...
with exec_init_struct: mem -> block -> Z -> list (offset * type) -> initializer_list -> mem -> Prop :=
...
Lemma transl_init_rec_sound:
   (forall m b ofs bf ty i m',
    ...
    match_state s m b ->
    match bf with
    | Full => transl_init_rec coe s ty i ofs
    | Bits sz sg p w => transl_init_bitfield coe s ty sz p w i ofs
    end = OK s' ->
    match_state s' m' b)
/\ (forall m b ofs ty sz il m',
    exec_init_array m b ofs ty sz il m' ->
    forall s s',
    match_state s m b ->
    transl_init_array coe s ty il ofs = OK s' ->
    match_state s' m' b)
/\ (forall m b ofs flds il m',
    ...
    transl_init_struct coe s ms il ofs pos = OK s' ->
    match_state s' m' b).
Proof.
...
Theorem transl_init_sound:
  ...
  transl_init (composite_offsets_of_program p) ty i = OK data ->
  exists m2,
...
Proof.
  ...
  change (sizeof (composite_offsets_of_program p) ty) with (sizeof ge ty) in EQ.
  ...
  { eapply (proj1 (transl_init_rec_sound ge p.(prog_offsets) p.(prog_offsets_eq))); eauto. }
