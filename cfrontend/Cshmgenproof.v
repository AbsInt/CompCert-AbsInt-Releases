...
Inductive match_fundef (p: Clight.program) : Clight.fundef -> Csharpminor.fundef -> Prop :=
  | match_fundef_internal: forall f tf,
      transl_function (composite_offsets_of_program p) f = OK tf ->
...
Lemma make_memcpy_correct:
  ...
  step ge (State f s k e le m) E0 (State f Sskip k e le m').
Proof.
  assert (forall c p, make_ptrofs c = OK p -> Ptrofs.unsigned p = c).
  { unfold make_ptrofs. intros.
    destruct (zlt (-1) c); inv H.
    destruct (zlt c Ptrofs.max_signed); inv H1.
    generalize Ptrofs.max_signed_unsigned. intros.
    apply Ptrofs.unsigned_repr; lia.
  }
  intros. inv H2; try congruence.
  monadInv H4.
  exploit transl_alignof_blockcopy. eexact LINK. eexact EQ. intros [A B]. rewrite A, B in *.
  change le with (set_optvar None Vundef le) at 2.
  econstructor. econstructor; eauto. econstructor; eauto. constructor.
  generalize (H (alignof_blockcopy (prog_comp_env prog) ty) x1 EQ0). intros.
  generalize (H (Ctypes.sizeof (prog_comp_env prog) ty) x0 EQ1). intros. simpl.
  rewrite H2, H4.
  econstructor; eauto.
  apply alignof_blockcopy_1248. generalize (Ptrofs.unsigned_range x0). intros. lia.
...
Lemma match_env_alloc_variables:
  ...
  forall tvars te1,
  mmap (transl_var (composite_offsets_of_program cunit)) vars = OK tvars ->
  match_env e1 te1 ->
  exists te2,
...
Lemma transl_expr_lvalue:
  ...
  transl_expr (composite_offsets_of_program cunit) a = OK ta ->
  exists tb, transl_lvalue (composite_offsets_of_program cunit) a = OK (tb, bf)
...
- (* field struct *)
  ...
  { rewrite H0 in EQ1. unfold make_field_access in EQ1.
    destruct (lookup_struct_offset (composite_offsets_of_program cunit) id i) eqn:E; try discriminate.
    monadInv EQ1.
    destruct ((lookup_struct_offset_sound (composite_offsets_of_program cunit)) cunit.(prog_offsets_eq) id i o E) as [co' [H3 H4]].
    exploit field_offset_stable. eexact LINK. eauto. instantiate (1 := i). intros [A B].
    simpl in A, H1, H2, H4. congruence.
  }
  subst x1.
...
- (* field union *)
  ...
  assert (x1 = bf).
  { rewrite H0 in EQ1. unfold make_field_access in EQ1.
    destruct (lookup_union_offset (composite_offsets_of_program cunit) id i) eqn:E; try discriminate.
    monadInv EQ1.
    destruct ((lookup_union_offset_sound (composite_offsets_of_program cunit)) cunit.(prog_offsets_eq) id i o E) as [co' [H3 H4]].
...
Lemma transl_expr_lvalue_correct:
  (forall a v,
   ...
   forall ta (TR: transl_expr (composite_offsets_of_program cunit) a = OK ta) ,
   Csharpminor.eval_expr tge te le m ta v)
/\(forall a b ofs bf,
   Clight.eval_lvalue ge e le m a b ofs bf ->
   forall ta bf' (TR: transl_lvalue (composite_offsets_of_program cunit) a = OK (ta, bf')),
...
- (* field struct *)
  ...
  monadInv EQ0. simpl in EQ1, H2.
  pose proof (lookup_struct_offset_sound (composite_offsets_of_program cunit) cunit.(prog_offsets_eq)) id i (x0, bf') EQ1 as [co' [CO EQ2]].
  exploit field_offset_stable. eexact LINK. eauto. instantiate (1 := i). intros [A B].
  simpl in EQ2.
  rewrite <- B in EQ2. 
...
- (* field union *)
  unfold make_field_access in EQ0. rewrite H1 in EQ0.
  monadInv EQ0. simpl in EQ1, H2.
  pose proof (lookup_union_offset_sound (composite_offsets_of_program cunit) cunit.(prog_offsets_eq)) id i (x0, bf') EQ1 as [co' [CO EQ2]].
  exploit union_field_offset_stable. eexact LINK. eauto. instantiate (1 := i). intros [A B].
  simpl in EQ2.
  rewrite <- B in EQ2. 
  assert (x0 = delta) by (unfold ge in *; simpl in *; congruence).
...
Lemma transl_expr_correct:
   forall a v,
   Clight.eval_expr ge e le m a v ->
   forall ta, transl_expr (composite_offsets_of_program cunit) a = OK ta ->
   Csharpminor.eval_expr tge te le m ta v.
...
Lemma transl_lvalue_correct:
   forall a b ofs bf,
   Clight.eval_lvalue ge e le m a b ofs bf ->
   forall ta bf', transl_lvalue (composite_offsets_of_program cunit) a = OK (ta, bf') ->
   bf = bf' /\ Csharpminor.eval_expr tge te le m ta (Vptr b ofs).
...
Lemma transl_arglist_correct:
  ...
  forall tal, transl_arglist (composite_offsets_of_program cunit) al tyl = OK tal ->
...
Inductive match_cont: composite_offset_env -> type -> nat -> nat -> Clight.cont -> Csharpminor.cont -> Prop :=
  ...
  | match_Kcall: forall ce tyret nbrk ncnt nbrk' ncnt' f e k id tf te le tk cu,
      linkorder cu prog ->
      transl_function (composite_offsets_of_program cu) f = OK tf ->
      match_env e te ->
      match_cont (composite_offsets_of_program cu) (Clight.fn_return f) nbrk' ncnt' k tk ->
      match_cont ce tyret nbrk ncnt
...
Inductive match_states: Clight.state -> Csharpminor.state -> Prop :=
  | match_state:
      forall f nbrk ncnt s k e le m tf ts tk te ts' tk' cu
          ...
          (TRF: transl_function (composite_offsets_of_program cu) f = OK tf)
          (TR: transl_statement (composite_offsets_of_program cu) (Clight.fn_return f) nbrk ncnt s = OK ts)
          (MTR: match_transl ts tk ts' tk')
          (MENV: match_env e te)
          (MK: match_cont (composite_offsets_of_program cu) (Clight.fn_return f) nbrk ncnt k tk),
...
Section FIND_LABEL.
Variable ce: composite_offset_env.
Variable lbl: label.
Variable tyret: type.
...
- (* call *)
  ...
  + (* without normalization of return value *)
    ...
    econstructor; eauto.
    eapply match_Kcall with (ce := (composite_offsets_of_program cu')) (cu := cu); eauto.
    exact I.
  ...
  + (* with normalization of return value *)
    ...
    traceEq.
    econstructor; eauto.
    eapply match_Kcall_normalize  with (ce := (composite_offsets_of_program cu')) (cu := cu); eauto.
...
- (* return none *)
  ...
  eapply match_returnstate with (ce := (composite_offsets_of_program cu)); eauto.
...
- (* goto *)
  ...
  exploit (transl_find_label (composite_offsets_of_program cu) lbl). eexact EQ. eapply match_cont_call_cont. eauto.
  rewrite H.
  intros [ts' [tk'' [nbrk' [ncnt' [A [B C]]]]]].
  econstructor; split.
...
Proof.
  ...
  econstructor; eauto. instantiate (1 := (composite_offsets_of_program cu)). constructor; auto. exact I. constructor.
Qed.
