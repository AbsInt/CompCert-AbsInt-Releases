...
Definition is_bitfield_access (l: expr) : mon bitfield :=
  ...
  | Efield r f _ =>
      match typeof r with
      | Tstruct id _ => match lookup_struct_offset ce id f with | Error e => error e | OK (_, bf) => ret bf end 
      | Tunion id _  => match lookup_union_offset ce id f with | Error e => error e | OK (_, bf) => ret bf end
...
Definition transl_program (p: Csyntax.program) : res program :=
  do p1 <- AST.transform_partial_program (transl_fundef (composite_offsets_of_program p)) p;
  OK {| prog_defs := AST.prog_defs p1;
        ...
        prog_offsets := prog_offsets p;
        prog_offsets_eq := prog_offsets_eq p |}.
