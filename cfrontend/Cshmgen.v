...
Definition make_ptrofs (c : Z) : res ptrofs :=
  if zlt (-1) c && zlt c Ptrofs.max_signed then
    OK (Ptrofs.repr c)
  else
    Error (msg "Chsmgen.make_ptrofs_const").

Definition make_memcpy (ce: composite_env) (dst src: expr) (ty: type) :=
  do sz <- sizeof ce ty;
  do sz <- make_ptrofs sz;
  do al <- make_ptrofs (Ctypes.alignof_blockcopy ce ty);
  OK (Sbuiltin None (EF_memcpy sz al)
        (dst :: src :: nil)).
...
Definition make_field_access (ce: composite_offset_env) (ty: type) (f: ident) (a: expr) : res (expr * bitfield) :=
  do (ofs, bf) <-
    match ty with
    | Tstruct id _ => lookup_struct_offset ce id f
    | Tunion id _ => lookup_union_offset ce id f
    | _ =>
...
Fixpoint transl_expr (ce: composite_offset_env) (a: Clight.expr) {struct a} : res expr :=
  match a with
...
with transl_lvalue (ce: composite_offset_env) (a: Clight.expr) {struct a} : res (expr * bitfield) :=
  match a with
  | Clight.Evar id _ =>
      OK (Eaddrof id, Full)
  | Clight.Ederef b _ =>
...
Fixpoint transl_arglist (ce: composite_offset_env) (al: list Clight.expr) (tyl: list type)
...
Fixpoint transl_statement (ce: composite_offset_env) (tyret: type) (nbrk ncnt: nat)
...
with transl_lbl_stmt (ce: composite_offset_env) (tyret: type) (nbrk ncnt: nat)
...
Definition transl_var (ce: composite_offset_env) (v: ident * type) :=
...
Definition transl_function (ce: composite_offset_env) (f: Clight.function) : res function :=
...
Definition transl_fundef (ce: composite_offset_env) (id: ident) (f: Clight.fundef) : res fundef :=
  match f with
  | Internal g =>
...
Definition transl_program (p: Clight.program) : res program :=
  transform_partial_program2 (transl_fundef (composite_offsets_of_program p)) transl_globvar p.
