...
Variable ce_oe_match: precompute_offsets ce.(coe_env) = ce.(coe_offsets).
...
Definition tr_is_bitfield_access (l: expr) (bf: bitfield) : Prop :=
  ...
  | Efield r f _ =>
      ...
      match typeof r with
      | Tstruct id _ =>
          ce.(coe_env)!id = Some co /\ field_offset ce f (co_members co) = OK (ofs, bf)
      | Tunion id _ =>
          ce.(coe_env)!id = Some co /\ union_field_offset ce f (co_members co) = OK (ofs, bf)
      | _ => False
      end
  | _ => bf = Full
  end.
...
Proof.
  ...
  destruct (typeof l); try discriminate. 
  - destruct (lookup_struct_offset ce i0 i) as [[ofs1 bf1]|] eqn:E; inv H.
    pose proof (lookup_struct_offset_sound ce ce_oe_match i0 i (ofs1, bf) E) as [co H1].
    exists co, ofs1; assumption.
  - destruct (lookup_union_offset ce i0 i) as [[ofs1 bf1]|] eqn:E; inv H.
    pose proof (lookup_union_offset_sound ce ce_oe_match i0 i (ofs1, bf) E) as [co H1].
    exists co, ofs1; assumption.
Qed.
...
Inductive tr_fundef (p: Csyntax.program): Csyntax.fundef -> Clight.fundef -> Prop :=
  | tr_internal: forall f tf,
      tr_function (composite_offsets_of_program p) f tf ->
...
Lemma transl_fundef_spec:
  forall p fd tfd,
  transl_fundef (composite_offsets_of_program p) fd = OK tfd ->
  tr_fundef p fd tfd.
Proof.
  unfold transl_fundef; intros.
  destruct fd; Errors.monadInv H.
+ constructor. eapply transl_function_spec. exact p.(prog_offsets_eq). exact EQ.
+ constructor.
Qed.
