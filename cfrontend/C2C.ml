...
let check_composite_size su id =
  let id' = intern_string id.name in
  match Maps.PTree.get id' !comp_env with
  | Some co ->
...
let debug_set_struct_mem_ofs sid ((id, byte_ofs), bits) =
  ...
  | Bits (sz, sg, bit_pos, width) ->
    ...
    let sz = Z.to_int64 (bitalignof_intsize sz) in
