...
(*- E_COMPCERT_CODE_C2C_literal_status_001 *)
(*- #Justify_Derived "Type definitions" *)
type literal_status =
  | No_literal
  | String_literal
...
type atom_info =
  { a_storage: C.storage;              (* storage class *)
    ...
    a_bol: bool;                       (* atom supports tricore bol addressing *)
}
(*- #End *)
...
(*- E_COMPCERT_CODE_C2C_atom_is_external_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_C2C_atom_needs_GOT_access_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_C2C_atom_allows_bol_001 *)
(*- #Justify_Derived "Utility function" *)
let atom_allows_bol a =
  try
    (Hashtbl.find decl_atom a).a_bol
  with Not_found ->
    false
(*- #End *)

(*- E_COMPCERT_CODE_C2C_atom_literal_001 *)
(*- #Justify_Derived "Utility function" *)
let atom_literal a =
  try
...
(*- #Justify_Derived "Variable for global state" *)
let comp_env : composite_offset_env ref = ref empty_composite_offset_env
(*- #End *)
...
let builtins_generic = {
  ...
  builtin_functions =
    ...
    "__builtin_subl",
      (TInt(ILongLong,[]), [TInt(ILongLong,[]); TInt(ILongLong,[])], false);
    "__builtin_mull",
      (TInt(ILongLong,[]), [TInt(IUInt,[]); TInt(IUInt,[])], false);
    ...
    (TFloat (FDouble, []),  [TPtr(TInt(IChar, [AConst]), [])], false);
    "__builtin_isnanf",
    (TInt(C.IBool, []), [TFloat(FFloat, [])], false);
    "__builtin_isnan",
    (TInt(C.IBool, []), [TFloat(FDouble, [])], false);
    "__builtin_isinff",
    (TInt(C.IBool, []), [TFloat(FFloat, [])], false);
    "__builtin_isinf",
    (TInt(C.IBool, []), [TFloat(FDouble, [])], false);
    ...
    (TInt(C.IBool, []), [TFloat(FDouble, [])], false);
    "__builtin_inf",
    (TFloat (FDouble, []), [TVoid []], false);
    "__builtin_inff",
    (TFloat (FFloat, []), [TVoid []], false);
    "__builtin_huge_val",
    (TFloat (FDouble, []), [TVoid []], false);
    "__builtin_huge_valf",
    (TFloat (FFloat, []), [TVoid []], false);
...
] @ (if Configuration.arch = "tricore" then
    [
      (* object-related *)
      ("use_bol", Cutil.Attr_object);
      ("no_use_bol", Cutil.Attr_object);
]
  else
    [])
  @ (if Configuration.alias_supported then
       [("alias", Cutil.Attr_function);]
     else
       [])
(*- #End *)
...
let constant_size_t a =
  ...
  | Errors.OK(Vint n) ->
    ...
    Some(Integers.Ptrofs.of_int n)
  | Errors.OK(Vlong n) ->
    assert (Archi.ptr64);
    Some(Integers.Ptrofs.of_int64 n)
  | _ -> None
(*- #End *)

(*- E_COMPCERT_CODE_C2C_ezero_001 *)
(*- #Justify_Derived "Utility constant" *)
let ezero = Eval(Vint _0l, type_int32s)
(*- #End *)
...
let make_builtin_memcpy args =
  ...
  | Econs(dst, Econs(src, Econs(sz, Econs(al, Enil)))) ->
      let sz1 =
        ...
        | Some n -> n
        | None -> error "size argument of '__builtin_memcpy_aligned' must be a constant"; Integers.Ptrofs.zero in
      let al1 =
        match constant_size_t al with
        | Some n -> n
        | None -> error "alignment argument of '__builtin_memcpy_aligned' must be a constant"; Integers.Ptrofs.repr (Z.one) in
      if not (Z.is_power2 (Integers.Ptrofs.unsigned al1)) then
        error "alignment argument of '__builtin_memcpy_aligned' must be a power of 2";
      if not (Z.eq (Z.modulo (Integers.Ptrofs.unsigned sz1) (Integers.Ptrofs.unsigned al1)) Z.zero) then
...
let va_list_ptr e =
  if not CBuiltins.va_list_scalar then e else
    ...
    (*- #Link_to E_COMPCERT_TR_Robustness_C2C_038 *)
...
let make_builtin_va_arg env ty e =
  ...
  | Tlong _ ->
      make_builtin_va_arg_by_val
        "__compcert_va_int64" ty (Tlong(Unsigned, noattr)) e
  | Tfloat _ ->
      if Configuration.has_double then
        make_builtin_va_arg_by_val
  ...
  | _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_035 *)
...
let convertFkind k a : coq_type =
  match k with
  | C.FFloat16 ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_047 *)
  ...
  | C.FDouble ->
    ...
    if Configuration.has_double then begin
      Tfloat (F64, a)
    end else
      Tfloat (F32, a)
  | C.FLongDouble ->
...
let convertField env sid f =
  ...
  | Some w ->
      ...
      | _ ->
          (*- #Link_to E_COMPCERT_TR_Robustness_C2C_046 *)
...
(*- E_COMPCERT_CODE_C2C_gensym_string_literal_001 *)
(*- #Link_to E_COMPCERT_TR_Function_C2C_009 *)
let rec gensym_string_literal md5 =
  incr stringNum;
  let name = Printf.sprintf "__stringlit_%d_%s" !stringNum md5 in
  if Hashtbl.mem atom_of_string name
  then gensym_string_literal md5
  else name
(*- #End *)
...
let name_for_string_literal s =
  ...
  with Not_found ->
    let md5 = Digest.(to_hex (string s)) in
    let name = gensym_string_literal md5 in
...
let name_for_wide_string_literal s ik =
  ...
  with Not_found ->
    let md5 =
      (* Create a string from the int64 list containing the data *)
      let data_string = String.concat "" (List.map Int64.to_string s) in
      (* Compute the Digest and convert it to a hex string *)
      Digest.(to_hex (string data_string)) in
    let name = gensym_string_literal md5 in
    let id = intern_string name in
...
let global_for_wide_string (s, ik) id =
  ...
  let init_of_char =
    ...
    | _ -> assert false in
  let add_char c =
    (* Conversion is safe here since c is checked to be a 32 bit constant *)
...
let checkFloatOverflow f typ =
  ...
  | Binary.B754_nan _ ->
      (*- #Link_to E_COMPCERT_TR_Function_C2C_045 *)
      warning Diagnostics.Literal_range "floating-point converts to 'NaN'"
(*- #End *)
  ...
  | FBits pl ->
    begin match kind with
      | FFloat16 ->
        (*- #Link_to E_COMPCERT_TR_Robustness_C2C_047 *)
        unsupported "'_Float16' type";
        Ctyping.econst_single (Float32.of_bits (coqint_of_camlint (Int64.to_int32 pl)))
      | FFloat ->
        Ctyping.econst_single (Float32.of_bits (coqint_of_camlint (Int64.to_int32 pl)))
      | FDouble | FLongDouble ->
        if Configuration.has_double then
          ...
          Ctyping.econst_single (Float.to_single (Float.of_bits (coqint_of_camlint64 pl)))
      end
...
let ewrap = function
  | Errors.OK e -> e
  | Errors.Error msg ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_036 *)
...
let rec convertExpr env e =
  ...
  | C.ECompound(ty1, ie) ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_039 *)
 ...
 | C.ECall({edesc = C.EVar {name = "__builtin_memcpy_aligned"}}, args) ->
  ...
  | C.ECall({edesc = C.EVar {name = "__builtin_fabs"}}, [arg]) when Configuration.has_double ->
  ...
  | C.ECall({edesc = C.EVar {name = "__builtin_va_copy"}}, [arg1; arg2]) ->
      ...
      Ebuiltin(AST.EF_memcpy(Integers.Ptrofs.repr (Z.of_uint CBuiltins.size_va_list), Integers.Ptrofs.repr (Z.of_uint 4)),
  ...
  (*- #Link_to E_COMPCERT_TR_Function_C2C_049 *)
  | C.ECall({edesc = C.EVar {name = "__builtin_assume"}}, [arg]) ->
      Ecast ((Ctyping.econst_int _0l Signed), Tvoid)
  ...
  | C.ECall(fn, args) ->
      ...
      | None ->
          (*- #Link_to E_COMPCERT_TR_Robustness_C2C_037 *)
...
and convertLvalue env e =
  ...
  | C.EUnop(C.Odot id, e1) ->
      ewrap (Ctyping.efield !comp_env.coe_env (convertExpr env e1) (intern_string id))
  | C.EUnop(C.Oarrow id, e1) ->
      let e1' = convertExpr env e1 in
      let e2' = ewrap (Ctyping.ederef e1') in
      let e3' = ewrap (Ctyping.evalof e2') in
      ewrap (Ctyping.efield !comp_env.coe_env e3' (intern_string id))
  ...
  | _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_043 *)
...
let rec convertStmt env s =
  ...
  | C.Sblock _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_032 *)
      unsupported "nested blocks"; Csyntax.Sskip
  | C.Sdecl _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_033 *)
...
(*- E_COMPCERT_CODE_C2C_convertSwitchCase_001 *)
(*- #Link_to E_COMPCERT_TR_Function_C2C_007 *)
and convertSwitchCase env is_64 lbl s k =
  let lbl' =
    ...
    | C.Scase (e, n) ->
        begin
          match Initializers.constval !comp_env (convertExpr env e) with
          | Errors.OK (Vint v) ->
            assert (not is_64);
            assert ((Int64.to_int32 n) = camlint_of_coqint v);
            let v = Integers.Int.unsigned v in
            Some v
          | Errors.OK (Vlong v) ->
            assert is_64;
            assert (n = camlint64_of_coqint v);
            let v = Integers.Int64.unsigned v in
            Some v
          | Errors.OK _ ->
            error "expression of 'case' label is not an integer constant expression"; None
          | Errors.Error msg ->
            error "retyping error: %s" (string_of_errmsg msg); None
        end
    | _ -> assert false in
    ...
    List.map
      (fun (sto, id, ty, init) ->
        if sto = Storage_extern || sto = Storage_static then
          (*- #Link_to E_COMPCERT_TR_Robustness_C2C_040 *)
          unsupported "'static' or 'extern' local variable";
        if init <> None then
          (*- #Link_to E_COMPCERT_TR_Robustness_C2C_041 *)
...
(*- E_COMPCERT_CODE_C2C_decl_weak_alias_001 *)
(*- #Justify_Derived "Variable for global state" *)
let decl_weak_alias : (AST.ident, AST.ident ) Hashtbl.t = Hashtbl.create 10
(*- #End *)

(*- E_COMPCERT_CODE_C2C_alias_targets_001 *)
(*- #Justify_Derived "Variable for global state" *)
let alias_targets : AST.ident list ref = ref []
(*- #End *)

(*- E_COMPCERT_CODE_C2C_find_weak_alias_001 *)
(*- #Justify_Derived "Utility function" *)
let find_weak_alias = Hashtbl.find_opt decl_weak_alias
(*- #End *)

(*- E_COMPCERT_CODE_C2C_handle_alias_001 *)
(*- #Justify_Derived "Utility function" *)
let handle_alias id attr =
  match Cutil.get_alias_attribute attr with
  | None -> ()
  | Some s ->
    let s = intern_string s in
    Hashtbl.add decl_weak_alias id s;
    alias_targets := s :: !alias_targets
(*- #End *)
...
(*- E_COMPCERT_CODE_C2C_size_limit_001 *)
(*- #Justify_Derived "Utility constant" *)
let size_limit = Ptrofs.max_signed
(*- #End *)

(*- E_COMPCERT_CODE_C2C_size_limit_string_001 *)
(*- #Justify_Derived "Utility constant" *)
let size_limit_string = Z.to_string size_limit
(*- #End *)
...
let convertGlobvar loc env (sto, id, ty, optinit) =
  ...
  let sz = Ctypes.sizeof !comp_env.coe_env ty' in
  let al = Z.max (Ctypes.alignof !comp_env.coe_env ty') (Memdata.min_safe_alignment sz) in
  let attr = Cutil.attributes_of_type env ty in
  let init' =
    match optinit with
    | None ->
  ...
  if Z.ge sz size_limit then
    (*- #Link_to E_COMPCERT_TR_Robustness_C2C_016 *)
    fatal_error "size of variable '%s' (%s bytes) exceeds allowed maximum (%s bytes)"
                   id.name (Z.to_string sz) size_limit_string;
  ...
  let a_bol =
    if !Clflags.option_muse_bol then
      (* If the cmd line option is given we must check if it has no no_use_bol attribute *)
      not (Cutil.has_no_use_bol_attribute attr)
    else
      (* Otherwise we must check if it has a use_bol and no no_use_bol attribute *)
      Cutil.has_use_bol_attribute attr
      && not (Cutil.has_no_use_bol_attribute attr) in
  Hashtbl.add decl_atom id'
    { a_storage = sto;
...
(*- E_COMPCERT_CODE_C2C_check_composite_size_001 *)
let check_composite_size su id =
  let id' = intern_string id.name in
  match Maps.PTree.get id' !comp_env.coe_env with
  | Some co ->
    if Z.ge co.co_sizeof size_limit then
      (*- #Link_to E_COMPCERT_TR_Robustness_C2C_048 *)
      fatal_error "size of type '%s %s' (%s bytes) exceeds allowed maximum (%s bytes)"
        (match co.co_su with Ctypes.Struct -> "struct" | Ctypes.Union -> "union") id.name
        (Z.to_string co.co_sizeof) size_limit_string;
  | None -> ()
(*- #End *)
...
let rec convertGlobdecls env res gl =
  ...
  | g :: gl' ->
      ...
      | C.Gcompositedef(su, id, _, _) ->
        check_composite_size su id;
  ...
  let classify_def g =
    ...
    | C.Gfundef fd ->
        if IdentSet.mem fd.fd_name !strong then
          (*- #Link_to E_COMPCERT_TR_Robustness_C2C_034 *)
...
let public_globals gl =
  List.fold_left
    (fun accu (id, g) ->
       if atom_is_static id && not (atom_is_used id)
          && not (List.mem id !alias_targets)
          && not (Hashtbl.mem decl_weak_alias id) then
         accu
       else id :: accu)
    [] gl
(*- #End *)
...
(*- E_COMPCERT_CODE_C2C_debug_set_struct_ofs_001 *)
(*- #Justify_Derived "Functional decomposition" *)
  ...
  if !Clflags.option_g then
    List.iter (function
        | Composite (sid, Ctypes.Struct, ms, a) ->
          ...
          let ofs = List.fold_left (fun acc m ->
              if not (field_zero_or_padding m) then
                let id = Ctypes.name_member m in
                match Maps.PTree.get id layout with
                | Some (Errors.OK (byte_ofs, bits)) ->
                  (id, byte_ofs, bits) :: acc
                | _ -> acc
              else
                acc) [] ms in
          Debug.set_members_offset ~str_id:sid ofs
        | _ -> ()) typs
(*- #End *)
