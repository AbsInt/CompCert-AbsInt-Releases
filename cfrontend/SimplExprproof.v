...
Let ce := composite_offsets_of_program cunit.
...
Proof.
  assert (A: forall id co co',
             tge.(genv_cenv)!id = Some co -> ce.(coe_env)!id = Some co' ->
...
Inductive match_cont : composite_offset_env -> Csem.cont -> cont -> Prop :=
  ...
  | match_Kcall: forall f e C ty k optid tf le sl tk a dest tmps cu ce,
      linkorder cu prog ->
      tr_function (composite_offsets_of_program cu) f tf ->
      leftcontext RV RV C ->
      (forall v m, tr_top (composite_offsets_of_program cu) tge e (set_opttemp optid v le) m dest (C (Csyntax.Eval v ty)) sl a tmps) ->
      match_cont_exp (composite_offsets_of_program cu) dest a k tk ->
      match_cont ce (Csem.Kcall f e C ty k)
                    (Kcall optid tf e le (Kseqlist sl tk))

with match_cont_exp : composite_offset_env -> destination -> expr -> Csem.cont -> cont -> Prop :=
...
Inductive match_states: Csem.state -> state -> Prop :=
  | match_exprstates: forall f r k e m tf sl tk le dest a tmps cu
      ...
      (TRF: tr_function (composite_offsets_of_program cu) f tf)
      (TR: tr_top (composite_offsets_of_program cu) tge e le m dest r sl a tmps)
      (MK: match_cont_exp (composite_offsets_of_program cu) dest a k tk),
  ...
  | match_regularstates: forall f s k e m tf ts tk le cu
      (LINK: linkorder cu prog)
      (TRF: tr_function (composite_offsets_of_program cu) f tf)
      (TR: tr_stmt (composite_offsets_of_program cu) s ts)
      (MK: match_cont (composite_offsets_of_program cu) k tk),
...
- (* expr *)
  assert (tr_expr (composite_offsets_of_program cu) le dest r sl a tmps).
...
- (* return *)
  specialize (MK empty_composite_offset_env). inv MK.
  econstructor; split.
  left; apply plus_one. constructor.
  econstructor; eauto.
Qed.
...
Proof.
  intros. inv H0. inv H. specialize (MK empty_composite_offset_env). inv MK. constructor.
Qed.
