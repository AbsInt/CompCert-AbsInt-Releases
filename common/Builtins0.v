...
(*- E_COMPCERT_FTR_Function_Builtins0_valxty_001 *)
(*- #Justify_Derived "Auxiliary definition" *)
...
Definition standard_builtin_sig (b: standard_builtin) : signature :=
  match b with
  | BI_select t =>
      let t := inj_typ_xtype t in [Xint; t; t ---> t]
  | BI_fabs | BI_fsqrt =>
      [Xfloat ---> Xfloat]
  | BI_fabsf =>
      [Xsingle ---> Xsingle]
...
Program Definition standard_builtin_sem (b: standard_builtin) : builtin_sem (sig_res (standard_builtin_sig b)) :=
  ...
  | BI_i64_ftos => mkbuiltin_n1p Tsingle Xlong Float32.to_long
  | BI_i64_ftou => mkbuiltin_n1p Tsingle Xlong Float32.to_longu
  ...
  | BI_dtob => mkbuiltin_n1p Tfloat Xlong (fun f => if Binary.is_nan _ _ f then None else Some (Float.to_bits f))
  | BI_isnanf =>
      mkbuiltin_n1t Tsingle Xbool (fun f => if Binary.is_nan _ _ f then Int.one else Int.zero)
  | BI_isnan =>
      mkbuiltin_n1t Tfloat Xbool (fun f => if Binary.is_nan _ _ f then Int.one else Int.zero)
  | BI_isinff =>
      mkbuiltin_n1t Tsingle Xbool (fun f => if negb (Binary.is_finite _ _ f || Binary.is_nan _ _ f) then Int.one else Int.zero)
  | BI_isinf =>
      mkbuiltin_n1t Tfloat Xbool (fun f => if negb (Binary.is_finite _ _ f || Binary.is_nan _ _ f) then Int.one else Int.zero)
  | BI_isfinitef =>
      mkbuiltin_n1t Tsingle Xbool (fun f => if Binary.is_finite _ _ f then Int.one else Int.zero)
  | BI_isfinite =>
      mkbuiltin_n1t Tfloat Xbool (fun f => if Binary.is_finite _ _ f then Int.one else Int.zero)
  end.
Next Obligation. 
  red. destruct vl; auto. destruct v; auto. 
  destruct vl; auto. destruct vl; auto. destruct vl; auto.
  apply Val.has_inj_typ_xtype. apply Val.normalize_type.
Qed.
Next Obligation.
...
Qed.
Next Obligation.
  symmetry. apply Int.zero_ext_range_eq. change Int.zwordsize with 32; lia.
  unfold Int.ltu. apply zlt_true.
  change (Int.unsigned (Int.repr (two_p 16))) with 65536.
  unfold decode_int.
  generalize (int_of_bytes_range (rev_if_be (rev (encode_int 2 (Int.unsigned n))))).
  rewrite !rev_if_be_length. try rewrite List.rev_length. rewrite encode_int_length. 
  change (two_p (Z.of_nat 2 * 8)) with 65536. intros.
