...
Lemma range_merge:
  ...
  m |= range b lo hi ** P.
Proof.
  intros. rewrite <- sep_assoc in H0. eapply sep_imp; eauto.
  split; simpl; intros.
- intuition auto.
  destruct (zlt i mid).
  + apply H7; lia.
...
Lemma contains_range:
  forall chunk b ofs P spec m,
  m |= contains chunk b ofs spec ** P ->
  ofs + size_chunk chunk <= Ptrofs.modulus ->
  m |= range b ofs (ofs + size_chunk chunk) ** P.
Proof.
  intros. eapply sep_imp. exact H. 2: trivial.
  unfold massert_imp. split.
  - intros. unfold contains in H1. destruct H1 as (A & B & C). unfold range, m_pred.
    split. lia. split. lia. intros. unfold Mem.valid_access, Mem.range_perm in B.
    destruct B. apply Mem.perm_implies with (p1:=Freeable). 2: apply perm_F_any.
    destruct k.
    apply Mem.perm_cur_max, H2. lia.
    apply H2. lia.
  - simpl. trivial.
Qed.
...
Lemma range_empty_1:
  forall b i,
  0 <= i <= Ptrofs.modulus ->
  massert_imp (pure True) (range b i i).
Proof.
  intros. red; simpl; split; intros.
- split. lia. split. lia. intros. lia.
- destruct H0. lia.
Qed.
...
Lemma range_empty_2:
  ...
  m |= P -> m |= range b i i ** P.
Proof.
  intros. eapply sep_imp. apply sep_pure.
  exact (conj I H0). apply range_empty_1. assumption.
  reflexivity.
Qed.
...
Lemma free_parallel_rule_0:
  forall j m1 b1 sz m1' m2 b2 P,
  m2 |= minjection j m1 ** P ->
  Mem.free m1 b1 0 sz = Some m1' ->
  j b1 = Some (b2, 0) ->
  exists m2',
     Mem.free m2 b2 0 sz = Some m2'
  /\ m2' |= minjection j m1' ** P.
Proof.
  intros.
  destruct H as (A & B & C).
  ...
  intros (m2' & D & E).
  rewrite 2 Z.add_0_r in D.
  exists m2'; split; auto. split; [|split].
  - assumption.
  - apply (m_invar P) with m2; auto.
    eapply Mem.free_unchanged_on; eauto.
    intros; red; intros. eelim C; eauto. simpl.
    exists b1, 0; split; auto.
    apply Mem.perm_cur_max. apply Mem.perm_implies with Freeable; auto with mem.
    eapply Mem.free_range_perm; eauto. lia.
  - red; simpl; intros. eelim C; eauto.
    simpl. destruct H as (b0 & delta0 & U & V).
    exists b0, delta0; split; auto.
    eapply Mem.perm_free_3; eauto.
Qed.
...
Lemma free_rule:
  forall m b lo hi P,
  m |= range b lo hi ** P ->
  exists m', Mem.free m b lo hi = Some m' /\ m' |= P.
Proof.
  intros.
  destruct (Mem.range_perm_free m b lo hi) as (m' & ?).
  unfold range in H. simpl in H. decompose [Logic.and] H.
  unfold Mem.range_perm. intros. apply H4. assumption.
  exists m'. split; auto.
  eapply m_invar. eapply sep_proj2. exact H.
  eapply Mem.free_unchanged_on. exact e.
  intros. unfold not. intros.
  unfold sepconj in H. simpl in H. decompose [Logic.and] H.
  unfold disjoint_footprint in H7. eapply H7. 2: exact H1. simpl. split; auto.
Qed.
...
Lemma alloc_parallel_rule_0:
  forall (F V: Type) (ge: Genv.t F V) m1 sz m1' b m2 P j,
  m1' |= minjection j m1 ** globalenv_inject ge j ** P ->
  Mem.alloc m1 0 sz = (m2, b) ->
  exists j1 m2' b',
     m2' |= minjection j1 m2 ** globalenv_inject ge j1 ** P
  /\ Mem.alloc m1' 0 sz = (m2', b')
  /\ inject_incr j j1
  /\ j1 b = Some(b', 0).
Proof.
  intros until j; intros SEP ALLOC1.
  rewrite <- sep_assoc in SEP.
  destruct SEP as (A & B & C).
  destruct A as (INJ & GINJ & D).
  exploit (Mem.alloc_parallel_inject j m1 m1' 0 sz m2 b 0 sz); eauto.
  lia. lia.
  intros (j1 & m2' & b' & ALLOC2 & INJ' & INCR & ? & ?).
  assert (FRESH': ~Mem.valid_block m1' b') by (eapply Mem.fresh_block_alloc; eauto).

  exists j1, m2', b'; split; auto.
  rewrite <- sep_assoc.
  split; [split; [|split]|split].
  - assumption.
  - simpl. destruct GINJ as (bound&?&?).
    exists bound. split. erewrite Mem.nextblock_alloc; eauto. extlia.
    destruct H2. split; intros; eauto.
    rename b2 into b1'.
    eapply IMAGE. 2: assumption.
    destruct (eq_block b1 b).
    * (* contradiction: b3 < nextblock m2 but it's not allocated yet in m2. *)
      exfalso.
      subst b1. rewrite H in H2. inv H2.
      apply Mem.alloc_result in ALLOC2.
      extlia.
    * rewrite <- H0; eauto.
  - red; simpl; intros. contradiction.
  - apply (m_invar P) with m1'; auto. eapply Mem.alloc_unchanged_on; eauto.
  - red; intros.
    destruct H1 as [|[]].
    rename b0 into b1'.
    assert (VALID: Mem.valid_block m1' b1') by (eapply (m_valid P); eauto).
    simpl in H1. red in C.
    destruct H1 as (b1 & delta & ? & ?).
    destruct (eq_block b1 b).
    + (* contradiiction: b1' = b' and b1' is valid in m1' but b' has not been allocated yet in m1'. *)
      subst b1. rewrite H1 in H. inv H.
      contradiction.
    + eapply C. 2: eassumption.
      simpl. left. exists b1, delta. split.
      rewrite <- H0; eauto.
