...
Lemma subtype_refl ty: subtype ty ty = true.
Proof.
...
(*- E_COMPCERT_FTR_Function_AST_inj_typ_xtype_001 *)
(*- #Justify_Derived "Internal function" *)
Definition inj_typ_xtype (t: typ) : xtype :=
  match t with
  | Tint => Xint
  | Tfloat => Xfloat
  | Tlong => Xlong
...
(*- E_COMPCERT_FTR_Function_AST_proj_xtype_typ_001 *)
(*- #Justify_Derived "Internal function" *)
Definition proj_xtype_typ (x: xtype) : typ :=
  match x with
...
Lemma proj_inj_typ_xtype: forall t, proj_xtype_typ (inj_typ_xtype t) = t.
Proof.
  destruct t; auto.
Qed.

Definition inj_typ_xtype_list (ls: list typ): list xtype := List.map inj_typ_xtype ls.

Definition list_xtype_eq: forall (l1 l2: list xtype), {l1=l2} + {l1<>l2} := 
  list_eq_dec xtype_eq.

Inductive ptype : Type :=
  | Ptyp (t: typ)
  | Pptr.

Lemma ptype_eq: forall (t1 t2: ptype), {t1=t2} + {t1<>t2}.
Proof. generalize typ_eq; decide equality. Defined.
Global Opaque ptype_eq.

Ltac destruct_ptype pt :=
  let t := fresh "t" in
  let Eq := fresh "Eq" in
  destruct pt as [t|] eqn:Eq; [destruct t|].

Definition inj_typ_ptype : typ -> ptype := Ptyp.

Definition proj_ptype_typ (p: ptype) : typ :=
  match p with
  | Pptr => Tptr 
  | Ptyp t => t
  end.

Definition proj_xtype_ptype (x: xtype) : ptype :=
  match x with
  | Xptr => Pptr
  | _ => Ptyp (proj_xtype_typ x)
  end.

Lemma proj_inj_typ_ptype: forall t, proj_ptype_typ (inj_typ_ptype t) = t.
Proof.
  destruct t; auto.
Qed.

Lemma proj_inj_typ_ptype_list (xs: list typ): List.map proj_ptype_typ (List.map inj_typ_ptype xs) = xs. 
Proof.
  induction xs.
  - reflexivity.
  - cbn. f_equal. apply IHxs.
Qed.

Definition list_ptype_eq: forall (l1 l2: list ptype), {l1=l2} + {l1<>l2} := 
  list_eq_dec ptype_eq.

Lemma proj_xtype_ptype_typ:
  forall xt, proj_xtype_typ xt = proj_ptype_typ (proj_xtype_ptype xt).
Proof.
  intros. destruct xt; reflexivity.
Qed.

Lemma proj_xtype_ptype_typ_list:
  forall xts, map proj_xtype_typ xts = map proj_ptype_typ (map proj_xtype_ptype xts).
Proof.
  intros.
  rewrite map_map.
  apply (map_ext _ _ proj_xtype_ptype_typ).
Qed.

(** Subtyping on ptype *)
Definition subptype_bool x y := 
  match x, y with
  | Ptyp ty, Ptyp ty' => if typ_eq ty ty' then true else false
  | Ptyp ty, Pptr => false
  | Pptr, Pptr => true
  | Pptr, Ptyp Tint => if Archi.ptr64 then false else true
  | Pptr, Ptyp Tlong => if Archi.ptr64 then true else false
  | Pptr, _ => false
  end.

Definition subptype x y := subptype_bool x y = true.

Lemma subptype_refl x : subptype x x.
Proof.
  unfold subptype. destruct_ptype x; cbn; trivial.
Qed.

Lemma subptype_pptr_tptr:
  subptype Pptr (Ptyp Tptr).
Proof.
  unfold subptype, Tptr.
  cbn. now destruct Archi.ptr64.
Qed.

Lemma subptype_trans x y z: subptype x y -> subptype y z -> subptype x z.
Proof.
  unfold subptype in *.
  destruct_ptype x; destruct_ptype y; destruct_ptype z.
  all: try (discriminate || contradiction).
  all: trivial.
Qed.

Lemma subptype_antisymmetric:
  forall ty1 ty2, subptype ty1 ty2 -> subptype ty2 ty1 -> ty1 = ty2.
Proof.
  unfold subptype.
  intros.
  destruct ty1, ty2; cbn in *; try discriminate.
  - destruct (typ_eq t t0) as [->|]; [|discriminate]. reflexivity.
  - reflexivity.
Qed.

Definition subptype_dec x y: {subptype x y} + {not (subptype x y)}.
Proof.
  unfold subptype; destruct x, y; cbn. 
  - destruct t, t0. 
    all: try now left.
    all: right; discriminate.
  - right. congruence. 
  - destruct t; cbn; try eauto.
    all: destruct Archi.ptr64; cbn.
    all: intuition congruence.
  - left. reflexivity.
Defined.

(** Low and high bounds for a given type. *)
Definition low_bound (ty: ptype): ptype :=
  match ty with
  | Ptyp Tint => if Archi.ptr64 then ty else Pptr
  | Ptyp Tlong => if Archi.ptr64 then Pptr else ty
  | _ => ty
  end.

Lemma low_bound_subptype: forall t, subptype (low_bound t) t.
Proof.
  unfold subptype.
  intros.
  destruct_ptype t; cbn. 
  all: unfold Tptr; destruct Archi.ptr64 eqn:SF; cbn in *.
  all: try rewrite SF.
  all: reflexivity.
Qed.

Definition high_bound (ty: ptype): ptype :=
  match ty with
  | Pptr => Ptyp Tptr
  | _ => ty
  end.

Lemma high_bound_subptype: forall t, subptype t (high_bound t).
Proof.
  unfold subptype.
  intros.
  destruct_ptype t; cbn. 
...
Lemma subptype_to_subtype:
  forall ty1 ty2,
  subptype ty1 ty2 -> subtype (proj_ptype_typ ty1) (proj_ptype_typ ty2) = true.
Proof.
  destruct_ptype ty1; destruct_ptype ty2; destruct Archi.ptr64 eqn:SF;
    unfold subptype; simpl;
    unfold Tptr; simpl;
    try rewrite SF; (contradiction || discriminate || tauto).
Qed.

Lemma subptype_to_subtype':
  forall ty1 ty2,
  subptype ty1 (high_bound ty2) -> subtype (proj_ptype_typ ty1) (proj_ptype_typ ty2) = true.
Proof.
  destruct_ptype ty1; destruct_ptype ty2; destruct Archi.ptr64 eqn:SF;
    unfold subptype; simpl;
    unfold Tptr; simpl;
...
Lemma proj_inj_high_bound:
  forall ty, inj_typ_ptype (proj_ptype_typ ty) = high_bound ty.
Proof.
  destruct_ptype ty; reflexivity.
Qed.

Lemma proj_inj_high_bound_list:
  forall tyl, map inj_typ_ptype (map proj_ptype_typ tyl) = map high_bound tyl.
Proof.
  induction tyl.
  - reflexivity.
  - cbn. f_equal.
    apply proj_inj_high_bound.
    auto.
Qed.

Global Hint Resolve subptype_pptr_tptr | 2 : ty.

Global Hint Extern 2 (subptype ?S ?S) => simple apply subptype_refl : ty.
Global Hint Extern 2 (subptype ?S ?U) =>
  match goal with
  | H: subptype S ?T |- _ => eapply (@subptype_trans S T U)
  end: ty.
Global Hint Extern 2 (subptype ?S ?U) =>
  match goal with
  | H: subptype ?T U |-  _ => eapply (@subptype_trans S T U)
  end: ty.
Global Hint Extern 2 (subptype (low_bound ?S) ?U) =>
  match goal with
  | H: subptype S _ |- _ => simple apply (@subptype_trans (low_bound S) S U (low_bound_subptype S) H)
  end: ty.
Global Hint Extern 2 (subptype ?S (high_bound ?U)) => 
  match goal with
  | H: subptype _ U |-  _ => simple apply (@subptype_trans S U (high_bound U) H (high_bound_subptype U))
  end: ty.
...
(*- #Justify_Derived "Internal function" *)
Definition proj_sig_args (s: signature) : list typ := List.map proj_xtype_typ s.(sig_args).
(*- #End *)

Definition proj_sig_args_ptype (s: signature): list ptype := List.map proj_xtype_ptype s.(sig_args).
                                        
(*- E_COMPCERT_FTR_Function_AST_proj_sig_res_001 *)
(*- #Justify_Derived "Internal function" *)
Definition proj_sig_res (s: signature) : typ := proj_xtype_typ s.(sig_res).
(*- #End *)

Definition proj_sig_res_ptype (s: signature): ptype := proj_xtype_ptype s.(sig_res).
...
Lemma proj_xtype_typ_of_chunk:
  forall chunk, proj_xtype_typ (xtype_of_chunk chunk) = type_of_chunk chunk.
Proof.
  destruct chunk; auto.
Qed.
...
Lemma transform_program_preserves_main p: prog_main p = prog_main (transform_program p).
Proof.
  reflexivity.
Qed.

Lemma transform_program_preserves_public p: prog_public p = prog_public (transform_program p).
Proof.
  reflexivity.
Qed.  
End TRANSF_PROGRAM.
...
Definition ef_sig (ef: external_function): signature :=
  ...
  | EF_annot kind text targs => mksignature (List.map inj_typ_xtype targs) Xvoid cc_default
  | EF_annot_val kind text targ => [inj_typ_xtype targ ---> inj_typ_xtype targ]
  | EF_inline_asm text sg clob => sg
  | EF_debug kind text targs => mksignature (List.map inj_typ_xtype targs) Xvoid cc_default
  end.
(*- #End *)
...
Proof.
  generalize ident_eq string_dec signature_eq chunk_eq typ_eq xtype_eq list_eq_dec zeq Int.eq_dec Ptrofs.eq_dec; intros.
  decide equality.
Defined.
...
Lemma in_params_of_builtin_args A al b (a:A): In b al -> In a (params_of_builtin_arg b) -> In a (params_of_builtin_args al).
Proof.
  intros. induction al; cbn in *.
  - tauto.
  - apply in_or_app.  destruct H as [eq | H].
    + subst. left.  auto.
    + right. now apply IHal.
Qed.      
...
(* extra type information contains register-register (T(r1) <: T(r2)) constraints *)
Definition extra_type_info (Reg: Type) : Type := list (Reg * Reg).
Definition eti_nil {Reg: Type} : extra_type_info Reg := nil.
