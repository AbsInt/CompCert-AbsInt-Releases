...
(*- E_COMPCERT_FTR_Function_AST_typ_eq_001 *)
...
(*- E_COMPCERT_FTR_Function_AST_rettype_0_001 *)
(*- #Justify_Derived "Internal type" *)
Inductive rettype : Type :=
  ...
  | Trptr                               (**r Pointer return type *)
  | Tvoid.                              (**r no value returned *)
(*- #End *)

Coercion Tret: typ >-> rettype.

(*- E_COMPCERT_FTR_Function_AST_rettype_eq_001 *)
(*- #Justify_Derived "Internal function" *)
...
(*- E_COMPCERT_FTR_Function_AST_proj_rettype_001 *)
(*- #Justify_Derived "Internal function" *)
...
(** Simplification function for the return type used for defining the tailcall_matching_rettype.
    For tailcalls we still need to differentiate [Tvoid] from [Tint] and for some architectures we
    need to still differentiate functions returning [Trptr] from [Tret Tptr]. *)
Definition simpl_rettype (keep_rptr: bool) (r: rettype) : rettype :=
  match r with
  | Tret t => t
  | Trptr => if keep_rptr then Trptr else Tret Tptr
  | Tint8signed | Tint8unsigned | Tint16signed | Tint16unsigned => Tret Tint
  | Tvoid => Tvoid
  end.

Definition simpl_rettype_match (keep_rptr : bool) (r1 r2: rettype) : Prop :=
  simpl_rettype keep_rptr r1 = simpl_rettype keep_rptr r2.

Definition simpl_matching_rettype (keep_rptr : bool) (r1 r2: rettype) : bool :=
  rettype_eq (simpl_rettype keep_rptr r1) (simpl_rettype keep_rptr r2).

Lemma simpl_matching_rettype_correct:
  forall keep_rptr r1 r2,
  simpl_matching_rettype keep_rptr r1 r2 = true <-> simpl_rettype_match keep_rptr r1 r2.
Proof.
  unfold simpl_matching_rettype, simpl_rettype_match. intros. split; intros.
  destruct (simpl_rettype keep_rptr r1), (simpl_rettype keep_rptr r2); inv H; auto. destruct t, t0; inv H1; auto.
  rewrite H. destruct (simpl_rettype keep_rptr r2); auto. destruct t; auto.
Qed.

Lemma simpl_rettype_match_proj_rettype:
  forall keep_rptr r1 r2,
  simpl_rettype_match keep_rptr r1 r2 -> proj_rettype r1 = proj_rettype r2.
Proof.
  unfold simpl_rettype_match, simpl_rettype. intros.
  destruct r1, r2, keep_rptr; inv H; auto.
Qed.

(*- E_COMPCERT_FTR_Function_AST_argtype_0_001 *)
(*- #Justify_Derived "Internal type" *)
Inductive argtype : Type :=
  | Targ (t: typ)
  | Taptr.
(*- #End *)

Coercion Targ: typ >-> argtype.

(*- E_COMPCERT_FTR_Function_AST_argtype_eq_001 *)
(*- #Justify_Derived "Internal function" *)
Lemma argtype_eq: forall (t1 t2: argtype), {t1=t2} + {t1<>t2}.
Proof. generalize typ_eq; decide equality. Defined.
Global Opaque argtype_eq.
(*- #End *)

(*- E_COMPCERT_FTR_Function_AST_list_argtype_eq_001 *)
(*- #Justify_Derived "Internal function" *)
Definition list_argtype_eq: forall (l1 l2: list argtype), {l1=l2} + {l1<>l2}
                     := list_eq_dec argtype_eq.
(*- #End *)

(*- E_COMPCERT_FTR_Function_AST_proj_argtype_001 *)
(*- #Justify_Derived "Internal function" *)
Definition proj_argtype (a: argtype) : typ :=
  match a with
  | Targ t => t
  | Taptr => Tptr
  end.
...
(*- #Justify_Derived "Internal function" *)
Definition proj_sig_args (s: signature) : list typ := List.map proj_argtype s.(sig_args).
(*- #End *)
...
Proof.
  generalize rettype_eq, list_argtype_eq, calling_convention_eq; decide equality.
Defined.
...
(*- E_COMPCERT_FTR_Function_AST_rettype_of_chunk_001 *)
(*- #Justify_Derived "Internal function" *)
...
Definition argtype_of_chunk (c: memory_chunk) : argtype := Targ (type_of_chunk c).

(*- E_COMPCERT_FTR_Function_AST_argtype_of_chunk_001 *)
(*- #Justify_Derived "Internal function" *)
Lemma proj_argtype_of_chunk:
  forall chunk, proj_argtype (argtype_of_chunk chunk) = type_of_chunk chunk.
Proof.
  destruct chunk; auto.
Qed.
(*- #End *)
...
Definition ef_sig (ef: external_function): signature :=
  ...
  | EF_vload chunk => mksignature (Taptr :: nil) (rettype_of_chunk chunk) cc_default
  | EF_vstore chunk => mksignature (Taptr :: (argtype_of_chunk chunk) :: nil) Tvoid cc_default
  | EF_malloc => mksignature (Targ Tptr :: nil) Trptr cc_default
  | EF_free => mksignature (Taptr :: nil) Tvoid cc_default
  | EF_memcpy sz al => mksignature (Taptr :: Taptr :: nil) Tvoid cc_default
  | EF_annot kind text targs => mksignature (List.map Targ targs) Tvoid cc_default
  | EF_annot_val kind text targ => mksignature (Targ targ :: nil) targ cc_default
  | EF_inline_asm text sg clob => sg
  | EF_debug kind text targs => mksignature (List.map Targ targs) Tvoid cc_default
  end.
(*- #End *)
