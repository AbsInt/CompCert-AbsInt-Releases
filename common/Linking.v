...
(*- E_COMPCERT_FTR_Function_Linking_Linker_0_001 *)
...
Theorem match_transform_partial_program2':
  forall {C F1 V1 F2 V2: Type} {LC: Linker C} {LF: Linker F1} {LV: Linker V1}
         (match_fundef: C -> F1 -> F2 -> Prop)
         (match_varinfo: V1 -> V2 -> Prop)
         (transf_fun: ident -> F2 -> res F1)
         (transf_var: ident -> V2 -> res V1)
         (ctx: C) (p: program F2 V2) (tp: program F1 V1),
  transform_partial_program2 transf_fun transf_var p = OK tp ->
  (forall i f tf, transf_fun i f = OK tf -> match_fundef ctx tf f) ->
  (forall i v tv, transf_var i v = OK tv -> match_varinfo tv v) ->
  match_program_gen match_fundef match_varinfo ctx tp p.
Proof.
  unfold transform_partial_program2; intros. monadInv H.
  red; simpl; split; auto.
  revert x EQ. generalize (prog_defs p).
...
Theorem match_transform_partial_program_contextual':
  forall {A B V: Type} {LA: Linker A} {LV: Linker V}
         (match_fundef: program A V -> A -> B -> Prop)
         (transf_fun: B -> res A)
         (p: program B V) (tp: program A V),
  transform_partial_program transf_fun p = OK tp ->
  (forall f tf, transf_fun f = OK tf -> match_fundef tp tf f) ->
  match_program match_fundef eq tp p.
Proof.
  intros.
  eapply match_transform_partial_program2'. eexact H.
  auto.
...
Theorem match_transform_program_contextual':
  forall {A B V: Type} {LA: Linker A} {LV: Linker V}
         ...
         (transf_fun: B -> A)
         (p: program B V),
  (forall f, match_fundef (transform_program transf_fun p) (transf_fun f) f) ->
  match_program match_fundef eq (transform_program transf_fun p) p.
Proof.
  intros.
  eapply match_transform_partial_program_contextual'.
  apply transform_program_partial_program with (transf_fun := transf_fun).
...
Theorem match_transform_program':
  forall {A B V: Type} {LA: Linker A} {LV: Linker V}
         (transf: B -> A)
         (p: program B V),
  match_program (fun cu f tf => f = transf tf) eq (transform_program transf p) p.
Proof.
  intros. apply match_transform_program_contextual'. auto.
Qed.
...
Global Instance TransfNoop
           {A V: Type} {LV: Linker V}:
  TransfLink (fun (p1: program (fundef A) V) (p2: program (fundef A) V) =>
                  p1 = p2).
Proof.
  red. intros. 
  exists p. subst. now split.
Qed.
...
(*- E_COMPCERT_FTR_Function_Linking_link_list_001 *)
(*- #Link_to E_COMPCERT_TOR_Function_SEMANTIC_PRESERVATION_002 *)
