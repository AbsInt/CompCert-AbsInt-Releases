...
let astargtype p = function
  | AST.Targ t -> fprintf p "AST.Targ %a" asttype t
  | AST.Taptr -> fprintf p "AST.Taptr"
...
let signatur p sg =
  fprintf p "@[<hov 2>(mksignature@ %a@ %a@ %a)@]"
     (print_list astargtype) sg.sig_args
