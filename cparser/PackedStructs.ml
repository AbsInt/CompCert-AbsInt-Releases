...
(*- #Link_to E_COMPCERT_TR_Robustness_PACKED_STRUCT_017 *)
...
let transf_struct_decl mfa msa swapped loc env struct_id attrs ml =
  let packed = find_custom_attributes ["packed";"__packed__"] attrs <> [] in
  let attrs' =
  ...
  if msa = 0 && (not packed) then (attrs', ml') else begin
    ...
    (*- #Link_to E_COMPCERT_TR_Robustness_PACKED_STRUCT_019 *)
...
(*- #Link_to E_COMPCERT_TR_Robustness_PACKED_STRUCT_018 *)
...
(*- E_COMPCERT_CODE_PackedStructs_transf_expr_002 *)
(*- #Justify_Derived "Utility function" *)
  in texp ctx e
(*- #End *)
