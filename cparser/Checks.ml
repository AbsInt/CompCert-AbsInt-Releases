...
let safe_expr vars env e =
  ...
  and binop op e1 e2 =
    ...
    | Oadd | Osub | Omul | Oand | Oor | Oxor | Oshl | Oshr ->
      expr e1 && expr e2
    | Odiv ->
      let is_fp = is_float_type env e1.etyp && is_float_type env e2.etyp in
      is_fp && expr e1 && expr e2
...
let has_move_for_type env ty =
  ...
  let not_too_large_int =
    if is_integer_type env ty then
      ...
      | Some sz -> Int64.of_int Machine.(!config.sizeof_intreg) >= sz
    else
      true in
  ...
  | "peaktop" | "riscV" | "x86" -> not (is_float_type env ty) && not_too_large_int
  | _ -> assert false
(*- #End *)
...
let non_linear_conditional p =
  (* No check for architectures that don't support select *)
...
(*- E_COMPCERT_CODE_Checks_alias_001 *)
(*- #Link_to E_COMPCERT_TR_Robustness_CHECKS_003 *)
let alias p global_defines =
  if  Configuration.alias_supported then
    let alias_targets_info : (typ * location * string) StringMap.t ref = ref StringMap.empty in
    (* Check that function definition have no alias *)
    let fundef env loc fd =
      if has_alias_attribute fd.fd_attrib then
        error loc "definition '%s' cannot also be an alias" fd.fd_name.name
    in
    (* Collect all alias targets and information *)
    let decl env loc (sto, id, ty, init) =
      let attr = attributes_of_type env ty in
      let weak = has_weak_attribute attr in
      match find_custom_attributes ["alias"; "__alias__"] attr with
      | [] -> ()
      | [[C.AString alias]] ->
        if not weak then
          error loc "attribute 'alias' without 'weak'";
        if StringSet.mem id.name global_defines then
          error loc "redefinition of '%s'" id.name;
        (* Check that the target is defined toplevel  *)
        if not (StringSet.mem alias global_defines) then
          error loc "'%s' aliased to undefined symbol '%s'" id.name alias;
        alias_targets_info := StringMap.add alias (ty, loc, id.name) !alias_targets_info
      | [[_]] ->
        error loc "attribute 'alias' argument not a string"
      | [[]] | [_::_] ->
        error loc "wrong number of arguments specified for 'alias' attribute"
      | _ ->
        error loc "'%s' defined both normally and as 'alias' attribute" id.name
    in
    let alias_targets : IdentSet.t ref = ref IdentSet.empty in
    (* Check that the target is not a variable *)
    let decl' env loc (sto, id, ty, init) =
      try
        let (_, _, name) = StringMap.find id.name !alias_targets_info in
        if not (is_function_type env ty) then
          error loc "'%s' alias between function and non-function types are not supported" name
      with Not_found -> ()
    in
    (* Check that the target is a function with compatible type *)
    let fundef' env loc fd =
      try
        let (ty, loc', name) = StringMap.find fd.fd_name.name !alias_targets_info in
        let iso_inline = fd.fd_storage = Storage_default && fd.fd_inline in
        if iso_inline then
          error loc "'%s' alias to external symbol '%s'" name fd.fd_name.name;
        let fd_ty = fundef_typ fd in
        if not (compatible_types AttrIgnoreAll env ty fd_ty) then begin
          let ty' = strip_attributes_type ty []
          and ty_id' = strip_attributes_type fd_ty [] in
          error loc' "'%s' alias between functions of incompatible types '%a' and '%a'"
            name Cprint.typ_raw ty' Cprint.typ_raw ty_id'
        end;
        alias_targets := IdentSet.add fd.fd_name !alias_targets
      with Not_found -> ()
    in
    let p = List.rev p in
    traverse_program ~decl:decl ~fundef:fundef p;
    (* If we did not find any alias we are done here *)
    if StringMap.is_empty !alias_targets_info then
      IdentSet.empty
    else begin
      (* Check that the targets have the correct type *)
      traverse_program ~decl:decl' ~fundef:fundef' p;
      !alias_targets
    end
  else
