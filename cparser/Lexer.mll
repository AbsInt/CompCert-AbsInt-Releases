...
(*- E_COMPCERT_CODE_Lexer_ident_or_keyword_001 *)
let ident_or_keyword lb id =
  ...
  with Not_found ->
    if String.contains id '$' then begin
      if id = "$" then
        (*- #Link_to E_COMPCERT_TR_Robustness_LEXER_018 *)
        error lb "not supported: identifier consisting of a single '$' sign"
      else
        (*- #Link_to E_COMPCERT_TR_Function_LEXER_017 *)
...
rule initial = parse
  ...
  | "//"                          { singleline_comment lexbuf; initial_linebegin lexbuf } (* singleline_comment consumes the terminating newline *)
...
and char = parse
  | universal_character_name
      { try
        ...
        with Failure _ ->
          (*- #Link_to E_COMPCERT_TR_Robustness_LEXER_016 *)
