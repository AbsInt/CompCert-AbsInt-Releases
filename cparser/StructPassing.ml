...
let classify_return env ty =
  if is_composite_type env ty then begin
    ...
    | Some sz, Some al ->
        ...
        | SR_int1248 when sz = 1L || sz = 2L || sz = 4L ->
            Ret_value (TInt(IUInt, []), sz, al)
        | SR_int1248 when sz = 8L ->
            Ret_value (TInt(IULongLong, []), sz, al)
        | (SR_int1to4 | SR_int1to8) when sz <= 4L ->
            Ret_value (TInt(IUInt, []), sz, al)
        | SR_int1to8 when sz > 4L && sz <= 8L ->
            Ret_value (TInt(IULongLong, []), sz, al)
        | _ ->
            Ret_ref
        end
...
type param_kind =
  ...
  | Param_flattened of int64 * int64 * int (**r passed as N integer arguments
                          (N, size, alignment) *)
  | Param_int of int64 * int (**r passed as single integer argument
                          (size, alignment) *)
(*- #End *)
...
let classify_param env ty =
  if is_composite_type env ty then begin
    ...
    | SP_split_args ->
      ...
      | Some sz, Some al ->
          Param_flattened (Int64.(div (add sz 3L) 4L), sz, al)
      | _, _ ->
          Param_unchanged  (* should not happen *)
      end
    | SP_int1to8 ->
      begin match sizeof env ty, alignof env ty with
        | Some sz, Some al when sz <= 8L ->
          Param_int (sz, al)
        | _, _ -> Param_unchanged
      end
  end else
...
(*- #Justify_Derived "Utility function" *)
let list_map_n f n =
  let rec map i = if i >= n then [] else f i :: map (Int64.add i 1L)
  in map 0L
(*- #End *)
...
let ebuffer_index base idx =
  { edesc = EBinop(Oindex, base, intconst idx IInt, uintptr);
    etyp = uint }
(*- #End *)
...
(*- #Justify_Derived "Utility function" *)
let lshift a nbytes =
  if nbytes = 0L then a else
    { edesc = EBinop(Oshl, a, intconst (Int64.mul nbytes 8L) IInt, uint);
      etyp = uint }
(*- #End *)
...
let offsetptr base ofs =
  if ofs = 0L then base else
  { edesc = EBinop(Oadd, base, intconst ofs IInt, ucharptr);
    etyp = ucharptr }
(*- #End *)
...
let lshift_ll a nbytes =
  ...
  if nbytes = 0L then a else
    { edesc = EBinop(Oshl, a, intconst (Int64.mul nbytes 8L) IInt, ulonglong);
      etyp = ulonglong }
(*- #End *)
...
let load_word base ofs sz al =
  match sz with
  | 0L -> intconst 0L IInt
  | 1L -> load1 base ofs 0L 3L
  | 2L -> if al >= 2 || (!config).supports_unaligned_accesses then
           load2 base ofs 0L 2L
         else
           or2 (load1 base ofs 0L 3L)
               (load1 base (Int64.add ofs 1L) 1L 2L)
  | 3L -> if al >= 2 || (!config).supports_unaligned_accesses then
           or2 (load2 base ofs 0L 2L)
               (load1 base (Int64.add ofs 2L) 2L 1L)
         else
           or3 (load1 base ofs 0L 3L)
               (load1 base (Int64.add ofs 1L) 1L 2L)
               (load1 base (Int64.add ofs 2L) 2L 1L)
  | 4L -> if al >= 4 || (!config).supports_unaligned_accesses then
           load4 base ofs
         else if al >= 2 then
           or2 (load2 base ofs 0L 2L)
               (load2 base (Int64.add ofs 2L) 2L 0L)
         else
           or4 (load1 base ofs 0L 3L)
               (load1 base (Int64.add ofs 1L) 1L 2L)
               (load1 base (Int64.add ofs 2L) 2L 1L)
               (load1 base (Int64.add ofs 3L) 3L 0L)
  | _ -> assert false
(*- #End *)
...
let rec load_words base ofs sz al =
  if ofs >= sz then []
  else if Int64.add ofs 4L >= sz then [load_word base ofs (Int64.sub sz ofs) al]
  else load_word base ofs 4L al :: load_words base (Int64.add ofs 4L) sz al
(*- #End *)
...
let load_result base sz al =
  assert (sz <= 8L);
  if sz <= 4L then
    load_word base 0L sz al
  else if sz = 8L && (al >= 8 || (!config).supports_unaligned_accesses) then
    load8 base 0L
  else begin
    let (shift1, shift2) = if (!config).bigendian then (4L, 0L) else (0L, 4L) in
    or2_ll (lshift_ll (load_word base 0L 4L al) shift1)
           (lshift_ll (load_word base 4L (Int64.sub sz 4L) al) shift2)
  end
(*- #End *)
...
and transf_funargs env = function
  ...
  | (id, t) :: args ->
      ...
      | Param_flattened(n, sz, al) ->
          ...
          @ args'
      | Param_int (sz, al) ->
        let typ = if sz <= 4L then uint else ulonglong in
  ...
  | arg :: args ->
      ...
      | Param_int (sz, al) ->
        ...
        if translates_to_extended_lvalue arg then begin
          ...
          let base_ty = if sz <= 4L then uint else ulonglong in
...
let rec transf_funparams loc env params =
  ...
  | (x, tx) :: params ->
      ...
      | Param_int(sz, al) ->
          let ty_base = if sz <= 4L then uint else ulonglong in
