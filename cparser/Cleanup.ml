...
(*- E_COMPCERT_CODE_Cleanup_used_types_001 *)
(*- #Justify_Derived "Variable for global state" *)
let used_types = ref IdentSet.empty
(*- #End *)

(*- E_COMPCERT_CODE_Cleanup_alias_targets_001 *)
(*- #Justify_Derived "Variable for global state" *)
let alias_targets = ref IdentSet.empty
(*- #End *)
...
(* Saturate the set of referenced identifiers, starting with externally
   ...
   - Declarations of types marked as needed
*)
...
let visible_decl (sto, id, ty, init) =
  ...
  let alias = has_alias_attribute attr in
  used || alias ||
  sto = Storage_default &&
  match ty with TFun _ -> false | _ -> true
(*- #End *)

(*- E_COMPCERT_CODE_Cleanup_visible_type_001 *)
(*- #Justify_Derived "Utility function" *)
let visible_type id =
  IdentSet.mem id !used_types
(*- #End *)
...
let visible_fundef f =
  ...
  | Storage_static ->
    has_used_attribute f.fd_attrib || IdentSet.mem f.fd_name !alias_targets
...
let rec add_init_globdecls accu = function
  ...
  | g :: rem ->
      ...
      | Gcompositedef(_, id, _ , flds) when visible_type id ->
        List.iter add_field flds; addref id; add_init_globdecls accu rem
      | Gtypedef (id, ty) when visible_type id ->
        add_typ ty; addref id; add_init_globdecls accu rem
...
(*- #Justify_Derived "Functional decomposition" *)
let remove_unused_debug used g =
  let unused x = not (IdentSet.mem x used) in
  match g.gdesc with
  | Gdecl (_,id,_,_) when unused id -> Debug.remove_unused id
  | Gfundef f when unused f.fd_name -> Debug.remove_unused_function f.fd_name
  | _ -> ()
(*- #End *)

(*- E_COMPCERT_CODE_Cleanup_used_debug_001 *)
(*- #Justify_Derived "Functional decomposition" *)
let used_debug ids g =
  match g.gdesc with
  | Gdecl (_, id, _, _) -> IdentSet.add id ids
  | Gfundef f -> IdentSet.add f.fd_name ids
  | _ -> ids
(*- #End *)

(*- E_COMPCERT_CODE_Cleanup_simpl_globdecls_001 *)
(*- #Justify_Derived "Functional decomposition" *)
let rec simpl_globdecls (accu, used) = function
  | [] -> accu, used
  | g :: rem ->
      let need =
        match g.gdesc with
        ...
        | Gcompositedef(_, id, _, flds) -> visible_type id || needed id
        | Gtypedef(id, ty) -> needed id
        | Genumdef(id, _, enu) ->
            needed id || List.exists (fun (id, _, _) -> needed id) enu
        | Gpragma s -> true in
      if need then
        let used = used_debug used g in
        simpl_globdecls ((g :: accu), used) rem
      else
        simpl_globdecls (accu, used) rem
(*- #End *)

(*- E_COMPCERT_CODE_Cleanup_program_001 *)
let program p types alias =
  alias_targets := alias;
  used_types := types;
  ...
  let (p', u) = simpl_globdecls ([], IdentSet.empty) p in
  List.iter (remove_unused_debug u) p;
  referenced := IdentSet.empty;
  p'
(*- #End *)
