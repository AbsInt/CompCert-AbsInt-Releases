...
(*- E_COMPCERT_CODE_Elab_monadic_map_002 *)
(*- #Justify_Derived "Utility function" *)
...
*)
  ...
  | CALL ((VARIABLE "__builtin_nanf") as a1, al) ->
      let conv i =
  ...
  | CALL ((VARIABLE "__builtin_nansf") as a1, al) ->
      ...
      elab_nanf_variants env conv "nansf" a1 al
  (*- #End *)
  ...
  | CALL(VARIABLE "__builtin_isnan", al) ->
      let s_id = "__builtin_isnanf"
      and d_id = "__builtin_isnan" in
      elab_float_generic_builtin_one_arg env d_id d_id s_id al

  | CALL(VARIABLE "__builtin_isninf", al) ->
      let s_id = "__builtin_isinff"
      and d_id = "__builtin_isinf" in
      elab_float_generic_builtin_one_arg env d_id d_id s_id al

  | CALL(VARIABLE "__builtin_isfinite", al) ->
...
(* Type-checking of function arguments *)
  ...
  (* Helper function for the single argument type generic float builtins *)
  and elab_float_generic_builtin_one_arg env id double_id single_id al =
    let lookup env id =
    ...
    begin match al with
      | [a1] ->
        ...
        let ident  =
          if compat FFloat then
            lookup env single_id
          else if compat FDouble then
            lookup env double_id
          else if compat FLongDouble then
            lookup env double_id (* Works only if long double is double *)
          else
            fatal_error "non-floating-point argument in call to function '%s'" id
        in
      ...
      | _ ->
        fatal_error "'%s' expects one argument" id
    end

  and elab_nanf_variants env conv id a1 al =
    (* We elab the call to get the types etc. *)
    let (b1, env) = elab env a1 in
    let (bl, env) = mmap elab env al in
    let (res, proto, vararg) = match unroll env b1.etyp with
      ...
      | [bl] ->
        begin match eval_int_of_string_literal env bl with
          | None ->
            ...
            let (b1, env) = elab env (VARIABLE id) in
            { edesc = ECall(b1, [bl]); etyp = res },env
          | Some i ->
            let i = conv i in
            {edesc = EConst (CFloat (FBits i, FFloat)); etyp = TFloat (FFloat, [])}, env
        end
