...
(*- E_COMPCERT_CODE_Elab_needed_types_001 *)
(*- #Justify_Derived "Variable for global state" *)
let needed_types = ref IdentSet.empty
(*- #End *)

(*- E_COMPCERT_CODE_Elab_add_needed_type_001 *)
(*- #Justify_Derived "Utility function" *)
let add_needed_type ty =
  let add_need id = needed_types := IdentSet.add id !needed_types in
  let rec add_typ = function
    ...
    | TNamed(id, _) | TStruct(id, _) | TUnion(id, _) | TEnum(id, _) -> add_need id
    | _ -> ()
  and add_vars vl =
    List.iter (fun (id, ty) -> add_typ ty) vl in
  add_typ ty
(*- #End *)
...
let elaborated_program () =
  ...
  let alias_targets = Checks.alias p !global_defines in
  top_declarations := [];
  (* Reverse it and eliminate unreferenced declarations *)
  Cleanup.program p !needed_types alias_targets
(*- #End *)
...
(*- #Link_to E_COMPCERT_TR_Robustness_ELAB_247 *)
...
let combine_toplevel_definitions loc env s old_sto old_ty sto ty =
  ...
  let new_sto =
    ...
    | _,Storage_register
    | Storage_register,_ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_236 *)
...
(*- E_COMPCERT_CODE_Elab_elab_elab_expr_f_001 *)
(*- #Justify_Derived "Forward declaration for utility function" *)
let elab_expr_f : (Cabs.loc -> Env.t -> Cabs.expression -> C.exp * Env.t) ref
  = ref (fun _ _ _ -> assert false)
(*- #End *)

(*- E_COMPCERT_CODE_Elab_elab_elab_funbody_f_001 *)
(*- #Justify_Derived "Forward declaration for utility function" *)
...
(*- E_COMPCERT_CODE_Elab_check_char_range_001 *)
...
(*- E_COMPCERT_CODE_Elab_ikind_of_encoding_001 *)
(*- #Justify_Derived "Utility function" *)
...
let elab_char_constant loc enc chars =
  ...
  let v =
    ...
    | [], _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_234 *)
      error loc "empty character constant"; 0L
    | [c], _ -> c
    | _, EncNone ->
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_019 *)
    ...
    | _, _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_235 *)
  ...
  if v' <> v then
    (*- #Link_to E_COMPCERT_TR_Function_ELAB_233 *)
...
(*- E_COMPCERT_CODE_Elab_warn_C11_literals_001 *)
(*- #Link_to E_COMPCERT_TR_Function_ELAB_154 *)
(*- #Link_to E_COMPCERT_TR_Function_ELAB_232 *)
...
let warn_if_reduced_alignment  ?(field:bool=false) loc ~actual ~natural =
  match actual, natural with
  | Some act, Some nat when act < nat ->
    (*- #Link_to E_COMPCERT_TR_Function_ELAB_162 *)
    if Configuration.arch = "tricore" && field then begin
      error loc "requested alignment (%d) is less than natural alignment (%d), this is unsupported as the architecture traps on misaligned memory accesses"
        act nat
    end else begin
...
(*- #Justify_Derived "Utility function" *)
let check_reduced_alignment ?(field:bool=false) loc env typ =
  warn_if_reduced_alignment ~field:field loc
...
(*- E_COMPCERT_CODE_Elab_check_bitfield_001 *)
let check_bitfield loc env id ty ik n =
  let max = if ik = IBool then 1L else Int64.of_int(sizeof_ikind ik * 8) in
  ...
  if n < 0L then begin
    ...
    | TEnum(eid, _) ->
      ...
      (*- #Link_to E_COMPCERT_TR_Function_ELAB_227 *)
...
let rec elab_specifier ?(only = false) loc env specifier =
  ...
  | SpecStorage st ->
      ...
      | TYPEDEF ->
          (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_245 *)
          if !typedef then
...
and elab_parameter env (PARAM (spec, id, decl, attr, loc)) =
  ...
  (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_241 *)
  if tydef then
...
(*- #Link_to E_COMPCERT_TR_Robustness_ELAB_240 *)
...
| Field_group (spec, fieldlist, loc) ->
  ...
  if sto <> Storage_default then
    ...
    (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_246 *)
...
and elab_struct_or_union_info kind loc env members attrs =
  ...
  let rec duplicate acc = function
    ...
    | fld::rest ->
       if fld.fld_anonymous then begin
         ...
         if StringSet.mem fld.fld_name acc then
           error loc "duplicate member '%a'" pp_field fld.fld_name;
         duplicate (StringSet.add fld.fld_name acc) rest
       end else
         duplicate acc rest in
  duplicate StringSet.empty m;
  (* Check if there is a max member alignment constraint given by a packed attribute *)
  let mfa =
    let (mfa, _, _) = packing_parameters attrs in
    (* 0 means no member alignment constraint *)
    if mfa = 0 then None else Some mfa
  in
  (* Check whether the field alignment is reduced *)
  let check_field_alignment fld_typ =
    match mfa with
    | None ->
      (* No alignment constraints by packed of the composite *)
      check_reduced_alignment ~field:true loc env' fld_typ
    | Some mfa ->
      (* Compute the natural alignment of the field type  *)
      let natural = wrap alignof loc env' (erase_attributes_type env' fld_typ) in
      (* Compute the actual alignment of the field *)
      let actual =
        begin match wrap alignof loc env' fld_typ with
        | Some actual ->
          (* We have an alignment constraint from the composite and one given
             by additional attributes, take the minimum. *)
          Some (min mfa actual)
        | None -> Some mfa
        end
      in
      (* Call the warning function  *)
      warn_if_reduced_alignment ~field:true loc ~actual:actual ~natural:natural
  in
  ...
  | fld :: rem ->
      ...
      check_field_alignment fld.fld_typ;
      (* Unnamed bit-fields don't count as member *)
      let only = only && fld.fld_name = "" && fld.fld_bitfield <> None in
      check_incomplete only rem in
...
and elab_struct_or_union only kind loc tag optmembers attrs env =
  ...
  | _, None ->
      ...
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_243 *)
      if tag = "" then
...
let elab_type loc env spec decl =
  ...
  (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_237 *)
...
and elab_single env zi a il =
  ...
  | _ ->
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_244 *)
...
(* 6.5.1.1 Generic selection *)
  ...
  (*- E_COMPCERT_CODE_Elab_elab_expr_elab_046 *)
  (*- #Link_to E_COMPCERT_TR_Function_ELAB_228 *)
  | GENERIC(a1, assoc) ->
      (*- #Link_to E_COMPCERT_TR_Function_ELAB_231 *)
      ...
      add_needed_type ty;
      List.iter (fun (ty, _) -> match ty with Some ty -> add_needed_type ty | None -> ()) bssoc;
      ...
      | (_, b) :: others ->
          if others <> [] then
            (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_230 *)
...
*)
  ...
  (*- #Link_to E_COMPCERT_TR_Function_ELAB_A_181 *)
  ...
  (*- #Link_to E_COMPCERT_TR_Function_ELAB_A_181 *)
  (*- #Link_to E_COMPCERT_TR_Function_ELAB_B_181 *)
  | CALL(a1, al) ->
      let b1,env =
...
(* 6.5.3 Unary expressions *)
  ...
  | BUILTIN_OFFSETOF ((spec,dcl), mem) ->
    ...
    let checked_add a b =
      ...
      (* a and b should be positive and there should be no overflow *)
      if Int64.add a b < a || a < 0L || b < 0L then
        fatal_error "'offsetof' overflows";
      Int64.add a b in
    add_needed_type ty;
    ...
    let rec offset_of_list acc env ty = function
      ...
      | fld::rest ->
        ...
        offset_of_list (checked_add acc off) env fld.fld_typ rest in
    let offset_of_member (env,off_accu,ty) mem =
      ...
      | INFIELD_INIT mem,ty ->
        ...
        let off,ty = offset_of_list 0L env ty flds in
        (env, checked_add off_accu off, ty)
      | ATINDEX_INIT e,TArray (sub_ty,_,_) ->
        ...
        let off_accu = match cautious_mul e size with
          ...
          | Some s -> checked_add off_accu s in
        (env, off_accu, sub_ty)
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_087 *)
      | ATINDEX_INIT _,_ -> fatal_error "subscripted value is not an array" in
    let env,offset,_ = List.fold_left offset_of_member (env,0L,ty) mem in
    let size_t = size_t_ikind () in
    (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_089 *)
    if not (integer_representable offset size_t) then
      fatal_error "'offsetof' overflows";
    (*- #Link_to E_COMPCERT_TR_Function_CEVAL_007 *)
    let offset = Ceval.normalize_int offset size_t in
  ...
  (*- #Link_to E_COMPCERT_TR_Function_ELAB_184 *)
  | UNARY(ADDROF, a1) ->
      let rec check_register = function
        | EVar id ->
        ...
        | EUnop(Odot _, e)-> check_register e.edesc
        | _ -> () in
      ...
      | EVar id ->
          check_register b1.edesc
      | EUnop(Odot f, b2) ->
          check_register b2.edesc;
        ...
        if is_arith_type env b1.etyp && is_arith_type env b2.etyp then begin
          ...
          | (TPtr(ty1, a1) | TArray(ty1, _, a1)),
            (TPtr(ty2, a2) | TArray(ty2, _, a2)) ->
              ...
              (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_094 *)
              if wrap sizeof loc env ty1 = Some 0L then
          ...
          let tyres =
            if is_void_type env ty1 || is_void_type env ty2 then
              ...
              | None ->
                  ...
                  TPtr(TVoid (add_attributes a1 a2), [])
              | Some ty ->
                (* Remove the noreturn attribute from a function if it is not contained on both arguments. Otherwise the
                   analysis for noreturn will think that this expression might not return. *)
                let ty1_noret = has_noret_attribute (attributes_of_type env ty1)
                and ty2_noret = has_noret_attribute (attributes_of_type env ty2) in
                if ty1_noret <> ty2_noret then
                  (* either b1 or b2 does not contain a pointer to a noreturn function, so we must remove it *)
                  begin match unroll env ty with
                    | TPtr (ty, a) ->
                      let ty = change_attributes_type env (remove_custom_attributes ["noreturn"; "__noreturn__"]) ty in
                      TPtr (ty, a)
                    | _ -> ty
                  end
                else
...
(* Type-checking of function arguments *)
  ...
  and elab_float_generic_builtin_one_arg env b_id double_id single_id al =
    match al with
      | [a1] ->
        ...
        let double_id = if Configuration.has_double then double_id else single_id in
        let ident  =
  ...
  (*- E_COMPCERT_CODE_Elab_elab_generic_association_001 *)
  (*- #Link_to E_COMPCERT_TR_Function_ELAB_228 *)
    ...
    let rec elab_gen env accu = function
      ...
      | (None, a) :: l ->
          if List.exists (fun (oty, _) -> oty = None) accu then
            (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_229 *)
...
(*- E_COMPCERT_CODE_Elab_elab_elab_expr_f_002 *)
...
let enter_typedef loc env sto (s, ty, init) =
  (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_242 *)
...
let elab_KR_function_parameters env params defs loc =
  ...
  | d -> (* Should never be produced by the parser *)
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_248 *)
...
let elab_fundef genv spec name defs body loc =
  ...
  let (ty, extra_decls, lenv) =
    ...
    | TFun(ty_ret, None, false, attr), Some params ->
        ...
        (ty, extra_decls, lenv)
    | _, _ ->
        (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_249 *)
...
let elab_definition (for_loop: bool) (local: bool) (nonstatic_inline: bool)
  ...
  | FUNDEF(spec, name, defs, body, loc) ->
      (* This should actually never be triggered, catched by pre-parser *)
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_238 *)
...
(* Unsupported *)
  ...
  (*- E_COMPCERT_CODE_Elab_elab_stmt_017 *)
  | DEFINITION def ->
      (*- #Link_to E_COMPCERT_TR_Robustness_ELAB_239 *)
...
(*- E_COMPCERT_CODE_Elab_elab_elab_funbody_f_002 *)
(*- #Justify_Derived "Initializer" *)
let _ = elab_funbody_f := elab_funbody
(*- #End *)

(*- E_COMPCERT_CODE_Elab_elab_enter_lib_fun_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Elab_elab_known_lib_funs_001 *)
