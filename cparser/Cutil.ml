...
let align x boundary =
  ...
  Int64.(logand (sub (add x boundary) 1L) (lognot (sub boundary 1L)))
(*- #End *)
...
(*- #Justify_Derived "Utility function" *)
let cautious_mul (a: int64) (b: int64) =
  if b = 0L || a <= Int64.div Int64.max_int b
  then Some(Int64.mul a b)
  else None
(*- #End *)
...
let rec sizeof env t =
  let to_int64_opt = function None -> None | Some s -> Some (Int64.of_int s) in
  match t with
  | TVoid _ -> to_int64_opt !config.sizeof_void
  | TInt(ik, _) -> Some(Int64.of_int (sizeof_ikind ik))
  | TFloat(fk, _) -> Some(Int64.of_int (sizeof_fkind fk))
  | TPtr(_, _) -> Some(Int64.of_int (!config.sizeof_ptr))
  ...
  | TArray(ty, Some n, _) as t' ->
      ...
      | Some s ->
          ...
          | None -> error no_loc "sizeof(%a) overflows" Cprint.typ t'; Some 1L
      end
  | TFun(_, _, _, _) -> to_int64_opt !config.sizeof_fun
  ...
  | TEnum(_, _) -> Some(Int64.of_int (sizeof_ikind enum_ikind))
(*- #End *)
...
let sizeof_layout_struct env members ma =
  ...
  let align_bit_offset pos a =
    align pos Int64.(mul 8L (of_int (if ma > 0 && a > ma then ma else a))) in

  let record_field name pos =
    assert (Int64.rem pos 8L = 0L);
    (name, Int64.div pos 8L) in
  ...
  | m :: rem ->
      ...
      | Some a, Some s ->
          ...
          | None ->
              let pos = align_bit_offset pos a in
              sizeof_rec Int64.(add pos (mul s 8L))
          ...
          | Some width ->
              ...
              let al' = Int64.(mul (of_int a) 8L) in
              let curr = Int64.(mul (div pos al') al') in
              let next = Int64.(add curr (mul s 8L)) in
              let width' = Int64.of_int width in
              let pos' =
                if width <= 0 then align pos al'
                else if Int64.add pos width' <= next then Int64.add pos width'
                else Int64.add next width' in
...
let sizeof_struct env members ma =
  ...
  | None -> None
  | Some(bitsize, offsets) -> Some (Int64.(div (add bitsize 7L) 8L))
(*- #End *)
...
let composite_info_def env su attr m =
  ...
  let sz =
    ...
    | Union -> sizeof_union env m in
  let min_struct_align =
    (* For TriCore we need to increase the minimal struct alignment from 1 to 2 if: *)
    if Configuration.arch = "tricore"
    (* - The alignment of all members is 1 *)
    && natural_align = Some 1
    (* - The size is larger than 1 *)
    && (match sz with Some n -> n > 1L | None -> false)
    (* - The struct is not packed *)
    && find_custom_attributes ["packed";"__packed__"] attr = []
    (* - The alignment is not changed via attribute *)
    && attr_align = 0
    then
      2
    else
  ...
  { ci_kind = su; ci_members = m;
    ...
    ci_sizeof =
      ...
      | Some s, Some a -> Some (align s (Int64.of_int a))
      | _, _ -> None
      end;
    ci_attr = attr }
(*- #End *)
...
(*- #Justify_Derived "Utility function" *)
let size_t_ikind () = !config.size_t_ikind
(*- #End *)
...
(*- #Justify_Derived "Utility function" *)
let ptrdiff_t_ikind () = !config.ptrdiff_t_ikind
(*- #End *)
...
(*- #Justify_Derived "Utility function" *)
let rec contains_flex_array_mem env ty =
  match unroll env ty with
  | TStruct (id,_) ->
    find_flex_array_mem env id <> None
  | TUnion (id, _) ->
    let ci = Env.find_union env id in
    List.exists (fun fld -> contains_flex_array_mem env fld.fld_typ) ci.ci_members
  | _ -> false
(*- #End *)
...
(*- E_COMPCERT_CODE_Cutil_has_weak_attribute_001 *)
(*- #Justify_Derived "Utility function" *)
let has_weak_attribute attr =
  find_custom_attributes ["weak"; "__weak__"] attr <> []
(*- #End *)

(*- E_COMPCERT_CODE_Cutil_has_noret_attribute_001 *)
(*- #Justify_Derived "Utility function" *)
let has_noret_attribute attr =
  find_custom_attributes ["noreturn"; "__noreturn__"] attr <> []
(*- #End *)

(*- E_COMPCERT_CODE_Cutil_has_use_bol_attribute_001 *)
(*- #Justify_Derived "Utility function" *)
let has_use_bol_attribute attr =
  find_custom_attributes ["use_bol"; "__use_bol__"] attr <> []
(*- #End *)

(*- E_COMPCERT_CODE_Cutil_has_no_use_bol_attribute_001 *)
(*- #Justify_Derived "Utility function" *)
let has_no_use_bol_attribute attr =
  find_custom_attributes ["no_use_bol"; "__no_use_bol__"] attr <> []
(*- #End *)

(*- E_COMPCERT_CODE_Cutil_has_alias_attribute_001 *)
(*- #Justify_Derived "Utility function" *)
let has_alias_attribute attr =
  find_custom_attributes ["alias"; "__alias__"] attr <> []
(*- #End *)

(*- E_COMPCERT_CODE_Cutil_get_alias_attribute_001 *)
(*- #Justify_Derived "Utility function" *)
let get_alias_attribute attr =
  match find_custom_attributes ["alias"; "__alias__"] attr with
  | [[AString name ]] -> Some name
  | _ -> None
(*- #End *)
