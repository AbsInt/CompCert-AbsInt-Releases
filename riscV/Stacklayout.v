...
Definition split_used_callee_save := default_callee_save.

Definition callee_save_correspond := default_callee_save_correspond.

Definition callee_save_wellformed := default_callee_save_wellformed.

Definition make_env (b: bounds) : frame_env :=
  let calleesave := split_used_callee_save b.(used_callee_save) in
  ...
  let ol :=  align (size_callee_save_area_rec (flatten_group_list calleesave) ocs) 8 in (* locals *)
  ...
  {| fe_size := sz;
     ...
     fe_used_callee_save := calleesave |}.
...
Local Opaque Z.add Z.mul sepconj range.
  ...
  change (size_callee_save_area_rec _ ocs) with (size_callee_save_area fe ocs).
(* Reorder as:
     outgoing
     back link
     retaddr
...
Proof.
  ...
  set (oretaddr := olink + w).
  set (ocs := oretaddr + w).
  change (size_callee_save_area_rec _ _) with (size_callee_save_area fe ocs).
  set (ol :=  align (size_callee_save_area fe ocs) 8).
  set (ostkdata := align (ol + 4 * b.(bound_local)) 8).
  assert (0 < w) by (unfold w; destruct Archi.ptr64; lia).
  generalize b.(bound_local_pos) b.(bound_outgoing_pos) b.(bound_stack_data_pos); intros.
