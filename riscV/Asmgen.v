...
Proof.
  unfold iregs_of_rpair. unfold preg_rpair_of. simpl. intros.
  unfold ireg_of. destruct (preg_of hi); inv H. destruct (preg_of lo); inv H1. auto.
Qed.
...
Definition transl_op
  ...
  | Omove, a1 :: nil =>
      ...
      | One (IR r), One (IR a) => OK (Pmv r a :: k)
      | One (FR r), One (FR a) => OK (Pfmv r a :: k)
  ...
  | Oadd, a1 :: a2 :: nil =>
      do rd <- ireg_of_rpair res; do rs1 <- ireg_of_rpair a1; do rs2 <- ireg_of_rpair a2;
  ...
  | Oaddl, a1 :: a2 :: nil =>
      if Archi.ptr64 then
        do rd <- ireg_of_rpair res;
        do rs1 <- ireg_of_rpair a1;
        do rs2 <- ireg_of_rpair a2;
        ...
        if ireg_eq rl ah || ireg_eq rl bh || ireg_eq rl bl then
          OK (Paddw X31 al bl :: Psltuw X1 X31 bl :: Paddw rh ah bh :: Paddw rh rh X1 :: Pmv rl X31 :: k)
        else
          OK (Paddw rl al bl :: Psltuw X1 rl bl :: Paddw rh ah bh :: Paddw rh rh X1 :: k)
  | Oaddlimm n, a1 :: nil =>
      do rd  <- ireg_of_rpair res; do rs <- ireg_of_rpair a1;
  ...
  | Onegl, a1 :: nil =>
      if Archi.ptr64 then
        ...
        if (ireg_eq rl ah) then
          OK (Psltuw X1 X0 al :: Psubw X31 X0 al :: Psubw rh X0 ah :: Psubw rh rh X1 :: Pmv rl X31 :: k)
        else
          OK (Psltuw X1 X0 al :: Psubw rl X0 al :: Psubw rh X0 ah :: Psubw rh rh X1 :: k)
  | Osubl, a1 :: a2 :: nil =>
      if Archi.ptr64 then
        do rd <- ireg_of_rpair res;
        do rs1 <- ireg_of_rpair a1;
        ...
        do (bh, bl) <- iregs_of_rpair a2;
        if ireg_eq rl ah || ireg_eq rl bh  || ireg_eq rl bl  then
          OK (Psltuw X1 al bl :: Psubw X31 al bl :: Psubw rh ah bh :: Psubw rh rh X1 :: Pmv rl X31 :: k)
        else
          OK (Psltuw X1 al bl :: Psubw rl al bl :: Psubw rh ah bh :: Psubw rh rh X1 :: k)
  | Omull, a1 :: a2 :: nil =>
      do rd <- ireg_of_rpair res; do rs1 <- ireg_of_rpair a1; do rs2 <- ireg_of_rpair a2;
  ...
  | Onegf, a1 :: nil =>
      do rd <- freg_of_rpair res; do rs <- freg_of_rpair a1;
      OK (Pfnegd rd rs :: k)
  | Oabsf, a1 :: nil =>
      do rd <- freg_of_rpair res; do rs <- freg_of_rpair a1;
      OK (Pfabsd rd rs :: k)
  | Oaddf, a1 :: a2 :: nil =>
      do rd <- freg_of_rpair res; do rs1 <- freg_of_rpair a1; do rs2 <- freg_of_rpair a2;
  ...
  | Ointoffloat, a1 :: nil =>
      do rd <- ireg_of_rpair res; do rs <- freg_of_rpair a1;
      OK (Pfcvtwd rd rs :: k)
  | Ointuoffloat, a1 :: nil =>
      do rd <- ireg_of_rpair res; do rs <- freg_of_rpair a1;
      OK (Pfcvtwud rd rs :: k)
  | Ofloatofint, a1 :: nil =>
      do rd <- freg_of_rpair res; do rs <- ireg_of_rpair a1;
      OK (Pfcvtdw rd rs :: k)
  | Ofloatofintu, a1 :: nil =>
      do rd <- freg_of_rpair res; do rs <- ireg_of_rpair a1;
      OK (Pfcvtdwu rd rs :: k)
  ...
  | Osel ty, a1 :: a2 :: a3 :: nil =>
      do rd <- ireg_of_rpair res;
      do rs1 <- ireg_of_rpair a1;
      do rs2 <- ireg_of_rpair a2;
      do rs3 <- ireg_of_rpair a3;
      if ireg_eq rs2 rs3 then
        OK (Pmv rd rs2 :: k)
      else if Archi.zicond_support tt then
        if typ_eq ty Tint then
          OK (Pczero_eqz X31 rs2 rs1 :: Pczero_nez rd rs3 rs1 :: Paddw rd rd X31 :: k)
        else
          assertion (typ_eq ty Tlong);
          OK (Pczerol_eqz X31 rs2 rs1 :: Pczerol_nez rd rs3 rs1 :: Paddl rd rd X31 :: k)
      else
        OK (Pcsel rd rs1 rs2 rs3 :: k)
  | Oczero_eq, a1 :: a2 :: nil =>
      do rd <- ireg_of_rpair res;
      do rs1 <- ireg_of_rpair a1;
      do rs2 <- ireg_of_rpair a2;
      if Archi.zicond_support tt then
        OK (Pczero_eqz rd rs2 rs1 :: k)
      else
        Error (msg "Asmgen.transl_op zicond not supported")
  | Oczero_ne, a1 :: a2 :: nil =>
      do rd <- ireg_of_rpair res;
      do rs1 <- ireg_of_rpair a1;
      do rs2 <- ireg_of_rpair a2;
      if Archi.zicond_support tt then
        OK (Pczero_nez rd rs2 rs1 :: k)
      else
        Error (msg "Asmgen.transl_op zicond not supported")
  | _, _ =>
...
Definition it1_is_parent (before: bool) (i: Mach.instruction) : bool :=
  ...
  | Mgetparam ofs ty dst => forallb_rpair (fun x => negb (mreg_eq x R30)) dst
  | Mop op args res => before && forallb_rpair (fun x => negb (mreg_eq x R30)) res
  | _ => false
  end.
...
Definition transl_function (f: Mach.function) :=
  ...
  OK (mkfunction f.(Mach.fn_sig)
        (Pallocframe f.(fn_stacksize) ::
          storeind_ptr X30 SP f.(fn_link_ofs)
           (storeind_ptr RA SP f.(fn_retaddr_ofs) (Pcfi_rel_offset f.(fn_retaddr_ofs):: c)))).
