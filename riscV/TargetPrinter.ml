...
(* Basic printing functions *)
    ...
    (* The ABI names are the ABI Mnemonic names as found here:
...
(* Printing of instructions *)
    ...
    let print_instruction oc = function
      ...
      (* Zicond extension *)
      | Pczero_eqz (rd, rs1, rs2) ->
        fprintf oc "	czero.eqz	%a, %a, %a\n" ireg rd ireg rs1 ireg rs2
      | Pczero_nez (rd, rs1, rs2) ->
        fprintf oc "	czero.nez	%a, %a, %a\n" ireg rd ireg rs1 ireg rs2
      | Pczerol_eqz (rd, rs1, rs2) ->
        assert Archi.ptr64;
        fprintf oc "	czero.eqz	%a, %a, %a\n" ireg rd ireg rs1 ireg rs2
      | Pczerol_nez (rd, rs1, rs2) ->
        assert Archi.ptr64;
        fprintf oc "	czero.nez	%a, %a, %a\n" ireg rd ireg rs1 ireg rs2
      ...
      | Pcsrwi (csr, imm) ->
        fprintf oc "	csrwi	%a, %a\n" coqint csr coqint imm

      | Pcsrrw (rd, csr, r1) ->
        fprintf oc "	csrrw	%a, %a, %a\n" ireg rd coqint csr ireg r1

      | Pcsrrwi (rd, csr, imm) ->
        fprintf oc "	csrrwi	%a, %a, %a\n" ireg rd coqint csr coqint imm

      | Pcsrrc (rd, csr, r1) ->
        fprintf oc "	csrrc	%a, %a, %a\n" ireg rd coqint csr ireg r1

      | Pcsrrci (rd, csr, imm) ->
        fprintf oc "	csrrci	%a, %a, %a\n" ireg rd coqint csr coqint imm

      | Pcsrrs (rd, csr, r1) ->
        fprintf oc "	csrrs	%a, %a, %a\n" ireg rd coqint csr ireg r1

      | Pcsrrsi (rd, csr, imm) ->
        fprintf oc "	csrrsi	%a, %a, %a\n" ireg rd coqint csr coqint imm
      ...
      (*- #Link_to E_COMPCERT_TR_Function_PRINT_PLOADSYMBOL_001 *)
      | Ploadsymbol(rd, id, ofs) ->
         loadsymbol oc rd id ofs

      (*- #Link_to E_COMPCERT_TR_Function_PRINT_PLOADSYMBOL_HIGH_001 *)
