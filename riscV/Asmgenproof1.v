...
Proof.
  ...
  - set (ty := mreg_type r) in *.
    ...
    + simpl in EQ. inversion EQ. simpl in LOAD. inversion LOAD. exists rs1'.
      ...
      intros. destruct (preg_eq r0 (preg_of r)). rewrite e. rewrite Pregmap.gss. assumption.
      Simpl.
...
Proof.
  ...
  - set (ty := mreg_type r) in *.
    ...
    + exploit IHl. rewrite <- B in STOR; eauto with asmgen.
      eapply store_stack_pregs_swap. rewrite <- B in STOR; auto with asmgen.
      apply STOR. auto with asmgen. discriminate.
      intros [rs' [C D]].
      exists rs'. split.
