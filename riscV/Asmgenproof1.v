...
Lemma ireg_of_not_X31'':
  forall m r, ireg_of m = OK r -> IR X31 <> IR r.
Proof.
  intros. destruct m; inv H; congruence.
Qed.

Lemma ireg_of_not_X31''':
  forall m r, ireg_of m = OK r -> X31 <> r.
Proof.
  intros. destruct m; inv H; congruence.
Qed.

Lemma ireg_of_not_X1:
  forall m r, ireg_of m = OK r -> IR r <> IR X1.
Proof.
  intros. erewrite <- ireg_of_eq; eauto with asmgen.
Qed.

Lemma ireg_of_not_X1':
  forall m r, ireg_of m = OK r -> r <> X1.
Proof.
  intros. apply ireg_of_not_X1 in H. congruence.
Qed.

Lemma ireg_of_not_X1'':
  forall m r, ireg_of m = OK r -> IR X1 <> IR r.
Proof.
  intros. destruct m; inv H; congruence.
Qed.

Lemma ireg_of_not_X1''':
  forall m r, ireg_of m = OK r -> X1 <> r.
Proof.
  intros. destruct m; inv H; congruence.
Qed.

Global Hint Resolve ireg_of_not_X31 ireg_of_not_X31' ireg_of_not_X31'' ireg_of_not_X31''' ireg_of_not_X1 ireg_of_not_X1' ireg_of_not_X1'' ireg_of_not_X1''': asmgen.
...
Lemma transl_op_correct:
  ...
  eval_operation ge (rs#SP) op (map (fun x => get_pair x rs) (map preg_rpair_of args)) m = Some v ->
  exists rs',
     exec_straight ge fn c rs m k rs' m
...
Opaque Int.eq.
  intros. unfold transl_op in H; destruct op; ArgsInv; simpl in H0; SimplEval H0; try (inv H0); try (TranslOpSplitter; fail).
- (* move *)
  destruct res, r; inv H.
  destruct (preg_of r0) eqn:RES; destruct (preg_of r) eqn:ARG; inv H1; TranslOpSplitter.
  destruct r0; inv H1.
- (* intconst *)
...
+ set (rs1 := nextinstr (rs#x <- (Genv.symbol_address ge id Ptrofs.zero))).
  ...
  repeat Splitter'.
  intros. rewrite C; eauto with asmgen. unfold rs1; Simpl.
+ TranslOpSimpl.
- (* stackoffset *)
...
- (* xorimm *)
  ...
  exists rs'; split; eauto. rewrite B; auto with asmgen.
- (* shrximm *)
  exploit Val.shrx_shr_2; eauto. intros E.
...
- (* addl *)
  destruct Archi.ptr64 eqn:?; monadInv H; try TranslOpSplitter.
  assert (x <> x0) by (destruct x, x0; try congruence; inv Heqb0).
  destruct (ireg_eq x0 x1 || ireg_eq x0 x3 || ireg_eq x0 x4) eqn:?; inv EQ3.
  + econstructor; split.
    eapply exec_straight_five; simpl; reflexivity.
    repeat Splitter'.
    destruct (rs x1), (rs x2), (rs x3), (rs x4); auto. unfold Val.cmpu. simpl.
    rewrite Heqb.
    rewrite Int64.decompose_add, Int64.hi_ofwords.
    rewrite Int.add_carry_lt.
    destruct (Int.ltu (Int.add i0 i2) i2); auto.
    destruct (rs x1), (rs x2), (rs x3), (rs x4); auto. simpl.
    rewrite Int64.decompose_add, Int64.lo_ofwords. auto.
    intuition Splitter.
  + rewrite ! orb_false_iff in Heqb2. destruct Heqb2. destruct H0.
    assert (x0 <> x1) by (destruct x0, x1; try congruence; inv H0).
    assert (x0 <> x4) by (destruct x0, x4; try congruence; inv H1).
    assert (x0 <> x3) by (destruct x0, x3; try congruence; inv H2).
    econstructor; split.
    eapply exec_straight_four; simpl; reflexivity.
    repeat Splitter'.
    destruct (rs x1), (rs x2), (rs x3), (rs x4); auto. unfold Val.cmpu. simpl.
    rewrite Heqb.
    rewrite Int64.decompose_add, Int64.hi_ofwords.
...
- (* negl *)
  ...
  + econstructor; split.
    eapply exec_straight_five; simpl; reflexivity.
    repeat Splitter'.
    destruct (rs x1), (rs x2); auto. unfold Val.cmpu. simpl.
    rewrite Int64.decompose_neg, Int.sub_borrow_lt, Int64.hi_ofwords.
    destruct (Int.ltu Int.zero i0); simpl; rewrite <- Int.sub_zero_r; auto.
    destruct (rs x1), (rs x2); auto. simpl.
    rewrite Int64.decompose_neg, Int64.lo_ofwords. rewrite <- Int.sub_zero_r. auto.
    intuition Simpl.
  + econstructor; split.
    eapply exec_straight_four; simpl; reflexivity.
    repeat Splitter'.
...
- (* subl *)
  ...
  + econstructor; split.
    ...
    repeat Splitter'.
    destruct (rs x1), (rs x2), (rs x3), (rs x4); auto. unfold Val.cmpu. simpl.
    rewrite Int64.decompose_sub, Int64.hi_ofwords.
    rewrite Int.sub_borrow_lt.
    destruct (Int.ltu i0 i2); auto.
    destruct (rs x1), (rs x2), (rs x3), (rs x4); auto. simpl.
...
- (* xorimm *)
  ...
  exists rs'; split; eauto. rewrite B; auto with asmgen.
- (* shrxlimm *)
  exploit Val.shrxl_shrl_2; eauto. intros E.
  destruct (Int.eq n Int.zero).
+ econstructor; split. apply exec_straight_one. simpl; eauto. auto.
  split; intros; Simpl. 
...
- (* cond *)
  ...
  erewrite <- get_pairs_singles in B; eauto. eauto with asmgen.
- (* sel *)
  destruct (ireg_eq x1 x2); [|destruct (Archi.zicond_support tt)].
  + inv EQ4. econstructor; split; [|split].
    apply exec_straight_one; simpl; auto.
    Simpl. unfold Val.select_bool. destruct (Val.to_optbool (rs x0)); auto. simpl.
    destruct b; auto using Val.lessdef_normalize.
    intros; Simpl.
  + destruct (typ_eq ty Tint) eqn:?; monadInv EQ4.
    * econstructor; split; [|split].
      eapply exec_straight_three; simpl; reflexivity; auto. Simpl.
      unfold Val.select_bool, Val.to_optbool, Val.cmp_bool. simpl.
      destruct (rs x0); auto.
      destruct (Int.eq i Int.one) eqn:?; simpl; auto.
      rewrite ! (Int.same_if_eq i Int.one Heqb).
      rewrite ! Int.eq_false by apply Int.one_not_zero.
      destruct (rs x1); auto; simpl; [rewrite Int.add_zero_l | rewrite Ptrofs.add_zero]; auto.
      destruct (Int.eq i Int.zero) eqn:?; simpl; auto.
      destruct (rs x2); auto; simpl; [rewrite Int.add_zero | rewrite Ptrofs.add_zero]; auto.
      intuition Simpl.
    * econstructor; split; [|split].
      eapply exec_straight_three; simpl; reflexivity; auto. Simpl.
      unfold Val.select_bool, Val.to_optbool, Val.cmp_bool. simpl.
      ...
      rewrite ! (Int.same_if_eq i Int.one Heqb).
      rewrite ! Int.eq_false by apply Int.one_not_zero.
      destruct (rs x1); auto; simpl; [rewrite Int64.add_zero_l | rewrite Ptrofs.add_zero]; auto.
      destruct (Int.eq i Int.zero) eqn:?; simpl; auto.
      destruct (rs x2); auto; simpl; [rewrite Int64.add_zero | rewrite Ptrofs.add_zero]; auto.
      intuition Simpl.
  + inv EQ4. econstructor; split; [|split].
    apply exec_straight_one; simpl; eauto.
    Simpl. unfold Val.select_bool.
    destruct (rs x0); auto. simpl.
    destruct (Int.eq i Int.one) eqn:?; auto. simpl.
    rewrite (Int.same_if_eq i Int.one Heqb). apply Val.lessdef_normalize.
    destruct (Int.eq i Int.zero) eqn:?; auto. simpl.
    rewrite (Int.same_if_eq i Int.zero Heqb0). apply Val.lessdef_normalize.
    intros; Simpl.
  - destruct (Archi.zicond_support tt); inv EQ3.
    econstructor; split. eapply exec_straight_one; simpl; auto.
    split; intros; Simpl.
    destruct (rs x0); auto. simpl. destruct (Int.eq i Int.zero); auto.
    destruct (rs x1); auto. simpl. destruct Archi.ptr64; auto.
  - destruct (Archi.zicond_support tt); inv EQ3.
    econstructor; split. eapply exec_straight_one; simpl; auto.
    split; intros; Simpl.
    destruct (rs x0); auto. simpl. destruct (Int.eq i Int.zero); auto.
