...
(*c Boolean tests: *)
  ...
  | Osel (ty: typ)           (**r [rd = r1] if condition holds, [rd = r2] otherwise. *)
  | Oczero_eq                (**r [rd = c == 0 ? 0 : r1 ] *)
  | Oczero_ne.               (**r [rd = c != 0 ? 0 : r1 ] *)
...
Definition eval_operation
  ...
  | Osel ty, v1::v2::v3::nil => Some (Val.select_bool v1 v2 v3 ty)
  | Oczero_eq, c::v1::nil => Some (Val.select (Val.cmp_bool Ceq c Vzero) Vzero v1 Tint)
  | Oczero_ne, c::v1::nil => Some (Val.select (Val.cmp_bool Cne c Vzero) Vzero v1 Tint)
  | _, _ => None
  end.
...
Definition type_of_condition_typ (c: condition) : list typ :=
  match c with
...
Definition type_of_operation_typ (op: operation) : list typ * typ :=
  match op with
  ...
  | Ocmp c => (type_of_condition_typ c, Tint)
  | Osel ty => (Tint :: ty :: ty :: nil, ty)
  | Oczero => (Tint :: Tint :: nil, Tint)
  end.

Definition type_of_addressing_typ (addr: addressing) : list typ :=
  match addr with
  | Aindexed _ => Tptr :: nil
  | Aglobal _ _ => nil
  | Ainstack _ => nil
...
Definition type_of_condition (c: condition) : list S.bounds := 
  map (fun t => S.brefl (inj_typ_ptype t)) (type_of_condition_typ c).

Definition type_of_operation {Reg: Type} (op: operation) (args: list Reg) (r: Reg): res (list S.bounds * S.bounds * extra_type_info Reg) :=
  let '(targs, tres) := type_of_operation_typ op in
  OK (map (fun t => S.brefl (inj_typ_ptype t)) targs, S.brefl (inj_typ_ptype tres), eti_nil).

Definition type_of_addressing (addr: addressing) : list S.bounds :=
  map (fun t => S.brefl (inj_typ_ptype t)) (type_of_addressing_typ addr).
...
Proof with (try exact I; try reflexivity; auto using Val.Vptr_has_type).
  ...
  destruct op; simpl in H0; FuncInv; subst; simpl;
    compute in H1; inv H1; subst; simpl.
  (* move *)
  - congruence.
  ...
  (* sel *)
  - destruct v0... unfold Val.select_bool. simpl.
    destruct (Int.eq i Int.one), (Int.eq i Int.zero)... apply Val.normalize_type.
    apply Val.normalize_type. apply Val.normalize_type.
  - (* czero_eq *)
    destruct v0... unfold Val.select. simpl.
    destruct (Int.eq Int.zero); auto; apply Val.normalize_type.
  - (* czero_ne *)
    destruct v0... unfold Val.select. simpl.
    destruct (Int.eq Int.zero); auto; apply Val.normalize_type.
Qed.
...
Lemma type_shift_stack_operation {Reg}:
  forall delta op, @type_of_operation Reg (shift_stack_operation delta op) = @type_of_operation Reg op.
Proof.
...
Definition aindexed_addr (p: ptrofs) := Aindexed p.
...
Proof.
  ...
  intros. simpl. rewrite Ptrofs.add_zero_l. reflexivity.
Qed.
...
Proof.
  ...
  - subst v1. destruct (eval_condition cond vl1 m1) eqn:?.
    ...
    simpl; constructor.
  (* sel *)
  - apply Val.select_bool_inject; auto.
  (* czero_eq *)
  - apply Val.select_inject; auto.
    destruct (Val.cmp_bool Ceq v Vzero) eqn:?; auto.
    right; symmetry. inv H4; auto. inv Heqo. constructor.
  (* czero_ne *)
  - apply Val.select_inject; auto.
