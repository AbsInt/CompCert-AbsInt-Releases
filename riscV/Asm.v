...
(** 64-bit integer register-register instructions *)
  ...
  | Pczero_eqz (rd rs1 rs2: ireg)                   (**r conditional zero if condition is equal to zero *)
  | Pczero_nez (rd rs1 rs2: ireg)                   (**r conditional zero if condition is nonzero *)
  | Pczerol_eqz (rd rs1 rs2: ireg)                  (**r conditional zero if condition is equal to zero 64 bit *)
  | Pczerol_nez (rd rs1 rs2: ireg)                  (**r conditional zero if condition is nonzero  64 bit *)

  (* Zicsr extension *)
  | Pcsrr (rd: ireg) (csr: int)                     (**r read from csr regsiter *)
  | Pcsrw (csr: int) (r: ireg)                      (**r write to a csr register *)
  | Pcsrwi (csr imm: int)                           (**r write immediate to a csr register *)
  | Pcsrrw (rd: ireg) (csr: int) (r: ireg)          (**r read/write to a csr register *)
  | Pcsrrwi (rd: ireg) (csr imm: int)               (**r read/write immediate to a csr register *)
  | Pcsrrc (rd:ireg) (csr: int) (r: ireg)           (**r read and clear bits in csr register *)
  | Pcsrrci (rd:ireg) (csr imm: int)                (**r read and clear bits in csr register with immediate mask *)
  | Pcsrrs (rd:ireg) (csr: int) (r: ireg)           (**r read and set bits in csr register *)
  | Pcsrrsi (rd:ireg) (csr imm: int)                (**r read and set bits in csr register with immediate mask *)
...
(** 64-bit integer register-register instructions *)
  ...
  | Pcvtl2w d s =>
      Next (nextinstr (rs#d <- (Val.lowordoflong rs##s))) m
  | Pcvtw2l r =>
...
(** Pseudo-instructions *)
  | Pallocframe sz =>
      ...
      Next (nextinstr (rs #X30 <- (rs SP) #SP <- sp #X31 <- Vundef)) m1
  | Pfreeframe sz pos =>
  ...
  (**  Zicond extension *)
  | Pczero_eqz d rs1 rs2 =>
      let v := match rs#rs2 with
               | Vint i => if Int.eq i Int.zero then Vzero else rs#rs1
               | _ => Vundef
               end in
      Next (nextinstr (rs#d <- v)) m
  | Pczero_nez d rs1 rs2 =>
      let v := match rs#rs2 with
               | Vint i => if Int.eq i Int.zero then rs#rs1 else Vzero
               | _ => Vundef
               end in
      Next (nextinstr (rs#d <- v)) m
  | Pczerol_eqz d rs1 rs2 =>
      let v :=  match rs#rs2 with
               | Vint i => if Int.eq i Int.zero then Vlong Int64.zero else rs#rs1
               | _ => Vundef
               end in
      Next (nextinstr (rs#d <- v)) m
  | Pczerol_nez d rs1 rs2 =>
      let v := match rs#rs2 with
               | Vint i => if Int.eq i Int.zero then rs#rs1 else Vlong Int64.zero
               | _ => Vundef
               end in
      Next (nextinstr (rs#d <- v)) m
