...
- monadInv H. destruct (ireg_eq x0 x1 || ireg_eq x0 x3 || ireg_eq x0 x4); TailNoLabel.
- apply opimm64_label; intros; exact I.
- monadInv H. destruct (ireg_eq x0 x1); TailNoLabel.
- monadInv H. destruct (ireg_eq x0 x1 || ireg_eq x0 x3 || ireg_eq x0 x4); TailNoLabel.
- apply opimm64_label; intros; exact I.
- apply opimm64_label; intros; exact I.
- apply opimm64_label; intros; exact I.
...
- destruct m; monadInv EQ0; eapply transl_memory_access_label; eauto; intros; exact I.
- destruct m; monadInv EQ0; eapply transl_memory_access_label; eauto; intros; exact I.
...
Proof.
  ...
  simpl.
  erewrite tail_nolabel_find_label by (apply storeind_ptr_label). simpl.
  erewrite tail_nolabel_find_label by (apply storeind_ptr_label). simpl.
...
- intros. monadInv H0.
  ...
  eapply storeind_ptr_label.
  eapply tail_nolabel_trans.
  eapply storeind_ptr_label. TailNoLabel.
...
(* GPR11 does not contain parent *)
  ...
  exploit loadind_correct. eauto. instantiate (2 := rs1). rewrite Q. eauto. congruence.
  intros [rs2 [S [T U]]].
...
- (* Mop *)
  ...
  rewrite R; auto. destruct res; [| split; simpl in H3; rewrite andb_true_iff in H3; destruct H3]; simpl; apply preg_of_not_X30; auto.
...
- (* Mtailcall *)
  ...
  assert (NOOV: list_length_z tf.(fn_code) <= Ptrofs.max_unsigned).
    eapply transf_function_no_overflow; eauto.  exploit Mem.loadv_extends. eauto. eexact H3. auto. simpl. intros [parent' [A B]].
...
- (* internal function *)
  ...
  set (tfbody := Pallocframe (fn_stacksize f) ::
                 storeind_ptr X30 SP (fn_link_ofs f)
                 (storeind_ptr RA SP (fn_retaddr_ofs f) x1)) in *.
  set (tf := {| fn_sig := Mach.fn_sig f; fn_code := tfbody |}) in *.
  set (rs2 := nextinstr (rs0#X30 <- (parent_sp s) #SP <- sp #X31 <- Vundef)).
  exploit (storeind_ptr_correct tge tf SP (fn_link_ofs f) X30); eauto with asmgen.
  rewrite chunk_of_Tptr in F. instantiate (2 := rs2).
  change (rs2 X2) with sp. eexact F.
  intros (rs3 & U & V).
  exploit (storeind_ptr_correct tge tf SP (fn_retaddr_ofs f) RA x1 rs3 m2'); eauto with asmgen.
  rewrite chunk_of_Tptr in P. rewrite !V by eauto with asmgen. change (rs2 X1) with (rs0 X1). rewrite ATLR.
  ...
  { change (fn_code tf) with tfbody; unfold tfbody.
    ...
    rewrite <- (sp_val _ _ _ AG). rewrite chunk_of_Tptr in F. unfold rs2. reflexivity.
    reflexivity.
    eapply exec_straight_trans.
    eexact U. eapply exec_straight_trans. eexact U'. eapply exec_straight_one; eauto.
  }
...
Local Transparent destroyed_at_function_entry.
  ...
  intros. unfold rs5; Simpl. rewrite V', V; eauto with asmgen.
  intros. unfold rs5; Simpl. rewrite V', V; eauto with asmgen.
- (* external function *)
