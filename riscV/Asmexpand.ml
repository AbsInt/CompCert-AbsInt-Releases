...
(*- E_COMPCERT_CODE_Asmexpand_expand_csel_001 *)
...
let valid_csr csr =
  Integers.(Int.cmpu Cle _0l csr) && Integers.(Int.cmpu Clt csr _4096l)

let expand_csr_generic name arg csr_imm csr_reg =
  match arg with
  | BA_int v ->
    if Integers.(Int.cmpu Cle _0l v && Int.cmpu Clt v _32l) then
      csr_imm v
    else begin
      expand_loadimm32 X31 v;
      csr_reg X31
    end
  | BA_long v ->
    if Integers.(Int64.cmpu Cle _0L v && Int64.cmpu Clt v _32L) then begin
      let imm = Integers.(Int.repr (Int64.unsigned v)) in
      csr_imm imm
    end else begin
      expand_loadimm64 X31 v;
      csr_reg X31
    end
  | BA (IR r) -> csr_reg r
  | _ -> raise (AsmexpandError ("the argument of "^name^" must be a either a register or immediate"))
...
let expand_builtin_inline name args res =
  ...
  (*- #Link_to E_COMPCERT_TR_Function_EXPAND_CSR_READ_001 *)
  | "__builtin_csr_read", [BA_int csr], BR (IR res) when valid_csr csr ->
    emit (Pcsrr (res, csr))
  (*- #Link_to E_COMPCERT_TR_Robustness_DIAG_CSR_001 *)
  | "__builtin_csr_read", _, _ ->
    raise (AsmexpandError "the argument of __builtin_csr_read must be a valid csr register")
  (*- #End *)

  (*- E_COMPCERT_CODE_Asmexpand_builtin_inline_026 *)
  (*- #Link_to E_COMPCERT_TR_Function_EXPAND_CSR_WRITE_001 *)
  | "__builtin_csr_write", [BA_int csr; arg], _ when valid_csr csr ->
    expand_csr_generic "__builtin_csr_write" arg
      (fun imm -> emit (Pcsrwi (csr,imm))) (fun r -> emit (Pcsrw (csr, r)))
  (*- #Link_to E_COMPCERT_TR_Robustness_DIAG_CSR_001 *)
  | "__builtin_csr_write", _, _ ->
    raise (AsmexpandError "the first argument of __builtin_csr_write must be a valid csr register")
  (*- #End *)

  (*- E_COMPCERT_CODE_Asmexpand_builtin_inline_027 *)
  (*- #Link_to E_COMPCERT_TR_Function_EXPAND_CSR_READWRITE_001 *)
  | "__builtin_csr_readwrite", [BA_int csr; arg], BR (IR res) when valid_csr csr ->
    expand_csr_generic "__builtin_csr_readwrite" arg
      (fun imm -> emit (Pcsrrwi (res, csr,imm))) (fun r -> emit (Pcsrrw (res, csr, r)))
  (*- #Link_to E_COMPCERT_TR_Robustness_DIAG_CSR_001 *)
  | "__builtin_csr_readwrite", _, _ ->
    raise (AsmexpandError "the first argument of __builtin_csr_readwrite must be a valid csr register")
  (*- #End *)

  (*- E_COMPCERT_CODE_Asmexpand_builtin_inline_028 *)
  (*- #Link_to E_COMPCERT_TR_Function_EXPAND_CSR_CLEAR_BITS_001 *)
  | "__builtin_csr_clear_bits", [BA_int csr; arg], BR (IR res) when valid_csr csr ->
    expand_csr_generic "__builtin_csr_clear_bits" arg
      (fun imm -> emit (Pcsrrci (res, csr, imm))) (fun r -> emit (Pcsrrc (res, csr, r)))
  (*- #Link_to E_COMPCERT_TR_Robustness_DIAG_CSR_001 *)
  | "__builtin_csr_clear_bits", _, _ ->
    raise (AsmexpandError "the first argument of __builtin_csr_clear_bits must be a valid csr register")
  (*- #End *)

  (*- E_COMPCERT_CODE_Asmexpand_builtin_inline_029 *)
  (*- #Link_to E_COMPCERT_TR_Function_EXPAND_CSR_SET_BITS_001 *)
  | "__builtin_csr_set_bits", [BA_int csr; arg], BR (IR res) when valid_csr csr ->
    expand_csr_generic "__builtin_csr_set_bits" arg
      (fun imm -> emit (Pcsrrsi (res, csr, imm))) (fun r -> emit (Pcsrrs (res, csr, r)))
  (*- #Link_to E_COMPCERT_TR_Robustness_DIAG_CSR_001 *)
  | "__builtin_csr_set_bits", _, _ ->
    raise (AsmexpandError "the first argument of __builtin_csr_set_bits must be a valid csr register")
  (*- #End *)
