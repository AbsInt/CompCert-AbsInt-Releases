...
Proof.
  ...
  TrivialExists. econstructor. econstructor. eexact H0. reflexivity.
  econstructor; eauto. econstructor; eauto. econstructor.
  simpl. unfold Val.select_bool. rewrite Val.to_optbool_of_optbool. auto.
Qed.
...
Proof.
  ...
  assert (eval_expr ge sp e m le (Eop (Ointconst Int.zero) Enil) Vzero).
  { econstructor. econstructor. auto. }
  destruct bf; try discriminate.
  - inv ARGS; try discriminate. inv H1; try discriminate. inv H3; try discriminate.
    inv SEM. inv SEL. destruct (Archi.zicond_support tt); inv H3.
    + econstructor; split.
      econstructor. econstructor; eauto. econstructor; eauto. econstructor.
      reflexivity.
      destruct v1, v0; auto. simpl. destruct (Int.eq i Int.zero); auto.
    + generalize (eval_comp Cne le a1 v1 (Eop (Ointconst Int.zero) Enil) Vzero H0 H).
      intros (v2 & A & B).
      exploit eval_sub. eexact H. eexact A.
      intros (v3 & A1 & B1).
      exploit eval_and. eexact H2. eexact A1.
      intros (v4 & A2 & B2).
      exists v4; split; eauto.
      destruct v1, v0; auto. unfold Val.cmp in B.
      simpl in B. destruct (Int.eq i Int.zero); auto.
      inv B. inv B1. inv B2. simpl. rewrite Int.sub_zero_l.
      rewrite Int.and_zero. auto.
      inv B. inv B1. inv B2. simpl. rewrite Int.sub_zero_r.
      change (Int.neg Int.one) with Int.mone. rewrite Int.and_mone.
      auto.
  - inv ARGS; try discriminate. inv H1; try discriminate. inv H3; try discriminate.
    inv SEM. inv SEL. destruct (Archi.zicond_support tt); inv H3.
    + econstructor; split.
      ...
      reflexivity.
      destruct v1, v0; auto. simpl. destruct (Int.eq i Int.zero); auto.
    + generalize (eval_comp Ceq le a1 v1 (Eop (Ointconst Int.zero) Enil) Vzero H0 H).
      intros (v2 & A & B).
      exploit eval_sub. eexact H. eexact A.
      intros (v3 & A1 & B1).
      exploit eval_and. eexact H2. eexact A1.
...
Proof.
  ...
  unfold lowlong; case (lowlong_match a); destruct Archi.ptr64 eqn:?; intros; inv H0;  InvEval; inv H0.
