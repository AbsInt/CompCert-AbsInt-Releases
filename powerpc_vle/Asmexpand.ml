...
let expand_volatile_access
  ...
  | BA_addrstack ofs ->
      if offset_in_range ofs then
        mk1 (Sreg GPR1) (Cint ofs)
      else begin
        emit (Pe_lis((Sreg GPR0), (Cint (Asmgen.high_s ofs))));
        emit (Pe_add16i((Sreg GPR0), (Sreg GPR0), Cint (Asmgen.low_s ofs)));
        mk2 (Sreg GPR1) (Sreg GPR0)
      end
  | BA_addrglobal(id, ofs) ->
      if symbol_is_small_data id ofs then begin
        emit (Pse_li(GPR0, _0));
        emit (Pe_add16i(temp, (Sreg GPR0), (Csymbol_sda (id, ofs))));
        mk1 temp (Cint _0)
      end else if symbol_is_rel_data id ofs then begin
        emit (Pe_li((Sreg GPR0), Cint _0));
        emit (Pe_add16i((Sreg GPR0), (Sreg GPR0), Csymbol_rel_low(id, ofs)));
        emit (Pe_add2is((Sreg GPR0), Csymbol_rel_high(id, ofs)));
        mk1 temp (Cint _0)
      end else begin
        emit (Pe_lis(temp, Csymbol_high(id, ofs)));
  ...
  | BA_addptr(BA(IR r), BA_int n) ->
      if offset_in_range n then
        ...
        emit_move_rr temp r;
        emit (Pe_add2is(temp,Cint (Asmgen.high_s n)));
        mk1 temp (Cint (Asmgen.low_s n))
      end
  | BA_addptr(BA_addrglobal(id, ofs), BA(IR r)) ->
      if symbol_is_small_data id ofs then begin
        emit (Pe_li((Sreg GPR0), Cint _0 ));
        emit (Pe_add16i((Sreg GPR0), (Sreg GPR0), (Csymbol_sda (id, ofs))));
        mk2 r (Sreg GPR0)
      end else if symbol_is_rel_data id ofs then begin
        emit_move_rr temp r;
        emit (Pse_li(GPR0, _0));
        ...
        emit (Pe_add16i(temp, r, Csymbol_low(id, ofs)));
        emit (Pe_add2is(temp, Csymbol_high(id, ofs)));
        mk1 temp (Cint _0)
      end
...
let expand_load_int64 hi lo base ofs_hi ofs_lo =
  if hi <> base then begin
    emit (Pe_lwz(hi, ofs_hi, base));
    emit (Pe_lwz(lo, ofs_lo, base))
  end else begin
    emit (Pe_lwz(lo, ofs_lo, base));
    emit (Pe_lwz(hi, ofs_hi, base))
  end
...
let expand_builtin_vload_1 chunk addr res =
  ...
  | Mint8unsigned, BR(IR res) ->
      expand_volatile_access
        (fun r c -> emit (Pe_lbz(res, c, r)))
        (fun r1 r2 -> emit (Plbzx(res, r1, r2)))
        addr (Areg GPR11)
  | Mint8signed, BR(IR res) ->
      expand_volatile_access
        (fun r c -> emit (Pe_lbz(res, c, r)); emit (Pextsb(res, res)))
        (fun r1 r2 -> emit (Plbzx(res, r1, r2)); emit (Pextsb(res, res)))
        addr (Areg GPR11)
  | Mint16unsigned, BR(IR res) ->
      expand_volatile_access
        (fun r c -> emit (Pe_lhz(res, c, r)))
        (fun r1 r2 -> emit (Plhzx(res, r1, r2)))
        addr (Areg GPR11)
  | Mint16signed, BR(IR res) ->
      expand_volatile_access
        (fun r c -> emit (Pe_lha(res, c, r)))
        (fun r1 r2 -> emit (Plhax(res, r1, r2)))
        addr (Areg GPR11)
  | (Mint32 | Many32), BR(IR res) ->
      expand_volatile_access
        (fun r c -> emit (Pe_lwz(res, c, r)))
        (fun r1 r2 -> emit (Plwzx(res, r1, r2)))
        addr (Areg GPR11)
  | Mfloat32, BR(IR res) ->
      expand_volatile_access
        (fun r c -> emit (Pfe_lwz(res, c, r)))
        (fun r1 r2 -> emit (Pflwzx(res, r1, r2)))
        addr (Areg GPR11)
  | Mint64, BR_splitlong(BR(IR hi), BR(IR lo)) ->
      expand_volatile_access
        (fun r c ->
           ...
           | None ->
               emit (Pe_add16i((Areg GPR11), r, c));
               expand_load_int64 hi lo (Areg GPR11) (Cint _0) (Cint _4))
        (fun r1 r2 ->
           emit (Padd((Areg GPR11), r1, r2));
           expand_load_int64 hi lo (Areg GPR11) (Cint _0) (Cint _4))
        addr (Areg GPR11)
  | _, _ ->
      assert false
...
let expand_store_int64 hi lo base ofs_hi ofs_lo =
  emit (Pe_stw(hi, ofs_hi, base));
  emit (Pe_stw(lo, ofs_lo, base))
(*- #End *)
...
let expand_builtin_vstore_1 chunk addr src =
  ...
  | (Mint8signed | Mint8unsigned), BA(IR src) ->
      expand_volatile_access
        (fun r c -> emit (Pe_stb(src, c, r)))
        (fun r1 r2 -> emit (Pstbx(src, r1, r2)))
        addr temp
  | (Mint16signed | Mint16unsigned), BA(IR src) ->
      expand_volatile_access
        (fun r c -> emit (Pe_sth(src, c, r)))
        (fun r1 r2 -> emit (Psthx(src, r1, r2)))
        addr temp
  | (Mint32 | Many32), BA(IR src) ->
      expand_volatile_access
        (fun r c -> emit (Pe_stw(src, c, r)))
        (fun r1 r2 -> emit (Pstwx(src, r1, r2)))
        addr temp
  | Mfloat32, BA(IR src) ->
      expand_volatile_access
        (fun r c -> emit (Pfe_stw(src, c, r)))
        (fun r1 r2 -> emit (Pfstwx(src, r1, r2)))
        addr temp
