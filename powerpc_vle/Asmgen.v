...
Require Import Mach.
Require Import Asm.
Require Import Compopts Stacklayout.
...
Definition freg_of (r: mreg) : res ireg :=
...
(** Check if an immediate is in SCI8 format
    The SCI8 format is a 8 bit immediate shifted by 0,1,2 or 3 byte positions.
    The remaining bits are either all zeros or all ones. *)
Definition is_sci8 (n: int) :=
  let b0 := Int.repr (4294967040) in (* Bytes 0 all zero rest one *)
  let b1 := Int.repr (4294902015) in (* Bytes 1 all zero rest one *)
  let b2 := Int.repr (4278255615) in (* Bytes 2 all zero rest one *)
  let b3 := Int.repr (16777215) in (* Bytes 3 all zero rest one *)
  let m0 := Int.and n b0 in
  let m1 := Int.and n b1 in
  let m2 := Int.and n b2 in
  let m3 := Int.and n b3 in
  (* n & bi is either bi or zero if n is in SCI8 format *)
  Int.eq m0 Int.zero || Int.eq m0 b0 || Int.eq m1 Int.zero || Int.eq m1 b1
  || Int.eq m2 Int.zero || Int.eq m2 b2 || Int.eq m3 Int.zero || Int.eq m3 b3.
...
Definition mulimm (r1 r2: ireg) (n: int) (k: code) :=
  if is_sci8 n then
    Pe_mulli r1 r2 n :: k
  else if ireg_eq r1 r2 && (Int.eq (high_s n) Int.zero) then
    Pe_mull2i r1 n :: k
  else
    loadimm GPR0 n (select_encoding_instr Pse_mullw Pmullw true r1 r2 GPR0 k).

Definition andimm_base (r1 r2: ireg) (n: int) (k: code) :=
  if is_sci8 n then
    Pe_andi_ r1 r2 n :: k
  else if Int.eq (high_u n) Int.zero then
...
Inductive condition_class: condition -> list mreg -> Type :=
  ...
  | condition_le:
      forall n r, Int.signed n < Int.max_signed -> condition_class (Ccompimm Cle n) (r :: nil)
  | condition_leu:
      forall n r, Int.unsigned n < Int.max_unsigned -> condition_class (Ccompuimm Cle n) (r :: nil)
  | condition_ge:
      forall n r, Int.min_signed < Int.signed n -> condition_class (Ccompimm Cge n) (r :: nil)
  | condition_geu:
      forall n r, 0 < Int.unsigned n -> condition_class (Ccompuimm Cge n) (r :: nil)
  | condition_default:
      forall c rl, condition_class c rl.

(* Check for the case that the upper 16 bit of the immediate change from
   zero to non-zero if immediate is decreased/increased. *)

Definition high_s_check (n1 n2: int) : bool :=
  negb (Int.eq (high_s n1) Int.zero) || (Int.eq (high_s n2) Int.zero).

Definition high_u_check (n1 n2 : int) : bool :=
  negb (Int.eq (high_u n1) Int.zero) || (Int.eq (high_u n2) Int.zero).
...
Definition classify_condition (c: condition) (args: list mreg): condition_class c args :=
  ...
  | Ccompimm Cge n, r :: nil =>
      ...
      | right _ =>
          if high_s_check n (Int.sub n Int.one) then
            match zlt Int.min_signed (Int.signed n) with
            | left EQ => condition_ge n r EQ
            | right _ => condition_default (Ccompimm Cge n) (r :: nil)
            end
          else
            condition_default (Ccompimm Cge n) (r :: nil)
      end
  ...
  | Ccompimm Cle n, r :: nil =>
      if  high_s_check n (Int.sub n Int.one) then
        match zlt (Int.signed n) Int.max_signed with
        | left EQ => condition_le n r EQ
        | right _ => condition_default (Ccompimm Cle n) (r :: nil)
        end
      else
        condition_default (Ccompimm Cle n) (r :: nil)
  | Ccompuimm Cle n, r :: nil =>
      if high_u_check n (Int.add n Int.one) then
        match zlt (Int.unsigned n) Int.max_unsigned with
        | left EQ => condition_leu n r EQ
        | right _ => condition_default (Ccompuimm Cle n) (r :: nil)
        end
      else
        condition_default (Ccompuimm Cle n) (r :: nil)
  | Ccompuimm Cge n, r :: nil =>
      match zlt 0 (Int.unsigned n) with
      | left EQ => condition_geu n r EQ
      | right _ => condition_default (Ccompuimm Cge n) (r :: nil)
      end
  | x, y =>
...
Definition transl_cond_op
  ...
  | condition_le n a _ =>
      transl_cond (Ccompimm Clt (Int.add n Int.one)) (a :: nil) ((Pmfcrbit r' CRbit_0) :: k)
  | condition_leu n a _ =>
      transl_cond (Ccompuimm Clt (Int.add n Int.one)) (a :: nil) ((Pmfcrbit r' CRbit_0) :: k)
  | condition_ge n a _ =>
      transl_cond (Ccompimm Cgt (Int.sub n Int.one)) (a :: nil) ((Pmfcrbit r' CRbit_1) :: k)
  | condition_geu n a _ =>
      transl_cond (Ccompuimm Cgt (Int.sub n Int.one)) (a :: nil) ((Pmfcrbit r' CRbit_1) :: k)
...
Definition transl_op
  ...
  | Omove, a1 :: nil =>
      ...
      | One (IR r), One (IR a) => OK (move_rr_base r a k) (* We need a move here *)
      | _   , _    => Error(msg "Asmgen.Omove")
      end
  | Ointconst n, nil =>
      do r <- ireg_of_rpair res; OK (loadimm r n k)
  | Osingleconst f, nil =>
      do r <- ireg_of_rpair res; OK (Plfis r f :: k)
  ...
  | Oaddrstack n, nil =>
      do r <- ireg_of_rpair res; OK (addimm r GPR1 (Ptrofs.to_int n) k)
  | Ocast8signed, a1 :: nil =>
  ...
  | Oadd, a1 :: a2 :: nil =>
      do r1 <- ireg_of_rpair a1; do r2 <- ireg_of_rpair a2; do r <- ireg_of_rpair res;
  ...
  | Oaddl, a1 :: a2 :: nil =>
      ...
      if ireg_eq rl ah || ireg_eq rl bh then
        OK (Paddc GPR0 al bl :: Padde rh ah bh :: (move_rr rl GPR0 k))
      else
        OK (Paddc rl al bl :: Padde rh ah bh :: k)
  | Oaddimm n, a1 :: nil =>
      do r1 <- ireg_of_rpair a1; do r <- ireg_of_rpair res; OK (addimm r r1 n k)
  | Oaddsymbol s ofs, a1 :: nil =>
       do r1 <- ireg_of_rpair a1; do r <- ireg_of_rpair res;
       OK (if symbol_is_small_data s ofs then
  ...
  | Osubl, a1::a2::nil =>
      ...
      do (bh, bl) <- iregs_of_rpair a2;
      if ireg_eq rl ah || ireg_eq rl bh then
        OK (Psubfc GPR0 bl al :: Psubfe rh bh ah :: (move_rr rl GPR0 k))
      else
        OK (Psubfc rl bl al :: Psubfe rh bh ah :: k)
  | Osubimm n, a1 :: nil =>
      do r1 <- ireg_of_rpair a1; do r <- ireg_of_rpair res;
  ...
  | Onegl, a1 :: nil =>
      ...
      if ireg_eq rl ah then
        OK (Pe_subfic GPR0 al Int.zero :: Psubfze rh ah :: (move_rr rl GPR0 k))
      else
        OK (Pe_subfic rl al Int.zero :: Psubfze rh ah :: k)
  | Odiv, a1 :: a2 :: nil =>
      do r1 <- ireg_of_rpair a1; do r2 <- ireg_of_rpair a2; do r <- ireg_of_rpair res;
      OK (Pdivw r r1 r2 :: k)
  | Odivu, a1 :: a2 :: nil =>
...
(* We split the memory access code into smaller functions in order to reuse
   them in Asmexpand for volatile load/store. *)

Definition aindexed
     (mk : constant -> ireg -> code -> code)
     (r1 temp: ireg) (ofs: int) (k: code) :=
  if Int.eq (high_s ofs) Int.zero then
    mk (Cint ofs) r1 k
  else
    move_rr temp r1 (Pe_add2is temp (Cint (high_s ofs)) ::
                     mk (Cint (low_s ofs)) temp k).
...
Definition transl_memory_access
  ...
  | Aindexed ofs, a1 :: nil =>
      do r1 <- ireg_of a1;
      OK (aindexed (fun c r k => mk1 c r :: k) r1 temp ofs k)
  | Aindexed2, a1 :: a2 :: nil =>
      do r1 <- ireg_of a1; do r2 <- ireg_of a2;
      OK (aindexed2 (fun r1 r2 k => mk2 r1 r2 :: k) r1 r2 k)
  | Aglobal symb ofs, nil =>
      OK (aglobal (fun c r k => mk1 c r :: k) (fun r1 r2 k => mk2 r1 r2 :: k) temp symb ofs k)
  | Abased symb ofs, a1 :: nil =>
      do r1 <- ireg_of a1;
      OK (abased (fun c r k => mk1 c r :: k) (fun r1 r2 k => mk2 r1 r2 :: k) r1 temp symb ofs k)
  | Ainstack ofs, nil =>
      let ofs := Ptrofs.to_int ofs in
      OK (ainstack (fun c r k => mk1 c r :: k) (fun r1 r2 k => mk2 r1 r2 :: k) ofs k)
  | _, _ =>
...
Definition transl_load_store_multiple_condition l ofs :=
  match l with
  | m1 :: m2 :: l0 => list_eq_dec preg_eq (map preg_of (m1 :: m2 :: l0)) (map IR (compute_ireg_list (preg_of m1)))
                     && Int.eq (high_s_24 (Int.repr ofs)) Int.zero && (Int.eq (Int.mods (Int.repr ofs) (Int.repr 4)) Int.zero)
  | _ => false
  end.
...
Definition transl_instr (f: Mach.function) (i: Mach.instruction)
  ...
  | Mrestorecallee ofs l =>
      do l <- mmap (@error_single mreg) l;
      if transl_load_store_multiple_condition l ofs then
        match l with
        | m1 :: l1 => do r1 <- ireg_of m1;
                    OK (Pe_lmw r1 SP (Cint (Int.repr (align ofs 4))) :: k)
        | _ => Error (msg "Mrestorecallee nil")
        end
      else
        Error (msg "Mrestorecallee constraint violated")
  | Msavecallee ofs l =>
      do l <- mmap (@error_single mreg) l;
      if transl_load_store_multiple_condition l ofs then
        match l with
        | m1 :: l1 => do r1 <- ireg_of m1;
                    OK (Pe_stmw r1 SP (Cint (Int.repr (align ofs 4))) :: k)
        | _ => Error (msg "Msavecallee nil")
        end
      else
        Error (msg "Msavecallee constraint violated")
...
Definition it1_is_parent (before: bool) (i: Mach.instruction) : bool :=
  ...
  | Mgetparam ofs ty dst => forallb_rpair (fun x => negb (mreg_eq x R11)) dst
  | Mop Omove args res => before && forallb_rpair (fun x => negb (mreg_eq x R11)) res
  | _ => false
  end.
