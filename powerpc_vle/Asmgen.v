...
Definition addimm_base (r1 r2 : ireg) (n: int) (k: code) :=
  if Int.eq (high_s n) Int.zero then
    Pe_add16i r1 r2 (Cint n) :: k
  else if Int.eq (low_s n) Int.zero then
    move_rr r1 r2 (Pe_add2is r1 (Cint (high_s n)) :: k)
  else
    Pe_add16i r1 r2 (Cint (low_s n)) ::
    Pe_add2is r1 (Cint (high_s n)) :: k.

Definition addimm (r1 r2: ireg) (n: int) (k: code) :=
  match r1 with
  | Sreg s1 => if Int.lt Int.zero n && Int.lt n (Int.repr 33) && ireg_eq r1 r2 then
               Pse_addi s1 n :: k
             else
               addimm_base r1 r2 n k
  | _ => addimm_base r1 r2 n k
  end.
