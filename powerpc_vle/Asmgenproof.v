...
Remark select_encoding_instr_label:
  forall (mk1 : sreg -> sreg -> instruction)
    (mk2 : ireg -> ireg -> ireg -> instruction)
    commut rd r1 r2 k,
  (forall rd r1, nolabel (mk1 rd r1)) ->
  (forall rd r1 r2, nolabel (mk2 rd r1 r2)) ->
  tail_nolabel k (select_encoding_instr mk1 mk2 commut rd r1 r2 k).
Proof.
  unfold select_encoding_instr; intros. destruct rd, r1, r2; TailNoLabel.
  destruct (sreg_eq s s0), commut, (sreg_eq s s1); simpl; TailNoLabel.
Qed.
Hint Resolve select_encoding_instr_label: labels.

Remark loadimm_default_label:
  forall r n k, tail_nolabel k (loadimm_default r n k).
Proof.
  intros. unfold loadimm_default.
  case (Int.eq (Int.shru n (Int.repr 7)) Int.zero). destruct r; TailNoLabel.
  case (Int.eq (high_s_12 n) Int.zero). TailNoLabel.
  case (Int.eq (low_s n) Int.zero); TailNoLabel.
Qed.
Hint Resolve loadimm_default_label: labels.

Remark loadimm_label:
  forall r n k, tail_nolabel k (loadimm r n k).
Proof.
  intros. unfold loadimm.
  destruct r, (Int.is_power2 n); TailNoLabel.
Qed.
Hint Resolve loadimm_label: labels.
Opaque loadimm.

Remark move_rr_base_label:
  forall r1 r2 k, tail_nolabel k (move_rr_base r1 r2 k).
Proof.
  intros. unfold move_rr_base. destruct r1,r2; TailNoLabel.
Qed.
Hint Resolve move_rr_base_label: labels.

Remark move_rr_label:
  forall r1 r2 k, tail_nolabel k (move_rr r1 r2 k).
Proof.
  intros. unfold move_rr. destruct (ireg_eq r1 r2); TailNoLabel.
Qed.
Hint Resolve move_rr_label: labels.

Remark addi_low_label:
  forall r1 r2 n k, tail_nolabel k (addi_low r1 r2 n k).
Proof.
  intros. unfold addi_low. destruct r1; TailNoLabel.
  destruct (ireg_eq s r2); simpl; TailNoLabel.
  destruct (is_oim5 n); TailNoLabel.
  destruct (is_oim5 (Int.neg n)); TailNoLabel.
Qed.
Hint Resolve addi_low_label: labels.
...
Proof.
  ...
  [TailNoLabel |destruct (is_sci8 n); TailNoLabel].
  destruct (Int.eq (low_s n) Int.zero);
    eapply tail_nolabel_trans. eapply move_rr_label. TailNoLabel.
   eapply addi_low_label. TailNoLabel.
Qed.
...
Proof.
  ...
  destruct (is_sci8 n); TailNoLabel.
  case (Int.eq (high_u n) Int.zero).
  eapply tail_nolabel_trans. eapply move_rr_label.
  TailNoLabel.
...
Remark mulimm_label:
  forall r1 r2 n k, tail_nolabel k (mulimm r1 r2 n k).
Proof.
  intros; unfold mulimm.
  destruct (is_sci8 n); [|destruct (ireg_eq r1 r2 && Int.eq (high_s n) Int.zero)]; TailNoLabel.
...
Remark shrimm_label:
  forall r1 r2 n k, tail_nolabel k (shrimm r1 r2 n k).
Proof.
  intros; unfold shrimm.
  destruct r1,r2; TailNoLabel. destruct (sreg_eq s s0); TailNoLabel.
Qed.
Hint Resolve shrimm_label: labels.

Lemma select_mem_access_label:
  forall access_size
    (mk1 : sreg -> int -> sreg -> instruction)
    (mk2 : ireg -> constant -> ireg -> instruction)
    rd cst base,
  (forall rd c r, nolabel (mk1 rd c r)) ->
  (forall rd r1 r2, nolabel (mk2 rd r1 r2)) ->
  nolabel (select_mem_access access_size mk1 mk2 rd cst base).
Proof.
  intros. unfold select_mem_access.
  destruct rd; auto.
  destruct cst; auto.
  destruct base; auto.
  destruct (is_sd4_const access_size i); auto.
Qed.
Hint Resolve select_mem_access_label: labels.
...
Proof.
  ...
  destruct ty; try discriminate; destruct (preg_of dst); try discriminate;
    destruct (Int.eq (high_s ofs') Int.zero); inv H;
      try apply tail_nolabel_cons; TailNoLabel;
        eapply tail_nolabel_trans; TailNoLabel.
Qed.
...
Lemma transl_restorecallee_unroll_label:
  forall l ofs k c,
  transl_restorecallee_unroll ofs l k = OK c ->
  tail_nolabel k c.
Proof.
  induction l as [ | h l]; intros; simpl in H.
  - monadInv H. apply tail_nolabel_refl.
  - monadInv H. eapply tail_nolabel_trans. eapply loadind_label. apply EQ0. eapply IHl; eauto.
Qed.

Lemma transl_savecallee_unroll_label:
  forall l ofs k c,
...
Proof.
  induction l as [ | h l]; intros; simpl in H.
  - monadInv H. apply tail_nolabel_refl.
  - monadInv H. eapply tail_nolabel_trans. eapply storeind_label. apply EQ0. eapply IHl; eauto.
Qed.
...
Proof.
  unfold transl_cond; intros; destruct cond; TailNoLabel;
    eapply tail_nolabel_trans; TailNoLabel.
  destruct x, x0; TailNoLabel.
  destruct x; TailNoLabel.
  destruct x0; TailNoLabel.
  destruct x; TailNoLabel.
  unfold floatcomp; destruct c0; TailNoLabel.
  unfold floatcomp; destruct c0; TailNoLabel.
Qed.
...
Opaque Int.eq.
  ...
  - destruct (symbol_is_small_data); [|destruct (symbol_is_rel_data)]; TailNoLabel.
  - destruct x0; TailNoLabel. eapply tail_nolabel_trans. eapply move_rr_label. TailNoLabel.
  - destruct x0; TailNoLabel. eapply tail_nolabel_trans. eapply move_rr_label. TailNoLabel.
  - destruct (symbol_is_small_data); [|destruct (symbol_is_rel_data)]; TailNoLabel.
  - destruct x1, x , x0; TailNoLabel. destruct (sreg_eq s s0), (sreg_eq s s1); TailNoLabel.
  - eapply tail_nolabel_trans. eapply loadimm_label. TailNoLabel.
  - eapply tail_nolabel_trans. eapply shrimm_label. TailNoLabel.
  - eapply transl_cond_op_label; eauto.
  - destruct (preg_of r); monadInv H. eapply transl_select_op_label; eauto.
Qed.

Remark transl_memory_access_label:
  forall (mk1: constant -> ireg -> instruction)
  ...
  tail_nolabel k c.
Proof.
  unfold transl_memory_access, aindexed, aindexed2, aglobal, abased, ainstack.
  intros; destruct addr; TailNoLabel.
...
Proof.
  ...
  destruct (split_by_type l); destruct l0; destruct l1; try discriminate. eapply transl_restorecallee_unroll_label; eauto.
  eapply transl_restorecallee_unroll_label; eauto. monadInv H. TailNoLabel. eapply transl_restorecallee_unroll_label; eauto.
  monadInv H. TailNoLabel. eapply transl_restorecallee_unroll_label; eauto. 
  eapply transl_restorecallee_unroll_label; eauto.
  destruct (split_by_type l); destruct l0; destruct l1; try discriminate. eapply transl_savecallee_unroll_label; eauto.
  eapply transl_savecallee_unroll_label; eauto. monadInv H. TailNoLabel. eapply transl_savecallee_unroll_label; eauto.
  monadInv H. TailNoLabel. eapply transl_savecallee_unroll_label; eauto. 
  eapply transl_savecallee_unroll_label; eauto.
  eapply loadind_label; eauto.
  ...
  destruct s0; monadInv H; TailNoLabel. destruct x; TailNoLabel.
  destruct s0; monadInv H; TailNoLabel.
  destruct x; inv EQ0. eapply tail_nolabel_trans with (c2 := transl_epilogue f (Pse_bctr s :: k)); TailNoLabel.
  eapply tail_nolabel_trans. apply transl_epilogue_label. TailNoLabel.
  eapply tail_nolabel_trans with (c2 := transl_epilogue f (Pse_bctr s :: k)); TailNoLabel.
  eapply tail_nolabel_trans. apply transl_epilogue_label. TailNoLabel.
  eapply tail_nolabel_trans. apply transl_epilogue_label. TailNoLabel.
  destruct (snd (crbit_for_cond c0)); TailNoLabel.
  eapply tail_nolabel_trans. eapply transl_cond_label. apply H. TailNoLabel.
  eapply tail_nolabel_trans. eapply transl_cond_label. apply H. TailNoLabel.
  eapply tail_nolabel_trans; [apply transl_epilogue_label | TailNoLabel].
Qed.
...
- (* Mrestorecallee *)
  exploit load_stack_regs_extends; eauto. intros [rs2' [A B]].
  rewrite (sp_val _ _ _ AG) in A.
  left; eapply exec_straight_steps; eauto.
  intros. unfold transl_instr in TR.
  remember (transl_load_store_multiple_condition l ofs).
  destruct b; cycle 1.
  destruct l. discriminate.
  (* Recursive case *)
  exploit transl_restorecallee_unroll_correct; eauto. discriminate.
  intros [rs2 [C D]].
  exists rs2; split. eauto. split.
  eapply agree_exten; eauto with asmgen.
  split. discriminate.
  rewrite D; eauto with asmgen.
  eapply load_stack_pregs_mregs; eauto with asmgen.
  (* Conditions hold *)
  unfold transl_load_store_multiple_condition in Heqb.
  remember (split_by_type l). destruct p.
  destruct l0. discriminate. destruct l0. discriminate.
  monadInv TR.
  exploit load_stack_pregs_sublists; eauto.
  apply split_by_type_same; eauto. intros [rs1' [C D]].
  exploit compute_ireg_list_correct.
  destruct (list_eq_dec preg_eq preg_of ## (m0 :: m1 :: l0) IR ## (compute_ireg_list (preg_of m0))). apply e.
  simpl in Heqb. discriminate. intros E.
  assert (Pnt: forall o, Val.add (rs0 GPR1) (Vint (Int.repr o)) = Val.offset_ptr (rs0 GPR1) (Ptrofs.repr o)).
  { intro. destruct (rs0 GPR1); try discriminate. simpl. f_equal; f_equal. auto with ptrofs. }
  assert (COND: load_store_multiple_condition x GPR1 (Cint (Int.repr (align ofs 4))) = true).
  { unfold load_store_multiple_condition. rewrite andb_true_r.
    rewrite <- (ireg_of_eq _ _ EQ). destruct m0; reflexivity.
  }
  exploit exec_lmw_correct.
  apply C. apply E. rewrite <- Pnt. rewrite (ireg_of_eq _ _ EQ).
  intros.
  destruct l1.
  (* Float list empty *)
  simpl in D. inversion D. inversion EQ1. rewrite <- H3.
  exists (nextinstr rs1').
  split. 
  econstructor. simpl.
  rewrite COND.
  apply H0.
  rewrite nextinstr_pc. f_equal. eapply load_stack_pregs_mregs; eauto.
  split.
  rewrite H2. eapply agree_exten; eauto. intros. apply nextinstr_inv1; assumption.
  split. intros. discriminate.
  rewrite nextinstr_inv; auto with asmgen. eapply load_stack_pregs_mregs; eauto with asmgen.
  (* Float list nonempty *)
  exploit load_stack_pregs_swap. apply D. intros. apply nextinstr_inv1; assumption.
  intros [rs2 [F [G1 G2]]].
  exploit transl_restorecallee_unroll_correct. apply EQ1.
  replace (rs0 GPR1) with (nextinstr rs0 GPR1) in D by eauto with asmgen.
  replace (rs0 GPR1) with (nextinstr rs1' GPR1) in F; cycle 1.
  rewrite nextinstr_inv; eauto with asmgen.
  eapply (load_stack_pregs_mregs (m0 :: m1 :: l0) SP); eauto with asmgen. apply F.
  discriminate. intros [rs1 [H1 H2]]. exists rs1.
  split. eapply exec_straight_trans.
  econstructor. unfold exec_instr. rewrite COND. apply H0.
  rewrite nextinstr_pc. f_equal. eapply load_stack_pregs_mregs; eauto.
  apply H1.
  split.
  eapply agree_exten; eauto. intros. rewrite H2; eauto with asmgen.
  split. discriminate.
  rewrite H2; eauto with asmgen. rewrite G2; eauto with asmgen.
  rewrite nextinstr_inv; eauto with asmgen.
  eapply load_stack_pregs_mregs; eauto with asmgen. discriminate.

- (* Msavecallee *) 
  exploit store_stack_regs_extends; eauto. intros [m1 [A B]].
  rewrite (sp_val _ _ _ AG) in A.
  left; eapply exec_straight_steps; eauto.
  intros. unfold transl_instr in TR.
  remember (transl_load_store_multiple_condition l ofs).
  ...
  destruct l. discriminate.
  (* Recursive case *)
  exploit transl_savecallee_unroll_correct; eauto. discriminate.
  intros [rs2 [C D]].
  exists rs2; split. eauto. split.
  eapply agree_undef_regs. apply AG. auto with asmgen.
  ...
  apply split_by_type_same; eauto. intros [m'1 [C D]].
  exploit compute_ireg_list_correct.
  destruct (list_eq_dec preg_eq preg_of ## (m0 :: m2 :: l0) IR ## (compute_ireg_list (preg_of m0))). apply e.
  simpl in Heqb. discriminate. intros E.
  assert (Pnt: forall o, Val.add (rs0 GPR1) (Vint (Int.repr o)) = Val.offset_ptr (rs0 GPR1) (Ptrofs.repr o)).
  { intro. destruct (rs0 GPR1); try discriminate. simpl. f_equal; f_equal. auto with ptrofs. }
  assert (COND: load_store_multiple_condition x GPR1 (Cint (Int.repr (align ofs 4))) = true).
  ...
  exploit exec_stmw_correct.
  apply C. apply E. rewrite <- Pnt. rewrite (ireg_of_eq _ _ EQ).
  intros.
  destruct l1.
  (* Float list empty *)
  simpl in D. inversion D. inversion EQ1. rewrite <- H3.
  exists (nextinstr rs0).
  split. 
  econstructor. simpl.
  rewrite COND.
  apply H1.
  reflexivity. split.
  eapply agree_undef_regs; eauto. intros. apply nextinstr_inv1; assumption.
  easy.
  (* Float list nonempty *)
  exploit transl_savecallee_unroll_correct. apply EQ1.
  replace (rs0 GPR1) with (nextinstr rs0 GPR1) in D by eauto with asmgen.
  eapply store_stack_pregs_swap. apply D.
  intros. Simpl. discriminate.
  intros [rs2 [G1 G2]]. exists rs2.
  split. eapply exec_straight_trans. 
  econstructor. unfold exec_instr.
  rewrite COND. simpl. apply H1.
  reflexivity.
  apply G1. split.
  eapply agree_undef_regs. apply AG. intros.
  rewrite G2; eauto with asmgen. apply nextinstr_inv1; auto.
  simpl. split;
  rewrite G2; eauto with asmgen. 
...
- (* Mcall *)
  ...
  assert (NOOV: list_length_z tf.(fn_code) <= Int.max_unsigned).
    eapply transf_function_no_overflow; eauto.
  destruct ros as [rf|fid]; simpl in H; monadInv H5; [destruct x0|].
  + (* Indirect call function address in sreg *)
    inv EQ2.
    assert (rs rf = Vptr f' Ptrofs.zero).
    destruct (rs rf); try discriminate.
    revert H; predSpec Ptrofs.eq Ptrofs.eq_spec i Ptrofs.zero; intros; congruence.
    assert (rs0 s0 = Vptr f' Ptrofs.zero).
  ...
  + (* Indirect call function address in areg *)
    inv EQ2.
    assert (rs rf = Vptr f' Ptrofs.zero).
    destruct (rs rf); try discriminate.
    revert H; predSpec Ptrofs.eq Ptrofs.eq_spec i Ptrofs.zero; intros; congruence.
    assert (rs0 a = Vptr f' Ptrofs.zero).
    exploit ireg_val; eauto. rewrite H5; intros LD; inv LD; auto.
    generalize (code_tail_next_int _ _ _ _ NOOV H6). intro CT1.
    generalize (code_tail_next_int _ _ _ _ NOOV CT1). intro CT2.
    assert (TCA: transl_code_at_pc ge (Vptr fb (Ptrofs.add (Ptrofs.add (Ptrofs.add ofs Ptrofs.one) Ptrofs.one) Ptrofs.one)) fb f c false tf x).
    econstructor; eauto.
    ...
    eapply functions_transl; eauto. eapply find_instr_tail; eauto.
    simpl. eauto.
    eapply star_two. eapply exec_step_internal. Simpl. rewrite <- H2; simpl; eauto.
    eapply functions_transl; eauto. eapply find_instr_tail; eauto.
    simpl. eauto.
    eapply exec_step_internal. Simpl. rewrite <- H2; simpl; eauto.
    eapply functions_transl; eauto. eapply find_instr_tail; eauto.
    simpl. eauto.
    traceEq.
    traceEq.
...
- (* Mtailcall *)
  ...
  assert (NOOV: list_length_z tf.(fn_code) <= Ptrofs.max_unsigned).
    eapply transf_function_no_overflow; eauto.
  destruct ros as [rf|fid]; simpl in H; monadInv H7;[destruct x0|].
+ (* Indirect call function address in sreg *)
  inv EQ2.
  assert (rs rf = Vptr f' Ptrofs.zero).
    destruct (rs rf); try discriminate.
  ...
  assert (A': exec_straight tge tf
            (Pse_mtctr s0 :: transl_epilogue f (Pse_bctr sig :: x))
            rs0 m'0
            (Pse_bctr sig :: x) rs2 m2').
...
+ (* Indirect call function address in areg *)
  ...
  assert (rs0 a = Vptr f' Ptrofs.zero).
    exploit ireg_val; eauto. rewrite H7; intros LD; inv LD; auto.
  set (rs1 := nextinstr (nextinstr rs0 # GPR0 <- (Vptr f' Ptrofs.zero)) # CTR <- (Vptr f' Ptrofs.zero)).
  assert (AG1: agree rs (Vptr stk soff) rs1). { apply agree_nextinstr. apply agree_set_other; auto. apply agree_nextinstr.  apply agree_set_other; auto. }
  exploit transl_epilogue_correct; eauto.
  intros (rs2 & m2' & A & B & C & D & E & F).
  assert (A': exec_straight tge tf
            (Pse_mfar GPR0 a :: Pse_mtctr GPR0 :: transl_epilogue f (Pse_bctr sig :: x))
            rs0 m'0
            (Pse_bctr sig :: x) rs2 m2').
  { eapply exec_straight_trans.
    eapply exec_straight_two; simpl; reflexivity. rewrite H9. Simpl. eexact A. }
  clear A.
  exploit exec_straight_steps_2; eauto using functions_transl.
  intros (ofs' & U & V).
  set (rs3 := rs2#PC <- (rs2 CTR)).
...
- (* Mjumptable *)
  ...
  instantiate (2 := rs0#GPR12 <- Vundef #GPR0 <- Vundef #CTR <- Vundef).
  Simpl. eauto.
  eauto.
  intros [tc' [rs' [A [B C]]]].
...
- (* internal function *)
  ...
  set (tfbody := Pallocframe (fn_stacksize f) (fn_link_ofs f)
                      (fn_retaddr_ofs f)
                    :: Pse_mflr GPR0
                       :: Pe_stw GPR0 (Cint (Ptrofs.to_int (fn_retaddr_ofs f)))
                            GPR1
                          :: Pcfi_rel_offset
