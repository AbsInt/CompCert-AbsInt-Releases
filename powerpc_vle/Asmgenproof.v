...
Lemma transf_function_retaddr_saved:
  forall f tf,
  transf_function f = OK tf -> f.(fn_retaddr_saved) = true.
Proof.
  intros. unfold transf_function in H.
  destruct (f.(fn_retaddr_saved)); inv H. reflexivity.
Qed.
...
Proof.
  ...
  eapply transl_cond_label; eauto. TailNoLabel.
  eapply tail_nolabel_trans.
  eapply transl_cond_label; eauto. TailNoLabel.
  eapply tail_nolabel_trans.
  eapply transl_cond_label; eauto. TailNoLabel.
...
Opaque Int.eq.
  ...
  - destruct r; inv H. destruct (preg_of r); inv H1. destruct r0; inv H0. destruct (preg_of r0); inv H1. TailNoLabel.
  ...
  - destruct x0; TailNoLabel. eapply tail_nolabel_trans. eapply move_rr_label. TailNoLabel.
  - monadInv H. destruct (ireg_eq x0 x1 || ireg_eq x0 x3); inv EQ3; TailNoLabel.
  - destruct (symbol_is_small_data); [|destruct (symbol_is_rel_data)]; TailNoLabel.
  - destruct x1, x , x0; TailNoLabel. destruct (sreg_eq s s0), (sreg_eq s s1); TailNoLabel.
  - monadInv H. destruct (ireg_eq x0 x1 || ireg_eq x0 x3); inv EQ3; TailNoLabel.
  - eapply tail_nolabel_trans. eapply loadimm_label. TailNoLabel.
  - monadInv H. destruct (ireg_eq x0 x1); inv EQ2; TailNoLabel.
  - eapply tail_nolabel_trans. eapply shrimm_label. TailNoLabel.
  - eapply transl_cond_op_label; eauto.
  - destruct r; inv EQ0. destruct (preg_of r); monadInv H0. eapply transl_select_op_label; eauto.
Qed.
...
Proof.
  ...
  - destruct s0; monadInv H; TailNoLabel.
    ...
    eapply tail_nolabel_trans. apply transl_epilogue_label. TailNoLabel.
  - destruct (snd (crbit_for_cond c0)); TailNoLabel.
    eapply tail_nolabel_trans. eapply transl_cond_label. apply EQ0. TailNoLabel.
    eapply tail_nolabel_trans. eapply transl_cond_label. apply EQ0. TailNoLabel.
  - eapply tail_nolabel_trans; [apply transl_epilogue_label | TailNoLabel].
Qed.
...
Lemma exec_straight_steps:
  forall s fb f rs1 i c ep tf tc m1' m2 sp ms2,
  match_stack ge s ->
  Genv.find_funct_ptr ge fb = Some (Internal f) ->
...
Proof.
  ...
  exploit H3; eauto. intros (rs2 & m2' & A & B & C & D & F).
  exists (State m2' rs2); split.
...
- (* Mlabel *)
  ...
  monadInv TR. econstructor; econstructor; split. apply exec_straight_one. simpl; eauto. auto.
...
- (* Mgetstack *)
  ...
  econstructor; exists rs'; split. eauto.
  split. eapply agree_set_mreg; eauto with asmgen. congruence.
  split. simpl; congruence.
  split; auto. apply R; auto with asmgen.
...
- (* Mrestorecallee *)
  exploit load_stack_rpairs_extends; eauto. intros [rs2' [A B]].
  rewrite (sp_val _ _ _ AG) in A.
  left; eapply exec_straight_steps; eauto. intros. monadInv TR. unfold transl_load_store_multiple_condition in EQ0. Destructor; try discriminate. InvBooleans. monadInv EQ0.
  assert (COND: load_store_multiple_condition x0 GPR1 = true).
  { unfold load_store_multiple_condition. rewrite andb_true_r.
    rewrite <- (ireg_of_eq _ _ EQ1). destruct m0; reflexivity. }
  assert (Pnt: forall o, Val.add (rs0 GPR1) (Vint (Int.repr o)) = Val.offset_ptr (rs0 GPR1) (Ptrofs.repr o)).
  { destruct l. inv EQ. simpl in A. unfold restore_callee_pair, load_stack in A. destruct (rs0 GPR1); Destructor; try discriminate.
    intros. simpl. f_equal; f_equal. auto with ptrofs. }
  exploit compute_ireg_list_correct; eauto. intros. exploit exec_lmw_correct; eauto. rewrite <- (sp_val _ _ _ AG); eauto.
  intros (rs3' & C & D & F).
  econstructor; econstructor. split. econstructor. simpl. rewrite COND. simpl. rewrite Pnt. erewrite <- (ireg_of_eq _ x0); eauto.
  rewrite D; auto. split. eapply agree_set_other; eauto. constructor; inv F; inv AG; auto. intros. eapply Val.lessdef_trans; eauto.
  split. simpl. congruence. rewrite D; auto.
...
- (* Msavecallee *)
  ...
  { unfold load_store_multiple_condition. rewrite andb_true_r.
    rewrite <- (ireg_of_eq _ _ EQ1). destruct m0; reflexivity. }
  assert (Pnt: forall o, Val.add (rs0 GPR1) (Vint (Int.repr o)) = Val.offset_ptr (rs0 GPR1) (Ptrofs.repr o)).
  { destruct l. inv EQ. simpl in H. unfold save_callee_pair, store_stack in H.
    destruct (rs0 GPR1); Destructor; try discriminate. simpl. intro. f_equal; f_equal. auto with ptrofs. }
  exploit compute_ireg_list_correct; eauto. intros. exploit exec_stmw_correct; eauto. rewrite <- (sp_val _ _ _ AG); eauto.
  intros (m2' & A & B). econstructor. econstructor. split.
  econstructor. simpl. rewrite COND. rewrite Pnt. erewrite <- (ireg_of_eq _ x0); eauto. auto. split.
  eapply agree_undef_regs; eauto. intros; eauto. Simpl. split; eauto.
...
Opaque loadind.
  ...
  + (* GPR11 contains parent *)
    ...
    intros [rs1 [P [Q R]]].
    econstructor; exists rs1; split; eauto.
  ...
  + (* GPR11 does not contain parent *)
    ...
    intros [rs2 [S [T U]]].
    econstructor; exists rs2; split. eapply exec_straight_trans; eauto.
...
- (* Mop *)
  ...
  econstructor; exists rs2; split. eauto. split.
  assert (S: lessdef' v (preg_rpair_of res) rs2) by (eapply lessdef_lessdef'_trans; eauto).
  eapply agree_set_undef_mreg_rpair; eauto with asmgen. split; auto.
  simpl. destruct op; try congruence. destruct ep; simpl; try congruence. intros.
  rewrite R; auto.
  destruct res; try split; simpl in H1; InvBooleans; apply preg_of_not_GPR11; auto.
  change (destroyed_by_op Omove) with (@nil mreg). simpl; auto.
  split; auto. apply R; auto with asmgen.
  destruct res; try split;  apply preg_of_not_LR.
...
- (* Mload *)
  ...
  econstructor; exists rs2; split. eauto.
...
- (* Mstore *)
  ...
  split. eapply agree_undef_regs; eauto with asmgen.
  split. simpl; congruence.
  split; auto. apply Q; auto with asmgen.
...
- (* Mcall *)
  ...
  assert (NOOV: list_length_z tf.(fn_code) <= Int.max_unsigned).
    eapply transf_function_no_overflow; eauto.
  destruct ros as [rf|fid]; simpl in H; monadInv H6; [destruct x0|].
  + (* Indirect call function address in sreg *)
    inv EQ2.
    assert (rs rf = Vptr fb' Ptrofs.zero).
    destruct (rs rf); try discriminate.
    revert H; predSpec Ptrofs.eq Ptrofs.eq_spec i Ptrofs.zero; intros; congruence.
    assert (rs0 s0 = Vptr fb' Ptrofs.zero).
  ...
  + (* Indirect call function address in areg *)
    ...
    destruct (rs rf); try discriminate.
    revert H; predSpec Ptrofs.eq Ptrofs.eq_spec i Ptrofs.zero; intros; congruence.
    assert (rs0 a = Vptr fb' Ptrofs.zero).
    exploit ireg_val; eauto. rewrite H6; intros LD; inv LD; auto.
    generalize (code_tail_next_int _ _ _ _ NOOV H7). intro CT1.
    generalize (code_tail_next_int _ _ _ _ NOOV CT1). intro CT2.
    ...
    eapply functions_transl; eauto. eapply find_instr_tail; eauto.
    simpl. eauto.
    eapply star_two. eapply exec_step_internal. Simpl. rewrite <- H3; simpl; eauto.
    eapply functions_transl; eauto. eapply find_instr_tail; eauto.
    simpl. eauto.
    eapply exec_step_internal. Simpl. rewrite <- H3; simpl; eauto.
    eapply functions_transl; eauto. eapply find_instr_tail; eauto.
    simpl. eauto.
    traceEq.
    traceEq.
...
- (* Mtailcall *)
  assert (f0 = f) by congruence.  subst f0.
  inversion AT; subst.
  exploit (transf_function_retaddr_saved f tf); auto. intros Hretaddr.
  specialize (H4 Hretaddr).
  assert (NOOV: list_length_z tf.(fn_code) <= Ptrofs.max_unsigned).
    eapply transf_function_no_overflow; eauto.
  destruct ros as [rf|fid]; simpl in H0; monadInv H8;[destruct x0|].
+ (* Indirect call function address in sreg *)
  inv EQ2.
  assert (rs rf = Vptr fb' Ptrofs.zero).
    destruct (rs rf); try discriminate.
  ...
  set (rs1 := nextinstr (rs0#CTR <- (Vptr fb' Ptrofs.zero))).
  ...
  { apply exec_straight_step with rs1 m'0. simpl. rewrite H10. reflexivity. reflexivity. eexact A. }
  clear A.
...
+ (* Indirect call function address in areg *)
  ...
  assert (rs0 a = Vptr fb' Ptrofs.zero).
    exploit ireg_val; eauto. rewrite H8; intros LD; inv LD; auto.
  set (rs1 := nextinstr (nextinstr rs0 # GPR0 <- (Vptr fb' Ptrofs.zero)) # CTR <- (Vptr fb' Ptrofs.zero)).
  ...
  { eapply exec_straight_trans.
    eapply exec_straight_two; simpl; reflexivity. rewrite H10. Simpl. eexact A. }
  clear A.
  exploit exec_straight_steps_2; eauto using functions_transl.
  intros (ofs' & U & V).
  set (rs3 := rs2#PC <- (rs2 CTR)).
...
+ (* Direct call *)
  ...
  exploit exec_straight_steps_2; eauto using functions_transl.
  intros (ofs' & U & V).
  set (rs3 := rs2#PC <- (Vptr fb' Ptrofs.zero)).
  left; exists (State rs3 m2'); split.
  (* execution *)
  eapply plus_right'. eapply exec_straight_exec; eauto.
  econstructor; eauto using functions_transl, find_instr_tail.
  simpl. unfold Genv.symbol_address. rewrite symbols_preserved, H0. reflexivity.
  traceEq.
  (* match states *)
  econstructor; eauto. apply agree_set_other; auto.
...
- (* Mreturn *)
  ...
  traceEq.
  (* match states *)
  econstructor; eauto. apply agree_restore_auto_save_regs. apply agree_set_other; auto.
  ...
  set (tfbody := Pallocframe (fn_stacksize f) (fn_link_ofs f)
                    ...
                    :: Pse_mflr GPR0
                       :: Pe_stw GPR0 (Cint (Ptrofs.to_int (fn_retaddr_ofs f)))
                            GPR1
                          :: Pcfi_rel_offset (fn_retaddr_ofs f) :: x0) in *.
...
- (* external function *)
  ...
  apply agree_restore_auto_save_regs. apply agree_undef_caller_save_regs; auto. 
