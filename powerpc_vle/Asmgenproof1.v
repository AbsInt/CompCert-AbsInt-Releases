...
Proof.
  ...
  - inversion MAP. inversion LOAD. simpl. reflexivity.
  - monadInv MAP.
    assert (TY: (mreg_type r) = Tany32).
    { unfold mreg_type. destruct r; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
    rewrite TY in LOAD. simpl in LOAD. simpl.
    rewrite <- (ireg_of_eq _ _ EQ).
    destruct (Mem.loadv Many32 m (Val.offset_ptr sp (Ptrofs.repr (align ofs 4))));[|discriminate].
...
Proof.
  ...
  - inversion MAP. inversion STOR. simpl. reflexivity.
  - monadInv MAP.
    assert (TY: (mreg_type r) = Tany32).
    { unfold mreg_type. destruct r; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
...
Proof.
  ...
  - remember (Mem.loadv (chunk_of_type (mreg_type r)) m
    ...
    destruct l1. exists rs1. simpl in H0. rewrite H0. split. reflexivity. simpl. rewrite <- Heqo. assumption.
...
Lemma transl_restorecallee_unroll_correct:
  forall l m k (rs1:regset) rs2 ofs c,
    transl_restorecallee_unroll ofs l k = OK c ->
    load_stack_pregs m (rs1 SP) ofs l rs1 = Some rs2 ->
    l <> nil ->
    exists rs2' : regset,
      exec_straight ge fn c rs1 m k rs2' m
      /\ forall r, r <> PC -> r <> GPR0 -> rs2'#r = rs2#r.
Proof.
  induction l as [ | r l]; simpl; intros until c; intros TR LOAD NNIL.
  - contradiction.
  - set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    monadInv TR.
    remember (Mem.loadv (chunk_of_type ty) m (Val.offset_ptr (rs1 SP) (Ptrofs.repr ofs1))).
    destruct o; cycle 1. discriminate.
    exploit loadind_correct; eauto with asmgen.
    intros [rs1' [A [B C]]].
    destruct l.
    + simpl in EQ. inversion EQ. simpl in LOAD. inversion LOAD. exists rs1'.
      split; auto with asmgen.
      intros. destruct (preg_eq r0 (preg_of r)). rewrite e. Simpl.
      Simpl.
    + exploit load_stack_pregs_swap. apply LOAD. instantiate (1:= rs1').
      intros. destruct (preg_eq r0 (preg_of r)).
      rewrite e. Simpl. 
      Simpl.
      intros [rs3 [LOAD' [D E]]]. rewrite <- C in LOAD'; auto with asmgen.
      exploit IHl. apply EQ. apply LOAD'.
      discriminate. intros [rs2' [F G]].
      exists rs2'. split. eapply exec_straight_trans; eauto.
      intros. specialize D with r0. specialize E with r0. specialize C with r0.
      rewrite G; eauto with asmgen.
      remember (data_preg r0). destruct b.
      rewrite D; eauto with asmgen.
      assert (r0 <> (preg_of r)).
      { red. intro. rewrite H1 in Heqb. rewrite preg_of_data in Heqb. discriminate. }
      rewrite E; auto. rewrite C; eauto with asmgen.
      exploit load_stack_pregs_mregs. left. symmetry. apply Heqb.
      apply LOAD.
...
Lemma transl_savecallee_unroll_correct:
  forall l ofs k c (rs: regset) m1 m2,
    ...
    exists rs',
      ...
      /\ forall r, r <> PC -> r <> GPR0 -> rs'#r = rs#r.
Proof. 
  induction l as [ | r l]; simpl; intros until m2; intros TR STOR NNIL.
  - contradiction.
  - set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    monadInv TR.
    remember (Mem.storev (chunk_of_type ty) m1 (Val.offset_ptr (rs SP) (Ptrofs.repr ofs1)) (rs (preg_of r))).
    destruct o; cycle 1. discriminate.
    exploit storeind_correct; eauto with asmgen.
    intros [rs1 [A B]].
    destruct l.
    + simpl in EQ. inversion EQ. simpl in STOR. inversion STOR. exists rs1.
      rewrite <- H1. split; auto with asmgen.
    + exploit IHl. rewrite <- B in STOR; eauto with asmgen.
      eapply store_stack_pregs_swap. rewrite <- B in STOR; auto with asmgen. apply STOR. auto with asmgen. discriminate.
      intros [rs' [C D]].
      exists rs'. split.
      eapply exec_straight_trans; eauto. intros. rewrite <- B; eauto with asmgen.
Qed.
