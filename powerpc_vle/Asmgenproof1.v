...
Lemma ireg_of_not_GPR0'':
  forall m r, ireg_of m = OK r -> IR GPR0 <>  IR r.
Proof.
...
Lemma ireg_of_not_GPR0''':
  forall m r, ireg_of m = OK r -> Sreg GPR0 <> r.
Proof.
  intros. destruct m; inv H; congruence.
Qed.
Global Hint Resolve ireg_of_not_GPR0''': asmgen.

Lemma ireg_rpair_of_not_GPR0:
  forall m r, ireg_of_rpair m = OK r -> IR r <> IR GPR0.
Proof.
  intros. destruct m; simpl in *; auto with asmgen.
  unfold ireg_of in H. destruct r0; inv H; try congruence.
Qed.
Global Hint Resolve ireg_rpair_of_not_GPR0: asmgen.

Lemma ireg_rpair_of_not_GPR0':
  forall m r, ireg_of_rpair m = OK r -> r <> GPR0.
Proof.
  intros.  generalize (ireg_rpair_of_not_GPR0 _ _ H). congruence.
Qed.
Global Hint Resolve ireg_rpair_of_not_GPR0': asmgen.
...
(** Mull integer immediate. *)
Lemma mulimm_correct:
  forall (r1 r2: ireg) n k rs m,
  r1 <> GPR0 ->
  r2 <> GPR0 ->
  exists rs',
     exec_straight ge fn (mulimm r1 r2 n k) rs m  k rs' m
  /\ rs'#r1 = Val.mul rs#r2 (Vint n)
  /\ forall r': preg, important_preg r' = true -> r' <> r1 -> rs'#r' = rs#r'.
Proof.
  intros. unfold mulimm.
  destruct (is_sci8 n); [|destruct (ireg_eq r1 r2 && Int.eq (high_s n) Int.zero) eqn:?].
  - (* e_mulli *)
    econstructor; split. apply exec_straight_one; simpl; reflexivity.
    intuition Simpl.
  - (* e_mul2i *)
    destruct (ireg_eq r1 r2); inv Heqb.
    econstructor; split. apply exec_straight_one; simpl; reflexivity.
    intuition Simpl.
  - (* Pmullw *)
    exploit (loadimm_correct GPR0 n (select_encoding_instr Pse_mullw Pmullw true r1 r2 GPR0 k)).
    intros [rs' [EX [RES OTH]]].
    exploit (select_encoding_instr_correct Pse_mullw Pmullw Val.mul true r1 r2 GPR0 k rs' m); eauto with asmgen.
    intros. apply Val.mul_commut.
    intros [rs'' [EX' [RES' OTH']]].
    exists rs''; split.
    eapply exec_straight_trans. eexact EX. eexact EX'.
    rewrite RES'. rewrite RES. rewrite (OTH r2); eauto with asmgen.
    split. reflexivity. intros. rewrite OTH'; auto. apply OTH; eauto with asmgen.
Qed.
...
Remark ptr_conversion_correct:
  forall a sp ofs,
    a > 0 ->
    Val.offset_ptr (Val.offset_ptr sp (Ptrofs.repr (align ofs a))) (Ptrofs.repr a) = Val.offset_ptr sp (Ptrofs.repr (align ((align ofs a) + a) a)).
Proof.
  intros.
...
Lemma exec_lmw_correct:
  forall l l' rs1 sp ofs m rs2 rs1' l'',
    load_stack_rpairs m sp ofs l rs1 = Some rs2 ->
    mmap (@error_single mreg) l = OK l' ->
    mmap ireg_of l' = OK l'' ->
    agree rs1 sp rs1' ->
    exists rs2',
    exec_lmw (Val.offset_ptr sp (Ptrofs.repr (align ofs 4))) l'' rs1' m = Next (nextinstr rs2') m
    /\ (forall r, (data_preg r = false) -> (rs1' r) = (rs2' r))
    /\ agree rs2 sp rs2'.
Proof.
  induction l as [|r l]; simpl; intros until l''; intros LOAD ERR MAP AG.
  - exists rs1'. inv ERR. inv MAP. inv LOAD. split; auto.
  - monadInv ERR. simpl in MAP. monadInv MAP.
    assert (TY: (mreg_type x) = Tany32).
    { unfold mreg_type. destruct x; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
    unfold Bounds.compute_size in LOAD. ErrorSingle. rewrite TY in LOAD. unfold load_stack in LOAD. simpl in LOAD.
    rewrite <- (ireg_of_eq _ _ EQ0). destruct (Mem.loadv); try discriminate.
    rewrite ptr_conversion_correct by lia.
    exploit IHl; eauto. eapply agree_set_mreg_parallel; eauto. intros (rs2' & A & B & C).
    econstructor. split; eauto. split; auto.
    intros. rewrite <- B. destruct (preg_eq (preg_of x) r0). subst r0.
    rewrite preg_of_data in H. discriminate.
    rewrite Pregmap.gso; eauto. apply H.
Qed.

Lemma exec_stmw_correct:
  forall l l' (rs1: Mach.regset) (rs1': regset) sp ofs m1 m2 m1' l'',
    ...
    Mem.extends m1 m1' ->
    exists m2',
    exec_stmw (Val.offset_ptr sp (Ptrofs.repr (align ofs 4))) l'' rs1' m1' = Next (nextinstr rs1') m2'
    /\ Mem.extends m2 m2'.
  Proof.
  induction l as [|r l]; simpl; intros until l''; intros STORE ERR MAP AG MEXT.
  - exists m1'. inv ERR. inv MAP. inv STORE. split; auto.
  - monadInv ERR. simpl in MAP. monadInv MAP.
    assert (TY: (mreg_type x) = Tany32).
    { unfold mreg_type. destruct x; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
    unfold Bounds.compute_size in STORE. ErrorSingle. rewrite TY in STORE. unfold store_stack in STORE. simpl in STORE.
    rewrite <- (ireg_of_eq _ _ EQ0). Destructor; try discriminate.
    rewrite ptr_conversion_correct by lia. exploit Mem.storev_extends. eauto. eauto. eauto. inv AG; auto. intros (m' & A & B).
    exploit IHl; eauto. intros (m2' & C & D).
    econstructor. rewrite A. split. apply C. auto.
Qed.
...
Opaque Int.eq.
 ...
 -  (* Ccomp *)
  ...
  - (* Ccompimm *)
    ...
    + econstructor; split.
      destruct (Int.eq (Int.shru i (Int.repr 5)) Int.zero); [destruct x|]; inv EQ0;
      econstructor; simpl; reflexivity.
      unfold compare_sint.
      rewrite <- Val.negate_cmp.
      destruct c0; intuition Simpl.
...
Proof.
  ...
  destruct (classify_condition cond args); intros; monadInv H; simpl;
    try rewrite (ireg_of_eq _ _ EQ); try rewrite (ireg_of_eq _ _ EQ1).
- (* eq 0 *)
  econstructor; split.
  eapply exec_straight_two; simpl; reflexivity.
...
- (* lt 0 *)
  ...
  intros; Simpl.
- (* le n *)
  set (cond := Ccompimm Clt (Int.add n Int.one)) in *.
  destruct (transl_cond_correct_1 cond (r :: nil) (Pmfcrbit x CRbit_0 :: k) rs m c EQ0) as [rs1 [EX1 [RES1 AG1]]].
  rewrite Val.cmp_bool_le_lt; auto.
  econstructor; split. eapply exec_straight_trans. eexact EX1.
  eapply exec_straight_one; simpl; reflexivity. simpl in RES1.
  rewrite RES1. intuition Simpl.
- (* leu n*)
  set (cond := Ccompuimm Clt (Int.add n Int.one)) in *.
  destruct (transl_cond_correct_1 cond (r :: nil) (Pmfcrbit x CRbit_0 :: k) rs m c EQ0) as [rs1 [EX1 [RES1 AG1]]].
  rewrite Val.cmpu_bool_le_lt; auto.
  econstructor; split. eapply exec_straight_trans. eexact EX1.
  eapply exec_straight_one; simpl; reflexivity. simpl in RES1.
  rewrite RES1. intuition Simpl.
- (* ge n *)
  set (cond := Ccompimm Cgt (Int.sub n Int.one)) in *.
  destruct (transl_cond_correct_1 cond (r :: nil) (Pmfcrbit x CRbit_1 :: k) rs m c EQ0) as [rs1 [EX1 [RES1 AG1]]].
  rewrite Val.cmp_bool_ge_gt; auto.
  econstructor; split. eapply exec_straight_trans. eexact EX1.
  eapply exec_straight_one; simpl; reflexivity. simpl in RES1.
  rewrite RES1. intuition Simpl.
- (* geu n*)
  set (cond := Ccompuimm Cgt (Int.sub n Int.one)) in *.
  destruct (transl_cond_correct_1 cond (r :: nil) (Pmfcrbit x CRbit_1 :: k) rs m c EQ0) as [rs1 [EX1 [RES1 AG1]]].
  rewrite Val.cmpu_bool_ge_gt; eauto.
  econstructor; split. eapply exec_straight_trans. eexact EX1.
  eapply exec_straight_one; simpl; reflexivity. simpl in RES1.
  rewrite RES1. intuition Simpl.
- (* default *)
...
Lemma transl_op_correct:
  ...
  eval_operation ge (rs#GPR1) op (get_pairs preg_rpair_of ## args rs) m = Some v ->
  exists rs',
     exec_straight ge fn c rs m k rs' m
  /\ lessdef' v (preg_rpair_of res) rs'
  /\ forall r, important_preg r = true ->forall_rpair (fun x => r <> (preg_of x)) res -> preg_notin r (destroyed_by_op op) -> rs' r = rs r.
Proof.
...
-(* Omove *)
  ...
  destruct r; inv H0.
  destruct (preg_of r) eqn:ARG; inv H1.
  exploit (move_rr_base_correct i i0). intros [rs' [EX [RS OTH]]].
  exists rs'. split. eexact EX. split. Splitter. intros. simpl in H0.
...
- (* Oaddrsymbol *)
  ...
  + (* small data *)
    Opaque Val.add.
    econstructor; split. apply exec_straight_one; simpl; reflexivity. Splitter.
    subst v'. rewrite small_data_area_addressing by auto.
    auto. intuition Simpl.
  + (* relative data *)
    econstructor; split. eapply exec_straight_three; simpl; reflexivity.
    Splitter.
    rewrite Val.add_assoc. rewrite (Val.add_commut (low_half ge i i0)).
...
- (* Oaddrstack *)
  ...
  exists rs'; split. auto. repeat Splitter'.
  rewrite RES. destruct (rs GPR1); simpl; auto.
  Transparent Val.add.
  simpl. rewrite Ptrofs.of_int_to_int; auto.
- (* Ocast8signed *)
  destruct x0; try TranslOpSplitter.
  exploit (move_rr_correct s x). intros [rs' [EX [RS OTH]]].
  econstructor. split. eapply exec_straight_opt_right. eexact EX.
  apply exec_straight_one; simpl; reflexivity. Splitter. rewrite RS. auto.
  intuition Simpl. rewrite OTH; auto with asmgen.
  intros. rewrite H4 in H. inv H.
- (* Ocast16signed *)
  destruct x0; try TranslOpSplitter.
  exploit (move_rr_correct s x). intros [rs' [EX [RS OTH]]].
  econstructor. split. eapply exec_straight_opt_right. eexact EX.
  apply exec_straight_one; simpl; reflexivity. Splitter. rewrite RS. split; intros; Simpl.
  intuition Simpl. rewrite OTH; auto with asmgen. intros. rewrite H4 in H. inv H.
- (* Oadd *)
...
- (* Oaddl *)
  monadInv H.
  assert (x <> x0) by (destruct x, x0; try congruence; [destruct s,s0|destruct a,a0]; try congruence; inv Heqb).
  generalize Val.addl_hiword_lessdef.
  generalize Val.addl_loword_lessdef.
  intros.
  destruct (ireg_eq x0 x1 || ireg_eq x0 x3) eqn:?; inv EQ3.
  + set (rs1 := nextinstr (rs#GPR0 <- (Val.add rs#x2 rs#x4)
                            #CARRY <- (Val.add_carry rs#x2 rs#x4 Vzero))).
    set (rs2 := nextinstr (rs1#x <- (Val.add (Val.add rs1#x1 rs1#x3) rs1#CARRY)
                              #CARRY <- (Val.add_carry rs1#x1 rs1#x3 rs1#CARRY))).
    generalize (move_rr_correct x0 GPR0 k rs2 m). intros [rs' [A B]].
    exists rs'; split.
    eapply exec_straight_step; try reflexivity.
    eapply exec_straight_opt_left. 2: eexact A.
    eapply exec_straight_one; try reflexivity.
    repeat Splitter'; [rewrite H15| rewrite H14|]; eauto with asmgen;
    unfold rs2, rs1; Simpl.
    intros. destruct H17. rewrite H15; eauto with asmgen.
    unfold rs2, rs1; Simpl.
  + rewrite orb_false_iff in Heqb1. destruct Heqb1.
    assert (x0 <> x1).
    { destruct x0, x1; try congruence. destruct s, s0; try congruence; inv H2.
      destruct a, a0; try congruence; inv H2.
    }
    assert (x0 <> x3).
    { destruct x0, x3; try congruence. destruct s, s0; try congruence; inv H3.
      destruct a, a0; try congruence; inv H3.
    }
...
- (* Oaddsymbol *)
  ...
  + (* small data *)
    econstructor; split. eapply exec_straight_two; simpl; reflexivity.
    Splitter.
    rewrite small_data_area_addressing by auto. rewrite Val.add_commut. reflexivity.
    intuition Simpl.
  + (* relative data *)
    econstructor; split.
  ...
  + (* absolute data *)
    ...
    intuition Simpl.
- (* Osub *)
  destruct x1, x, x0; try TranslOpSplitter.
  destruct (sreg_eq s s0); [rewrite e|]; try TranslOpSplitter.
  destruct (sreg_eq s s1); [rewrite e|]; TranslOpSplitter.
- (* Osubl *)
  monadInv H.
  assert (x <> x0) by (destruct x, x0; try congruence; [destruct s,s0|destruct a,a0]; try congruence; inv Heqb).
  generalize Val.subl'_hiword_lessdef.
  generalize Val.subl_loword_lessdef.
  intros.
  destruct (ireg_eq x0 x1 || ireg_eq x0 x3) eqn:?; inv EQ3.
  + set (rs1 := nextinstr (rs#GPR0 <- (Val.sub rs#x2 rs#x4)
                            #CARRY <- (Val.add_carry rs#x2 (Val.notint rs#x4) Vone))).
    set (rs2 := nextinstr (rs1#x <- (Val.add (Val.add rs1#x1 (Val.notint rs1#x3)) rs1#CARRY)
                              #CARRY <- (Val.add_carry rs1#x1 (Val.notint rs1#x3) rs1#CARRY))).
    generalize (move_rr_correct x0 GPR0 k rs2 m). intros [rs' [A B]].
    exists rs'; split.
    eapply exec_straight_step; try reflexivity.
    eapply exec_straight_opt_left. 2: eexact A.
...
- (* Osubimm *)
  ...
  Splitter.
  rewrite RES. rewrite OTH; eauto with asmgen.
  intuition Simpl. rewrite OTH; eauto with asmgen.
  intros. rewrite H4 in H. inv H. intros. rewrite H4 in H. inv H.
- (* Omull *)
  destruct (select_encoding_instr_correct Pse_mullw Pmullw Val.mul true x1 x x0 k rs m) as [rs' [EX [RES OTH]]]; eauto with asmgen.
  intros; apply Val.mul_commut.
  exists rs'. Splitter.
- (* Omullimm *)
  destruct (mulimm_correct x0 x i k rs m) as [rs' [A [B C]]]; eauto with asmgen.
  exists rs'; Splitter.
- (* Onegl *)
  monadInv H.
  assert (x <> x0) by (destruct x, x0; try congruence; [destruct s,s0|destruct a,a0]; try congruence; inv Heqb).
  destruct (ireg_eq x0 x1) eqn:?; inv EQ2.
  + set (rs1 := nextinstr (rs#GPR0 <- (Val.sub (Vint Int.zero) rs#x2)
                            #CARRY <- (Val.add_carry (Vint Int.zero) (Val.notint rs#x2 ) Vone))).
    set (rs2 := nextinstr (rs1#x <- (Val.add (Val.notint rs1#x1) rs1#CARRY))).
    generalize (move_rr_correct x1 GPR0 k rs2 m). intros [rs' [A B]].
    exists rs'; split.
    eapply exec_straight_step; try reflexivity.
    eapply exec_straight_opt_left. 2: eexact A.
    eapply exec_straight_one; try reflexivity. destruct B.
    repeat Splitter'; [rewrite H1| rewrite H0|]; eauto with asmgen;
    unfold rs2, rs1; Simpl.
    destruct (rs x1), (rs x2); auto. simpl.
    rewrite Int64.decompose_neg', Int64.hi_ofwords. auto.
    destruct (rs x1), (rs x2); auto. simpl.
    rewrite Int64.decompose_neg', Int64.lo_ofwords. auto.
    intros.  destruct H11. rewrite H1; eauto with asmgen.
    unfold rs2, rs1; Simpl.
  + econstructor. split. eapply exec_straight_two; simpl; eauto.
    repeat Splitter'.
    destruct (rs x1), (rs x2); auto. simpl.
...
- (* Oand *)
  ...
  split.
  rewrite D; auto with asmgen. unfold rs1; Simpl. intros. rewrite D; auto with asmgen.
  unfold rs1; Simpl.
- (* Oandimm *)
...
- (* Oshr *)
  unfold select_encoding_instr.
  destruct x1, x, x0; try TranslOpSplitter.
  destruct (sreg_eq s s0); try TranslOpSplitter. congruence.
- (* Oshrimm *)
...
- (* Oshrximm *)
  ...
  apply exec_straight_one; simpl; reflexivity.
  intuition Simpl. Splitter. rewrite B, C. erewrite Val.shrx_carry; eauto.
- (* Oshru *)
...
- (* Ocmp *)
  ErrorSingle.
  destruct (transl_cond_op_correct c0 x0 x k rs m c) as [rs' [A [B C]]]; auto.
  exists rs'; auto with asmgen. split; auto. ErrorSingle.
- (* Osel *)
  ...
  destruct res; inv EQ0. destruct (preg_of r1) eqn:RES; monadInv H0.
 destruct (transl_select_op_correct _ _ t _ _ _ _ rs m _ EQ4) as (rs' & A & B & C); eauto.
 destruct r; inv EQ0. eapply X; eauto.
 destruct r0; inv EQ2. eapply X; eauto.
 econstructor. split. eexact A. repeat Splitter'. ErrorSingle.
Qed.
...
Proof.
  ...
  - (* Mint8signed *)
    ...
    destruct (zle (Ptrofs.unsigned i + 1) Ptrofs.modulus); try discriminate.
...
Lemma compute_ireg_list_correct:
  forall l r,
    (preg_of ## (r::l)) = (IR ## (compute_ireg_list (preg_of r))) ->
    mmap ireg_of (r::l) = OK (compute_ireg_list (preg_of r)).
Proof.
  induction l as [| r1 l]; intros.
  - destruct r; try discriminate.
    simpl. f_equal.
  - simpl in H.
    specialize IHl with r1.
    simpl. simpl in IHl. 
    destruct r; rewrite IHl; try discriminate; simpl; f_equal; simpl in H; injection H;
      intros; rewrite H1; unfold compute_ireg_list; try rewrite H0; try reflexivity.
Qed.
