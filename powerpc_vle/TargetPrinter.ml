...
module type SYSTEM =
    sig
      ...
      val cfi_adjust: out_channel -> Integers.Ptrofs.int -> unit
      val cfi_rel_offset: out_channel -> string -> Integers.Ptrofs.int -> unit
...
module Diab_System : SYSTEM =
  struct
    ...
    let print_prologue oc =
      fprintf oc "	.xopt	align-fill-text=0x44004400\n";
  ...
  struct
    ...
    let print_instruction oc fallthrough = function
      ...
      | Pdcbi (r1,r2) ->
          fprintf oc "	dcbi	%a, %a\n" ireg r1 ireg r2
      | Pdcbt (c,r1,r2) ->
          fprintf oc "	dcbt	%a, %a, %a\n" coqint c ireg r1  ireg r2
      | Pdcbtst (c,r1,r2) ->
          fprintf oc "	dcbtst	%a, %a, %a\n" coqint c ireg r1 ireg r2
      | Pdcbtls (c,r1,r2) ->
          fprintf oc "	dcbtls	%a, %a, %a\n" coqint c ireg r1 ireg r2
      | Pdcbz (r1,r2) ->
          fprintf oc "	dcbz	%a, %a\n" ireg r1 ireg r2
      | Pdivw(r1, r2, r3) ->
      ...
      | Picbi (r1,r2) ->
          fprintf oc "	icbi	%a, %a\n" ireg r1 ireg r2
      | Picbtls (n,r1,r2) ->
          fprintf oc "	icbtls	%a, %a, %a\n" coqint n ireg r1 ireg r2
      | Pse_isync ->
          fprintf oc "	se_isync\n"
      | Plbzx(r1, r2, r3) ->
      ...
      | Pmfspr(rd, spr) ->
          fprintf oc "	mfspr	%a, %a\n" ireg rd coqint spr
      | Pmtspr(spr, rs) ->
          fprintf oc "	mtspr	%a, %a\n" coqint spr ireg rs
      | Pmr(r1, r2) ->
          fprintf oc "	mr	%a, %a\n" ireg r1 ireg r2
      | Pmullw(r1, r2, r3) ->
      ...
      | Pe_mulli (r1, r2 , imm) ->
          fprintf oc "	e_mulli	%a, %a, %a\n" ireg r1 ireg r2 coqint imm
      | Pe_mull2i (r , imm) ->
          fprintf oc "	e_mull2i	%a, %a\n" ireg r coqint imm
      | Pmulhw(r1, r2, r3) ->
      ...
      | Psrawi(r1, r2, n) ->
          fprintf oc "	srawi	%a, %a, %a\n" ireg r1 ireg r2 coqint n
      | Psrw(r1, r2, r3) ->
      ...
      | Pse_mfar (rd, r1) ->
        fprintf oc "	se_mfar	%a, %a\n" sreg rd areg r1
      | Pe_andi_ (rd, r1, imm) ->
          fprintf oc "	e_andi.	%a, %a, %a\n" ireg rd ireg r1 coqint imm
      | Pe_and2i_ (rd, c) ->
      ...
      | Pe_addi (r1, r2, imm) ->
          fprintf oc "	e_addi	%a, %a, %a\n" ireg r1 ireg r2 coqint imm
      | Pe_add2is (rd, cst) ->
          fprintf oc "	e_add2is	%a, %a\n" ireg rd constant cst
      | Pse_addi (r1, imm) ->
          fprintf oc "	se_addi	%a, %a\n" sreg r1 coqint imm
      | Pe_addic(r1, r2, c) ->
          fprintf oc "	e_addic	%a, %a, %a\n" ireg r1 ireg r2 coqint c
      | Pe_b lbl ->
          fprintf oc "	e_b	%a\n" label (transl_label lbl)
      | Pse_bctr sg ->
          fprintf oc "	se_bctr\n"
      ...
      | Pse_bgeni (rd, c) ->
          fprintf oc "	se_bgeni	%a, %a\n" sreg rd coqint c
      | Pse_li (rd, c) ->
          fprintf oc "	se_li	%a, %a\n" sreg rd coqint c
      | Pe_li (rd, c) ->
          fprintf oc "	e_li	%a, %a\n" ireg rd constant c
      | Pe_lis (rd, c) ->
      ...
      | Pe_lmw(rt, ra, d) ->
          fprintf oc "	e_lmw	%a, %a(%a)\n" ireg rt constant d ireg ra
      ...
      | Pe_ori (rd, r1, imm) ->
          fprintf oc "	e_ori	%a, %a, %a\n" ireg rd ireg r1 coqint imm
      | Pe_or2is (rd, c) ->
           fprintf oc "	e_or2is	%a, %a\n" ireg rd constant c
      | Pe_rlwinm(r1, r2, c1, c2) ->
          let (mb, me) = rolm_mask (camlint_of_coqint c2) in
          fprintf oc "	e_rlwinm	%a, %a, %a, %d, %d %s 0x%lx\n"
            ireg r1 ireg r2 coqint c1 mb me
            comment (camlint_of_coqint c2)
      | Pe_rlwimi(r1, r2, c1, c2) ->
          let (mb, me) = rolm_mask (camlint_of_coqint c2) in
          fprintf oc "	e_rlwimi	%a, %a, %a, %d, %d %s 0x%lx\n"
            ireg r1 ireg r2 coqint c1 mb me
            comment (camlint_of_coqint c2)
      | Pse_srawi (rd, imm) ->
          fprintf oc "	se_srawi	%a, %a\n" sreg rd coqint imm
      | Pe_stb (r1, c, r2) ->
      ...
      | Pe_stmw(rs, ra, d) ->
          fprintf oc "	e_stmw	%a, %a(%a)\n" ireg rs constant d ireg ra
      ...
      | Pe_subfic(r1, r2, c) ->
          fprintf oc "	e_subfic	%a, %a, %a\n" ireg r1 ireg r2 coqint c
      | Pe_xori(r1, r2, c) ->
          fprintf oc "	e_xori	%a, %a, %a\n" ireg r1 ireg r2 coqint c
      | Pefsabs (r1, r2) ->
