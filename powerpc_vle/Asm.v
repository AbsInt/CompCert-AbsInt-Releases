...
Inductive preg: Type :=
  ...
  | ErrorReg: preg                      (**r float registers *)
...
Module IndexedPreg <: INDEXED_TYPE.
  ...
  :: IR GPR28 :: IR GPR29 :: IR GPR30 :: IR GPR31 :: ErrorReg :: PC :: LR :: CTR
  :: CARRY :: CR0_0 :: CR0_1 :: CR0_2 :: CR0_3 :: CR1_1 :: CR1_2 :: nil.

  Lemma all_pregs_complete:
    forall (r: preg), In r all_pregs.
  Proof.
    assert (forall r, proj_sumbool (In_dec preg_eq r all_pregs) = true).
    { intros. case r; try intro r'; try case r'; try intro r''; try destruct r''; reflexivity. }
  ...
  Definition index (r: preg): positive :=
    ...
    | GPR30 => 31 | GPR31 => 32 | ErrorReg => 33 | PC => 34 | LR => 35
    | CTR => 36 | CARRY => 37 | CR0_0 => 38 | CR0_1 => 39 | CR0_2 => 40
    | CR0_3 => 41 | CR1_1 => 42 | CR1_2 => 43
    end.

  Local Open Scope positive_scope.
  Definition reverse_ireg_index (p: positive) : ireg :=
    match p with
...
Inductive instruction : Type :=
  ...
  | Pe_addi : ireg -> ireg -> int -> instruction              (**r add immediate *)
  ...
  | Pe_mulli : ireg -> ireg -> int -> instruction             (**r integer multiply immediate *)
  | Pe_mull2i : ireg -> int -> instruction                    (**r integer multiply immediate *)
  ...
  | Pe_andi_: ireg -> ireg -> int -> instruction              (**r and immediate and set conditions *)
  ...
  | Pe_ori : ireg -> ireg -> int -> instruction               (**r or immediate *)
  ...
  | Pe_lmw: ireg -> ireg -> constant -> instruction           (**r load multiple 32-bit int *)                              
  ...
  | Pe_stmw: ireg -> ireg -> constant -> instruction          (**r store multiple 32-bit int *)
...
Definition compute_ireg_list (r : preg) :=
  Pos.peano_rect (fun _ => list ireg) (Sreg GPR31 :: nil)
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pe_mulli rd r1 imm =>
      Next (nextinstr (rs#rd <- (Val.mul rs#r1 (Vint imm)))) m
  | Pe_mull2i rd imm =>
  ...
  | Pe_ori rd r1 imm =>
      Next (nextinstr (rs#rd <- (Val.or rs#r1 (Vint imm)))) m
  | Pe_or2i rd cst =>
