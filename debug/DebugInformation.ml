...
(* Simple id generator *)
(*- E_COMPCERT_CODE_DebugInformation_id_001 *)
(*- #Justify_Derived "Local state for generator of unique debug IDs" *)
let id = ref 0
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_next_id_001 *)
(*- #Justify_Derived "Accessor function for next unique debug ID" *)
let next_id () =
  let nid = !id in
  assert (nid >= 0);
  incr id; nid
...
(*- E_COMPCERT_CODE_DebugInformation_list_replace_001 *)
...
(*- E_COMPCERT_CODE_DebugInformation_strip_attributes_001 *)
(*- #Justify_Derived "Utility function" *)
let strip_attributes typ = strip_attributes_type typ [AConst; AVolatile]
(*- #End *)

(* Find the type id to an type *)
(*- E_COMPCERT_CODE_DebugInformation_find_type_001 *)
(*- #Justify_Derived "Utility function" *)
let find_type ty =
...
(*- E_COMPCERT_CODE_DebugInformation_insert_type_001 *)
(*- #Justify_Derived "Utility function" *)
let insert_type ty =
  let insert d_ty ty =
    let id = next_id ()
          ...
          | TFun (t,param,_,_) ->
              let param,prot =
                begin match param with
                  | None -> [],false
                  | Some p -> (List.map (fun (i,t) -> attr_aux t, i.name) p, true)
                end in
  ...
  and attr_aux ty =
    ...
    with
      Not_found ->
        ...
        | Some AConst,t ->
            let id = attr_aux t in
            insert (ConstType id) ty
        | Some AVolatile,t ->
            let id = attr_aux t in
            insert (VolatileType id) ty
...
(*- E_COMPCERT_CODE_DebugInformation_replace_composite_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_replace_enum_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_replace_typedef_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_find_fun_stamp_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_find_fun_atom_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_replace_var_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_replace_fun_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_replace_lvar_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_find_scope_id_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_replace_scope_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_gen_comp_typ_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_remove_unused_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_remove_unused_function_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_insert_global_declaration_001 *)
(*- #Justify_Derived "Utility function" *)
let insert_global_declaration env dec =
  ...
  | Genumdef (n,at,e) ->
      ...
      let enumerator = List.map (fun (i,c,_) -> (i.name,c)) e in
      replace_enum id (fun en ->
        {en with enum_file_loc = Some dec.gloc; enum_enumerators = enumerator;})
  | Gpragma _ -> ()
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_StringMap_001 *)
(*- #Justify_Derived "Type definition" *)
module StringMap = Map.Make(String)
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_set_members_offset_001 *)
(*- #Justify_Derived "Utility function" *)
let set_members_offset ~str_id ofs =
  let id = Hashtbl.find atom_to_definition str_id in
  let ofs = List.fold_left (fun m (id, byte_ofs, bits) ->
      StringMap.add (extern_atom id) (byte_ofs, bits) m) StringMap.empty ofs in
  replace_composite id (fun comp ->
      let members = List.map (fun m ->
          try
            let (byte_ofs, bits) = StringMap.find m.cfd_name ofs in
            let byte_ofs = Z.to_int64 byte_ofs in
            match bits with
            | Ctypes.Full -> {m with cfd_byte_offset = Some byte_ofs }
            | Ctypes.Bits (sz, sg, bit_pos, width) ->
                  let bit_pos = Z.to_int64 bit_pos in
                  let sz = Z.to_int64 (Ctypes.bitalignof_intsize sz) in
                  let bit_pos = if not !Machine.config.Machine.bitfields_msb_first then
                      Int64.(sub (sub sz bit_pos) (Z.to_int64 width))
                    else
                      bit_pos in
                  let size = Int64.div sz 8L in
                  { m with cfd_bit_offset = Some bit_pos; cfd_byte_size = Some size; cfd_byte_offset = Some byte_ofs }
            with Not_found -> m
        ) comp.ct_members in
      {comp with ct_members = members})
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_set_composite_size_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_atom_global_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_atom_parameter_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_add_fun_addr_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_atom_local_variable_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_add_lvar_scope_001 *)
(*- #Justify_Derived "Utility function" *)
let add_lvar_scope f_id var_id s_id =
  try
    let s_id',scope = find_scope_id f_id s_id in
    let var_id,_ = find_lvar_stamp var_id.stamp in
    replace_scope s_id' (var_id::scope)
  with Not_found -> ()
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_insert_local_declaration_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_new_scope_001 *)
(*- #Justify_Derived "Utility function" *)
let new_scope f_id sc_id =
  let id = next_id () in
  Hashtbl.add local_variables id (Scope []);
  Hashtbl.add scope_to_local (f_id,sc_id) id;
  id
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_enter_function_scope_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_close_scope_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_start_live_range_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_end_live_range_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_stack_variable_001 *)
(*- #Justify_Derived "Utility function" *)
let stack_variable (f,v) (sp,loc) =
  Hashtbl.add var_locations (f,v) (FunctionLoc (sp,loc))
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_compilation_section_start_001 *)
(*- #Justify_Derived "Variable for local state" *)
let compilation_section_start: (string,int) Hashtbl.t = Hashtbl.create 7
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_compilation_section_end_001 *)
(*- #Justify_Derived "Variable for local state" *)
let compilation_section_end: (string,int) Hashtbl.t = Hashtbl.create 7
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_section_start_001 *)
(*- #Justify_Derived "Utility function" *)
let section_start n = Hashtbl.find compilation_section_start n
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_fold_section_start_001 *)
(*- #Justify_Derived "Utility function" *)
let fold_section_start f a =
  List.fold_left (fun accu (sec, low) -> f sec low accu) a
    (List.fast_sort (fun (s1, _) (s2, _) -> String.compare s1 s2)
       (Hashtbl.fold (fun sec low accu -> (sec, low) :: accu) compilation_section_start []))
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_section_end_001 *)
(*- #Justify_Derived "Utility function" *)
let section_end n = Hashtbl.find compilation_section_end n
(*- #End *)

(*- E_COMPCERT_DEACTIVATED_CODE_DebugInformation_diab_001 *)
(*- #Condition "CompCert with Diab Compiler as hosting toolchain" *)
(*- #Justification "Code is unreachable for CompCert with GCC or LLVM based toolchains" *)

(*- E_COMPCERT_CODE_DebugInformation_diab_additional_001 *)
(*- #Justify_Derived "Variable for local state" *)
let diab_additional: (string,int * int * section_name) Hashtbl.t = Hashtbl.create 7
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_diab_additional_section_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_section_to_string_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_DEACTIVATED_CODE_DebugInformation_diab_002 *)
(*- #Condition "CompCert with Diab Compiler as hosting toolchain" *)
(*- #Justification "Code is unreachable for CompCert with GCC or LLVM based toolchains" *)

(*- E_COMPCERT_CODE_DebugInformation_add_diab_info_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_diab_add_fun_addr_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_gnu_add_fun_addr_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_exists_section_001 *)
(*- #Justify_Derived "Utility function" *)
let exists_section sec =
  Hashtbl.mem compilation_section_start (section_to_string sec)
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_filenum_001 *)
(*- #Justify_Derived "Variable for local state" *)
let filenum: (string * string,int) Hashtbl.t = Hashtbl.create 7
(*- #End *)

(*- E_COMPCERT_DEACTIVATED_CODE_DebugInformation_diab_003 *)
(*- #Condition "CompCert with Diab Compiler as hosting toolchain" *)
(*- #Justification "Code is unreachable for CompCert with GCC or LLVM based toolchains" *)

(*- E_COMPCERT_CODE_DebugInformation_diab_file_loc_001 *)
(*- #Justify_Derived "Utility function" *)
let diab_file_loc f l = Hashtbl.find filenum (f,l)
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_compute_diab_file_enum_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_DebugInformation_compute_gnu_file_enum_001 *)
(*- #Justify_Derived "Utility function" *)
let compute_gnu_file_enum f =
  StringSet.iter f !all_files
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_printed_symbols_001 *)
(*- #Justify_Derived "Variable for local state" *)
let printed_symbols: StringSet.t ref = ref StringSet.empty
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_is_symbol_printed_001 *)
(*- #Justify_Derived "Utility function" *)
let is_symbol_printed id = StringSet.mem id !printed_symbols
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_symbol_printed_001 *)
(*- #Justify_Derived "Utility function" *)
let symbol_printed id =
  printed_symbols := StringSet.add id !printed_symbols
(*- #End *)

(*- E_COMPCERT_CODE_DebugInformation_init_001 *)
(*- #Justify_Derived "Utility function" *)
let init name =
  id := 0;
  file_name := name;
...
(*- E_COMPCERT_CODE_DebugInformation_default_debug_001 *)
...
let default_debug =
  {
   ...
   set_members_offset = set_members_offset;
