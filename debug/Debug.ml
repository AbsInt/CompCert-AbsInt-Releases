...
(*- E_COMPCERT_CODE_Debug_implem_001 *)
(*- #Justify_Derived "Type definition" *)
type implem =
    {
      init: string -> unit;
      atom_global: ident -> atom -> unit;
      set_composite_size: ident -> atom -> struct_or_union -> int64 option -> unit;
      set_members_offset: str_id:atom -> (atom * Z.t * Ctypes.bitfield) list  -> unit;
      ...
      start_live_range: (atom * atom) -> positive -> int * (int * (int  * int) option) AST.builtin_arg -> unit;
      end_live_range: (atom * atom) -> positive -> unit;
      stack_variable: (atom * atom) -> int * (int * (int * int) option) AST.builtin_arg -> unit;
...
(*- E_COMPCERT_CODE_Debug_default_implem_001 *)
(*- #Justify_Derived "Default implementation for empty debug information" *)
let default_implem =
  {
   init = (fun _ -> ());
   atom_global = (fun _ _ -> ());
   set_composite_size = (fun _ _ _ _ -> ());
   set_members_offset = (fun ~str_id _ -> ());
...
(*- E_COMPCERT_CODE_Debug_implem_002 *)
(*- #Justify_Derived "Global state representing currently selected implementation of debug information" *)
let implem = ref default_implem
(*- #End *)

(*- E_COMPCERT_CODE_Debug_implem_accessors_001 *)
(*- #Justify_Derived "Utility accessor functions for selected implementation of debug information" *)
...
let set_members_offset ~str_id ofs = !implem.set_members_offset ~str_id:str_id ofs
