...
let f_set_char_signedness name sgn =
  [Exact("-f" ^ name), Unit (fun () -> Machine.config := {!Machine.config with char_signed = sgn});
   Exact("-fno-" ^ name), Unit (fun () -> Machine.config := {!Machine.config with char_signed = not sgn})]
(*- #End *)
...
let f_set_size_t name typ_name ikind =
  [Exact("-f" ^ name), Unit (fun () ->
       ...
       Machine.config := {!Machine.config with size_t_ikind = ikind})]
(*- #End *)
...
let f_set_ptrdiff_t name typ_name ikind =
  [Exact("-f" ^ name), Unit (fun () ->
       ...
       Machine.config := {!Machine.config with ptrdiff_t_ikind = ikind})]
(*- #End *)
