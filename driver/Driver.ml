...
(* BEGIN:untraced *)
(*- #Untraced E_COMPCERT_TR_Function_ENVIRONMENT_001 *)
(*- #Justify_Untraced "CompCert implements no specific resource limits. Its capacity is limited only by the computation time and available system space." *)
(* END:untraced *)

(* Name used for version string etc. *)
(*- E_COMPCERT_CODE_Driver_tool_name_001 *)
(*- #Justify_Derived "Utility constant" *)
let tool_name = "C verified compiler"
(*- #End *)

(* Optional sdump suffix *)
(*- E_COMPCERT_CODE_Driver_sdump_suffix_001 *)
(*- #Justify_Derived "Variable for global state" *)
let sdump_suffix = ref ".json"
(*- #End *)

(*- E_COMPCERT_CODE_Driver_nolink_001 *)
(*- #Justify_Derived "Utility function" *)
let nolink () =
  !option_c || !option_S || !option_E || !option_interp
(*- #End *)

(*- E_COMPCERT_CODE_Driver_object_filename_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Driver_compile_c_file_001 *)
(*- #Justify_Derived "Utility function" *)
let compile_c_file sourcename ifile ofile =
  ...
  set_dest PrintTypedRTL.destination option_drtl ".rtl";
  ...
  let asm =
    ...
    | Errors.OK asm ->
        asm
    | Errors.Error msg ->
      let loc = loc_of_error sourcename msg in
      fatal_error loc "%a" print_error msg in
  (* Dump Asm in binary and JSON format *)
  AsmToJSON.print_if asm sourcename;
  (* Print Asm in text form *)
  let oc = open_out ofile in
  PrintAsm.print_program oc sourcename asm;
  close_out oc
(*- #End *)

(* From C source to asm *)

(*- E_COMPCERT_CODE_Driver_compile_i_file_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Driver_process_c_file_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Driver_process_i_file_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Driver_process_s_file_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Driver_process_h_file_001 *)
(*- #Link_to E_COMPCERT_TR_Robustness_DRIVER_011 *)
...
(*- E_COMPCERT_CODE_Driver_target_help_001 *)
(*- #Justify_Derived "Utility constant" *)
let target_help =
...
|} ^
(if Configuration.model <> "armv6t2" then
{|  -midiv         Enable integer division extension
|} else "")
else if Configuration.arch = "tricore" then
{|  -muse-bol      Fold low 16-bit part of addresses into load/store offset when possible
  -mno-use-bol     Do not fold low 16-bit part of addresses into load/store offset
|}
else if Configuration.arch = "riscV" then
{|  -mzicond       Use integer conditional operations extension
  -mno-zicond    Do use the integer conditional operations extension
|}
else
  ""
(*- #End *)

(*- E_COMPCERT_CODE_Driver_toolchain_help_001 *)
(*- #Justify_Derived "Utility constant" *)
let toolchain_help =
...
(*- E_COMPCERT_CODE_Driver_usage_string_001 *)
(*- #Justify_Derived "Utility constant" *)
let usage_string =
...
{|Optimization options: (use -fno-<opt> to turn off -f<opt>)
  ...
  -fextract-addr Optimize re-use of base addresses in load/store [on]
...
Code generation options: (use -fno-<opt> to turn off -f<opt>)
  ...
  -fpie / -fPIE  Generate position-independent executables [off]
...
{|  -finline-runtime Inline the compiler runtime library
|} else "")
  ^
 target_help ^
 toolchain_help ^
...
(*- E_COMPCERT_CODE_Driver_print_usage_and_exit_001 *)
(*- #Justify_Derived "Utility function" *)
let print_usage_and_exit () =
  printf "%s" usage_string; exit 0
(*- #End *)

(*- E_COMPCERT_CODE_Driver_dump_mnemonics_001 *)
(*- #Justify_Derived "Utility function" *)
...
(*- E_COMPCERT_CODE_Driver_optimization_options_001 *)
(*- #Justify_Derived "Utility constant" *)
...
(*- E_COMPCERT_CODE_Driver_all_opts_setters_001 *)
(*- #Justify_Derived "Utility function" *)
let set_all opts () = List.iter (fun r -> r := true) opts
let unset_all opts () = List.iter (fun r -> r := false) opts
(*- #End *)

(*- E_COMPCERT_CODE_Driver_file_counters_001 *)
...
(*- E_COMPCERT_CODE_Driver_cmdline_actions_001 *)
...
let cmdline_actions =
  ...
  let check_align n =
    if n <= 0 || ((n land (n - 1)) <> 0) then
      (*- #Link_to E_COMPCERT_TR_Robustness_DRIVER_015 *)
      error no_loc "requested alignment %d is not a power of 2" n
    in
  let set_pic_mode () =
    (*- #Link_to E_COMPCERT_TR_Function_DRIVER_018 *)
...
(*- E_COMPCERT_DEACTIVATED_CODE_driver_001 *)
(*- #Condition "CompCert target is other than ARM." *)
(*- #Justification "Code is active only when CompCert is configured for an ARM target architecture." *)
  (if Configuration.arch = "arm" then
    (f_opt "inline-runtime" option_inline_runtime) @
    (if Configuration.model = "armv6" then
      ...
      (if Configuration.model <> "armv6t2" then
        [Exact "-midiv", Set option_midiv;]
       else []))
(*- #End_DEACTIVATED_CODE *)

(*- E_COMPCERT_DEACTIVATED_CODE_driver_002 *)
(*- #Condition "CompCert target is other than TriCore." *)
(*- #Justification "Code is active only when CompCert is configured for a TriCore target architecture." *)
   else if Configuration.arch = "tricore" then
     [ Exact "-muse-bol", Set option_muse_bol;
       Exact "-mno-use-bol", Unset option_muse_bol;]
   else if Configuration.arch = "riscV" then
     [ Exact "-mzicond", Set option_mzicond;
       Exact "-mno-zicond", Unset option_mzicond;]
   else []) @
(*- #End_DEACTIVATED_CODE *)
...
(* -f options: come in -f and -fno- variants *)
  ...
  (*- #Link_to E_COMPCERT_TR_Function_IFCONV_001 *)
  @ f_opt "if-conversion" option_fifconversion
  @ f_opt "const-prop" option_fconstprop
  @ f_opt "extract-addr" option_fextractaddr
...
(* Catch options that are not handled *)
  (*- #Link_to E_COMPCERT_TR_Robustness_DRIVER_014 *)
  Prefix "-", Self (fun s ->
...
(*- E_COMPCERT_CODE_Driver_main_001 *)
let _ =
  try
    Gc.set { (Gc.get()) with
