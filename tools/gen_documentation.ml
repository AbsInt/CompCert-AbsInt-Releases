...
let copy_and_rename new_platform_name file_dir = begin 
  ...
  Sys.chdir cwd;
  (* rename platform *)
  Sys.rename (Filename.concat file_dir "TargetPlatform") (Filename.concat file_dir new_platform_name)
end
...
let patch_file plat f = 
  ...
  (* Replace all ".TargetPlatform." with ".<platform>."*)
  let regx = Str.regexp {|\.TargetPlatform\.|} in 
...
let docs_for_platform new_platform_name = 
  ...
  (* Patch references to TargetPlatform to platform name *)
