...
(* a.d. TODO some configuration variables from the original shell script are not printed yet.
   Decide whether we need each one.

  COQDEVDIR=$(PREFIX)/lib/compcert/coq
  OCAML_NATIVE_COMP=true
  OCAML_OPT_COMP=true
  MENHIR_DIR=/home/absintrian/build/compcert_devel.ppc/usr/lib/ocaml/menhirLib
  CLIGHTGEN=true
  ...
  the last two are needed at least otherwise the Makefile build fails
 *)
...
(* This controls whether we generate default values for later use by the compcert-configedit program. *)
...
let _ = begin if !configedit_defaults then
  toolprefix := "%TOOLPATH%";
  libdir := "%LIBPATH%";
end
...
(* If the option has not been defined above and we are in manual mode or setting default values for compcert-configedit,
   ...
   Otherwise try deduce it from the configured assembler. *)
...
(* TODO Test Availability of Option '-no-pie' or '-nopie',  '-z nobtcfi' or '-Wl,-z,nobtcfi'*)
...
PIC_SUPPORTED=%B
" in
 sprintf s !abi_ref arch (Option.get !asm_supports_cfi) bitsize !casm !casm_options !casmruntime !cc !cc_options !clinker !clinker_options !cprepro !cprepro_options !archiver endianess !has_runtime_lib !has_standard_headers !has_double_ref !libmath model !system !responsefile !pic_supported
end
