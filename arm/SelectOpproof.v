...
Proof.
  ...
  - destruct (can_use_Aindexed2 chunk); simpl.
    ...
    exists (Vptr b ofs :: nil); split. constructor. EvalOp. simpl. congruence. constructor.
    simpl. rewrite Ptrofs.add_zero; auto.
  - destruct (Compopts.optim_extractaddr tt).
    + exists (Vptr b Ptrofs.zero :: nil).
      unfold Genv.symbol_address in *. destruct (Genv.find_symbol ge s) eqn:?; inv H0.
      split. constructor. EvalOp. simpl. unfold Genv.symbol_address. rewrite Heqo. reflexivity.
      constructor. simpl. rewrite Ptrofs.of_int_to_int; auto. rewrite Ptrofs.add_zero_l. reflexivity.
    + exists (Vptr b ofs :: nil); split. constructor. EvalOp. simpl. congruence. constructor.
      simpl. rewrite Ptrofs.add_zero; auto. congruence.
...
Proof.
  ...
  - (* BI_i64_udivl *)
    simpl in *.
    destruct Compopts.inlined_runtime; inv SEL.
    inv ARGS; try discriminate. inv H0; try discriminate. inv H2; inv SEM.
    exists v; split; auto.
    econstructor.  econstructor; eauto. econstructor; eauto.  econstructor.
    econstructor; eauto.
  - (* BI_dtob *)
    inv ARGS; try discriminate. inv H0; inv SEL.
    exists (Val.bitsoffloat v1); split. EvalOp.
    simpl in SEM. destruct v1; inv SEM.
    destruct (Binary.is_nan 53 1024 f) eqn:?; inv H1; auto.
Qed.
...
Proof.
  ...
  unfold lowlong; case (lowlong_match a); intros; inv H0.
  - InvEval. TrivialExists. inv H0. rewrite Val.loword_addl. auto.
  - InvEval. TrivialExists. inv H0. rewrite Val.loword_subl; auto.
  - inv H1. InvEval. TrivialExists. simpl. inv H6.
    destruct (eval_condition c vl m); auto. simpl. destruct b, v1, v0; auto.
  - InvEval. inv H0. TrivialExists. rewrite Val.loword_negl; auto.
Qed.
...
Proof.
  intros until v.
  assert (forall v a, eval_expr ge sp e m le a v -> eval_expr ge sp e m le (Eop Ohighlong (a ::: Enil)) (Val.hiwordoflong v)).
  {  intros. econstructor. econstructor; [|econstructor]. eexact H. auto. }
  unfold highlong; case (highlong_match a); intros; inv H0.
  inv H1. InvEval. TrivialExists. simpl. inv H6.
  destruct (eval_condition c vl m); auto. simpl. destruct b, v1, v0; auto.
Qed.
