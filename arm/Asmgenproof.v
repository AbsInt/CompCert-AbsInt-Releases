...
Remark transl_restorecallee_unroll_label:
  forall l ofs k c,
...
Proof.
  ...
  - monadInv H. eapply tail_nolabel_trans. eapply loadind_label. apply EQ0. eapply IHl. apply EQ.
Qed.

Remark transl_savecallee_unroll_label:
  forall l ofs k c,
...
Proof.
  induction l as [ | h l]; intros; simpl in H.
  - monadInv H. apply tail_nolabel_refl.
  - monadInv H. eapply tail_nolabel_trans. eapply storeind_label. apply EQ0. eapply IHl. apply EQ.
Qed.

Remark transl_restorecallee_label:
  forall l ofs k c ty,
  transl_restorecallee ofs l ty k = OK c ->
  tail_nolabel k c.
Proof.
  unfold transl_restorecallee. intros.
  destruct (length_sufficient l). destruct ty; try discriminate.
  TailNoLabel. eapply tail_nolabel_trans. apply addimm_label. TailNoLabel.
  eapply transl_restorecallee_unroll_label. apply H.
  TailNoLabel. eapply tail_nolabel_trans. apply addimm_label. TailNoLabel.
  eapply transl_restorecallee_unroll_label. apply H.
  eapply transl_restorecallee_unroll_label. apply H.
Qed.

Remark transl_savecallee_label:
  forall l ofs k ty c,
  transl_savecallee ofs l ty k = OK c ->
  tail_nolabel k c.
Proof.
  unfold transl_savecallee. intros.
  destruct (length_sufficient l). destruct ty; try discriminate.
  TailNoLabel. eapply tail_nolabel_trans. apply addimm_label. TailNoLabel.
  eapply transl_savecallee_unroll_label. apply H.
  TailNoLabel. eapply tail_nolabel_trans. apply addimm_label. TailNoLabel.
  eapply transl_savecallee_unroll_label. apply H.
  eapply transl_savecallee_unroll_label. apply H.
Qed.
...
Proof.
  ...
  destruct (split_by_type l); destruct (consecutive_exec_constraint l0 l1 (reg_list_offset l0 z));
    destruct l0; destruct l1; try discriminate; TailNoLabel; eapply tail_nolabel_trans;
  repeat match goal with
    | [ |- context[addimm _ _ _ _]] => apply addimm_label
    | [ |- tail_nolabel _ (Pldm _ _ :: _)] => TailNoLabel
    | _  => eapply transl_restorecallee_label; eauto
    end.
  destruct (split_by_type l); destruct (consecutive_exec_constraint l0 l1 (reg_list_offset l0 z));
    destruct l0; destruct l1; try discriminate; TailNoLabel; eapply tail_nolabel_trans;
    repeat match goal with
    | [ |- context[addimm _ _ _ _]] => apply addimm_label
    | [ |- tail_nolabel _ (Pstm _ _ :: _)] => TailNoLabel
    | _  => eapply transl_savecallee_label; eauto
    end.
...
- (* Mrestorecallee *)
  left. exploit load_stack_regs_extends; eauto. intros [rs2' [LOAD' AG']].
  eapply exec_straight_steps; eauto. intros. simpl in TR.
  rewrite (sp_val _ _ _ AG) in LOAD'.
  remember (split_by_type l). destruct p as [il fl].
  remember (consecutive_exec_constraint il fl (reg_list_offset il ofs)). destruct b.
  destruct il; destruct fl; unfold consecutive_exec_constraint in Heqb; symmetry in Heqb;
    repeat rewrite andb_false_r in Heqb; try discriminate.
  monadInv TR.
  exploit consecutive_load_multiple_correct.
  apply split_by_type_same in Heqp. rewrite <- Heqp in LOAD'. apply LOAD'.
  apply EQ. apply EQ1. apply Heqb.
  intros [rs2 [A B]].
  exists rs2. split. apply A. split. eapply agree_exten; eauto. discriminate.
  destruct il; destruct fl; monadInv TR.
  + unfold transl_restorecallee in EQ0. simpl in EQ0, EQ.
    apply split_by_type_same in Heqp. simpl in Heqp. rewrite <- Heqp in LOAD'.
    exploit transl_restorecallee_correct; eauto. discriminate. intros [rs2 [A B]].
    exists rs2. split. inversion EQ0. rewrite <- H1. apply A.
    split. eapply agree_exten; eauto.
    discriminate.
  + unfold transl_restorecallee in EQ. simpl in EQ0, EQ.
    apply split_by_type_same in Heqp. rewrite app_nil_r in Heqp. rewrite <- Heqp in LOAD'.
    exploit transl_restorecallee_correct; eauto. discriminate. intros [rs2 [A B]].
    exists rs2. split. inversion EQ. apply A.
    split. eapply agree_exten; eauto.
    discriminate.
  + exploit load_stack_pregs_sublists; eauto.
    apply split_by_type_same. apply Heqp. intros [rs1 [LOAD1 LOAD2]].
    exploit transl_restorecallee_correct. apply LOAD1. apply EQ0. discriminate.
    intros [rs1' [A B]].
    exploit (load_stack_pregs_mregs (m0::il) SP); eauto with asmgen. intros. rewrite <- H0 in LOAD2.
    exploit load_stack_pregs_swap. apply LOAD2. apply B. intros [rs3 [C [D E]]].
    rewrite <- B in C; eauto.
    exploit transl_restorecallee_correct; eauto. discriminate.
    intros [rs2 [F G]]. exists rs2. split.
    eapply exec_straight_trans. apply A. apply F. split.
    exploit agree_exten; eauto. intros. eapply agree_exten. apply H1. apply G.
    discriminate.
    
- (* Msavecallee *)
  left. exploit store_stack_regs_extends; eauto. intros [m2' [STOR' MEXT']].
  eapply exec_straight_steps; eauto. intros. simpl in TR.
  rewrite (sp_val _ _ _ AG) in STOR'.
  remember (split_by_type l).  destruct p as [il fl].
  remember (consecutive_exec_constraint il fl (reg_list_offset il ofs)). destruct b.
  destruct il; destruct fl; unfold consecutive_exec_constraint in Heqb; symmetry in Heqb;
    repeat rewrite andb_false_r in Heqb; try discriminate.
  monadInv TR.
  exploit consecutive_store_multiple_correct.
  apply split_by_type_same in Heqp. rewrite <- Heqp in STOR'. apply STOR'.
  apply EQ. apply EQ1. apply Heqb. intros [rs2 [A B]].
  exists rs2. split. apply A. 
  split. eapply agree_undef_regs; eauto.
  rewrite B; eauto with asmgen.
  destruct il; destruct fl; monadInv TR.
  + unfold transl_savecallee in EQ0. simpl in EQ0, EQ.
    apply split_by_type_same in Heqp. simpl in Heqp. rewrite <- Heqp in STOR'.
    exploit transl_savecallee_correct; eauto. discriminate. intros [rs2 [A B]].
    exists rs2. split. inversion EQ0. rewrite <- H2. apply A.
    split. eapply agree_undef_regs; eauto.
    rewrite B; auto with asmgen.
  + unfold transl_savecallee in EQ. simpl in EQ0, EQ.
    apply split_by_type_same in Heqp. rewrite app_nil_r in Heqp. rewrite <- Heqp in STOR'.
    exploit transl_savecallee_correct; eauto. discriminate. intros [rs2 [A B]].
    exists rs2. split. inversion EQ. apply A.
    split. eapply agree_undef_regs; eauto.
    rewrite B; auto with asmgen.
  + exploit store_stack_pregs_sublists; eauto.
    apply split_by_type_same. apply Heqp. intros [m3 [STOR1 STOR2]].
    exploit transl_savecallee_correct. apply STOR1. apply EQ0. discriminate.
    intros [rs2' [A B]].
    rewrite <- B in STOR2; eauto.
    exploit transl_savecallee_correct.
    eapply store_stack_pregs_swap. apply STOR2. auto with asmgen. apply EQ. discriminate.
    intros [rs2 [C D]]. exists rs2. split.
    eapply exec_straight_trans. apply A. apply C. split.
    eapply agree_undef_regs. eapply agree_exten; eauto. auto.
    rewrite D; eauto. rewrite B; eauto.
