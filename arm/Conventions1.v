...
(** Allocatable integer registers are split into two list,
    the first list are the registers that should be prefered and
    the second list contains the rest of the allocatable registers.
    Normally the prefered register should be the caller saved registers,
    for thumb we prefer the thumb registers. *)

Definition int_alloc_register (_ : unit) :=
  if thumb tt then
  (R0 :: R1 :: R2 :: R3 :: nil, R4 :: R5 :: R6 :: R7 :: R12 :: R8 :: R9 :: R10 :: R11 :: nil)
  else
  (int_caller_save_regs, int_callee_save_regs).
