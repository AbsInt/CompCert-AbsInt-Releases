...
Lemma exec_ldm_correct:
  forall l l' m sp ofs rs1 rs2 r1,
    load_stack_pregs m sp ofs l rs1 = Some rs2 ->
    mmap ireg_of l = OK l' ->
    ~ In r1 l' ->
...
Proof.
  induction l as [| r l]; simpl; intros until r1; intros LOAD MAP NIN.
  - inversion MAP. inversion LOAD. reflexivity.
  - apply bind_inversion in MAP as [x [EQ0 EQ1]]. apply bind_inversion in EQ1 as [xs [EQ1 EQ2]].
    assert (TY: (mreg_type r) = Tany32).
    { unfold mreg_type. destruct r; (reflexivity || unfold ireg_of in EQ0; simpl in EQ0; discriminate). }
    rewrite TY in LOAD.
    simpl in LOAD. inversion EQ2. 
    simpl. destruct (Mem.loadv Many32 m (Val.offset_ptr sp (Ptrofs.repr (align ofs 4)))); cycle 1. discriminate.
    rewrite TY. simpl.
    rewrite ptr_conversion_correct; cycle 1. lia.
    rewrite consecutive_align_correct; cycle 1. lia.
    rewrite <- (ireg_of_eq _ _ EQ0).
    apply IHl; auto.
    rewrite <- H0 in NIN. apply not_in_cons in NIN. apply NIN.
Qed.

Lemma exec_stm_correct:
  forall l l' (rs1 : regset) sp ofs r1 m1 m2,
    store_stack_pregs m1 sp ofs l rs1 = Some m2 ->
    mmap ireg_of l = OK l' ->
    let ofs' :=  Val.offset_ptr sp (Ptrofs.repr (reg_list_offset l (align ofs 4))) in
    exec_stm r1 (Val.offset_ptr sp (Ptrofs.repr (align ofs 4))) l' rs1 m1 = Next (nextinstr (rs1#(IR r1) <- ofs')) m2.
Proof.
  induction l as [|r l]; simpl; intros until m2; intros STOR MAP.
  - inversion MAP. inversion STOR. reflexivity.
  - monadInv MAP.
    assert (TY: (mreg_type r) = Tany32).
    { unfold mreg_type. destruct r; (reflexivity || unfold ireg_of in EQ; simpl in EQ; discriminate). }
    rewrite TY in STOR. simpl in STOR. simpl.
    rewrite <- (ireg_of_eq _ _ EQ).
    destruct (Mem.storev Many32 m1 (Val.offset_ptr sp (Ptrofs.repr (align ofs 4))) (rs1 (preg_of r)));[|discriminate].
    rewrite TY. simpl.
    rewrite ptr_conversion_correct; cycle 1. lia.
    rewrite consecutive_align_correct; cycle 1. lia.
    apply IHl; auto.
...
Lemma exec_fldm_correct:
  forall l l' m sp ofs rs1 rs2,
    load_stack_pregs m sp ofs l rs1 = Some rs2 ->
    mmap freg_of l = OK l' ->
    exec_fldm (Val.offset_ptr sp (Ptrofs.repr (align ofs 8))) l' rs1 m = Next (nextinstr rs2) m.
Proof.
    induction l as [| r l]; simpl; intros until rs2; intros LOAD MAP.
  - inversion MAP. inversion LOAD. reflexivity.
  - apply bind_inversion in MAP as [x [EQ0 EQ1]]. apply bind_inversion in EQ1 as [xs [EQ1 EQ2]].
    replace (mreg_type r) with Tany64 in LOAD; cycle 1.
    { unfold mreg_type. destruct r; (reflexivity || unfold ireg_of in EQ0; simpl in EQ0; discriminate). }
    simpl in LOAD. inversion EQ2. 
    simpl. destruct (Mem.loadv Many64 m (Val.offset_ptr sp (Ptrofs.repr (align ofs 8)))); cycle 1. discriminate.
    rewrite ptr_conversion_correct; cycle 1. lia.
    apply IHl.
    rewrite <- (freg_of_eq _ _ EQ0). apply LOAD. apply EQ1.
Qed.

Lemma exec_fstm_correct:
  forall l l' (rs1 : regset) sp ofs m1 m2,
    store_stack_pregs m1 sp ofs l rs1 = Some m2 ->
    mmap freg_of l = OK l' ->
...
Proof.
  ...
  - inversion MAP. inversion STOR. reflexivity.
  - monadInv MAP.
    assert (TY: (mreg_type r) = Tany64).
    { unfold mreg_type. destruct r; (reflexivity || unfold freg_of in EQ; simpl in EQ; discriminate). }
    rewrite TY in STOR. simpl in STOR. simpl.
    rewrite <- (freg_of_eq _ _ EQ). 
    destruct (Mem.storev Many64 m1 (Val.offset_ptr sp (Ptrofs.repr (align ofs 8))) (rs1 (preg_of r)));[|discriminate].
    rewrite ptr_conversion_correct; cycle 1. lia.
    apply IHl; auto.
Qed.
...
Lemma load_stack_pregs_sublists:
    forall l l1 l2 m sp ofs rs1 rs2,
    load_stack_pregs m sp ofs l rs1 = Some rs2 ->
    l1 ++ l2 = l ->
    exists rs3,
      load_stack_pregs m sp ofs l1 rs1 = Some rs3
      /\ load_stack_pregs m sp (reg_list_offset l1 ofs) l2 rs3 = Some rs2.
Proof.
  induction l as [| r l]; simpl; intros.
  - exists rs1. apply app_eq_nil in H0 as [A B]. rewrite A,B. easy.
  - remember (Mem.loadv (chunk_of_type (mreg_type r)) m
                (Val.offset_ptr sp (Ptrofs.repr (align ofs (AST.typesize (mreg_type r)))))).
    destruct o;[|discriminate].
    destruct l1.
    exists rs1. simpl in H0. rewrite H0. split. reflexivity. simpl. rewrite <- Heqo. assumption.
    simpl in H0. inversion H0. exploit IHl; eauto. intros [rs3 [A B]].
    exists rs3. simpl. rewrite <- Heqo. split; assumption.
Qed.

Lemma store_stack_pregs_sublists:
  forall l l1 l2 m1 sp ofs rs m2,
    store_stack_pregs m1 sp ofs l rs= Some m2 ->
    l1 ++ l2 = l ->
    exists m3,
      store_stack_pregs m1 sp ofs l1 rs = Some m3
      /\ store_stack_pregs m3 sp (reg_list_offset l1 ofs) l2 rs = Some m2.
Proof.
  induction l as [| r l]; simpl; intros.
  - exists m1. apply app_eq_nil in H0 as [A B]. rewrite A,B. easy.
  - remember (Mem.storev (chunk_of_type (mreg_type r)) m1
                (Val.offset_ptr sp (Ptrofs.repr (align ofs (AST.typesize (mreg_type r))))) (rs (preg_of r))).
    destruct o;[|discriminate].
    destruct l1. exists m1. simpl in H0. rewrite H0. split. reflexivity. simpl. rewrite <- Heqo. assumption.
    simpl in H0. inversion H0. exploit IHl; eauto. intros [m3 [A B]].
    exists m3. simpl. rewrite <- Heqo. split; assumption.
Qed.

Remark sorted_tail:
  forall (A:Type) (l : list A) P,
    LocallySorted P l -> LocallySorted P (tl l).
Proof.
  destruct l as [| r0 l]; simpl; intros.
  constructor.
  destruct l as [| r1 l].
  constructor.
...
Lemma mreg_sorted_prop:
  forall l l',
    LocallySorted RegOrd.lt l ->
    mmap ireg_of l = OK l' ->
    LocallySorted PregOrd.lt (IR ## l').
Proof.
  induction l as [ | r l]; intros.
  - inv H0. constructor.
  - simpl in H0. monadInv H0.
    destruct l. simpl in EQ1. inv EQ1. constructor.
    simpl in EQ1. monadInv EQ1. simpl.
    constructor. replace (IR x1 :: IR ## x2) with (IR ## (x1 :: x2)) by reflexivity.
    apply IHl. replace (t :: l) with (tl (r :: t :: l)) by reflexivity. apply sorted_tail; auto.
    simpl. rewrite EQ0, EQ1. reflexivity.
    inv H. rewrite <- (ireg_of_eq _ _ EQ). rewrite <- (ireg_of_eq _ _ EQ0).
    destruct r, t; auto.
Qed.

Lemma freg_consecutive_prop:
  forall l l',
  LocallySorted (fun r0 r1 => Pos.succ(IndexedMreg.index r0) = (IndexedMreg.index r1)) l ->
  mmap freg_of l = OK l' ->
  LocallySorted (fun r0 r1 => Pos.succ(IndexedPreg.index (FR r0)) = (IndexedPreg.index (FR r1))) l'.
Proof.
  induction l as [ | r l]; intros.
  - inv H0. constructor.
  - simpl in H0. monadInv H0.
    ...
    constructor.
    apply IHl. replace (m :: l) with (tl (r :: m :: l)) by reflexivity. apply sorted_tail; auto.
    simpl. rewrite EQ0, EQ1. reflexivity.
    inv H. rewrite <- (freg_of_eq _ _ EQ). rewrite <- (freg_of_eq _ _ EQ0).
    destruct r, m; (discriminate || reflexivity).
Qed.

Lemma split_by_type_same:
  forall l il fl,
    (il, fl) = split_by_type l ->
    il ++ fl = l.
Proof.
  induction l as [| r l]; simpl; intros.
  inversion H. reflexivity.
  destruct (typ_eq (mreg_type r) Tany32).
  destruct (split_by_type l).
  inversion H. simpl. f_equal. apply IHl. inversion H. reflexivity.
  inversion H. reflexivity.
Qed.

Remark ireg_of_eq_list:
  forall l l' r,
    mmap ireg_of l = OK l' ->
    In (IR r) (IR ## l') <-> In r l'.
Proof.
  induction l as [ | r0 l]; simpl; intros.
  - inv H. simpl. apply iff_refl.
  - monadInv H. simpl. apply iff_to_and. split.
    intros. destruct H. inv H. auto.
    right. apply IHl; auto. intros.
    destruct H. rewrite H. auto. right.
    apply in_map. assumption.
Qed.

Remark int_constraints:
  forall l r1 l',
    mmap ireg_of l = OK l' ->
    ~ In r1 l' ->
    LocallySorted RegOrd.lt l ->
    load_store_multiple_iregs_condition r1 (IR ## l') = true.
Proof.
  intros. unfold load_store_multiple_iregs_condition.
  apply andb_true_iff. split.
  apply proj_sumbool_is_true. eapply mreg_sorted_prop; eauto.
  rewrite negb_true_iff. rewrite <- not_true_iff_false. red; intro. InvBooleans.
  rewrite ireg_of_eq_list in H2; eauto.
Qed.

(* Correctness of save- and restorecallee *)

Lemma transl_restorecallee_unroll_correct:
  forall l m (rs1:regset) ofs rs2 k c,
    load_stack_pregs m (rs1 SP) ofs l rs1 = Some rs2 ->
    transl_restorecallee_unroll ofs l k = OK c ->
    l <> nil ->
    exists (rs2' : regset),
      exec_straight ge fn c rs1 m k rs2' m
      /\ forall r, data_preg r = true -> rs2'#r = rs2#r.
Proof.
  induction l as [| r l]; simpl; intros until c; intros LOAD TR NNIL.
  - contradiction.
  - set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    monadInv TR.
    remember (Mem.loadv (chunk_of_type ty) m (Val.offset_ptr (rs1 IR13) (Ptrofs.repr ofs1))).
    destruct o;[|discriminate].
    exploit loadind_correct; eauto. intros [rs' [A [B C]]].
    destruct l.
    + simpl in EQ. inversion EQ. simpl in LOAD. inversion LOAD. exists rs'. split. assumption. intros.
      destruct (preg_eq r0 (preg_of r)). rewrite e. rewrite B. symmetry. apply Pregmap.gss.
      Simpl.
    + exploit load_stack_pregs_swap. apply LOAD. instantiate (1:= rs').
      intros. destruct (preg_eq r0 (preg_of r)).
      rewrite e. rewrite B. symmetry. apply Pregmap.gss.
      Simpl. 
      intros [rs3 [LOAD' [D E]]]. rewrite <- C in LOAD'; auto with asmgen.
      exploit IHl. apply LOAD'.
      apply EQ. discriminate. intros [rs2' [F G]].
      exists rs2'. split. eapply exec_straight_trans; eauto.
      intros. rewrite G; auto with asmgen.
Qed.

Lemma transl_savecallee_unroll_correct:
  forall l (rs : regset) ofs m1 m2 k c,
    store_stack_pregs m1 (rs SP) ofs l rs = Some m2 ->
    transl_savecallee_unroll ofs l k = OK c ->
    l <> nil ->
    exists rs',
      exec_straight ge fn c rs m1 k rs' m2
      /\ forall r, data_preg r = true -> rs'#r = rs#r.
Proof.
  induction l as [| r l]; simpl; intros until c; intros STOR TR NNIL.
  - contradiction.
  - set (ty := mreg_type r) in *.
    set (sz := AST.typesize ty) in *.
    set (ofs1 := align ofs sz) in *.
    monadInv TR.
    remember (Mem.storev (chunk_of_type ty) m1 (Val.offset_ptr (rs IR13) (Ptrofs.repr ofs1)) (rs (preg_of r))).
    destruct o;[|discriminate].
    exploit storeind_correct; eauto. intros [rs' [A B]].
    destruct l.
    + simpl in EQ. inversion EQ. simpl in STOR. inversion STOR.
      exists rs'. rewrite <- H1. split. assumption. intros. apply B; auto with asmgen.
    + exploit IHl. rewrite <- B in STOR; eauto with asmgen.
      eapply store_stack_pregs_swap. apply STOR. auto with asmgen.
      apply EQ. discriminate. intros [rs2 [C D]].
      exists rs2. split. eapply exec_straight_trans; eauto.
      intros. rewrite D; auto with asmgen.
Qed.

Lemma transl_restorecallee_correct:
  forall l m (rs1:regset) ofs rs2 ty k c,
    load_stack_pregs m (rs1 SP) ofs l rs1 = Some rs2 ->
    transl_restorecallee ofs l ty k = OK c ->
    l <> nil ->
    exists (rs2' : regset),
      exec_straight ge fn c rs1 m k rs2' m
      /\ forall r, data_preg r = true -> rs2'#r = rs2#r.
Proof.
 intros until c; intros LOAD TR NNIL.
 unfold transl_restorecallee in TR.
 assert (Pnt: forall o, Val.add (rs1 IR13) (Vint (Int.repr o)) = Val.offset_ptr (rs1 IR13) (Ptrofs.repr o)).
 { destruct l. contradiction. intro. destruct (rs1 IR13); try discriminate. simpl. f_equal; f_equal. auto with ptrofs. }
 destruct (length_sufficient l). destruct ty; try discriminate.
 destruct (integer_list_constraint l).
 +  monadInv TR.
    exploit addimm_correct. intros [rs3 [A [B C]]].
    exploit load_stack_pregs_swap. apply LOAD. intros. instantiate (2:= IR14) in C. apply C; eauto with asmgen.
    intros [rs3' [D [E F]]].
    econstructor. split. eapply exec_straight_trans. apply A.
    constructor. simpl.
    unfold exec_instr.
    rewrite (int_constraints l); cycle 1.
    assumption. eapply ireg_of_not_R14_list. apply EQ. assumption.
    destruct l;[contradiction|]. monadInv EQ. destruct x. inversion H.
    rewrite B. rewrite Pnt. apply exec_ldm_correct.
    apply D. apply EQ. eapply ireg_of_not_R14_list. apply EQ.
    Simpl. rewrite F; eauto with asmgen. discriminate.
    intros. Simpl.
 + eapply transl_restorecallee_unroll_correct; eauto.
 + destruct (float_list_constraint l).
   monadInv TR.
   exploit addimm_correct. intros [rs3 [A [B C]]].
   exploit load_stack_pregs_swap. apply LOAD. intros. instantiate (2:= IR14) in C. apply C; eauto with asmgen.
   intros [rs3' [D [E F]]].
   econstructor. split. eapply exec_straight_trans. apply A.
   constructor. simpl. destruct LocallySorted_dec.
   destruct l;[contradiction|]. monadInv EQ. destruct x. inversion H. clear l2.
   rewrite B. rewrite Pnt.
   eapply exec_fldm_correct. apply D. apply EQ.
   eapply (freg_consecutive_prop _ x) in l1. contradiction. assumption.
   rewrite <- F; auto. simpl. discriminate.
   intros. rewrite nextinstr_inv1; auto.
   eapply transl_restorecallee_unroll_correct; eauto.
 + eapply transl_restorecallee_unroll_correct; eauto.
Qed.

Lemma transl_savecallee_correct:
  forall l m1 ofs (rs1 : regset) m2 ty k c,
    store_stack_pregs m1 (rs1 SP) ofs l rs1 = Some m2 ->
    transl_savecallee ofs l ty k = OK c ->
    l <> nil ->
    exists rs2: regset,
      exec_straight ge fn c rs1 m1 k rs2 m2
      /\ (forall r,data_preg r = true -> rs2#r = rs1#r).
Proof.
  intros until c; intros STOR TR NNIL.
  assert (Pnt: forall o, Val.add (rs1 IR13) (Vint (Int.repr o)) = Val.offset_ptr (rs1 IR13) (Ptrofs.repr o)).
  { destruct l. contradiction. intro. destruct (rs1 IR13); try discriminate. simpl. f_equal; f_equal. auto with ptrofs. }
  unfold transl_savecallee in TR.
  destruct (length_sufficient l). destruct ty; try discriminate.
  destruct (integer_list_constraint l).
  monadInv TR.
  exploit addimm_correct. intros [rs2 [A [B C]]].
  econstructor.
  split. eapply exec_straight_trans. apply A.
  constructor. unfold exec_instr. rewrite (int_constraints l); cycle 1.
  assumption. eapply ireg_of_not_R14_list. apply EQ. assumption.
  destruct l;[contradiction|]. monadInv EQ. destruct x. inversion H.
  rewrite B. rewrite Pnt. apply exec_stm_correct.
  eapply store_stack_pregs_swap. apply STOR. intros. apply C; eauto with asmgen. apply EQ.
  Simpl. intros. Simpl.
  eapply transl_savecallee_unroll_correct; eauto.
  destruct (float_list_constraint l).
  monadInv TR.
  exploit addimm_correct. intros [rs2 [A [B C]]].
  econstructor. split. eapply exec_straight_trans. apply A.
  constructor. simpl. destruct LocallySorted_dec.
  destruct l;[contradiction|]. monadInv EQ. destruct x. inversion H. clear l2.
  rewrite B. rewrite Pnt. eapply exec_fstm_correct.
  eapply store_stack_pregs_swap. apply STOR. intros. apply C; eauto with asmgen. apply EQ.
  eapply (freg_consecutive_prop _ x) in l1. contradiction. assumption. reflexivity.
  intros. rewrite <- C; eauto with asmgen. apply nextinstr_inv1; auto.
  eapply transl_savecallee_unroll_correct; eauto.
  eapply transl_savecallee_unroll_correct; eauto.
Qed.

Lemma consecutive_load_multiple_correct:
 forall l1 l2 irl frl m ofs (rs0 : regset) rs1 k,
   load_stack_pregs m (rs0 IR13) ofs (l1 ++ l2) rs0 = Some rs1 ->
   mmap ireg_of l1 = OK irl ->
   mmap freg_of l2 = OK frl ->
   consecutive_exec_constraint l1 l2 (reg_list_offset l1 ofs) = true ->
   exists rs2,
     exec_straight ge fn (addimm IR14 IR13 (Int.repr (align ofs 4)) (Pldm IR14 irl :: Pfldm IR14 frl :: k)) rs0 m k rs2 m
     /\ forall r, data_preg r = true -> rs2#r = rs1#r.
Proof.
  intros until k. intros LOAD MAP1 MAP2 COND.
  unfold consecutive_exec_constraint in COND. InvBooleans.
  destruct l1. destruct (Archi.thumb2_support);simpl in H1; apply Nat.nlt_0_r in H1; contradiction.
  destruct l2. destruct (Archi.thumb2_support);simpl in H3; apply Nat.nlt_0_r in H3; contradiction.
  exploit addimm_correct. intros [rs3 [A [B C]]].
  exploit load_stack_pregs_swap. apply LOAD. intros. instantiate (2 := IR14) in C. apply C; eauto with asmgen.
  intros [rs4 [D [E F]]].
  exploit load_stack_pregs_sublists. apply D. reflexivity. intros [rs5 [G I]].
  exploit exec_ldm_correct. apply G. apply MAP1.
  instantiate (1 := IR14). eapply ireg_of_not_R14_list. apply MAP1.
  intros J.
  exploit load_stack_pregs_swap. apply I.
  instantiate (1 := (nextinstr rs5 # IR14 <- (Val.offset_ptr (rs0 IR13) (Ptrofs.repr (reg_list_offset (m0::l1) (align ofs 4)))))).
  intros. Simpl. intros [rs6 [K [L M]]].
  exploit exec_fldm_correct. apply K. apply MAP2. intros N.
  econstructor. split.
  eapply exec_straight_trans. apply A. econstructor.
  unfold exec_instr.
  rewrite (int_constraints (m0::l1)); auto; cycle 1. eapply ireg_of_not_R14_list; eauto.
  simpl in MAP1. monadInv MAP1.
  rewrite B.
  assert (Pnt: forall o, Val.add (rs0 IR13) (Vint (Int.repr o)) = Val.offset_ptr (rs0 IR13) (Ptrofs.repr o)).
  { intro. destruct (rs0 IR13); try discriminate. simpl. f_equal; f_equal. auto with ptrofs. }
  rewrite Pnt. apply J.
  rewrite nextinstr_pc. f_equal. rewrite Pregmap.gso; auto with asmgen.
  eapply load_stack_pregs_mregs; eauto.
  constructor. unfold exec_instr. eapply freg_consecutive_prop in H2; eauto. rewrite pred_dec_true.
  simpl in MAP2. monadInv MAP2.
  Simpl.
  rewrite <- H0 in N.
  replace (reg_list_offset (m0::l1) (align ofs 4)) with (reg_list_offset (m0::l1) ofs) at 1; cycle 1.
  { simpl. replace (mreg_type m0) with Tany32. simpl. rewrite (align_same (align ofs 4)). reflexivity.
    lia. apply align_divides. lia. simpl in MAP1. monadInv MAP1. destruct m0; (discriminate || reflexivity). }
  apply N. assumption.
  rewrite <- M; auto. discriminate.
  intros. rewrite nextinstr_inv1; auto. rewrite L; auto.
Qed.

Lemma consecutive_store_multiple_correct:
 forall l1 l2 irl frl m1 m2 ofs (rs0 : regset) k,
   store_stack_pregs m1 (rs0 IR13) ofs (l1 ++ l2) rs0 = Some m2 ->
   mmap ireg_of l1 = OK irl ->
   mmap freg_of l2 = OK frl ->
   consecutive_exec_constraint l1 l2 (reg_list_offset l1 ofs) = true ->
   exists rs1,
     exec_straight ge fn (addimm IR14 IR13 (Int.repr (align ofs 4)) (Pstm IR14 irl :: Pfstm IR14 frl :: k)) rs0 m1 k rs1 m2
     /\ forall r, data_preg r = true -> rs1#r = rs0#r.
Proof.
  intros until k. intros STOR MAP1 MAP2 COND.
  unfold consecutive_exec_constraint in COND. InvBooleans.
  destruct l1. destruct (Archi.thumb2_support);simpl in H1; apply Nat.nlt_0_r in H1; contradiction.
  destruct l2. destruct (Archi.thumb2_support);simpl in H3; apply Nat.nlt_0_r in H3; contradiction.
  exploit addimm_correct. intros [rs1 [A [B C]]].
  exploit store_stack_pregs_sublists; eauto.
  intros [m3 [STOR1 STOR2]].
  econstructor. split.
  eapply exec_straight_trans. apply A.
  econstructor. unfold exec_instr. rewrite (int_constraints (m :: l1)); auto; cycle 1.
  eapply ireg_of_not_R14_list; eauto.
  simpl in MAP1. monadInv MAP1.
  rewrite B.
  assert (Pnt: forall o, Val.add (rs0 IR13) (Vint (Int.repr o)) = Val.offset_ptr (rs0 IR13) (Ptrofs.repr o)).
  { intro. destruct (rs0 IR13); try discriminate. simpl. f_equal; f_equal. auto with ptrofs. }
  rewrite Pnt. apply exec_stm_correct.
  eapply store_stack_pregs_swap. apply STOR1. auto with asmgen. simpl. rewrite EQ, EQ1. reflexivity.
  Simpl.
  constructor. unfold exec_instr.
  rewrite pred_dec_true; auto.
  simpl in MAP2. monadInv MAP2.
  Simpl.
  replace (reg_list_offset (m::l1) (align ofs 4)) with (reg_list_offset (m::l1) ofs) at 1; cycle 1.
  { simpl. replace (mreg_type m) with Tany32. simpl. rewrite (align_same (align ofs 4)). reflexivity.
    lia. apply align_divides. lia. destruct m; (discriminate || reflexivity). }
  rewrite H0. eapply exec_fstm_correct.
  eapply store_stack_pregs_swap. apply STOR2. intros.
  Simpl. 
  simpl. rewrite EQ, EQ1. reflexivity. eapply freg_consecutive_prop. eapply H2. assumption.
  reflexivity.
  intros. Simpl.
Qed.
