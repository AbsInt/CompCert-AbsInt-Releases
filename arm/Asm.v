...
Module IndexedPreg <: INDEXED_TYPE.
  Definition all_pregs : list preg :=
     ...
     :: CR CN :: CR CZ :: CR CC :: CR CV :: PC :: nil.

  Lemma all_pregs_complete:
    forall (r: preg), In r all_pregs.
  Proof.
    assert (forall r, proj_sumbool (In_dec preg_eq r all_pregs) = true) by (destruct r as [r | r | r |]; try destruct r; reflexivity).
...
Inductive instruction : Type :=
  ...
  | Pfldm: ireg -> list freg -> instruction        (**r any64 load multiple to FP regs *)                            
  ...
  | Pfstm: ireg -> list freg -> instruction        (**r any64 store from list of FP regs *)  
...
Fixpoint exec_fldm (base: val) (l: list freg) (rs: regset) (m: mem) :=
  match l with
  | r::l => match Mem.loadv Many64 m base with
          | Some v => exec_fldm (Val.offset_ptr base (Ptrofs.repr 8)) l (rs#r <- v) m
          | None => Stuck
          end
  | nil => Next (nextinstr rs) m
  end.

Fixpoint exec_fstm (base: val) (l: list freg) (rs: regset) (m: mem) :=
  match l with
  | r::l => match Mem.storev Many64 m base (rs r) with
...
Definition load_store_multiple_iregs_condition r1 (l: list preg) :=
  LocallySorted_dec PregOrd.lt PregOrdFacts.lt_dec l && negb (In_dec preg_eq (IR r1) l).
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pldm r1 rl =>
      if load_store_multiple_iregs_condition r1 (map IR rl)
      then
        match rl with
        | nil => Stuck
  ...
  | Pfldm r1 rl =>
      if LocallySorted_dec
           (fun r0 r1 => Pos.succ(IndexedPreg.index (FR r0)) = (IndexedPreg.index (FR r1)))
           (fun r0 r1 => Pos.eq_dec (Pos.succ(IndexedPreg.index (FR r0))) (IndexedPreg.index (FR r1))) rl
      then
        match rl with
        | nil => Stuck
