...
Inductive instruction : Type :=
  ...
  | Padd_sp :int -> instruction                 (**r thumb integer addition to sp *)
  ...
  | Psub_sp: int -> instruction  (**r thumb integer subtraction to sp *)
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Padd_sp imm =>
      Next (nextinstr_nf (rs#SP <- (Val.add rs#SP (Vint imm)))) m
  | Pand r1 r2 so =>
  ...
  | Psub_sp imm =>
      Next (nextinstr_nf (rs#SP <- (Val.sub rs#SP (Vint imm)))) m
  | Pudiv =>
