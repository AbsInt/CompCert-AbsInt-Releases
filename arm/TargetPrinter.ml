...
struct
  ...
  let print_instruction oc = function
    ...
    | Pmovitel(cond, (rh, rl), (ifsoh, ifsol), (ifnoth, ifnotl)) ->
      fprintf oc "	ittee	%s\n" (condition_name cond);
      fprintf oc "	mov%s	%a, %a\n"
        (condition_name cond) ireg rl ireg ifsol;
      fprintf oc "	mov%s	%a, %a\n"
        (condition_name cond) ireg rh ireg ifsoh;
      fprintf oc "	mov%s	%a, %a\n"
        (neg_condition_name cond) ireg rl ireg ifnotl;
      fprintf oc "	mov%s	%a, %a\n"
        (neg_condition_name cond) ireg rh ireg ifnoth
  ...
  let print_prologue oc =
    ...
    fprintf oc "	.eabi_attribute Tag_ABI_FP_number_model, 3\n"; (* uses all the IEEE econdings *)
