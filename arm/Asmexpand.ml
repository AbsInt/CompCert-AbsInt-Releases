...
let expand_builtin_inline name args res =
  ...
  | ("__builtin_fsqrt" | "__builtin_sqrt"), [BA(FR a1)], BR(FR res) ->
     emit (Pfsqrt (res,a1))
  | "__builtin_copysignf", [BA (FR a1); BA (FR a2)], BR(FR res) ->
     ...
     emit (Plabel lbl)
  | "__builtin_isnanf", [BA (FR a1)], BR (IR res) ->
    (* Compare value with itself which should return not equal for NaN *)
    emit (Pfcmps (a1, a1));
    emit (Pmovite (TCeq, res, SOimm _0, SOimm _1))
  | "__builtin_isnan", [BA (FR a1)], BR (IR res) ->
    (* Compare value with itself which should return not equal for NaN *)
    emit (Pfcmpd (a1, a1));
    emit (Pmovite (TCeq, res, SOimm _0, SOimm _1))
  | "__builtin_isinff", [BA (FR a1)], BR (IR res) ->
    assert (a1 = FR0);
    let single_large = coqfloat32_of_camlint 0x7F7FFFFFl in
    emit (Pflis (FR6, single_large));
    emit (Pfabss (a1, a1));
    emit (Pfcmps (a1, FR6));
    (* Compare value with large value *)
    emit (Pmovite (TCle, res, SOimm _0, SOimm _1))
  | "__builtin_isinf", [BA (FR a1)], BR (IR res) ->
    assert (a1 = FR0);
    let double_large = coqfloat64_of_camlint 0x7FEFFFFFFFFFFFFFL in
    emit (Pflid (FR6, double_large));
    emit (Pfabsd (a1, a1));
    emit (Pfcmpd (a1, FR6));
    (* Compare value with large value *)
    emit (Pmovite (TCle, res, SOimm _0, SOimm _1))
  | "__builtin_isfinitef", [BA (FR a1)], BR (IR res) ->
    assert (a1 = FR0);
    let single_large = coqfloat32_of_camlint 0x7F7FFFFFl in
    emit (Pflis (FR6, single_large));
    emit (Pfabss (a1, a1));
    emit (Pfcmps (a1, FR6));
    (* Compare value with large value *)
    emit (Pmovite (TChi, res, SOimm _0, SOimm _1));
  | "__builtin_isfinite", [BA (FR a1)], BR (IR res) ->
    assert (a1 = FR0);
    let double_large = coqfloat64_of_camlint 0x7FEFFFFFFFFFFFFFL in
    emit (Pflid (FR6, double_large));
    emit (Pfabsd (a1, a1));
