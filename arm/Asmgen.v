...
Definition iregs_of_rpair (r: rpair mreg) : res (ireg * ireg) :=
  match preg_rpair_of r with
  | Two (IR r1) (IR r2) => OK (r1, r2)
...
- For Thumb, all loads/stores for integers offer an offset range of
  [(-2^8,2^12)] (encoding as +/-imm8 or +imm12).

- For ARM classic, ldrb, strb, ldr, and str offer a range of [(-2^12,2^12)]
  while ldrsb, ldrsh, ldrh, and strh offer a reduced range of [(-2^8,2^8)].

- For 32- and 64-bit floating-points, immediate offsets are multiples of 4
  in [(-2^10,2^10)].
...
(* [(-2^12,2^12)] for ARM, [(-2^8,2^12)] for Thumb *)
...
(* [(-2^8,2^8)] for ARM, [(-2^8,2^12)] for Thumb *)
Definition mk_immed_mem_small (x: int): int :=
  if Int.lt x Int.zero then
    Int.neg (Int.zero_ext 8 (Int.neg x))
  else
    Int.zero_ext (if thumb tt then 12 else 8) x.

(* [(-2^10,2^10)] aligned to 4 bytes *)
...
Definition transl_op
  ...
  | Olongconst n, nil =>
      ...
      assertion (negb (ireg_eq rh rl));
      let hi := Int64.hiword n in
      let lo := Int64.loword n in
      if Int.eq hi lo then
        OK (loadimm rh hi (Pmov rl (SOreg rh) :: k))
      else
        OK (loadimm rh hi (loadimm rl lo k))
  | Ofloatconst f, nil =>
  ...
  | Onegl, a :: nil =>
      do (rh, rl) <- iregs_of_rpair res;
      assertion (negb (ireg_eq rh rl));
      do (ah, al) <- iregs_of_rpair a;
      if ireg_eq rl ah then
        OK (Prsbs IR14 al (SOimm Int.zero) ::
              if thumb tt then
                Pmvn' rh ah :: Padc rh rh (SOimm Int.zero) :: Pmov rl (SOreg IR14) :: k
              else
                Prsc rh ah (SOimm Int.zero) :: Pmov rl (SOreg IR14) :: k)
      else
        OK (Prsbs rl al (SOimm Int.zero) ::
              if thumb tt then
                Pmvn' rh ah :: Padc rh rh (SOimm Int.zero) :: k
              else
                Prsc rh ah (SOimm Int.zero) :: k)
  | Oadd, a1 :: a2 :: nil =>
  ...
  | Oaddl, a :: b :: nil =>
      ...
      assertion (negb (ireg_eq rh rl));
      do (ah, al) <- iregs_of_rpair a;
      do (bh, bl) <- iregs_of_rpair b;
      if ireg_eq rl ah || ireg_eq rl bh then
        OK (Padds IR14 al (SOreg bl) :: Padc rh ah (SOreg bh) :: Pmov rl (SOreg IR14) :: k)
      else
        OK (Padds rl al (SOreg bl) :: Padc rh ah (SOreg bh ):: k)
  ...
  | Osubl, a :: b :: nil =>
      ...
      do (bh, bl) <- iregs_of_rpair b;
      if ireg_eq rl ah || ireg_eq rl bh then
        OK (Psubs IR14 al (SOreg bl) :: Psbc rh ah (SOreg bh) :: Pmov rl (SOreg IR14) :: k)
      else
        OK (Psubs rl al (SOreg bl) :: Psbc rh ah (SOreg bh ):: k)
  ...
  | Oumull, a1 :: a2 :: nil =>
      do (rd2, rd1) <- iregs_of_rpair res;
      assertion (negb (ireg_eq rd1 rd2));
      do r1 <- ireg_of_rpair a1; do r2 <- ireg_of_rpair a2;
      OK (Pumull rd1 rd2 r1 r2 :: k)
  | Odiv, a1 :: a2 :: nil =>
      do r <- ireg_of_rpair res; do r1 <- ireg_of_rpair a1; do r2 <- ireg_of_rpair a2;
  ...
  | Oshr, a1 :: a2 :: nil =>
      do r <- ireg_of_rpair res; do r1 <- ireg_of_rpair a1; do r2 <- ireg_of_rpair a2;
      OK (Pasr r r1 (SOreg r2) :: k)
  | Oshru, a1 :: a2 :: nil =>
      do r <- ireg_of_rpair res; do r1 <- ireg_of_rpair a1; do r2 <- ireg_of_rpair a2;
      OK (Plsr r r1 (SOreg r2) :: k)
  | Oshift s, a1 :: nil =>
      do r <- ireg_of_rpair res; do r1 <- ireg_of_rpair a1;
  ...
  | Oabsf, a1 :: nil =>
      ...
      OK (Pfabsd rp p1 :: k)
  | Oaddf, a1 :: a2 :: nil =>
      do rp <- freg_pair_of_rpair res; do p1 <- freg_pair_of_rpair a1; do p2 <- freg_pair_of_rpair a2;
  ...
  | Osel cmp ty, a1 :: a2 :: args =>
      ...
      | Two (IR rdh) (IR rdl) =>
          assertion (negb (ireg_eq rdh rdl));
          do (ah, al) <- iregs_of_rpair a1;
          do (bh, bl) <- iregs_of_rpair a2;
          if ireg_eq rdl ah || ireg_eq rdl bh then
            if ireg_eq rdh al || ireg_eq rdh bl then
              transl_cond cmp args (Pmovitel (cond_for_cond cmp) (rdh, IR14) (ah, al) (bh, bl) :: Pmov rdl (SOreg IR14) :: k)
            else
              transl_cond cmp args (Pmovitel (cond_for_cond cmp) (rdl, rdh) (al, ah) (bl, bh) :: k)
          else
            transl_cond cmp args (Pmovitel (cond_for_cond cmp) (rdh, rdl) (ah, al) (bh, bl) :: k)
      | _ =>
...
(** Translation of Mrestorecallee and Msavecallee *)

Definition transl_restorecallee ofs mil mfl k :=
  do il <- mmap ireg_of mil;
  do fl <- mmap freg_pair_of_rpair mfl;
  if ldstm_iregs_condition IR14 il && ldstm_fregs_condition fl then
    match il, fl with
    | nil, nil => Error (msg "Asmgen.transl_restorecallee")
    | _, nil => OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pldm IR14 il  :: k))
    | nil, p :: nil => let ofs := Int.repr (align ofs 8) in OK (indexed_memory_access (Pfldd_a p) mk_immed_mem_float IR13 ofs k)
    | nil, _ => OK (addimm IR14 SP (Int.repr (align ofs 8)) (Pfldm IR14 fl  :: k))
    | _, _ => let ofs' := reg_list_offset mil ofs in
             if Z.eq_dec ofs' (align ofs' 8) then
               OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pldm IR14 il :: Pfldm IR14 fl:: k))
             else Error (msg "Mrestorecallee constraint violated")
    end
  else
    Error (msg "Asmgen.transl_restorecallee").

Definition transl_savecallee ofs mil mfl k :=
  do il <- mmap ireg_of mil;
  do fl <- mmap freg_pair_of_rpair mfl;
  if ldstm_iregs_condition IR14 il && ldstm_fregs_condition fl then
    match il, fl with
    | nil, nil => Error (msg "Asmgen.transl_savecallee")
    | _, nil => OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pstm IR14 il  :: k))
    | nil, p :: nil => let ofs := Int.repr (align ofs 8) in OK (indexed_memory_access (Pfstd_a p) mk_immed_mem_float IR13 ofs k)
    | nil, _ => OK (addimm IR14 SP (Int.repr (align ofs 8)) (Pfstm IR14 fl  :: k)) 
    | _, _ => let ofs' := reg_list_offset mil ofs in
             if Z.eq_dec ofs' (align ofs' 8) then
               OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pstm IR14 il :: Pfstm IR14 fl:: k))
             else Error (msg "Msavecallee constraint violated")
    end
  else
    Error (msg "Asmgen.transl_savecallee").

Fixpoint split_callee_saves (l: list (rpair mreg)) :=
  match l with
  | One r :: l' => let (il, fl) := split_callee_saves l' in (r::il, fl)
  | _ :: l' => (nil, l)
  | nil => (nil, nil)
  end.
...
Definition transl_instr (f: Mach.function) (i: Mach.instruction)
  ...
  | Mrestorecallee ofs l =>
      let (mil, mfl) := split_callee_saves l in
      transl_restorecallee ofs mil mfl k
  | Msavecallee ofs l =>
      let (mil, mfl) := split_callee_saves l in
      transl_savecallee ofs mil mfl k
