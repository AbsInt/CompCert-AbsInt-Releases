...
(** Translation of save- and restorecallee *)

Module RegOrd := OrderedIndexed(IndexedMreg).
Module RegOrdFacts := OrderedTypeFacts(RegOrd).

Definition integer_list_constraint :=
  LocallySorted_dec RegOrd.lt RegOrdFacts.lt_dec.

Definition float_list_constraint :=
  LocallySorted_dec
    (fun r0 r1 => Pos.succ(IndexedMreg.index r0) = (IndexedMreg.index r1))
    (fun r0 r1 => Pos.eq_dec (Pos.succ(IndexedMreg.index r0)) (IndexedMreg.index r1)).

Definition length_sufficient (l : list mreg) := lt_dec (if Archi.thumb2_support then 3 else 2) (length l).

Definition consecutive_exec_constraint il fl ofs:=
  length_sufficient il && integer_list_constraint il
  && length_sufficient fl && float_list_constraint fl
  && Z.eq_dec ofs (align ofs 8).
...
Fixpoint reg_list_offset l ofs :=
  match l with
  | r :: l => let sz := AST.typesize (mreg_type r) in reg_list_offset l ((align ofs sz) + sz)
  | nil => ofs
  end.

Fixpoint transl_restorecallee_unroll ofs l (k : code) :=
  match l with
  | nil => OK k
  | r :: l => let ty := mreg_type r in
            let sz := AST.typesize ty in
            let ofs1 := align ofs sz in
            do c <- (transl_restorecallee_unroll (ofs1 + sz) l k); (loadind SP (Ptrofs.repr ofs1) ty r c)
  end.

Fixpoint transl_savecallee_unroll ofs l (k : code) :=
  match l with
  | nil => OK k
  | r :: l => let ty := mreg_type r in
            let sz := AST.typesize ty in
            let ofs1 := align ofs sz in
            do c <- (transl_savecallee_unroll (ofs1 + sz) l k); (storeind r SP (Ptrofs.repr ofs1) ty c)
  end.

Definition transl_restorecallee ofs l ty k :=
  if length_sufficient l then
    match ty with
    | Tany32 => if integer_list_constraint l then
                 do rl <- mmap ireg_of l; OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pldm IR14 rl  :: k))
               else transl_restorecallee_unroll ofs l k
    | Tany64 => if float_list_constraint l then 
                 do rl <- mmap freg_of l; OK (addimm IR14 SP (Int.repr (align ofs 8)) (Pfldm IR14 rl  :: k))
               else transl_restorecallee_unroll ofs l k
    | _ => Error (msg "Invalid register type")
    end
  else
    transl_restorecallee_unroll ofs l k.

Definition transl_savecallee ofs l ty k :=
  if length_sufficient l then
    match ty with
    | Tany32 => if integer_list_constraint l then
                 do rl <- mmap ireg_of l; OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pstm IR14 rl  :: k))
               else transl_savecallee_unroll ofs l k
    | Tany64 => if float_list_constraint l then 
                 do rl <- mmap freg_of l; OK (addimm IR14 SP (Int.repr (align ofs 8)) (Pfstm IR14 rl  :: k))
               else transl_savecallee_unroll ofs l k
    | _ => Error (msg "Invalid register type")
    end
  else
    transl_savecallee_unroll ofs l k.
...
Definition transl_instr (f: Mach.function) (i: Mach.instruction)
  ...
  | Mrestorecallee ofs l =>
      match split_by_type l with
      | (nil, nil) => Error (msg "Mrestorecallee nil")
      | (il, fl) => let ofs' := (reg_list_offset il ofs) in
                   if consecutive_exec_constraint il fl ofs'
                   then
                     do irl <- mmap ireg_of il;
                     do frl <- mmap freg_of fl;
                     OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pldm IR14 irl :: Pfldm IR14 frl:: k))
                    else  
                      do c <- transl_restorecallee (reg_list_offset il ofs) fl Tany64 k;
                      transl_restorecallee ofs il Tany32 c
      end
  | Msavecallee ofs l =>
      match split_by_type l with
      | (nil, nil) => Error (msg "Msavecallee nil")
      | (il, fl) => let ofs' := (reg_list_offset il ofs) in
                   if consecutive_exec_constraint il fl ofs'
                   then
                     do irl <- mmap ireg_of il;
                     do frl <- mmap freg_of fl;
                     OK (addimm IR14 SP (Int.repr (align ofs 4)) (Pstm IR14 irl :: Pfstm IR14 frl:: k))
                    else  
                      do c <- transl_savecallee (reg_list_offset il ofs) fl Tany64 k;
                      transl_savecallee ofs il Tany32 c
      end
