...
Lemma exec_addimm_aux_64:
  ...
  /\ forall r, r <> PC -> r <> X16 -> r <> rd -> rs'#r = rs#r.
Proof.
  intros insn sem SEM ASSOC; intros. unfold addimm_aux.
...
- (* addlimm *)
  ...
  intros (rs' & A & B & C).
  exists rs'; split. eexact A. split. simpl in B; rewrite B; auto. auto with asmgen.
- (* subext *)
...
Lemma storestr_pair_correct: forall (r1 r2: ireg) ofs k m1 m2 m3 (rs: regset),
  Mem.storev Mint64 m1 (Val.offset_ptr rs#XSP ofs) rs#r1 = Some m2 ->
  Mem.storev Mint64 m2 (Val.offset_ptr rs#XSP (Ptrofs.add ofs (Ptrofs.repr 8))) rs#r2 = Some m3 ->
  r1 <> X16 -> r2 <> X16 -> r1 <> r2 ->
  exists rs',
     exec_straight ge fn (storestr_pair r1 r2 ofs k) rs m1 k rs' m3
  /\ forall r, r <> PC -> r <> X16 -> rs' r = rs r.
Proof.
  intros. unfold storestr_pair.
  destruct (is_stp_immediate (Ptrofs.to_int64 ofs)).
  - econstructor; split. apply exec_straight_one; simpl; eauto with asmgen.
    destruct (ireg_eq r1 r2); try congruence; simpl. unfold exec_store_pair.
    simpl in *. destruct (rs SP); simpl in *; try congruence. rewrite Ptrofs.of_int64_to_int64 in * by auto.
    rewrite H, Ptrofs.add_assoc, H0; eauto. reflexivity. intros; Simpl.
  - exploit (exec_addimm64 X16 XSP (Ptrofs.to_int64 ofs)); eauto with asmgen.
    intros (rs' & A & B & C).
    econstructor; split. eapply exec_straight_trans. eexact A.
    apply exec_straight_one; simpl; eauto with asmgen.
    destruct (ireg_eq r1 r2); try congruence. unfold exec_store_pair.
    simpl in *. rewrite B.  destruct (rs SP); simpl in *; try congruence. rewrite Ptrofs.of_int64_to_int64 in * by auto.
    change (Ptrofs.of_int64 Int64.zero) with Ptrofs.zero. rewrite Ptrofs.add_zero.
    rewrite ! C; eauto with asmgen. rewrite H, Ptrofs.add_assoc, H0; eauto with asmgen.
    reflexivity. intros. Simpl.
Qed.
