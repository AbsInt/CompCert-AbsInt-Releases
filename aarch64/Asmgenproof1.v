...
Remark load_pair_split_correct:
  forall rd1 rd2 k c ofs1 ofs2 m v1 v2 (rs:regset),
  Mem.loadv Many64 m (Val.offset_ptr (rs SP) ofs1) = Some v1 ->
  Mem.loadv Many64 m (Val.offset_ptr (rs SP) ofs2) = Some v2 ->
  bind (loadind XSP ofs2 Tany64 rd2 k)
         (fun c0 => loadind XSP ofs1 Tany64 rd1 c0) = OK c ->
  exists rs',
    exec_straight ge fn c rs m k rs' m
    /\ (rd1 <> rd2 -> rs'#(preg_of rd1) = v1 /\ rs'#(preg_of rd2) = v2)
    /\ (rd1 = rd2 -> rs'#(preg_of rd1) = v2)
    /\ forall r, data_preg r = true -> r <> (preg_of rd1) -> r <> (preg_of rd2) -> rs' r = rs r.
Proof.
  intros until rs. intros LM1 LM2 LP.
  monadInv LP.
  exploit loadind_correct. apply EQ0. apply LM1. discriminate. intros [rs1 [A [B C]]].
  replace (rs SP) with (rs1 SP) in LM2 by auto with asmgen.
  exploit loadind_correct. apply EQ. apply LM2. discriminate. intros [rs2 [D [E F]]].
  exists rs2. split.
  eapply exec_straight_trans; eauto.
  split. intros. split.
  rewrite F; eauto with asmgen. red. intro. apply preg_of_injective in H0. contradiction.
  assumption. split.
  intros. rewrite H. assumption.
  intros. rewrite F; eauto with asmgen.
Qed.

Remark store_pair_split_correct:
  forall m1 m2 m3 ofs1 ofs2 r1 r2 rs k c,
  Mem.storev Many64 m1 (Val.offset_ptr (rs SP) ofs1) (rs (preg_of r1)) = Some m2 ->
  Mem.storev Many64 m2 (Val.offset_ptr (rs SP) ofs2) (rs (preg_of r2)) = Some m3 ->
  bind (storeind r2 XSP ofs2 Tany64 k)
       (fun c0 => storeind r1 XSP ofs1 Tany64 c0) = OK c ->
  exists rs',
    exec_straight ge fn c rs m1 k rs' m3 /\ forall r, data_preg r = true -> rs' r = rs r.
Proof.
  intros until c. intros STM1 STM2 STP.
  monadInv STP.
  exploit storeind_correct. apply EQ0. apply STM1. discriminate. intros [rs1 [A B]].
  replace (rs SP) with (rs1 SP) in STM2 by auto with asmgen.
  replace (rs (preg_of r2)) with (rs1 (preg_of r2)) in STM2 by auto with asmgen.
  exploit storeind_correct. apply EQ. apply STM2. discriminate. intros [rs2 [C D]].
  exists rs2. split.
  eapply exec_straight_trans; eauto.
  intros. rewrite D; auto.
Qed.

Lemma load_pair_correct:
  forall rd1 rd2 k c ofs m v1 v2 (rs:regset),
    Mem.loadv Many64 m (Val.offset_ptr (rs SP) (Ptrofs.repr (align ofs 8))) = Some v1 ->
    Mem.loadv Many64 m (Val.offset_ptr (rs SP) (Ptrofs.repr (align ((align ofs 8) + 8) 8))) = Some v2 ->
    load_pair rd1 rd2 ofs k = OK c ->
    exists rs',
      exec_straight ge fn c rs m k rs' m
      /\ (rd1 <> rd2 -> rs'#(preg_of rd1) = v1 /\ rs'#(preg_of rd2) = v2)
      /\ (rd1 = rd2 -> rs'#(preg_of rd1) = v2)
      /\ forall r, data_preg r = true -> r <> (preg_of rd1) -> r <> (preg_of rd2) -> rs' r = rs r.
Proof.
  unfold load_pair. intros until rs. intros LM1 LM2 LP.
  assert (Pnt: forall o, (Val.addl (rs SP) (Vlong (Int64.repr o))) = Val.offset_ptr (rs SP) (Ptrofs.repr o)).
  { intros. destruct (rs SP); try discriminate. simpl. f_equal. f_equal. auto with ptrofs. }
  remember (Int64.eq (Int64.repr ofs) (Int64.sign_ext 9 (Int64.repr ofs)) && negb (mreg_eq rd1 rd2)) as b.
  destruct b.
  - remember (preg_of rd1). remember (preg_of rd2).
    symmetry in Heqb. InvBooleans. rewrite negb_true_iff in H0.
    assert (EQ: preg_of rd1 <> preg_of rd2).
    { destruct (mreg_eq rd1 rd2). discriminate.
      red; intro. apply preg_of_injective in H1. contradiction. }
    destruct p; destruct p0; rewrite Heqp; try rewrite Heqp0;
      try (eapply load_pair_split_correct; eauto; fail);
      rewrite <- Pnt in LM1; rewrite <- ptr_conversion_correct in LM2; cycle 1;
      try lia; rewrite <- Pnt in LM2.
    + inv LP. econstructor. split.
      econstructor. unfold exec_instr.
      rewrite Heqp. rewrite Heqp0. destruct (preg_eq (preg_of rd1) (preg_of rd2)). contradiction.
      unfold exec_load_pair. simpl.
      rewrite LM1. rewrite LM2.
      reflexivity.
      Simpl. repeat split; Simpl.
      intro. rewrite H1 in EQ. contradiction.
      intros. Simpl.
    + destruct rd1; discriminate.
    + destruct rd1; discriminate.
    + inv LP. econstructor. split.
      econstructor. unfold exec_instr.
      rewrite Heqp. rewrite Heqp0. destruct (preg_eq (preg_of rd1) (preg_of rd2)). contradiction.
      ...
      intro. rewrite H1 in EQ. contradiction.
      intros. Simpl.
  - eapply load_pair_split_correct; eauto.
Qed.

Lemma store_pair_correct:
  forall m1 m2 m3 ofs r1 r2 rs k c,
  Mem.storev Many64 m1 (Val.offset_ptr (rs SP) (Ptrofs.repr (align ofs 8))) (rs (preg_of r1)) = Some m2 ->
  Mem.storev Many64 m2 (Val.offset_ptr (rs SP) (Ptrofs.repr (align (align ofs 8 + 8) 8))) (rs (preg_of r2)) = Some m3 ->
  store_pair r1 r2 ofs k = OK c ->
  exists rs',
    exec_straight ge fn c rs m1 k rs' m3 /\ forall r, data_preg r = true -> rs' r = rs r.
Proof.
  unfold store_pair. intros until c. intros STM1 STM2 STP.
  assert (Pnt: forall o, (Val.addl (rs SP) (Vlong (Int64.repr o))) = Val.offset_ptr (rs SP) (Ptrofs.repr o)).
  { intros. destruct (rs SP); try discriminate. simpl. f_equal. f_equal. auto with ptrofs. }
  remember (Int64.eq (Int64.repr ofs) (Int64.sign_ext 9 (Int64.repr ofs)) && negb (mreg_eq r1 r2)) as b.
  destruct b.
  - remember (preg_of r1). remember (preg_of r2).
    symmetry in Heqb. InvBooleans. rewrite negb_true_iff in H0.
    assert (EQ: preg_of r1 <> preg_of r2).
    { destruct (mreg_eq r1 r2). discriminate.
      red; intro. apply preg_of_injective in H1. contradiction. }
    destruct p; destruct p0; rewrite Heqp in STM1; rewrite Heqp0 in STM2;
      try (eapply store_pair_split_correct; eauto; fail); try(destruct r1; discriminate);
      try(destruct r2; discriminate);
      rewrite <- Pnt in STM1; rewrite <- ptr_conversion_correct in STM2; cycle 1;
      try lia; rewrite <- Pnt in STM2.
    + inv STP. econstructor. split.
      econstructor. unfold exec_instr.
      rewrite Heqp. rewrite Heqp0. destruct (preg_eq (preg_of r1) (preg_of r2)). contradiction.
      unfold exec_store_pair. simpl.
      rewrite STM1. rewrite STM2.
      reflexivity. reflexivity. intros. apply nextinstr_inv1; auto.
    + inv STP. econstructor. split.
      econstructor. unfold exec_instr.
      rewrite Heqp. rewrite Heqp0. destruct (preg_eq (preg_of r1) (preg_of r2)). contradiction.
      unfold exec_store_pair. simpl.
      rewrite STM1. rewrite STM2.
      reflexivity. reflexivity. intros. apply nextinstr_inv1; auto.
  - eapply store_pair_split_correct; eauto.
Qed.

Lemma transl_restorecallee_correct:
  forall l ofs k m (rs1: regset) rs2 c,
    load_stack_pregs m (rs1 SP) ofs l rs1 = Some rs2 ->
    transl_restorecallee ofs l k = OK c ->
    l <> nil ->
    exists (rs2' : regset),
      exec_straight ge fn c rs1 m k rs2' m
      /\ forall r, data_preg r = true -> rs2'#r = rs2#r.
Proof.
  intros until m. functional induction transl_restorecallee ofs l k; intros.
  - monadInv H0. simpl in H.
    repeat match goal with [ H: match ?x with _ => _ end = Some _ |- _ ] =>
               let a := fresh "M" in remember x as a; destruct a; try discriminate end.
    exploit load_pair_correct. symmetry. apply HeqM. symmetry. apply HeqM0. apply EQ0.
    intros [rs0 [A [B [C D]]]].
    assert (E: forall r : preg, data_preg r = true -> rs0 r = (rs1 # (preg_of rd1) <- v) # (preg_of rd2) <- v0 r).
    { intros. destruct (preg_eq r (preg_of rd1)); destruct (preg_eq r (preg_of rd2)).
      rewrite <- e0. rewrite Pregmap.gss. rewrite e. apply C. rewrite e in e0. apply preg_of_injective in e0. assumption.
      rewrite Pregmap.gso; auto. rewrite e. rewrite Pregmap.gss. apply B. red. intro. rewrite <- H2 in n. contradiction.
      rewrite <- e. rewrite Pregmap.gss. rewrite e. apply B. red. intro. rewrite <- H2 in e. contradiction.
      rewrite Pregmap.gso; auto. rewrite Pregmap.gso; auto. }
    destruct l0.
    + simpl in EQ. inv EQ.
      simpl in H. inv H.
      exists rs0. split. assumption.
      apply E.
   + exploit load_stack_pregs_swap. apply H. instantiate (1 := rs0).
      apply E.
      intros [rs2'' [F [G I]]].
      exploit IHr.
      replace (rs1 SP) with (rs0 SP) in F by (rewrite D; eauto with asmgen).
      apply F.
      apply EQ.
      discriminate.
      intros [rs2' [J K]]. exists rs2'. split.
      eapply exec_straight_trans. apply A. apply J.
      intros. rewrite K; eauto with asmgen.
  - simpl in H.
    remember (Mem.loadv Many64 m (Val.offset_ptr (rs1 SP) (Ptrofs.repr (align ofs 8)))).
    destruct o; try discriminate.
    exploit loadind_correct; eauto. discriminate. intros [rs' [A [B C]]].
    exists rs'. split. auto.
    intros. destruct (preg_eq r (preg_of rd)).
    rewrite e. rewrite B. inversion H. symmetry. apply Pregmap.gss.
    rewrite C; auto. inversion H. symmetry. apply Pregmap.gso; auto.
  - contradiction.
Qed.

Lemma transl_savecallee_correct:
  forall l ofs k c (rs: regset) m1 m2,
  store_stack_pregs m1 (rs SP) ofs l rs = Some m2 ->
  transl_savecallee ofs l k = OK c ->
    l <> nil ->
    exists rs',
      exec_straight ge fn c rs m1 k rs' m2
      /\ forall r, data_preg r = true -> rs'#r = rs#r.
Proof.
  intros until k. functional induction transl_savecallee ofs l k; intros.
  - monadInv H0. simpl in H.
    repeat match goal with [ H: match ?x with _ => _ end = Some _ |- _ ] =>
               let a := fresh "M" in remember x as a; destruct a; try discriminate end.
    exploit store_pair_correct. symmetry. apply HeqM. symmetry. apply HeqM0. apply EQ0.
    intros [rs0 [A B]].
    destruct l0.
    + inv EQ. inv H.
      exists rs0. auto.
    + exploit store_stack_pregs_swap. apply H.
      intros. apply B; eauto with asmgen.
      intros C.
      replace (rs SP) with (rs0 SP) in C by auto with asmgen.
      exploit IHr.
      apply C. apply EQ. discriminate.
      intros [rs1 [D E]].
      exists rs1. split.
      eapply exec_straight_trans; eauto.
      intros. rewrite E; auto.
  - inv H.
    remember (Mem.storev Many64 m1 (Val.offset_ptr (rs SP) (Ptrofs.repr (align ofs 8))) (rs (preg_of rd))).
    destruct o; try discriminate.
    inv H3.
    eapply storeind_correct. apply H0. symmetry. apply Heqo. discriminate.
  - contradiction.
Qed.
End CONSTRUCTORS.
