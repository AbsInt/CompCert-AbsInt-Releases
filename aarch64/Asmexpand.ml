...
let expand_builtin_vload chunk args res =
  ...
  | [BA_addrstack ofs] ->
      if offset_in_range (Ptrofs.add ofs size_chunk) then
        expand_builtin_vload_common chunk XSP (Ptrofs.to_int64 ofs) res
      else begin
        expand_addimm64 (RR1 X16) XSP (Ptrofs.to_int64 ofs); (* X16 <- SP + ofs *)
        expand_builtin_vload_common chunk (RR1 X16) _0L res
      end
  | [BA_addptr(BA(IR addr), BA_long ofs)] ->
      if offset_in_range (Ptrofs.(add (of_int64 ofs) size_chunk)) then
...
let expand_builtin_vstore chunk args =
  ...
  | [BA_addrstack ofs; src] ->
      if offset_in_range (Ptrofs.add ofs size_chunk) then
        expand_builtin_vstore_common chunk XSP (Ptrofs.to_int64 ofs) src
      else begin
        expand_addimm64 (RR1 X16) XSP (Ptrofs.to_int64 ofs); (* X16 <- SP + ofs *)
