...
Require Import Coqlib AST Memory Separation.
...
Function split_used_callee_save (rl: list mreg) :=
  match rl with
  | r1 :: r2 :: l => Multiple (One r1 :: One r2 :: nil) :: (split_used_callee_save l)
  | r :: nil => Single r :: nil
  | nil => nil
  end.

Lemma callee_save_correspond:
  forall r l, In r (regs_of_rpairs (flatten_group_list (split_used_callee_save l))) <-> In r l.
Proof.
  intros. functional induction (split_used_callee_save l); simpl; auto; intuition auto.
Qed.

Lemma callee_save_wellformed:
  forall l p, In p (flatten_group_list (split_used_callee_save l)) -> pair_wf p.
Proof.
  intro. functional induction (split_used_callee_save l); intros.
  - simpl in H. destruct H; inv H; simpl; auto.
  - simpl in H; auto. destruct H; inv H; simpl; auto.
  - simpl in H. contradiction.
Qed.

Definition make_env (b: bounds) : frame_env :=
  let calleesaves := split_used_callee_save b.(used_callee_save) in
  ...
  let ol :=  align (size_callee_save_area_rec (flatten_group_list calleesaves) ocs) 8 in (* locals *)
  ...
  {| fe_size := sz;
     ...
     fe_used_callee_save := calleesaves |}.
...
Local Opaque Z.add Z.mul sepconj range.
  ...
  set (ocs := oretaddr + 8).
  set (ol :=  align (size_callee_save_area fe ocs) 8).
  ...
  assert (size_callee_save_area fe ocs <= ol) by (apply align_le; lia).
  assert (ol + 4 * b.(bound_local) <= ostkdata) by (apply align_le; lia).
  change (align (size_callee_save_area_rec (flatten_group_list (split_used_callee_save (used_callee_save b))) ocs) 8) with ol.
(* Reorder as:
     outgoing
     back link
     retaddr
...
Definition saves_retaddr (f: Linear.function) : bool := true.
Global Opaque saves_retaddr.
