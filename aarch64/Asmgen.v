...
Definition load_pair rd1 rd2 ofs k :=
  let ofs1 := align ofs 8 in
  if Int64.eq (Int64.repr ofs) (Int64.sign_ext 9 (Int64.repr ofs))
     && negb (mreg_eq rd1 rd2)
  then
    match preg_of rd1, preg_of rd2 with
    | IR r1, IR r2 =>
        OK (Pldpx_a r1 r2 (ADimm XSP (Int64.repr ofs1)) :: k)
    | FR r1, FR r2 =>
        OK (Pldpd_a r1 r2 (ADimm XSP (Int64.repr ofs1)) :: k)
    | _,_ =>
        do c <- loadind XSP (Ptrofs.repr (align (ofs1 + 8) 8)) Tany64 rd2 k;
        loadind XSP (Ptrofs.repr ofs1) Tany64 rd1 c
    end
  else
   do c <- loadind XSP (Ptrofs.repr (align (ofs1 + 8) 8)) Tany64 rd2 k;
    loadind XSP (Ptrofs.repr ofs1) Tany64 rd1 c.

Definition store_pair rd1 rd2 ofs k :=
  let ofs1 := align ofs 8 in
  if Int64.eq (Int64.repr ofs) (Int64.sign_ext 9 (Int64.repr ofs))
     && negb (mreg_eq rd1 rd2)
  then
    match preg_of rd1, preg_of rd2 with
    | IR r1, IR r2 =>
        OK (Pstpx_a r1 r2 (ADimm XSP (Int64.repr ofs1)) :: k)
    | FR r1, FR r2 =>
        OK (Pstpd_a r1 r2 (ADimm XSP (Int64.repr ofs1)) :: k)
    | _,_ =>
        do c <- storeind rd2 XSP (Ptrofs.repr (align (ofs1 + 8) 8)) Tany64 k;
        storeind rd1 XSP (Ptrofs.repr ofs1) Tany64 c
    end
  else
   do c <- storeind rd2 XSP (Ptrofs.repr (align (ofs1 + 8) 8)) Tany64 k;
    storeind rd1 XSP (Ptrofs.repr ofs1) Tany64 c.
...
Definition transl_instr (f: Mach.function) (i: Mach.instruction)
  ...
  | Mrestorecallee ofs l =>
      match l with
      | One r1 :: One r2 :: nil => if mreg_eq r1 r2
                          then Error (msg "Same registeres in load pair")
                          else load_pair r1 r2 ofs k
      | _ => Error (msg "Mrestorecallee unsupported parameters")
      end
  | Msavecallee ofs l =>
      match l with
      | One r1 :: One r2 :: nil => if mreg_eq r1 r2
                          then Error (msg "Same registers in store pair")
                          else store_pair r1 r2 ofs k
      | _ => Error (msg "Msavecallee unsuported parameters")
      end
...
(** For stp we must check that the offset is a multiple of 8 in the range -512 to 504 *)
Definition is_stp_immediate (ofs: int64) :=
  Int64.eq ofs (Int64.sign_ext 10 ofs) && Int64.eq (Int64.modu ofs (Int64.repr 8)) Int64.zero.

Definition storestr_pair (r1 r2: ireg) (ofs: ptrofs) (k : code) : code :=
  let ofs := Ptrofs.to_int64 ofs in
  if is_stp_immediate ofs
  then
    Pstp r1 r2 (ADimm XSP ofs) :: k
  else
    addimm64 X16 XSP ofs (Pstp r1 r2 (ADimm X16 Int64.zero) :: k).

Definition transl_function (f: Mach.function) :=
  do c <- transl_code' f f.(Mach.fn_code) true;
  assertion (Ptrofs.eq (Ptrofs.add f.(fn_link_ofs) (Ptrofs.repr 8)) f.(fn_retaddr_ofs));
  OK (mkfunction f.(Mach.fn_sig)
        (Pallocframe f.(fn_stacksize) ::
          (storestr_pair X15 RA f.(fn_link_ofs)
