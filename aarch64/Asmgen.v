...
Function transl_restorecallee ofs l k :=
  let ofs1 := align ofs 8 in
  match l with
  | rd1 :: rd2 :: l =>
      let ofs2 := align (ofs1 + 8) 8 in
      do c <- transl_restorecallee (ofs2 + 8) l k;
      load_pair rd1 rd2 ofs c
  | rd :: nil => loadind XSP (Ptrofs.repr ofs1) Tany64 rd k
  | nil => OK k
  end.

Function transl_savecallee ofs l k :=
  let ofs1 := align ofs 8 in
  match l with
  | rd1 :: rd2 :: l =>
      let ofs2 := align (ofs1 + 8) 8 in
      do c <- transl_savecallee (ofs2 + 8) l k;
      store_pair rd1 rd2 ofs c
  | rd :: nil => storeind rd XSP (Ptrofs.repr ofs1) Tany64 k
  | nil => OK k
  end.
...
Definition transl_instr (f: Mach.function) (i: Mach.instruction)
  ...
  | Mrestorecallee ofs l =>
      match l with
      | nil => Error (msg "Mrestorecallee nil")
      | _ => transl_restorecallee ofs l k
      end
  | Msavecallee ofs l =>
      match l with
      | nil => Error (msg "Msavecallee nil")
      | _ => transl_savecallee ofs l k
      end
