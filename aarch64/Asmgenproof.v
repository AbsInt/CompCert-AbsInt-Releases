...
Remark store_pair_label:
  forall rd1 rd2 ofs k c,
  store_pair rd1 rd2 ofs k = OK c ->
  tail_nolabel k c.
Proof.
  unfold store_pair. intros.
  remember (preg_of rd1). remember (preg_of rd2).
  destruct p; destruct p0; try (destruct rd1; discriminate); try (destruct rd2; discriminate);
  TailNoLabel; eapply tail_nolabel_trans; eapply storeind_label; eauto.
Qed.

Remark load_pair_label:
  forall rd1 rd2 ofs k c,
  load_pair rd1 rd2 ofs k = OK c ->
  tail_nolabel k c.
Proof.
  unfold load_pair. intros.
  remember (preg_of rd1). remember (preg_of rd2).
  destruct p; destruct p0; try (destruct rd1; discriminate); try (destruct rd2; discriminate);
  TailNoLabel; eapply tail_nolabel_trans; eapply loadind_label; eauto.
Qed.
...
Remark storestr_pair_label:
  forall r1 r2 ofs k, tail_nolabel k (storestr_pair r1 r2 ofs k).
Proof.
  intros. unfold storestr_pair.
  destruct (is_stp_immediate (Ptrofs.to_int64 ofs)); TailNoLabel.
  eapply tail_nolabel_trans. eapply addimm64_label. TailNoLabel.
Qed.
...
- Destructor; TailNoLabel; eapply store_pair_label; eauto.
- Destructor; TailNoLabel; eapply load_pair_label; eauto.
...
Proof.
  ...
  erewrite tail_nolabel_find_label by (apply storestr_pair_label). simpl.
...
- intros. monadInv H0.
  ...
  constructor. eapply tail_nolabel_trans. eapply storestr_pair_label.
  TailNoLabel.
...
- (* Msetstack *)
  ...
  assert (Val.lessdef (Mach.get_pair src rs) (get_pair (preg_rpair_of src) rs0)) by (eapply preg_rpair_val; eauto).
...
- (* Mrestorecallee *)
  exploit load_stack_rpairs_extends; eauto. intros [rs1 [A B]].
  left; eapply exec_straight_steps; eauto.
  intros. simpl in TR. Destructor; try monadInv TR.
  rewrite (sp_val _ _ _ AG) in A.
  simpl in A. unfold load_stack in A. destruct (Mem.loadv) eqn: E1; try discriminate. destruct (Mem.loadv) eqn: E2 in A; try discriminate.
  destruct (mreg_eq _); try discriminate.
  exploit load_pair_correct. eapply E1. eapply E2. eapply TR.
  intros (rs2 & C & D & F & G). clear a4. apply D in n as (D1 & D2).
  exists rs2; split. eauto. split; try discriminate.
  constructor; inv AG; auto. rewrite G; auto with asmgen.
  intros. eapply Val.lessdef_trans. eapply B. inv A.
  destruct (mreg_eq r r0), (mreg_eq r r2).
  + subst r. contradiction.
  + subst r. rewrite Regmap.gso; auto. rewrite Regmap.gss; auto.
  + subst r. rewrite Regmap.gss; auto.
  + rewrite !Regmap.gso; auto. eapply Val.lessdef_trans; eauto. rewrite G; auto with asmgen; red; intro; apply preg_of_injective in H0; contradiction.
- (* Msavecallee *)
  left; eapply exec_straight_steps'; eauto.
  intros. simpl in TR. Destructor; try discriminate.
  simpl in H. unfold store_stack in H. Destructor; try discriminate.
  eapply Mem.storev_extends with (v2 := (rs0 (preg_of r0))) in a5 as (m0' & STORE1 & STORE1'); eauto.
  eapply Mem.storev_extends with (v2 := (rs0 (preg_of r2))) in a6 as (m1' & STORE2 & STORE2'); eauto.
  rewrite (sp_val _ _ _ AG) in STORE1, STORE2.
  destruct (mreg_eq _); try discriminate.
  exploit store_pair_correct. apply STORE1. apply STORE2. apply TR. intros (rs2 & A & B).
  exists m1', rs2; split. eauto. split.
  eapply agree_undef_regs. apply AG. auto with asmgen.
  simpl. split. rewrite B; auto.
  inv H; auto. inv AG; auto. inv AG; auto.
...
(* X15 does not contain parent *)
  exploit loadptr_correct. instantiate (3 := XSP). eexact A. simpl; congruence. intros [rs1 [P [Q R]]].
  exploit loadind_correct. eexact EQ1. instantiate (2 := rs1). simpl; rewrite Q. eauto. simpl; congruence.
  intros [rs2 [S [T U]]].
...
- (* Mreturn *)
  ...
  (* match states *)
  econstructor; eauto.
  apply agree_restore_auto_save_regs. apply agree_set_other; auto with asmgen.
...
- (* internal function *)
  ...
  set (x1 := Pcfi_rel_offset (fn_retaddr_ofs f) :: x0) in *.
  set (tfbody := Pallocframe (fn_stacksize f) ::
                 storestr_pair X15 RA (fn_link_ofs f) x1) in *.
  set (tf := {| fn_sig := Mach.fn_sig f; fn_code := tfbody |}) in *.
  set (rs2 := nextinstr (rs0#X15 <- (parent_sp s) #SP <- sp #X16 <- Vundef)).
  exploit (storestr_pair_correct tge tf X15 RA (fn_link_ofs f)); eauto with asmgen.
  instantiate (2 := rs2). change (rs2 XSP) with sp. eexact F.
  change (rs2 XSP) with sp. rewrite (Ptrofs.same_if_eq (Ptrofs.add (fn_link_ofs f) (Ptrofs.repr 8)) (fn_retaddr_ofs f) Heqb0).
  change (rs2 X30) with (rs0 X30). rewrite ATLR. eexact P.
  intros (rs3 & U & V).
  ...
  { change (fn_code tf) with tfbody; unfold tfbody.
    ...
    eapply exec_straight_trans. eexact U.
