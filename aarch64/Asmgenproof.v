...
- intros. monadInv H0.
  ...
  constructor. eapply tail_nolabel_trans.
  eapply (storeptr_label X15 XSP (fn_link_ofs f0)). eapply tail_nolabel_trans.
  eapply (storeptr_label X30 XSP (fn_retaddr_ofs f0) (Pcfi_rel_offset (fn_retaddr_ofs f0) :: x)).
  TailNoLabel.
...
- (* internal function *)
  ...
  set (tfbody := Pallocframe (fn_stacksize f) ::
                 storeptr X15 XSP (fn_link_ofs f)
                  (storeptr RA XSP (fn_retaddr_ofs f) x1)) in *.
  set (tf := {| fn_sig := Mach.fn_sig f; fn_code := tfbody |}) in *.
  set (rs2 := nextinstr (rs0#X15 <- (parent_sp s) #SP <- sp #X16 <- Vundef)).
  exploit (storeptr_correct tge tf XSP (fn_link_ofs f) X15); eauto with asmgen.
  instantiate (2 := rs2). change (rs2 XSP) with sp. eexact F.
  intros (rs3 & U & V).
  exploit (storeptr_correct tge tf XSP (fn_retaddr_ofs f) RA x1); eauto with asmgen.
    simpl preg_of_iregsp. instantiate (2 := rs3). rewrite !V; eauto with asmgen.
    change (rs2 X30) with (rs0 X30). change (rs2 SP) with sp. rewrite ATLR. eexact P.
  intros (rs4 & U' & V').
  ...
  { change (fn_code tf) with tfbody; unfold tfbody.
    ...
    eapply exec_straight_trans. eexact U. eapply exec_straight_trans. eexact U'.
...
Local Transparent destroyed_at_function_entry. simpl.
  ...
  intros. unfold rs5. rewrite  <- V by eauto with asmgen; Simpl.
  intros. unfold rs5. rewrite nextinstr_inv by eauto with asmgen.
  rewrite V', V; eauto with asmgen.
- (* external function *)
