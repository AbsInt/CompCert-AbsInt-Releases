...
let intsz oc (sz, n) =
  match sz with X -> coqint64 oc (I64.repr n) | W -> coqint oc (I32.repr n)
...
module type SYSTEM =
  sig
    ...
    val symbol_offset_high: out_channel -> P.t * Ptrofs.int -> unit
    val symbol_offset_low: out_channel -> P.t * Ptrofs.int -> unit
...
module MacOS_System : SYSTEM =
  struct
    ...
    let symbol_offset_gen kind oc (id, ofs) =
      ...
      if not (Ptrofs.eq ofs Ptrofs.zero) then fprintf oc " + %a" ptrofs ofs
...
(* Printing of instructions *)
    ...
    | Pldp(rd1, rd2, a) | Pldpx_a(rd1, rd2, a) ->
    ...
    | Pstrh(rs, a) ->
        fprintf oc "	strh	%a, %a\n" wreg rs addressing a
    | Pstp(rs1, rs2, a) | Pstpx_a(rs1, rs2, a) ->
    ...
    | Pstrd(rd, a) | Pstrd_a(rd, a) ->
        fprintf oc "	str	%a, %a\n" dreg rd addressing a
    | Pstpd_a(rs1, rs2, a) ->
        fprintf oc "	stp	%a, %a, %a\n" dreg rs1 dreg rs2 addressing a
    (* Floating-point move *)
    | Pfmov(rd, r1) ->
        fprintf oc "	fmov	%a, %a\n" dreg rd dreg r1

    (*- #Link_to E_COMPCERT_TR_Function_PRINT_PFMOVIMMD_001 *)
    | Pfmovimmd(rd, f) ->
    ...
    (*- #Link_to E_COMPCERT_TR_Function_PRINT_PFMOVIMMS_001 *)
    | Pfmovimms(rd, f) ->
    ...
    | Pfmaxnm (sz, rd, r1, r2) ->
        fprintf oc "	fmaxnm	%a, %a, %a\n" freg (sz, rd) freg (sz, r1) freg (sz, r2)
    | Pfminnm (sz, rd, r1, r2) ->
        fprintf oc "	fminnm	%a, %a, %a\n" freg (sz, rd) freg (sz, r1) freg (sz, r2)
