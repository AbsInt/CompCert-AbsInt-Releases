...
Options:
  ...
  -clightgen-only      Only compile and install the clightgen tool
...
case "$target" in
  ...
  armv7m+nofp.dp-*)
      arch="arm"; model="armv7m"; endianness="little"; bitsize=32; has_double=false;;
  armeb-*|armebv7a-*)
  ...
  powerpcvle-*|ppcvle-*)
      arch="powerpc_vle"; model="ppc32"; endianness="big"; bitsize=32; has_double=false;;
  riscv32-*|rv32-*)
  ...
  peaktop)
      arch="peaktop"; model="default"; endianness="little"; abi="eabi"; bitsize=32; has_double=false;;
  tricore-eabi|tricore-eabi-llvm)
      arch="tricore"; model="tc161"; endianness="little"; abi="eabi"; bitsize=32; has_double=false;;
  manual)
      ;;
  "")
...
#
if test "$arch" = "powerpc_vle"; then

  case "$target" in
    eabi|eabi-diab|linux)
        ;;
    *)
        echo "Error: invalid eabi/system '$target' for architecture PowerPC VLE." 1>&2
        echo "$usage" 1>&2
        exit 2;;
  esac
...
#
# Peaktop Target Configuration
if test "$arch" = "peaktop"; then
    asm_supports_cfi=false
    casm="${toolprefix}gcc"
    casm_options="-c"
    cc="${toolprefix}gcc"
  ...
  "")
        ...
        case "$rocq_ver" in
             ...
             *) echo "Rocq prover version $rocq_ver found."
                echo "Please install the Coq wrapper for this version of Rocq.";;
        esac
        missingtools=true;;
esac
...
case "$ocaml_ver" in
  # a.d. TODO which OCaml 5.x versions do we support
  4.0[5-9].*|4.1?.*|5.3.*)
