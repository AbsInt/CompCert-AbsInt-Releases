...
(*- E_COMPCERT_FTR_Function_IEEE754_extra_Beq_dec_001 *)
(*- #Justify_Derived "Auxiliary definition used to hide flocq library" *)
...
(*- E_COMPCERT_FTR_Function_IEEE754_extra_BofZ_001 *)
(*- #Justify_Derived "Auxiliary definition used to hide flocq library" *)
...
(*- E_COMPCERT_FTR_Function_IEEE754_extra_ZofB_001 *)
(*- #Justify_Derived "Auxiliary definition used to hide flocq library" *)
...
Theorem ZofB_minus':
  forall minus_nan m f p q,
  0 < q < emax - 1  ->
  ZofB f = Some p -> 0 <= p < 2*(two_p q) -> (IZR (two_p q) <= B2R _ _ f)%R ->
  ZofB (Bminus _ _ _ _ minus_nan m f (BofZ (two_p q))) = Some (p - (two_p q)).
Proof.
  intros. set (q' := two_p q) in *.
  assert (RANGE: (-1 < B2R _ _ f < IZR (p + 1)%Z)%R) by (apply ZofB_range_nonneg; auto; lia).
  rewrite ZofB_correct in H0. destruct (is_finite prec emax f) eqn:FIN; try discriminate.
  assert (PQ2: (IZR (p + 1) <= IZR q' * 2)%R).
  { rewrite <- mult_IZR. apply IZR_le. lia. }
  assert (EXACT: round radix2 fexp (round_mode m) (B2R _ _ f - IZR q')%R = (B2R _ _ f - IZR q')%R).
  { apply round_generic. apply valid_rnd_round_mode.
    apply sterbenz_aux. now apply FLT_exp_valid. apply FLT_exp_monotone. apply generic_format_B2R.
    destruct (integer_representable_2p q). lia. replace q' with (2 ^ q). auto.
    unfold q'. rewrite two_p_correct. reflexivity.
    lra. }
  destruct (BofZ_representable q') as (A & B & C); auto.
  unfold q'. rewrite two_p_correct.
  apply (integer_representable_2p q). lia.
  generalize (Bminus_correct _ _ _ _ minus_nan m f (BofZ q') FIN B).
  rewrite Rlt_bool_true.
- fold emin; fold fexp. intros (D & E & F).
  rewrite ZofB_correct. rewrite E. rewrite D. rewrite A. rewrite EXACT.
  inversion H0. f_equal. rewrite ! Ztrunc_floor. apply Zfloor_minus.
  lra. lra.
- rewrite A. fold emin; fold fexp. rewrite EXACT.
  apply Rle_lt_trans with (bpow radix2 (emax - 1)).
  apply Rle_trans with (IZR q'). apply Rabs_le. lra.
  rewrite <- IZR_Zpower. apply IZR_le; auto. destruct H1. simpl. unfold generic_format in H3. destruct H3.
  unfold q'. rewrite <- two_p_correct.
  replace (two_p q) with (two_p q * 1) by lia.
  replace (emax - 1) with (q + (emax - 1 - q)) by lia.
  rewrite two_p_is_exp; try lia. apply Zmult_le_compat_l.
  assert (two_p (emax - 1 - q) > 0). apply two_p_gt_ZERO. lia. lia.
  assert (two_p q > 0). apply two_p_gt_ZERO. lia. lia. lia.
  apply bpow_lt. lia.
Qed.

(** A variant of [ZofB] that bounds the range of representable integers. *)

(*- E_COMPCERT_FTR_Function_IEEE754_extra_ZofB_range_001 *)
(*- #Justify_Derived "Auxiliary definition used to hide flocq library" *)
...
Theorem ZofB_range_minus':
  ...
  0 < q < emax - 1  ->
  let q' := two_p q in
  ZofB_range f 0 (2 * q' - 1) = Some p -> (IZR q' <= B2R _ _ f)%R ->
  ZofB_range (Bminus _ _ _ _ minus_nan m f (BofZ q')) (-q') (q' - 1) = Some (p - q').
Proof.
  intros. destruct (ZofB_range_inversion _ _ _ _ H0) as (A & B & C).
  set (f' := Bminus prec emax _ _ minus_nan m f (BofZ q')).
  assert (D: ZofB f' = Some (p - q')).
  { apply ZofB_minus'. auto. auto. lia.  auto. }
  unfold ZofB_range. rewrite D. rewrite Zle_bool_true by lia. rewrite Zle_bool_true by lia. auto.
Qed.
...
(** Translate SpecFloat.bounded into multiple inequalities to be used by lia. *)
Lemma binary_float_bounds:
  forall mx ex,
  bounded prec emax mx ex = true ->
  (3 - emax - prec <= ex <= emax - prec
   /\ Z.pos mx < 2 ^ prec
   /\ Z.pos (digits2_pos mx) <= prec
   /\ (Z.pos mx < 2 ^ (prec - 1) -> ex = 3 - emax - prec)).
Proof.
  intros.
  unfold bounded, canonical_mantissa in H.
  rewrite andb_true_iff in H. destruct H as [BND0 BND1].
  apply Zle_is_le_bool in BND1.
  apply Zeq_bool_eq in BND0.
  unfold fexp in BND0.
  unfold emin in BND0.
  assert (ex >= 3 - emax - prec).
  { 
    destruct (Z.max_spec (Z.pos (digits2_pos mx) + ex - prec) (3 - emax - prec)) as [[??]|[??]]. 
    rewrite H0 in BND0. lia.
    rewrite H0 in BND0. rewrite BND0 in H. lia.
  }
  assert (Z.pos (digits2_pos mx) <= prec).
  { 
    destruct (Z_le_gt_dec (Z.pos (digits2_pos mx)) prec).
    assumption.
    lia.
  }
  assert (Z.pos mx < 2 ^ prec).
  { 
    rewrite Digits.Zpos_digits2_pos in H0.
    apply Digits.Zpower_gt_Zdigits in H0. simpl in H0. 
    assumption.
  }
  repeat split; try lia.
  intros.
  assert (Z.pos (digits2_pos mx) <= prec - 1).
  { 
    rewrite Digits.Zpos_digits2_pos.
    apply Digits.Zdigits_le_Zpower. simpl.
    assumption.
  }
  destruct (Z.max_spec (Z.pos (digits2_pos mx) + ex - prec) (3 - emax - prec)) as [[??]|[??]]. 
  rewrite H5 in BND0. lia.
  rewrite H5 in BND0. lia.
Qed.
...
(*- E_COMPCERT_FTR_Function_IEEE754_extra_Bconv_001 *)
