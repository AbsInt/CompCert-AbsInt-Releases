...
Definition to_bits_int_arith (s: bool) mx ex :=
  ...
  let m' := if 0 <=? m then m else (Zpos mx) in
  let e := if 0 <=? m then (ex + 1075) else 0 in
...
Proof.
  ...
  destruct (0 <=? (Zpos mx - 2 ^ 52)) eqn:Esubn.
...
Proof.
  ...
  split.
  destruct (0 <=? (Zpos mx - 2 ^ 52)) eqn:Esubn; subst m'; subst e.
  ...
  split.
  destruct (0 <=? (Zpos mx - 2 ^ 52)) eqn:Esubn; subst e; lia.
  destruct (0 <=? (Zpos mx - 2 ^ 52)) eqn:Esubn; subst m'; lia.
Qed.
...
Lemma exponent_to_bits:
  forall s mx ex bnd,
    Int64.shru (Int64.shl (to_bits (B754_finite _ _ s mx ex bnd)) (Int64.one)) (Int64.repr 53) = Int64.repr (if 0 <=? (Zpos mx - Zpower 2 52)%Z then (ex + 1075) else 0).
Proof.
   intros. rewrite to_bits_int_arith_to_bits. unfold to_bits_int_arith, join_bits_arith.
  destruct (IEEE754_extra.binary_float_bounds _ _ _ _ bnd) as (?&?&?&_).
