...
Lemma areg_of_not_RA':
  forall r r', areg_of r = OK r' -> AREG r' <> RA.
Proof.
  intros. red; intros. destruct r; cbn in H; congruence.
Qed.
...
Global Hint Resolve ireg_of_not_TMP ireg_of_not_TMP' data_preg_not_TMP  areg_of_not_TMPA areg_of_not_TMPA' data_preg_not_TMPA
  data_preg_not_PC preg_of_dreg_not_TMP preg_of_areg_not_TMPA areg_of_not_SP areg_of_not_RA data_preg_not_PCXI
  areg_of_not_RA' areg_of_not_SP' preg_of_not_P12 preg_of_not_PCXI : asmgen.
...
Lemma make_epilogue_correct:
  ...
  exists rs1, exists m1',
     exec_straight ge fn (make_epilogue f k) rs tm k rs1 m1'
  /\ agree_inj j ms (parent_sp cs) rs1
  /\ rs1#RA = parent_ra cs
  /\ (forall r, r <> PC -> r <> RA -> r <> SP -> r <> TMP -> r <> TMPA -> rs1#r = rs#r)
  /\ m1' |= minjection j m1 ** globalenv_inject ge0 j ** P.
Proof.
  intros until tm; intros LP LRA FREE AG STKINJ MCS SEP.
  exploit loadv_parallel_rule. eapply sep_proj1. exact SEP. eexact LP.
  apply Val.offset_ptr_inject. apply AG.
  intros (parent' & LP' & IP').
  exploit loadv_parallel_rule. eapply sep_proj1. exact SEP. eexact LRA.
  apply Val.offset_ptr_inject. apply AG.
  intros (ra' & LRA' & IRA').
  ...
  exploit (loadind_ptr_correct A10 (fn_retaddr_ofs f) RA (Pfreeframe (fn_stacksize f) (fn_link_ofs f) :: k) rs tm).
    reflexivity. exact LRA'.
  intros (rs1 & A1 & B1 & C1).
  ...
  eapply exec_straight_trans. eexact A1. apply exec_straight_one. simpl.
    rewrite (C1 A10) by auto with asmgen. change (chunk_of_type Tptr) with Mint32 in LP'. rewrite LP'.
    rewrite H0.
    rewrite FREE'. eauto. auto.
  split. apply agree_inj_nextinstr.
    apply agree_inj_change_sp with (Vptr stk soff).
    apply agree_inj_exten with rs; auto. intros; apply C1; auto with asmgen.
    eapply parent_sp_def; eauto.
    assumption.
  Simpl.
  split. eapply ra_injects_flat.
    eassumption. apply sep_proj2, sep_proj1 in SEP. exact SEP.
    congruence.
  split. intros; Simpl.
  assumption.
Qed.
