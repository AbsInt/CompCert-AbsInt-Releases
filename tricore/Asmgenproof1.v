...
Proof.
  intros. rewrite <- (areg_of_eq _ _ H0). eapply preg_val; eauto.
Qed.
...
Global Hint Resolve ireg_of_not_TMP ireg_of_not_TMP' data_preg_not_TMP  areg_of_not_TMPA areg_of_not_TMPA' data_preg_not_TMPA
  data_preg_not_PC preg_of_dreg_not_TMP preg_of_areg_not_TMPA areg_of_not_SP areg_of_not_RA: asmgen.
...
Lemma move_rr_correct:
  forall rd r k rs m pcl,
  exists rs',
     exec_straight_opt ge fn (move_rr rd r k) rs m  k rs' m pcl
  /\ rs'#rd = rs#r
...
Lemma loadimm_correct:
 forall rd n k rs m pcl,
  exists rs',
     exec_straight ge fn (loadimm rd n k) rs m k rs' m pcl
  /\ rs'#rd = Vint n
...
Lemma loadimm_addr_correct:
 forall rd n k rs m pcl,
  exists rs',
     exec_straight ge fn (loadimm_addr rd n k) rs m k rs' m pcl
  /\ rs'#rd = Vint n
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
...
Lemma select_encoding_instr_correct':
  forall (instr1: dreg -> dreg -> instruction)
    ...
    (sem: val -> val -> val)
    commut rd r1 r2 k k' rs m pcl,
  (forall v1 v2, commut = true -> sem v1 v2 = sem v2 v1) ->
  (forall rd r1 rs m,
    exec_instr ge fn (instr1 rd r1) rs m pcl =
    Next (nextinstr (rs#rd <- (sem rs#rd rs#r1))) m pcl) ->
  (forall rd r1 r2 rs m,
    exec_instr ge fn (instr2 rd r1 r2) rs m pcl =
    Next (nextinstr (rs#rd <- (sem rs#r1 rs#r2))) m pcl) ->
  exists rs',
    exec_straight ge fn (select_encoding_instr instr1 instr2 commut rd r1 r2 k ++ k') rs m (k++k') rs' m pcl
  /\ rs'#rd = sem rs#r1 rs#r2
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
  intros. unfold select_encoding_instr.
  assert (BASE: exists rs',
           exec_straight ge fn (instr2 rd r1 r2 :: k) rs m k rs' m pcl
           /\ rs'#rd = sem (rs r1) (rs r2)
           /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r).
...
Lemma select_encoding_instr_correct:
  forall (instr1: dreg -> dreg -> instruction)
    (instr2: dreg -> dreg -> dreg -> instruction)
    (sem: val -> val -> val)
    commut rd r1 r2 k rs m pcl,
  (forall v1 v2, commut = true -> sem v1 v2 = sem v2 v1) ->
  (forall rd r1 rs m pcl,
    exec_instr ge fn (instr1 rd r1) rs m pcl =
    Next (nextinstr (rs#rd <- (sem rs#rd rs#r1))) m pcl) ->
  (forall rd r1 r2 rs m pcl,
    exec_instr ge fn (instr2 rd r1 r2) rs m pcl =
    Next (nextinstr (rs#rd <- (sem rs#r1 rs#r2))) m pcl) ->
  exists rs',
    exec_straight ge fn (select_encoding_instr instr1 instr2 commut rd r1 r2 k) rs m k rs' m pcl
  /\ rs'#rd = sem rs#r1 rs#r2
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
...
Lemma select_encoding_instr_correct_aa:
  ...
  (forall rd r1 r2 rs m,
    ...
    Next (nextinstr (rs#rd <- (sem rs#r1 rs#r2))) m pcl) ->
  exists rs',
    exec_straight ge fn (select_encoding_instr_aa instr1 instr2 commut rd r1 r2 k) rs m k rs' m pcl
  /\ rs'#rd = sem rs#r1 rs#r2
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
...
Lemma add_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn (add rd r1 r2 k) rs m k rs' m pcl
...
Lemma add_aa_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn (add_aa rd r1 r2 k) rs m k rs' m pcl
  /\ rs'#rd = Val.add (rs#r1) (rs#r2)
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
...
Lemma add_d_correct: forall (rd r1 : dreg) (r2: areg) (k : code) (rs : regset) (m : mem) pcl,
 rd <> TMP -> exists rs' : regset,
  exec_straight ge fn (add_d rd r1 r2 k) rs m k rs' m pcl /\
...
Lemma sub_correct':
  forall rd r1 r2 k k' rs m pcl,
  exists rs',
  exec_straight ge fn ((sub rd r1 r2 k) ++ k') rs m (k++k') rs' m pcl
...
Lemma sub_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn ((sub rd r1 r2 k)) rs m (k) rs' m pcl
  /\ rs'#rd = Val.sub (rs#r1) (rs#r2)
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
...
Lemma mul_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn (mul rd r1 r2 k) rs m k rs' m pcl
...
Lemma and_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn (and rd r1 r2 k) rs m k rs' m pcl
...
Lemma or_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn (or rd r1 r2 k) rs m k rs' m pcl
...
Lemma xor_correct:
  forall rd r1 r2 k rs m pcl,
  exists rs',
  exec_straight ge fn (xor rd r1 r2 k) rs m k rs' m pcl
...
Lemma addimm_correct:
  forall (rd r : dreg) n k rs m pcl,
  exists rs',
    exec_straight ge fn (addimm rd r n k) rs m k rs' m pcl
...
Lemma addimm_addr_correct:
  forall (rd r : areg) n k rs m pcl,
  exists rs',
    exec_straight ge fn (addimm_addr rd r n k) rs m k rs' m pcl
  /\ rs'#rd = Val.add (rs#r) (Vint n)
...
Lemma rsubimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (rsubimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* rsub *)
    ...
    split; intuition Simpl.
  - (* loadimm + sub *)
    destruct (loadimm_correct TMP n (sub rd TMP r k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (sub_correct rd TMP r k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split.
...
Lemma mulimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (mulimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* loadimm + mul *)
    destruct (loadimm_correct TMP n (mul rd r TMP k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (mul_correct rd r TMP k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split.
    eapply exec_straight_trans. eexact EX. eexact EX'.
    rewrite RES'. rewrite RES. rewrite (OTH r); try congruence.
    split. reflexivity. intros. rewrite OTH'; auto.
...
Lemma slimm_correct:
  forall (rd r : dreg) (n: amount32) k rs m pcl,
  r <> TMP ->
  exists rs',
    exec_straight ge fn (slimm rd r n k) rs m k rs' m pcl
...
Lemma lsrimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (lsrimm rd r n k) rs m k rs' m pcl
...
Lemma asrimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (asrimm rd r n k) rs m k rs' m pcl
...
Lemma maddimm_correct:
  forall (rd r1 r2: dreg) n k rs m pcl,
  r1 <> TMP ->
  r2 <> TMP ->
  exists rs',
    exec_straight ge fn (maddimm rd r1 r2 n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* madd_sc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + madd *)
    destruct (loadimm_correct TMP n (Pmadd rd r1 r2 TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    econstructor; split.
...
Lemma msubimm_correct:
  ...
  r2 <> TMP ->
  exists rs',
    exec_straight ge fn (msubimm rd r1 r2 n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* msub_sc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + msub *)
    destruct (loadimm_correct TMP n (Pmsub rd r1 r2 TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    econstructor; split.
    eapply exec_straight_trans. eexact EX.
    apply exec_straight_one; simpl; reflexivity.
    rewrite RES. rewrite !OTH; auto with asmgen.
...
Lemma andimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (andimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* and_ruc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + and *)
    destruct (loadimm_correct TMP n (and rd r TMP k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (and_correct rd r TMP k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split. apply (exec_straight_trans ge fn _ _ m _ _ m _ _ _ _ EX EX').
...
Lemma nandimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (nandimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* nand_uc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + nand *)
    destruct (loadimm_correct TMP n (Pnand rd r TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    econstructor; split.
    eapply exec_straight_trans. eexact EX.
    apply exec_straight_one; simpl; reflexivity.
    rewrite RES. rewrite OTH; auto with asmgen.
...
Lemma orimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (orimm rd r n k) rs m k rs' m pcl
...
- (* orn_uc9 *)
  ...
  - (* or_ruc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + or *)
    destruct (loadimm_correct TMP n (or rd r TMP k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (or_correct rd r TMP k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split. apply (exec_straight_trans ge fn _ _ m _ _ m _ _ _ _ EX EX').
    rewrite RES', RES, (OTH r); auto with asmgen.
    split; auto. intros. rewrite OTH', OTH; auto with asmgen.
Qed.
...
Lemma norimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (norimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* nor_uc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + nor *)
    destruct (loadimm_correct TMP n (Pnor rd r TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    econstructor; split.
    eapply exec_straight_trans. eexact EX.
    apply exec_straight_one; simpl; reflexivity.
    rewrite RES. rewrite OTH; auto with asmgen.
...
Lemma xorimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (xorimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* xor_uc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + xor *)
    destruct (loadimm_correct TMP n (xor rd r TMP k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (xor_correct rd r TMP k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split. apply (exec_straight_trans ge fn _ _ m _ _ m _ _ _ _ EX EX').
    rewrite RES', RES, (OTH r); auto with asmgen.
    split; auto. intros. rewrite OTH', OTH; auto with asmgen.
Qed.
...
Lemma xnorimm_correct:
  ...
  r <> TMP ->
  exists rs',
    exec_straight ge fn (xnorimm rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  - (* xnor_uc9 *)
    ...
    split; intuition Simpl.
  - (* loadimm + xnor *)
    destruct (loadimm_correct TMP n (Pxnor rd r TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    econstructor; split.
    eapply exec_straight_trans. eexact EX.
    apply exec_straight_one; simpl; reflexivity.
    rewrite RES. rewrite OTH; auto with asmgen.
...
Lemma transl_cbranch_int32s_correct:
  forall cmp (r1 r2: dreg) lbl (rs: regset) m b pcl,
  Val.cmp_bool cmp rs#r1 rs#r2 = Some b ->
  exec_instr ge fn (transl_cbranch_int32s cmp r1 r2 lbl) rs m pcl =
  eval_branch fn lbl rs m (Some b) pcl.
Proof.
...
Lemma transl_cbranch_int32u_correct:
  ...
  exec_instr ge fn (transl_cbranch_int32u cmp r1 r2 lbl) rs m pcl =
  eval_branch fn lbl rs m (Some b) pcl.
Proof.
  intros. destruct cmp; simpl; rewrite ? H; auto.
...
Lemma transl_cbranch_int32s_imm_correct:
  forall cmp n r lbl k c m b (rs: regset) pcl,
  r <> TMP ->
  transl_cbranch_int32s_imm cmp n r lbl k = c ->
  Val.cmp_bool cmp (rs r) (Vint n) = Some b ->
  exists rs', exists insn,
     exec_straight_opt ge fn c rs m (insn :: k) rs' m pcl
  /\ exec_instr ge fn insn rs' m pcl = eval_branch fn lbl rs' m (Some b) pcl
  /\ forall r, r <> PC -> r <> TMP -> rs'#r = rs#r.
Proof.
  unfold transl_cbranch_int32s_imm. intros.
  assert (DEF:
           exists (rs' : regset) (insn : instruction),
           exec_straight_opt ge fn (loadimm D0 n (transl_cbranch_int32s cmp r D0 lbl :: k)) rs m (insn :: k) rs' m pcl /\
             exec_instr ge fn insn rs' m pcl = eval_branch fn lbl rs' m (Some b) pcl /\
...
Proof.
 ...
 assert (DEF:
           exists (rs' : regset) (insn : instruction),
           exec_straight_opt ge fn (loadimm D0 n (transl_cbranch_int32u cmp r D0 lbl :: k)) rs m (insn :: k) rs' m pcl /\
             exec_instr ge fn insn rs' m pcl = eval_branch fn lbl rs' m (Some b) pcl /\
             (forall r0 : preg, r0 <> PC -> r0 <> D0 -> rs' r0 = rs r0)).
  {
    exploit (loadimm_correct TMP n); eauto. intros (rs' & A & B & C).
...
Lemma transl_cond_single_branch_correct:
  forall cmp (rd r1 r2: dreg) rs k m pcl,
  exists rs',
    exec_straight ge fn (transl_cond_single_branch cmp rd r1 r2 k) rs m k rs' m pcl
...
Lemma switch_A_D_via_TMP_correct:
  forall ra rd k rs m pcl,
  exists rs',
     exec_straight ge fn (switch_A_D_via_TMP ra rd k) rs m k rs' m pcl
...
Lemma switch_A_D_via_TMPA_correct:
  forall ra rd k rs m pcl,
  exists rs',
     exec_straight ge fn (switch_A_D_via_TMPA ra rd k) rs m k rs' m pcl
...
Lemma transl_cond_int32s_correct:
  forall cmp rd r1 r2 k rs m pcl,
  exists rs',
     exec_straight ge fn (transl_cond_int32s cmp rd r1 r2 :: k) rs m k rs' m pcl
...
Lemma transl_cond_int32u_correct:
  forall cmp rd r1 r2 k rs m pcl,
  exists rs',
     exec_straight ge fn (transl_cond_int32u cmp rd r1 r2 :: k) rs m k rs' m pcl
...
Lemma transl_cond_addr_correct:
  forall cmp rd r1 r2 k rs m pcl,
  exists rs',
     exec_straight ge fn (transl_cond_addr cmp rd r1 r2 :: k) rs m k rs' m pcl
  /\ rs'#rd = Val.cmpu (Mem.valid_pointer m) cmp rs#r1 rs#r2
  /\ forall r, r <> PC -> r <> rd -> rs'#r = rs#r.
Proof.
  intros. destruct cmp; simpl.
...
Lemma transl_condimm_int32s_correct:
  forall cmp rd r n k rs m pcl,
  r <> TMP ->
  exists rs',
     exec_straight ge fn (transl_condimm_int32s cmp rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  assert (DEF:
           exists rs',
           exec_straight ge fn (loadimm TMP n (transl_cond_int32s cmp rd r TMP :: k)) rs m k rs' m pcl
           /\ Val.lessdef (Val.cmp cmp rs#r (Vint n)) rs'#rd
           /\ forall r, r <> PC -> r <> TMP -> r <> rd -> rs'#r = rs#r).
  {
    destruct (loadimm_correct TMP n (transl_cond_int32s cmp rd r TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (transl_cond_int32s_correct cmp rd r TMP k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split.
    eapply exec_straight_trans. eexact EX. eexact EX'.
...
Lemma transl_condimm_int32u_correct:
  ...
  r <> TMP ->
  exists rs',
     exec_straight ge fn (transl_condimm_int32u cmp rd r n k) rs m k rs' m pcl
...
Proof.
  ...
  assert (DEF:
           exists rs',
           exec_straight ge fn (loadimm TMP n (transl_cond_int32u cmp rd r TMP :: k)) rs m k rs' m pcl
           /\ Val.lessdef (Val.cmpu (Mem.valid_pointer m) cmp rs#r (Vint n)) rs'#rd
           /\ forall r, r <> PC -> r <> TMP -> r <> rd -> rs'#r = rs#r).
  {
    destruct (loadimm_correct TMP n (transl_cond_int32u cmp rd r TMP :: k) rs m pcl) as [rs' [EX [RES OTH]]].
    destruct (transl_cond_int32u_correct cmp rd r TMP k rs' m pcl) as [rs'' [EX' [RES' OTH']]].
    exists rs''; split.
    eapply exec_straight_trans. eexact EX. eexact EX'.
    rewrite RES'. rewrite RES. rewrite (OTH r); try congruence.
    split; auto. intros. rewrite OTH'; auto.
...
Lemma transl_condimm_addr_correct:
  forall cmp rd r n k rs m pcl c,
  transl_condimm_addr cmp rd r n = Some c ->
  exists rs',
     exec_straight ge fn (c :: k) rs m k rs' m pcl
...
Lemma notbool_nor_or:
  forall rd r1 r2 bit_1 bit_2 k rs m pcl (inv : bool),
  exists rs',
    exec_straight ge fn ((if inv then Pnor_t rd r1 bit_1 r2 bit_2 else Por_t rd r1 bit_1 r2 bit_2) :: k) rs m k rs' m pcl
...
Lemma transl_cond_single_op_correct:
  forall cmp rd r1 r2 k rs m pcl inv,
  exists rs',
     exec_straight ge fn (transl_cond_single_op inv cmp rd r1 r2 k) rs m k rs' m pcl
...
Proof.
  ...
  - subst cmp. simpl.
    ...
    destruct (notbool_nor_or rd rd rd bit1 bit1 k rs' m pcl (negb inv)) as [rs'' [EX [RES OTH]]].
  ...
  - assert (exists bit_1 bit_2,
    ...
    rewrite H2.
    set (rs' := nextinstr rs # rd <- (cmpfs (rs r1) (rs r2))).
    destruct (notbool_nor_or rd rd rd bit_1 bit_2 k rs' m pcl inv) as [rs'' [EX [RES OTH]]].
    econstructor; split.
...
Lemma translate_bin_comp_correct cmp rd a1 a2 k c get_val rs m pcl:
  translate_bin_comp rd a1 a2 cmp k = OK c ->
  (forall a1 a2 rs k, exists rs' : regset,
    exec_straight ge fn (transl_cond_int32s cmp rd a1 a2 :: k) rs m k rs' m pcl 
    /\ Val.lessdef (get_val (rs a1) (rs a2)) (rs' rd) 
    /\ (forall r : preg, r <> PC -> r <> rd -> rs' r = rs r)) ->
  exists rs',
    exec_straight ge fn c rs m k rs' m pcl
...
Lemma translate_bin_compu_correct cmp rd a1 a2 k c get_val rs m pcl:
  translate_bin_compu rd a1 a2 cmp k = OK c ->
  (forall a1 a2 rs k, exists rs' : regset,
  exec_straight ge fn (transl_cond_int32u cmp rd a1 a2 :: k) rs m k rs' m pcl /\
  Val.lessdef (get_val (rs a1) (rs a2)) (rs' rd) /\
  (forall r : preg, r <> PC -> r <> rd -> rs' r = rs r)) ->
  (forall a1 a2 rs k, exists rs' : regset,
  exec_straight ge fn (transl_cond_addr cmp rd a1 a2 :: k) rs m k rs' m pcl /\
  Val.lessdef (get_val (rs a1) (rs a2)) (rs' rd) /\
  (forall r : preg, r <> PC -> r <> rd -> rs' r = rs r)) ->
  exists rs',
    exec_straight ge fn c rs m k rs' m pcl
...
Lemma translate_imm_comp_correct cond rd a1 k c n get_val rs m pcl:
  translate_imm_comp rd a1 cond n k = OK c ->
  (forall a1 rs k, a1 <> TMP -> exists rs' : regset,
  exec_straight ge fn (transl_condimm_int32s cond rd a1 n k) rs m k rs' m pcl /\
...
Lemma translate_imm_compu_correct cmp rd a1 k c n get_val rs m pcl:
  translate_imm_compu rd a1 cmp n k = OK c ->
  (forall a1 rs k, a1 <> TMP -> exists rs' : regset,
  exec_straight ge fn (transl_condimm_int32u cmp rd a1 n k) rs m k rs' m pcl /\
  Val.lessdef (get_val (rs a1) (Vint n)) (rs' rd) /\
  (forall r : preg, r <> PC -> r <> rd -> r <> TMP -> rs' r = rs r)) ->
  (forall a1 rs k c,
...
Lemma transl_cond_op_correct:
  forall cond rd args k c rs m pcl,
...
Proof.
  ...
  - (* cmpuimm *)
    ...
    + intros (rs' & A & B & C). exists rs'. eauto.
  - (* cmpfs *)
    destruct ( transl_cond_single_op_correct c0 rd x x0 k rs m pcl false) as [rs' [EX [RES OTH]]].
    exists rs'. split; eauto. rewrite RES.
    fold (Val.cmpfs c0 (rs x) (rs x0)).
    split; auto.
  - (* notcmpfs *)
    destruct ( transl_cond_single_op_correct c0 rd x x0 k rs m pcl true) as [rs' [EX [RES OTH]]].
    exists rs'. split; eauto. rewrite RES.
...
Lemma translate_bin_cbranch_correct r1 r2 cond transl_f lbl k c f (rs: regset) m pcl:
...
exec_instr ge fn (transl_f r1 r2 lbl) rs m pcl =
  eval_branch fn lbl rs m (Some b) pcl)
...
Lemma transl_cbranch_correct_1:
  forall cond args lbl k c m pcl ms b sp rs m',
...
Lemma transl_cbranch_correct_true:
  forall cond args lbl k c m pcl ms sp rs m',
  transl_cbranch cond args lbl k = OK c ->
  ...
  exists rs', exists insn,
     exec_straight_opt ge fn c rs m' (insn :: k) rs' m' pcl
  /\ exec_instr ge fn insn rs' m' pcl = goto_label fn lbl rs' m' pcl
  /\ forall r, r <> PC -> r <> TMP -> r <> TMPA -> rs'#r = rs#r.
Proof.
...
Lemma transl_op_correct:
  forall op args res k (rs: regset) m pcl v c,
...
Opaque Int.eq.   Opaque preg_eq.
  ...
  - (* andimm *)
    ...
    + exploit (andimm_correct x x n); eauto with asmgen.
      ...
      eexact A. rewrite B.
      split; intros; Simpl.
      rewrite C; eauto with asmgen. Simpl.
  - (* select *)
    destruct r; inv EQ2.
    destruct (preg_of r) eqn:?; destruct r0; inv H0;
    destruct (preg_of r0) eqn:?; inv H2.
    + exploit transl_cond_op_correct; eauto. intros (rs' & A & B & C).
      simpl. rewrite Heqp, Heqp0.
      econstructor; split. eapply exec_straight_trans. eexact A.
      apply exec_straight_one; simpl; auto.
      erewrite get_pairs_singles; eauto. split.
      destruct (eval_condition c0 rs ## (preg_of ## x0) m) as [b|]; simpl in *; auto.
      Simpl. destruct b; inv B; simpl; change (Int.eq Int.one Int.zero) with false; change (Int.eq Int.zero Int.zero) with true; simpl;
        rewrite C; eauto with asmgen; apply Val.lessdef_normalize.
      intros.  Simpl.
    + erewrite get_pairs_singles; eauto.
      destruct (dreg_eq x r2).
      * subst x.
        exploit transl_cond_op_correct; eauto. intros (rs' & A & B & C).
        exploit switch_A_D_via_TMPA_correct. intros (rs'' & A' & B' & C' & D').
        exploit switch_A_D_via_TMP_correct. intros (rs''' & A'' & B'' & C'' & D'').
        exists rs'''. repeat split.
         ...
         rewrite D''; auto with asmgen.
         Simpl. rewrite D'; auto with asmgen.
        destruct (eval_condition c0 rs ## (preg_of ## x0) m) as [b|]; simpl in *; auto.
        assert (rs' TMP = if b then Vtrue else Vfalse) by (destruct b, (rs' TMP); inv B; auto).
        rewrite H. rewrite (D' r2); eauto with asmgen.
        rewrite C; eauto with asmgen.  rewrite <- B'; eauto with asmgen.
        rewrite C; eauto with asmgen.
        destruct b; simpl; change (Int.eq Int.one Int.zero) with false; change (Int.eq Int.zero Int.zero) with true; simpl.
        rewrite Heqp. apply Val.lessdef_normalize.
        rewrite Heqp0. apply Val.lessdef_normalize.
        apply preg_of_areg_not_TMPA in Heqp0. congruence.
        destruct (dreg_eq r2 D2); congruence.
        destruct (dreg_eq r2 D2); congruence.
        apply preg_of_dreg_not_TMP in Heqp; congruence.
        destruct (dreg_eq r2 D2); congruence.
        intros. rewrite <- C; eauto with asmgen.
        destruct (preg_eq r4 (AREG r3)).
        subst. rewrite <- C''.
        Simpl. rewrite B'.
        rewrite Pregmap.gso; auto.  destruct (dreg_eq r2 D2); congruence.
        apply preg_of_areg_not_TMPA in Heqp0; congruence.
        destruct (dreg_eq r2 D2); congruence.
        destruct (preg_eq r4 (DREG (if dreg_eq r2 D2 then D3 else D2))).
        subst. rewrite <- B''; eauto with asmgen.
        rewrite D''; eauto with asmgen. Simpl.
      *  exploit transl_cond_op_correct; eauto. intros (rs' & A & B & C).
         econstructor; split.
         eapply exec_straight_trans. eexact A. eapply exec_straight_two; simpl; eauto.
         split; Simpl.
         destruct (eval_condition c0 rs ## (preg_of ## x0) m) as [b|]; simpl in *; auto.
         assert (rs' x = if b then Vtrue else Vfalse) by (destruct b, (rs' x); inv B; auto).
         rewrite H. rewrite !C; eauto with asmgen.
         destruct b; simpl; change (Int.eq Int.one Int.zero) with false; change (Int.eq Int.zero Int.zero) with true; simpl;
           [rewrite Heqp| rewrite Heqp0];  apply Val.lessdef_normalize.
         intros. Simpl.
    + erewrite get_pairs_singles; eauto.
      destruct (dreg_eq x r3); subst.
      * exploit transl_cond_op_correct; eauto. intros (rs' & A & B & C).
        exploit switch_A_D_via_TMPA_correct. intros (rs'' & A' & B' & C' & D').
        exploit switch_A_D_via_TMP_correct. intros (rs''' & A'' & B'' & C'' & D'').
        exists rs'''. repeat split.
        eapply exec_straight_trans. eexact A. eapply exec_straight_trans. eexact A'.
        eapply exec_straight_step. reflexivity. Simpl. eassumption.
        rewrite D''; eauto with asmgen.
        Simpl. rewrite D'; auto with asmgen.
        destruct (eval_condition c0 rs ## (preg_of ## x0) m) as [b|]; simpl in *; auto.
        assert (rs' TMP = if b then Vtrue else Vfalse) by (destruct b, (rs' TMP); inv B; auto).
        rewrite H. rewrite (D' r3); eauto with asmgen.
        rewrite C; eauto with asmgen.  rewrite <- B'; eauto with asmgen.
        rewrite C; eauto with asmgen.
        destruct b; simpl; change (Int.eq Int.one Int.zero) with false; change (Int.eq Int.zero Int.zero) with true; [rewrite Heqp | rewrite Heqp0];
                           simpl ; apply Val.lessdef_normalize.
        apply preg_of_areg_not_TMPA in Heqp. congruence.
        destruct (dreg_eq r3 D2); congruence.
        destruct (dreg_eq r3 D2); congruence.
        destruct (dreg_eq r3 D2); congruence.
        intros. rewrite <- C; eauto with asmgen.
        destruct (preg_eq r4 (AREG r2));subst.
        rewrite <- C''. Simpl. rewrite B'. rewrite Pregmap.gso; auto.
        destruct (dreg_eq r3 D2); congruence.
        apply preg_of_areg_not_TMPA in Heqp; congruence.
        destruct (dreg_eq r3 D2); congruence.
        destruct (preg_eq r4 (DREG (if dreg_eq r3 D2 then D3 else D2))).
        subst. rewrite <- B''; eauto with asmgen.
         rewrite D''; eauto with asmgen. Simpl.
      * exploit transl_cond_op_correct; eauto. intros (rs' & A & B & C).
        econstructor; split. eapply exec_straight_trans. eexact A.
        ...
        simpl; apply Val.lessdef_normalize.
        intros. Simpl.
    + erewrite get_pairs_singles; eauto. destruct (areg_eq r2 r3); subst.
      * inv H0. econstructor; split. apply exec_straight_one; simpl; auto. split; intros; Simpl.
        destruct (eval_condition c0 rs ## (preg_of ## x0) m); auto. simpl.
        rewrite Heqp, Heqp0. destruct b; apply Val.lessdef_normalize.
      * exploit transl_cond_op_correct; eauto. intros (rs' & A & B & C).
        exploit switch_A_D_via_TMP_correct. intros (rs'' & A' & B' & C' & D').
        exploit switch_A_D_via_TMP_correct. intros (rs''' & A'' & B'' & C'' & D'').
        exists rs'''. repeat split.
        eapply exec_straight_trans. eexact A. eapply exec_straight_trans. eexact A'.
        eapply exec_straight_step. reflexivity. Simpl. eapply exec_straight_step. reflexivity.
        Simpl. eexact A''.
        rewrite D''; eauto with asmgen.
        ...
        rewrite C; eauto with asmgen.
        rewrite Pregmap.gso.
        rewrite <- B'; eauto with asmgen.
        rewrite C; eauto with asmgen.
        destruct b; simpl; change (Int.eq Int.one Int.zero) with false; change (Int.eq Int.zero Int.zero) with true; [rewrite Heqp | rewrite Heqp0];
          simpl ; apply Val.lessdef_normalize.
        destruct (dreg_eq x D2); congruence.
        destruct (dreg_eq x D2); congruence.
        destruct (dreg_eq x D2); congruence.
        intros. rewrite <- C; eauto with asmgen.
        destruct (preg_eq r4 (AREG r2)).
        subst. rewrite <- C''.
        Simpl. rewrite B'.
        rewrite Pregmap.gso; auto.
        Simpl. rewrite Pregmap.gso; auto. destruct (dreg_eq x D2); congruence.
        destruct (dreg_eq x D2); congruence.
        destruct (dreg_eq x D2); congruence.
        destruct (preg_eq r4 (DREG (if dreg_eq x D2 then D3 else D2))).
  ...
  - (* msubimm *)
    ...
    + destruct (dreg_eq x0 x); inv H3.
      ...
      simpl. rewrite Eres. Simpl. intros. rewrite C; eauto with asmgen. Simpl.
  - (* mul *)
...
Lemma indexed_memory_access_correct:
  forall mk1 mk2 (base : index_reg) ofs k (rs: regset) m pcl,
    valid_index_reg base rs pcl = true ->
  (exists base' ofs' rs',
      valid_index_reg base' rs' pcl = true
     /\ exec_straight_opt ge fn (indexed_memory_access mk1 mk2 base ofs k) rs m
         (mk2 base' ofs' :: k) rs' m pcl
     /\ Val.offset_ptr rs'#base' (Ptrofs.of_int ofs') = Val.offset_ptr rs#base ofs
     /\ forall r, r <> PC -> r <> TMPA -> rs'#r = rs#r)
  \/ (exists base' rs',
        exec_straight_opt ge fn (indexed_memory_access mk1 mk2 base ofs k) rs m (mk1 base' :: k) rs' m pcl
...
Lemma indexed_load_access_correct:
  forall chunk
    ...
    (mk2: index_reg -> sconst16 -> instruction) rd m pcl,
  (forall base rs,
     exec_instr ge fn (mk1 base) rs m pcl = exec_load chunk rs m pcl rd base sconst16_zero) ->
  (forall base ofs rs,
      valid_index_reg base rs pcl = true ->
      exec_instr ge fn (mk2 base ofs) rs m pcl = exec_load chunk rs m pcl rd base ofs) ->
  forall (base: index_reg) ofs k (rs: regset) v,
  valid_index_reg base rs pcl = true ->
  Mem.loadv chunk m (Val.offset_ptr rs#base ofs) = Some v ->
  rd <> PC ->
  exists rs',
     exec_straight ge fn (indexed_memory_access mk1 mk2 base ofs k) rs m k rs' m pcl
  /\ rs'#rd = v
  /\ forall r, r <> PC -> r <> TMPA -> r <> rd -> rs'#r = rs#r.
Proof.
  intros until pcl; intros EXEC1 EXEC2; intros until v; intros IR LOAD NOTPC.
...
Lemma indexed_store_access_correct:
  forall chunk
    (mk1: areg -> instruction)
    (mk2: index_reg -> sconst16 -> instruction) r1 m pcl,
  (forall base rs,
     exec_instr ge fn (mk1 base) rs m pcl = exec_store chunk rs m pcl r1 base sconst16_zero) ->
  (forall base ofs rs,
      valid_index_reg base rs pcl = true ->
     exec_instr ge fn (mk2 base ofs) rs m pcl = exec_store chunk rs m pcl r1 base ofs) ->
  ...
  /\ forall r, r <> PC -> r <> TMPA -> rs'#r = rs#r.
Proof.
  intros until pcl; intros EXEC1 EXEC2; intros until m'; intros IR STORE NOT31' NOTPC.
  exploit indexed_memory_access_correct; eauto.
  intros. destruct H.
  - destruct H as (base' & ofs' & rs' & IV & EX & BP & OTH).
    econstructor; split.
...
Lemma loadind_correct:
  forall base ofs ty dst k c (rs: regset) m pcl v,
  valid_index_reg base rs pcl = true ->
...
Lemma storeind_correct:
  forall base ofs ty src k c (rs: regset) m m' pcl,
  valid_index_reg base rs pcl = true ->
...
Proof.
  unfold storeind.
  intros until pcl; intros IR TR STORE.
...
Lemma loadind_ptr_correct:
  forall (base: index_reg) ofs (dst: areg) k (rs: regset) m pcl v,
  valid_index_reg base rs pcl = true ->
  Mem.loadv Mptr m (Val.offset_ptr rs#base ofs) = Some v ->
  exists rs',
     exec_straight ge fn (loadind_ptr base ofs dst k) rs m k rs' m pcl
  /\ rs'#dst = v
...
Lemma storeind_ptr_correct:
  forall (base: index_reg) ofs (src: areg) k (rs: regset) m m' pcl,
  valid_index_reg base rs pcl = true ->
  Mem.storev Mptr m (Val.offset_ptr rs#base ofs) rs#src = Some m' ->
  src <> TMPA ->
  exists rs',
     exec_straight ge fn (storeind_ptr src base ofs k) rs m k rs' m' pcl
  /\ forall r, r <> PC -> r <> TMPA -> rs'#r = rs#r.
Proof.
  unfold Mptr. simpl. intros.
...
Lemma lea_correct:
  forall rd id ofs k rs m pcl,
  exists rs',
  exec_straight ge fn (lea rd id ofs k) rs m k rs' m pcl
...
Lemma transl_load_access_correct:
  forall chunk
    ...
    addr args k c rd (rs: regset) m pcl v v',
  (forall base rs,
     exec_instr ge fn (mk1 base) rs m pcl = exec_load chunk rs m pcl rd base sconst16_zero) ->
  (forall ofs rs,
     exec_instr ge fn (mk2 SP ofs) rs m pcl = exec_load chunk rs m pcl rd SP ofs) ->
  (forall (base: areg) id ofs (rs : regset),
     exec_instr ge fn (mk3 base id ofs) rs m pcl = exec_load2 chunk rs m pcl rd base (low_half ge id ofs)) ->
...
Proof.
  ...
  - (* Aglobal *)
    ...
    + (* bol *)
      ...
      Simpl. intuition Simpl.
    + (* non bol *)
      exploit (lea_correct TMPA id ofs (mk1 A14 :: k)).
      intros (rs' & EX & RES & OTH).
      econstructor; split.
      eapply exec_straight_trans. eexact EX.
...
Lemma transl_store_access_correct:
  forall chunk
    ...
    (mk2: index_reg -> sconst16 -> instruction)
    (mk3: areg -> ident -> ptrofs -> instruction)
    addr args k c r1 (rs: regset) m v m' pcl,
  (forall base rs,
     exec_instr ge fn (mk1 base) rs m pcl = exec_store chunk rs m pcl r1 base sconst16_zero) ->
  (forall ofs rs,
     exec_instr ge fn (mk2 SP ofs) rs m pcl = exec_store chunk rs m pcl r1 SP ofs) ->
  (forall (base: areg) id ofs (rs: regset),
     exec_instr ge fn (mk3 base id ofs) rs m pcl = exec_store2 chunk rs m pcl r1 base (low_half ge id ofs)) ->
  transl_memory_access mk1 mk2 mk3 addr args k = OK c ->
  eval_addressing ge rs#SP addr (map rs (map preg_of args)) = Some v ->
...
Proof.
  ...
  intros until pcl; intros INSTR1 INSTR2 INSTR3 TR EV STORE NOTPC NOTTMPA.
  unfold transl_memory_access in TR; destruct addr; ArgsInv.
  - (* Aindexed *)
    inv EV.
    destruct (Int.eq ofs Int.zero) eqn:?; inv EQ0.
...
Lemma transl_load_correct:
  forall chunk addr args dst k c (rs: regset) m pcl a v,
...
Lemma transl_store_correct:
  forall chunk addr args src k c (rs: regset) m a m' pcl,
  ...
  /\ forall r, r <> PC -> r <> TMPA -> rs'#r = rs#r.
Proof.
  intros until pcl; intros TR EV STORE.
  destruct chunk; inv TR; try monadInv H0.
...
Lemma make_epilogue_correct:
  forall ge0 f m stk soff cs m' ms rs k tm pcl,
  ...
  match_stack ge0 cs ->
  exists rs', exists tm',
     exec_straight ge fn (make_epilogue f k) rs tm k rs' tm' pcl
...
Proof.
  intros until pcl; intros LP LRA FREE AG MEXT MCS.
  ...
  simpl. auto. rewrite <- (sp_val _ _ _ AG). eexact LRA'.
  intros (rs1 & A1 & B1 & C1).
...
Lemma make_epilogue'_correct:
  forall ge0 f r m stk soff cs m' ms rs k tm pcl,
  r <> TMP ->
  load_stack m (Vptr stk soff) Tptr f.(fn_link_ofs) = Some (parent_sp cs) ->
  load_stack m (Vptr stk soff) Tptr f.(fn_retaddr_ofs) = Some (parent_ra cs) ->
  Mem.free m stk 0 f.(fn_stacksize) = Some m' ->
  ...
  match_stack ge0 cs ->
  exists rs', exists tm',
     exec_straight ge fn (make_epilogue' f r k) rs tm k rs' tm' pcl
  /\ agree ms (parent_sp cs) rs'
  /\ Mem.extends m' tm'
  /\ rs'#RA = parent_ra cs
  /\ rs'#SP = parent_sp cs
  ...
  /\ (forall r, r <> PC -> r <> RA -> r <> SP -> r <> TMP -> r <> TMPA -> rs'#r = rs#r).
Proof.
  intros until pcl; intros NOTT LP LRA FREE AG MEXT MCS.
  exploit Mem.loadv_extends. eauto. eexact LP. auto. simpl. intros (parent' & LP' & LDP').
  exploit Mem.loadv_extends. eauto. eexact LRA. auto. simpl. intros (ra' & LRA' & LDRA').
  exploit lessdef_parent_sp; eauto. intros EQ; subst parent'; clear LDP'.
  exploit lessdef_parent_ra; eauto. intros EQ; subst ra'; clear LDRA'.
  ...
  Simpl. simpl. auto. rewrite <- (sp_val _ _ _ AG). eexact LRA'.
  intros (rs1 & A1 & B1 & C1).
  econstructor; econstructor; split.
...
Section UPPER_CTX.
Local Transparent reg_cc. 
Lemma upper_ctx_regs:
  upper_ctx = (AREG A12 :: AREG A13 :: AREG A15 :: DREG D8 :: DREG D9 :: DREG D10 :: DREG D11 :: DREG D12 :: DREG D13 :: DREG D14 :: DREG D15 :: nil).
Proof.
...
Proof.
  ...
  - rewrite upper_ctx_regs.
    unfold is_auto_save in H. destruct r; (discriminate || firstorder).
...
Proof.
  ...
  - destruct (is_auto_save r) eqn:E; auto.
    exfalso. apply H, auto_save_regs_in_upper_ctx; auto.
Qed.
