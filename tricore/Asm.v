...
Inductive preg: Type :=
  ...
  | PCXI           (**r address of the previous context list *)
...
Notation "'TMPA'" := A3 (only parsing) : asm.
...
Inductive instruction: Type :=
  ...
  | Pret (sz: Z)                              (**r return instruction *)
...
Definition exec_load2 (chunk: memory_chunk) (rs: regset) (m: mem)
...
Definition exec_store2 (chunk: memory_chunk) (rs: regset) (m: mem)
...
Definition preg_of (r: mreg) : preg :=
  match r with
  | P2 => A2   | P4 => A4   | P5 => A5   | P6 => A6
  | P7 => A7   | P12 => A12 | P13 => A13 | P14 => A14
  | P15 => A15
...
Definition callee_may_modify_regs: list preg :=
  List.map preg_of (List.filter is_modifiable_by_callee all_mregs).

Definition undef_callee_may_modify_regs (rs: regset) : regset :=
  fun r =>
    if preg_eq r SP then rs r
    else 
      if In_dec preg_eq r callee_may_modify_regs
      then Vundef
      else rs r.

(** ** Task Context Handling *)

(** On the level of Asm we model context saving as an allocation of a new block for the CSA and an explicit memory store for each upper context register.
    Analogously, a context restore is modelled as a series of memory loads and freeing the CSA block.

    See TC1.6_ArchitectureManual_Volume1.pdf, Section 4, "Tasks and Functions". *)

(** [upper_ctx] contains the registers of the upper context.
    See TC1.6_ArchitectureManual_Volume1.pdf, Section 4, "Tasks and Functions", Figure 4-1 *)
Definition upper_ctx_mr := R8 :: R9 :: R10 :: R11 :: P12 :: P13 :: P14 :: P15 :: R12 :: R13 :: R14 :: R15 :: nil.
Definition upper_ctx: list preg :=
    PCXI :: PSW_C :: AREG SP :: AREG RA ::
    List.map preg_of upper_ctx_mr.

(** [save_ctx_rec base ofs ctx rs m] saves the value of each register of [ctx] in the CSA
    represented by [base]. [ofs] is the offset of the head register of [ctx] in the CSA.
    It returns the updated memory store. *)
Fixpoint save_ctx_rec (base: block) (ofs: Z) (ctx: list preg) (rs: regset) (m: mem) : option mem :=
  match ctx with
  | r::ctx' =>
      match Mem.store Many32 m base ofs (rs r) with
      | None => None
      | Some m' => save_ctx_rec base (ofs + 4) ctx' rs m'
      end
  | nil => Some m
  end.

(** [save_ctx ctx rs m] saves the context [ctx] to a newly allocated CSA, updating the value of [PCXI].
    It returns the updated memory store and register set. *)
Definition save_ctx (ctx: list preg) (rs: regset) (m: mem) : option (regset * mem) :=
  let (m1, b) := Mem.alloc m 0 csa_size in
  match save_ctx_rec b 0 ctx rs m1 with
  | None => None
  | Some m2 =>
      Some (rs#PCXI <- (Vptr b Ptrofs.zero), m2)
  end.

(** [exec_call ra pc rs m] models a function call, saving the upper context to memory and updating [RA] & [PC]. *)
Definition exec_call (ra: val) (pc: val) (rs: regset) (m: mem) : outcome :=
  match save_ctx upper_ctx rs m with
  | None => Stuck
  | Some (rs', m') => 
      Next (rs'#RA <- ra #PC <- pc) m'
  end.

(** [restore_ctx_rec base ofs ctx rs m] restores the value of each register of [ctx] in the CSA
    represented by [base]. [ofs] is the offset of the head register of [ctx] in the CSA.
    It returns the updated register set. *)
Fixpoint restore_ctx_rec (base: block) (ofs: Z) (ctx: list preg) (rs: regset) (m: mem) : option regset :=
  match ctx with
  | r::ctx' =>
    match Mem.load Many32 m base ofs with
    | None => None
    | Some v => restore_ctx_rec base (ofs + 4) ctx' (rs#r <- v) m
    end
  | nil => Some rs
  end.

(** [restore_ctx ctx rs m] restores the context [ctx] from the CSA pointed to by [PCXI].
    Restoring a context always updates [PCXI] to point to the previous context, so the CSA is freed afterwards.
    It returns the updated memory store and register set. *)
Definition restore_ctx (ctx: list preg) (rs: regset) (m: mem) : option (regset * mem) :=
  match rs#PCXI with
  | Vptr pcxib _ =>
      match restore_ctx_rec pcxib 0 ctx rs m with
      | None => None
      | Some rs' =>
          match Mem.free m pcxib 0 csa_size with
          | None => None
          | Some m' => Some (rs', m')
          end
      end
...
(** [exec_call ra pc rs m] models a function return, restoring the upper context from memory, deallocating the call frame, and updating [PC]. *)
Definition exec_ret (rs: regset) (m: mem) (sz: Z) : outcome :=
  let ra := rs#RA in
  match rs#SP with
  | Vptr stk ofs =>
      ...
      | None => Stuck
      | Some m1 =>
          match restore_ctx upper_ctx rs m1 with
          | None => Stuck
          | Some (rs1, m2) =>
              Next (rs1#PC <- ra) m2
          end
...
(* [external_call_regmem m rs] models an external function, executing a ret instruction in the end which restores the upper context. *)
Definition external_call_regmem (m: mem) (rs: regset) : option (regset * mem) :=
  restore_ctx upper_ctx (undef_callee_may_modify_regs rs) m.
...
Definition valid_index_reg (i: index_reg) (rs: regset) (m: mem) :=
  match i with
  | A10 => true
  | A12 =>
        match rs#PCXI with
        | Vptr pcxib _ =>
            (* Parent SP is saved at offset 8.
               NOTE: this only works if the head CSA holds the upper context from our parent stack frame.
                     This is not valid anymore if we implement saving lower contexts
                     or saving upper contexts in other situations than call instructions. *)
            match Mem.load Many32 m pcxib 8 with
            | Some v => Val.eq rs#A12 v
            | None => false
            end
        | _ => false
        end
...
Definition exec_instr (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
  ...
  | Pmovh_sa rd id ofs => Next (nextinstr (rs#rd <- (high_half ge id ofs))) m
  ...
  | Plea_sofs rd r id ofs => Next (nextinstr (rs#rd <- (Val.add rs#r (low_half ge id ofs)))) m
  ...
  | Peq_a rd r1 r2 =>
      Next (nextinstr (rs#rd <- (Val.cmpu (Mem.valid_pointer m) Ceq rs#r1 rs#r2))) m
  | Peqz_a rd r =>
      Next (nextinstr (rs#rd <- (Val.cmpu (Mem.valid_pointer m) Ceq rs#r (Vint Int.zero)))) m
  | Pne rd r1 r2 =>
  ...
  | Pne_a rd r1 r2 =>
      Next (nextinstr (rs#rd <- (Val.cmpu (Mem.valid_pointer m) Cne rs#r1 rs#r2))) m
  | Pnez_a rd r =>
      Next (nextinstr (rs#rd <- (Val.cmpu (Mem.valid_pointer m) Cne rs#r (Vint Int.zero)))) m
  | Plt rd r1 r2 =>
  ...
  | Paddsc_a rd r1 r2 imm =>
      ...
      (Next (nextinstr (rs#rd <- (Val.add rs#r1 v2)))) m
  | Paddih_a rd r1 imm =>
      Next (nextinstr (rs#rd <- (Val.add rs#r1 (Vint (Int.shl imm (Int.repr 16)))))) m
  ...
  | Psubc rd r1 r2 =>
      Next (nextinstr (rs#rd <- (Val.sub (Val.add (Val.sub rs#r1 rs#r2) rs#PSW_C) Vone)
                         #PSW_C <- (Val.add_carry rs#r1 (Val.notint rs#r2) rs#PSW_C))) m
  | Psubx rd r1 r2 =>
      Next (nextinstr (rs#rd <- (Val.sub rs#r1 rs#r2)
                         #PSW_C <- (Val.add_carry rs#r1 (Val.notint rs#r2) Vone))) m
  | Psub_a rd r1 r2 =>
      Next (nextinstr (rs#rd <- (Val.sub rs#r1 rs#r2))) m
  ...
  | Pldb d a ofs =>
      if valid_index_reg a rs m then
        exec_load Mint8signed rs m d a ofs
      else
        Stuck
  | Pldb_symb d a id ofs =>
      exec_load2 Mint8signed rs m d a (low_half ge id ofs)
  | Pldb_rr d a =>
  ...
  | Pldbu_symb d a id ofs =>
      exec_load2 Mint8unsigned rs m d a (low_half ge id ofs)
  | Pldbu_rr d a =>
  ...
  | Pldh_symb d a id ofs =>
      exec_load2 Mint16signed rs m d a (low_half ge id ofs)
  | Pldh_rr d a =>
  ...
  | Pldhu_symb d a id ofs =>
      exec_load2 Mint16unsigned rs m d a (low_half ge id ofs)
  | Pldhu_rr d a =>
  ...
  | Pldw_symb d a id ofs =>
      exec_load2 Mint32 rs m d a (low_half ge id ofs)
  | Pldw_rr d a =>
  ...
  | Pfldw_symb d a id ofs =>
      exec_load2 Mfloat32 rs m d a (low_half ge id ofs)
  | Pfldw_rr d a =>
  ...
  | Pldw_a_symb d a id ofs =>
      exec_load2 Many32 rs m d a (low_half ge id ofs)
  | Pldw_a_rr d a =>
  ...
  | Pstb_symb d a id ofs =>
      exec_store2 Mint8unsigned rs m d a (low_half ge id ofs)
  | Pstb_rr d a =>
  ...
  | Psth_symb d a id ofs =>
      exec_store2 Mint16unsigned rs m d a (low_half ge id ofs)
  | Psth_rr d a =>
  ...
  | Pstw_symb d a id ofs =>
      exec_store2 Mint32 rs m d a (low_half ge id ofs)
  | Pstw_rr d a =>
  ...
  | Pfstw_symb d a id ofs =>
      exec_store2 Mfloat32 rs m d a (low_half ge id ofs)
  | Pfstw_rr d a =>
  ...
  | Pstw_a_symb d a id ofs =>
      exec_store2 Many32 rs m d a (low_half ge id ofs)
  | Pstw_a_rr d a =>
  ...
  | Pallocframe sz =>
      ...
      Next (nextinstr (rs# A12 <- (rs#SP)# SP <- sp)) m1
  | Pfreeframe sz pos =>
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | exec_step_external:
      forall b ef args res rs m t rs1 rs2 m1 m2,
      rs PC = Vptr b Ptrofs.zero ->
      Genv.find_funct_ptr ge b = Some (External ef) ->
      external_call ef ge args m t res m1 ->
      extcall_arguments rs m (ef_sig ef) args ->
      external_call_regmem m1 rs = Some (rs1, m2) ->
      rs2 = (set_pair (loc_external_result (ef_sig ef)) res rs1)#PC <- (rs RA) ->
      step (State rs m) t (State rs2 m2).
...
Inductive initial_state (p: program): state -> Prop :=
  | initial_state_intro: forall m0 m1 m2 pcxi_init,
      let ge := Genv.globalenv p in
      Genv.init_mem p = Some m0 ->
      Mem.alloc m0 0 csa_size = (m1, pcxi_init) ->
      (* We store a Vnullptr in the initial CSA.
         The final ret instruction of the program loads it into SP so that [final_state] holds. *)
      Mem.store Many32 m1 pcxi_init 8 Vnullptr = Some m2 ->
      let rs0 :=
        (Pregmap.init Vundef)
        # PC <- (Genv.symbol_address ge p.(prog_main) Ptrofs.zero)
        # SP <- Vnullptr
        # RA <- Vnullptr
        # PCXI <- (Vptr pcxi_init Ptrofs.zero)
      in
...
- (* determ *)
  ...
  rewrite H11 in H5. injection H5. intros. subst. auto.
- (* trace length *)
