...
(** [exec_call ra pc rs m] models a function return, restoring the upper context from memory and updating [PC]. *)
Definition exec_ret (rs: regset) (m: mem) : outcome :=
  let ra := rs#RA in
  match restore_ctx upper_ctx rs m with
  | None => Stuck
  | Some (rs', m') =>
      Next (rs'#PC <- ra) m'
  end.
