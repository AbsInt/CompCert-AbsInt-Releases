...
Module CC.
  ...
  Definition reg_cc (r: mreg): RegCC :=
    match r with
    | R8 | R9 | R10 | R11 | R12 | R13 | R14 | R15 => Auto
    | P12 | P13 | P15 => Auto
    | _ => Caller
    end.
End CC.
...
Definition is_addr_reg (r:mreg): bool :=
  match r with
  | P2 | P3 | P4 | P5 | P6 | P7 | P12 | P13 | P15 => true
  | _ => false end. 
...
(** How to use registers for register allocation.
    ...
    a.d. FIXME one idea would be to put only the upper context registers in the preferred_regs field. But we 
    would have to test if that results in a better allocation. *)
  ...
  - destruct (typ_eq (proj_xtype_typ ty) Tlong).
    + destruct H.
      ...
      * unfold typesize in H. rewrite e in H. apply IHtyl with (ofs := ofs + typesize Tlong).
        cbn. lia. auto.
    + destruct H.
      * subst p. split. lia. destruct (proj_xtype_typ ty) eqn:E; cbn; try apply Z.divide_1_l.
        destruct ty; cbn in *; (discriminate || congruence).
