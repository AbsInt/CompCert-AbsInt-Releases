...
Lemma transf_function_not_retaddr_saved:
  forall f tf,
  transf_function f = OK tf -> f.(fn_retaddr_saved) = false.
Proof.
  intros. unfold transf_function in H.
  destruct (wt_function f); inv H.
  destruct (f.(fn_retaddr_saved)); inv H1. reflexivity.
Qed.
...
Proof.
  intros. unfold transf_function in H.
  destruct (wt_function f); inv H.
  destruct (f.(fn_retaddr_saved)); inv H1.
  monadInv H0. destruct (zlt Ptrofs.max_unsigned (list_length_z x.(fn_code))); inv EQ0.
  lia.
Qed.
...
Proof.
 ...
 - destruct s0; monadInv H; TailNoLabel; (eapply tail_nolabel_trans; TailNoLabel).
...
Proof.
  ...
  simpl.
  destruct (storeind_ptr_label A12 A10 (fn_link_ofs f) x) as [A B]; rewrite B.
...
- intros. unfold transf_function in H0.
  ...
  destruct (f0.(fn_retaddr_saved)); inv H2.
  monadInv H1.
...
Fixpoint prev_ctx_list (j: meminj) (s: list Mach.stackframe) (pcxis : list block) (pcxi: block) {struct s} : massert :=
  ...
  | Stackframe fb sp ra ms c :: s', pcxi' :: pcxis' =>
      ...
      ** contains Many32 pcxi  12 (fun v => v = parent_ra s')
...
Proof.
  ...
  assert (forall v, Val.inject j (parent_ra s') v -> Val.inject j' (parent_ra s') v).
  { intros. inv H1; econstructor. apply H in H3. eassumption. reflexivity. }
...
Lemma save_ctx_rec_ptr_correct:
  forall spec rs r v pcxi m_asm pos P pl,
  (v = Vnullptr \/ exists b ofs, v = Vptr b ofs) ->
  Val.has_type (rs#r) Tany32 ->
  spec v (rs#r) ->
  m_asm |= range pcxi pos (pos + 4) ** P ->
  (align_chunk Many32 | pos) ->
  exists m_asm',
     save_ctx_rec pcxi pos (r :: pl) rs m_asm = save_ctx_rec pcxi (pos + 4) pl rs m_asm'
  /\ m_asm' |= contains Many32 pcxi pos (fun v' => spec v v') ** P.
Proof.
  intros * Hv HTY INJ SEP ALIGN.
  simpl.

  apply (range_contains Many32) in SEP; [|assumption].
  specialize (store_rule Many32 _ _ _ (rs#r) _ (fun v' => spec v v') _ SEP).
  clear SEP. intros (m_asm' & -> & SEP).
  - destruct Hv as [-> | (b & ofs & ->)].
...
Lemma save_ctx_upper_correct:
  ...
  (rs#RA = Vnullptr \/ exists b ofs, rs#RA = Vptr b ofs) ->
  rs#RA = parent_ra s ->
...
Proof.
  intros * CSA HRA HRA' HSP TY INJ SEP.
  ...
  exploit (save_ctx_rec_ptr_correct (fun v v' => v' = v) rs PCXI (Vptr pcxi Ptrofs.zero)).
    right. exists pcxi, Ptrofs.zero. reflexivity.
    rewrite CSA. constructor. assumption. exact SEP. sep_lia.
  ...
  exploit (save_ctx_rec_ptr_correct (fun v v' => Val.inject j v v') rs SP).
    right. exists stk, sofs; reflexivity.
    inv HSP. constructor.
  ...
  exploit (save_ctx_rec_ptr_correct (fun v v' => v' = v) rs RA).
    exact HRA.
    destruct HRA as [->|(?&?&->)]; constructor.
    reflexivity. exact SEP. sep_lia.
...
Proof.
  ...
  - (* return not from main. Loads restore register contents. *)
    ...
    exploit restore_ctx_rec_step_correct.
      exact SEP. sep_lia. sep_lia.
    clear SEP. intros (rs3 & ra & -> & Hrs3 & Hrra & -> & SEP). 
...
Proof.
  intros * Hrpcxi HRA HRA' TY AG STKINJ SEP Hrs0. unfold exec_call.
  ...
  destruct H1 as (stk & sofs & -> & HSP).
  assert (forall r, Val.inject j (ms r) (rs0 (preg_of r))).
  { intros. rewrite Hrs0. Simpl. apply AG. }
  exploit save_ctx_upper_correct. 
    exact H. rewrite H0. exact HRA. rewrite H0. exact HRA'.
    exact HSP. exact TY. exact H1. exact SEP.
...
Lemma exec_call_correct:
  ...
  exists rs' m_asm' pcxi',
    ...
    /\ rs'#PCXI = (Vptr pcxi' Ptrofs.zero) /\ rs'#PC = pc /\ rs'#RA = ra.
Proof.
  intros * Hrpcxi HRA HRA' TY AG STKINJ SEP. unfold exec_call.
  assert (exists stk sofs, sp = Vptr stk sofs /\ Val.inject j sp rs#SP).
  { inv STKINJ. inv H.
    eexists. eexists. split. reflexivity. apply AG. }
  destruct H as (stk & sofs & -> & HSP).
  exploit save_ctx_upper_correct. 
    exact Hrpcxi. exact HRA. exact HRA'. apply HSP. exact TY. apply AG. exact SEP.
  clear SEP. intros (rs' & m_asm' & pcxi' & A & SEP & D & E).
  eexists _, m_asm', pcxi'. splitall.
  - rewrite A. reflexivity.
  - intros. apply SEP.
...
Lemma exec_ret_correct:
  forall j s ms (rs: regset) m_mach m_mach1 m_asm sz pcxis pcxi stk soff P,
  Mem.free m_mach stk 0 sz = Some m_mach1 ->
  agree_inj j ms (Vptr stk soff) rs ->
  ptr_injects j (Vptr stk soff) ->
  rs#PCXI = Vptr pcxi Ptrofs.zero ->
  rs#RA = parent_ra s ->
  m_asm |= prev_ctx_list j s pcxis pcxi ** minjection j m_mach ** P ->
  exists rs' m_asm',
       exec_ret rs m_asm sz = Next rs' m_asm'
    ...
    /\ match s, pcxis with
       | nil, nil =>
            ...
            /\ rs'#PC = Vnullptr
       | (Stackframe _ sp' ra' ms _)::s', pcxi' :: pcxis' =>
               m_asm' |= prev_ctx_list j s' pcxis' pcxi' ** minjection j m_mach1 ** P
            /\ (forall r, is_auto_save r = true -> Val.inject j (ms#r) (rs'#(preg_of r)))
            /\ rs'#PCXI = Vptr pcxi' Ptrofs.zero
            /\ rs'#PC = ra'
            /\ rs'#RA = parent_ra s'
...
Proof.
  intros * FREE AG SPINJ HrPCXI HrRA SEP.
  inv SPINJ. rename b' into stk'.
  exploit free_parallel_rule_0; eauto.
    apply sep_comm, sep_assoc in SEP. exact SEP.
  clear SEP. intros (m_asm1 & FREE' & SEP).
    rewrite <- sep_assoc, sep_comm in SEP.
  unfold exec_ret.
  assert (rs A10 = Vptr stk' soff).
  { destruct AG. inv agree_inj_sp.
    rewrite H0 in H3. inv H3.
    rewrite Ptrofs.add_zero. reflexivity. }
  exploit restore_ctx_upper_correct. exact HrPCXI. exact SEP.
  intros (rs1 & m_asm2 & A & B & C).
  eexists _, m_asm2. splitall.
  - rewrite H, FREE', A. reflexivity.
  - intros. Simpl.
...
Proof.
  ...
  exploit H3; eauto. intros (rs2 & A & B & C & D & E & F).
...
Proof.
  ...
  exploit H5; eauto. intros (jmp & k' & rs2 & A & B & C & D & E & F).
...
- (* Mop *)
  ...
  left; eapply exec_straight_steps; eauto; intros.
  exploit transl_op_correct; eauto. intros (rs2 & P & Q & R & S & T).
...
- (* Mcall *)
  ...
  + (* Indirect call *)
    ...
    assert (rs#RA = Vnullptr \/ exists b ofs', rs#RA = Vptr b ofs').
    { destruct s as [|[] s']; [left|right].
      - assumption.
      - simpl in ATLR.
        inv STACKS; inv H16. do 2 eexists; reflexivity. }
  ...
  + (* Direct call *)
    ...
    { destruct s as [|[] s']; [left|right].
      - assumption.
      - simpl in ATLR.
        inv STACKS; inv H14. do 2 eexists; reflexivity. }
    exploit exec_call_correct; eauto.
    clear SEP. intros (rs1 & m_asm1 & pcxi' &  A & SEP & C & D & E & F).
...
- (* Mtailcall *)
  ...
  + (* Indirect call *)
    ...
    { assert (Val.inject j (Vptr fb' Ptrofs.zero) (rs x0)).
      ...
      rewrite Ptrofs.add_zero. reflexivity. }
    exploit free_frame_correct; eauto with asmgen.
      inv H10. exists b'; assumption.
      rewrite sep_comm, sep_assoc in SEP. exact SEP.
    clear SEP. intros (rs1 & m_asm1 & U & V & W & SEP).
      rewrite <- sep_assoc, sep_comm in SEP.
    ...
    * Simpl. rewrite W; eauto with asmgen.
    * Simpl. rewrite W; eauto with asmgen.
    * Simpl. rewrite W; eauto with asmgen.
  + (* Direct call *)
    exploit free_frame_correct; eauto.
      inv H10. exists b'; assumption.
      rewrite sep_comm, sep_assoc in SEP. exact SEP.
    clear SEP. intros (rs1 & m_asm1 & U & V & W & SEP).
      rewrite <- sep_assoc, sep_comm in SEP.
...
- (* Mreturn *)
  ...
  assert (NOOV: list_length_z tf.(fn_code) <= Ptrofs.max_unsigned).
    eapply transf_function_no_overflow; eauto.
  (* compute j stk *)
  assert (exists stk', rs0 A10 = Vptr stk' soff).
  { inv SINJ; inv H6. destruct AG. inv agree_inj_sp.
    rewrite H10 in H12. inv H12.
    rewrite Ptrofs.add_zero. eexists. reflexivity. }
  destruct H6 as (stk' & ?).
  exploit (exec_ret_correct j s ms rs0); eauto.
    inv SINJ. assumption.
  intros (rs2' & m2' & A & B & C).
  left; econstructor; split.
  (* execution *)
  eapply plus_one.
  econstructor; eauto. eapply functions_transl; eauto. eapply find_instr_tail. eassumption.
  simpl. eassumption.
  (* match states *)
  destruct s as [|[] s'], pcxis as [|pcxi' pcxis']; try now contradiction C.
  + (* return from main. PCL & Stack empty *)
    destruct C as [C D].
    econstructor; eauto.
    simpl. constructor.
    * rewrite C. constructor.
    * unfold Vnullptr. simpl; congruence.
    * intros. unfold Mach.restore_auto_save_regs. destruct (is_auto_save r) eqn:Er.
      constructor. rewrite B; eauto with asmgen. apply AG. apply not_auto_save_regs_not_in_upper_ctx; auto.
    * constructor.
    * instantiate (1 := nil). assumption.

  + clear SEP. destruct C as (SEP & C & D & E & F & G). econstructor; eauto. simpl.
    constructor.
    * eassumption.
      (* a.d. TODO can this be returned by exec_ret_correct? *)
    * inv SINJ; inv H9; inv H12. congruence.
    * intros. unfold Mach.restore_auto_save_regs. destruct (is_auto_save r) eqn:Er.
      auto.
      rewrite B. apply AG. auto with asmgen. apply not_auto_save_regs_not_in_upper_ctx; auto.
    * inv SINJ. assumption.
    * instantiate (1 := pcxi' :: pcxis'). 
      simpl. splitall; assumption.
...
- (* internal function *)
  ...
  destruct (wt_function f); inv EQ'.
  destruct (f.(fn_retaddr_saved)) eqn:Eretaddr; inv H4. monadInv H5.
  ...
  { destruct s as [|[] s'] eqn:Es.
    - (* top of the callstack *)
      ...
      rewrite H3 in ST1.
      exists m2'. splitall; try eassumption.
      simpl. rewrite <- H3. unfold Vnullptr. simpl; constructor.
    - (* down the callstack *)
      eapply stackframes_inject_incr in SFINJ. 2: eassumption.
      revert Es.
      inv SFINJ. inv H5. intros.
      exploit storev_parallel_rule; eauto.
      { eapply Val.offset_ptr_inject. unfold sp. econstructor. eassumption. rewrite Ptrofs.add_zero. reflexivity. }
      ...
      { simpl in AG. destruct AG. inv agree_inj_sp.
        enough (j' b = Some (b2, delta)); [|now auto].
        enough (Some (b', 0) = Some (b2, delta)); [|congruence].
        inv H5. rewrite Ptrofs.add_zero. reflexivity. }
      rewrite H4 in ST1.
      exists m2'. splitall; try eassumption.
      simpl. rewrite <- H4. econstructor. eassumption. rewrite Ptrofs.add_zero. reflexivity. }

  clear SEP. destruct H3 as (m2' & ST1 & ? & SEP).

  (* Execution of function prologue *)
  monadInv EQ0. rewrite transl_code'_transl_code in EQ1.
  set (tfbody := Pallocframe (fn_stacksize f) ::
                  storeind_ptr A12 SP (fn_link_ofs f) x0) in *.
...
Local Transparent destroyed_at_function_entry.
  ...
  intros. rewrite V; auto with asmgen.
  rewrite V; auto with asmgen.
  rewrite V; auto with asmgen.
...
- (* external function *)
  ...
  + (* Case analysis on whether we are at the top of the callstack.
    ...
    { (* Down the call stack. *)
      clear SEP. destruct Hs as (SEP & V & W & X & Y).
      apply (stackframes_inject_incr _ _ _ R) in SFINJ.
      econstructor; eauto.
      unfold loc_external_result. apply agree_inj_set_other; auto. apply agree_inj_set_pair; auto.
      simpl. constructor.
      ...
      - intros. unfold external_call_regs, restore_auto_save_regs, Mach.undef_callee_may_modify_regs.
        ...
        + constructor. 
      - instantiate (1 := pcxi' :: pcxis'). simpl. splitall.
...
- (* return *)
  ...
  destruct PCL as (CSA & ATPC & ATLR & SEP).
  rewrite <- ATPC in H6.
  econstructor; eauto.
...
Proof.
  intros. inv H0. inv H. econstructor.
  - destruct pcxis; [assumption|contradiction].
