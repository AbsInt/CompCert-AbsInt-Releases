...
Proof.
  ...
  rewrite mk_amount32_eq; try apply (Int.is_power2_range n i); auto.
...
Proof.
  ...
  destruct (e#r1) eqn:?;
    ...
    inv H; auto.
  destruct (Int.is_power2 n) eqn:?;[destruct (Int.ltu i (Int.repr 31)) eqn:?|].
  - exists v; split; auto. simpl. rewrite mk_amount32_eq. eapply Val.divs_pow2; eauto. congruence.
...
Lemma make_insertimm_correct:
  forall n pos width r,
  let (op, args) := make_insertimm pos width n r in
  exists v, eval_operation ge (Vptr sp Ptrofs.zero) op e##args m = Some v /\ Val.lessdef (insert_bits pos width (e#r) (Vint n)) v.
Proof.
  intros; unfold make_insertimm.
  econstructor; split; eauto. auto.
Qed.

Lemma make_extr_correct:
  forall pos width r x,
  vmatch bc e#r x ->
  let (op, args) := make_extr pos width r x in
  exists v', eval_operation ge (Vptr sp Ptrofs.zero) op e##args m = Some v' /\ Val.lessdef (extr pos width e#r) v'.
Proof.
  intros; unfold make_extr.
  destruct pos, width. simpl.
  destruct (Int.eq a32_amount Int.zero) eqn:?; simpl; eauto.
  destruct (Int.ltu Int.zero a32_amount0) eqn:?; simpl; eauto.
  destruct (vincl x (Sgn Ptop (Int.unsigned a32_amount0))) eqn:?.
  - exists (e#r); split; auto.
    assert (V: vmatch bc e#r (Sgn Ptop (Int.unsigned a32_amount0))).
    { eapply vmatch_ge; eauto. apply vincl_ge; auto. } intros.
    unfold extr. rewrite (Int.same_if_eq a32_amount Int.zero Heqb), Int.sub_zero_l.
    rewrite Val.sign_ext_shr_shl. inv V; auto. rewrite is_sgn_sign_ext in H4 by auto. simpl. rewrite H4; auto.
    rewrite a32_range0, Heqb0. auto.
  - econstructor; split; simpl; eauto.
Qed.
...
- (* insert *)
  InvApproxRegs; SimplVM; inv H0; apply make_insertimm_correct; auto.
- (* cond *)
...
- (* extr *)
  inv H0. inv H. apply make_extr_correct; auto.
- (* default *)
  exists v; auto.
Qed.
...
- simpl. rewrite Genv.shift_symbol_address_32. econstructor; split; eauto.
  inv H0; simpl; auto. auto.
- rewrite Ptrofs.add_zero_l. econstructor; split; eauto.
  change (Vptr sp (Ptrofs.add n1 (Ptrofs.of_int n))) with (Val.offset_ptr (Vptr sp n1) (Ptrofs.of_int n)).
  inv H0; simpl; auto. rewrite H2. auto.
- exists res; auto.
Qed.
