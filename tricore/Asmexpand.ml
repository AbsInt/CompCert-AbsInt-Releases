...
let expand_builtin_memcpy  sz args =
  ...
  if Ptrofs.ltu Ptrofs.zero sz then begin
    ...
    emit_checked (Pmovh_ao (A14, (Asmgen.high_s sz)));
    emit_checked (Plea_sc16 (A14, A14, (Asmgen.low_s sz)));
    emit_checked (Plabel lbl);
    emit_checked (Pldbu_prr (D0, src_tmp));
    emit_checked (Pstb_prr (D0, dst_tmp));
    emit_checked (Ploop (A14, lbl))
  end
...
(* Since we don't use indexed addressing we need to add the offset before
   ...
   move the base to A14 since we need to update the base for the load of the
   other half. *)
let expand_vmem_addr chunk base ofs =
  if not (I32.eq ofs _0l) then begin
    expand_addimm_addr A14 base ofs;
    A14
  end else if chunk = Mint64 then begin
    if base <> A14 then emit_checked (Pmov_aa (A14, base));
    A14
  end else
    base
...
let expand_builtin_vload chunk args dst =
  ...
  | [BA (DREG addr)] ->
    emit_checked (Pmov_a (A14, addr));
    expand_builtin_vload_common chunk A14 _0l dst
  | [BA_addrstack ofs] ->
...
let expand_builtin_vstore chunk args =
  ...
  | [BA (DREG addr); src] ->
    emit_checked (Pmov_a (A14, addr));
    expand_builtin_vstore_common chunk A14 _0l src
...
let expand_builtin_va_start r =
  ...
  | Some ofs ->
     ...
     let r' = match r with
       | DREG r' -> emit_checked (Pmov_a (A14, r')); A14
       | AREG r' -> if r' = A13 then begin emit_checked (Pmov_aa (A14,r')); A14 end else r'
