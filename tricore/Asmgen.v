...
Definition transl_op
   ...
   | Osel cmp ty, a1 :: a2 :: args => (* Note that ty has type typ and not argtype, hence Taptr is not an option. Hence the result type is also never Taptr and res should always be a data register. *)
       do rd <- ireg_of_rpair res;
       do args <- mmap (@error_single mreg) args;
       let d := (if dreg_eq rd D2 then D3 else D2) in
       match (preg_rpair_of a1), (preg_rpair_of a2) with
       | One (DREG d1), One (DREG d2) => (transl_cond_op cmp TMP args (Psel rd TMP d1 d2 :: k))
       | One (DREG d1), One (AREG a2) => if (dreg_eq rd d1)
                            then
                              transl_cond_op cmp TMP args (switch_A_D_via_TMPA a2 d (Psel rd TMP d1 d::(switch_A_D_via_TMP a2 d k)))
                            else
                              transl_cond_op cmp rd args (Pmov_d TMP a2::Psel rd rd d1 TMP::k)
       | One (AREG a1), One (DREG d2) => if (dreg_eq rd d2)
                            then
                              transl_cond_op cmp TMP args (switch_A_D_via_TMPA a1 d (Psel rd TMP d d2::(switch_A_D_via_TMP a1 d k)))
                            else
                              transl_cond_op cmp rd args (Pmov_d TMP a1::Psel rd rd TMP d2::k)
       | One (AREG a1), One (AREG a2) => if (areg_eq a1 a2)
                            then
                              OK (Pmov_d rd a1::k)
                            else
                              transl_cond_op cmp rd args (switch_A_D_via_TMP a1 d (Pmov_d TMP a2:: Psel rd rd d TMP::switch_A_D_via_TMP a1 d k))
...
Definition transl_instr (f: Mach.function) (i: Mach.instruction)
  ...
  | Mtailcall sig (inl r) =>
      do r1 <- areg_of r;
      OK (make_epilogue f (Pji r1 sig :: k))
...
Definition transl_function (f: Mach.function) :=
  ...
  OK (mkfunction f.(Mach.fn_sig)
        (Pallocframe f.(fn_stacksize) ::
         storeind_ptr A12 SP f.(fn_link_ofs) (storeind_ptr RA SP f.(fn_retaddr_ofs) c))).
