...
Proof.
  intros. functional inversion H; subst.
  (* addimm - addimm *)
  - UseGetSound. FuncInv. simpl. exists r; split; auto.
    rewrite <- H0. simpl. rewrite <- H1. rewrite Val.add_assoc. auto.
  (* andimm - andimm *)
  - UseGetSound; simpl. exists r; split; auto. rewrite <- H1.
    generalize (Int.eq_spec p m0); rewrite H8; intros. rewrite <- H0. simpl. rewrite <- H1.
    rewrite <- H2. unfold p. rewrite Val.and_assoc. simpl. rewrite Int.and_assoc.
    rewrite Int.and_idem. reflexivity.
  - UseGetSound; simpl. exists r; split; auto. rewrite <- H0. simpl.
