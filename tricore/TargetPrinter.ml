...
struct
  ...
  let index_reg_name = function
    | A10 -> "%a10" | A12 -> "%a12"

  let areg_name = function
    | A0  -> "%a0"  | A1  -> "%a1"  | A2  -> "%a2"  | A3  -> "%a3"
    | A4  -> "%a4"  | A5  -> "%a5"  | A6  -> "%a6"  | A7  -> "%a7"
    | A8  -> "%a8"  | A9  -> "%a9"  | A11 -> "%a11" | A13 -> "%a13"
    | A14 -> "%a14" | A15 -> "%a15" | INDREG i -> index_reg_name i

  let dreg oc r = output_string oc (dreg_name r)

  let index_reg oc r = output_string oc (index_reg_name r)
...
(* Names of sections *)
  ...
  let name_of_section = function
    ...
    let low_half oc (id, ofs) =
       fprintf oc "lo:%a" symbol_offset (id, ofs)
    ...
    let print_instruction oc = function
      ...
      | Pmovh_as (a1, id) ->
         fprintf oc "	movh.a	%a, %a\n" areg a1 high_half (id, Ptrofs.zero)
      | Pmovh_sa (a1, id, ofs) ->
         fprintf oc "	movh.a	%a, %a\n" areg a1 high_half (id, ofs)
      ...
      | Plea_sofs (a1, a2, id, ofs) ->
         fprintf oc "	lea	%a, [%a], lo:%a\n" areg a1 areg a2 symbol_offset (id,ofs)
      | Plea_sc16 (a1, a2, imm) ->
        fprintf oc "	lea	%a, [%a], %a\n" areg a1 areg a2 coqint imm
      | Peq (rd, d1, d2) ->
        fprintf oc "	eq	%a, %a, %a\n" dreg rd dreg d1 dreg d2
      | Peq_sc9 (rd, d, imm) ->
        fprintf oc "	eq	%a, %a, %a\n" dreg rd dreg d coqint imm
      | Peq_a (rd, d1, d2) ->
        fprintf oc "	eq.a	%a, %a, %a\n" dreg rd areg d1 areg d2
      | Peqz_a (rd, d) ->
        fprintf oc "	eqz.a	%a, %a\n" dreg rd areg d
      | Pne (rd, d1, d2) ->
        fprintf oc "	ne	%a, %a, %a\n" dreg rd dreg d1 dreg d2
      | Pne_sc9 (rd, d, imm) ->
        fprintf oc "	ne	%a, %a, %a\n" dreg rd dreg d coqint imm
      | Pne_a (rd, d1, d2) ->
        fprintf oc "	ne.a	%a, %a, %a\n" dreg rd areg d1 areg d2
      | Pnez_a (rd, d) ->
        fprintf oc "	nez.a	%a, %a\n" dreg rd areg d
      | Plt (rd, d1, d2) ->
        fprintf oc "	lt	%a, %a, %a\n" dreg rd dreg d1 dreg d2
      | Plt_sc9 (rd, d, imm) ->
        fprintf oc "	lt	%a, %a, %a\n" dreg rd dreg d coqint imm
      ...
      | Padd_aa (rd, a) ->
        fprintf oc "	add.a	%a, %a\n" areg rd areg a
      | Paddsc_a (rd, a, d, imm) ->
        fprintf oc "	addsc.a	%a, %a, %a, %a\n"
          areg rd areg a dreg d coqint imm
      | Paddih_a (rd, a, imm) ->
        fprintf oc "	addih.a	%a, %a, %a\n" areg rd areg a coqint imm
      | Pcadd_sc9 (rd, c, d, imm) ->
        fprintf oc "	cadd	%a, %a, %a, %a\n"
          dreg rd dreg c dreg d coqint imm
      | Psub (rd, d1, d2) ->
        fprintf oc "	sub	%a, %a, %a\n" dreg rd dreg d1 dreg d2
      | Psub_rr (rd, d) ->
        fprintf oc "	sub	%a, %a\n" dreg rd dreg d
      | Prsub (rd, d ,imm) ->
        fprintf oc "	rsub	%a, %a, %a\n" dreg rd dreg d coqint imm
      | Prsub_r rd ->
        fprintf oc "	rsub	%a\n" dreg rd
      | Pcsub (rd, c, d1, d2) ->
      ...
      | Pmadd_sc9 (rd, d1, d2, imm) ->
        fprintf oc "	madd	%a, %a, %a, %a\n"
          dreg rd dreg d1 dreg d2 coqint imm
      | Pmsub (rd, d1, d2, d3) ->
      ...
      | Por_t (rd, d1, p1, d2, p2) ->
        fprintf oc "	or.t	%a, %a, %a, %a, %a\n"
          dreg rd dreg d1 coqint p1 dreg d2 coqint p2
      | Pnor_t (rd, d1, p1, d2, p2) ->
        fprintf oc "	nor.t	%a, %a, %a, %a, %a\n"
          dreg rd dreg d1 coqint p1 dreg d2 coqint p2
      | Pxor_ruc9 (rd, d , imm) ->
        fprintf oc "	xor	%a, %a, %a\n" dreg rd dreg d coqint imm
      | Pxnor (rd, d1, d2) ->
      ...
      | Pextr (rd, d, pos, width) ->
        fprintf oc "	extr	%a, %a, %a, %a\n"
          dreg rd dreg d coqint pos coqint width
      | Pextru (rd, d, pos, width) ->
        fprintf oc "	extr.u	%a, %a, %a, %a\n"
          dreg rd dreg d coqint pos coqint width
      | Pdextr (rd, r1, r2, pos) ->
        fprintf oc "	dextr	%a, %a, %a, %a\n"
          dreg rd dreg r1 dreg r2 coqint pos
      | Pinsert (rd, d1, d2, pos, width) ->
        fprintf oc "	insert	%a, %a, %a, %a, %a\n"
          dreg rd dreg d1 dreg d2 coqint pos coqint width
      | Pinsert_uc4 (rd, d, imm, pos, width) ->
        fprintf oc "	insert	%a, %a, %a, %a, %a\n"
          dreg rd dreg d coqint imm coqint pos coqint width
      | Pinsn_t (rd, r1, r2, pos1, pos2) ->
        fprintf oc "	insn.t	%a, %a, %a, %a, %a\n"
          dreg rd dreg r1 coqint pos1 dreg r2 coqint pos2
      | Psel (rd, c, d1, d2) ->
      ...
      | Pret _ -> fprintf oc "	ret\n"
      | Pjeq (d1, d2, lbl) ->
        fprintf oc "	jeq	%a, %a, %a\n" dreg d1 dreg d2 print_label lbl
      | Pjeq_sc4 (d, imm, lbl) ->
        fprintf oc "	jeq	%a, %a, %a\n" dreg d coqint imm print_label lbl
      | Pjge (d1, d2, lbl) ->
      ...
      | Pjne_sc4 (d, imm, lbl) ->
        fprintf oc "	jne	%a, %a, %a\n" dreg d coqint imm print_label lbl
      | Pjnz_t (d, b, lbl) ->
        fprintf oc "	jnz.t	%a, %a, %a\n" dreg d coqint b print_label lbl
      | Pjz_t (d, b, lbl) ->
        fprintf oc "	jz.t	%a, %a, %a\n" dreg d coqint b print_label lbl
      | Pldb (rd, base, imm) ->
        fprintf oc "	ld.b	%a, [%a]%a\n" dreg rd index_reg base coqint imm
      | Pldb_symb (rd, base, id, ofs) ->
        fprintf oc "	ld.b	%a, [%a]%a\n"
          dreg rd areg base low_half (id, ofs)
      | Pldb_rr (rd, base) ->
        fprintf oc "	ld.b	%a, [%a]\n" dreg rd areg base
      | Pldbu (rd, base, imm) ->
        fprintf oc "	ld.bu	%a, [%a]%a\n" dreg rd index_reg base coqint imm
      | Pldbu_symb (rd, base, id, ofs) ->
        fprintf oc "	ld.bu	%a, [%a]%a\n"
          dreg rd areg base low_half (id, ofs)
      | Pldbu_rr (rd, base) ->
      ...
      | Pldw_symb (rd, base, id, ofs)
      | Pfldw_symb (rd, base, id, ofs)
      | Pldw_a_symb (rd, base, id, ofs) ->
        fprintf oc "	ld.w	%a, [%a]%a\n"
          dreg rd areg base low_half (id, ofs)
      | Pldw_rr (rd, base)
      | Pfldw_rr (rd, base)
      | Pldw_a_rr (rd, base) ->
        fprintf oc "	ld.w	%a, [%a]\n" dreg rd areg base
      | Plda (rd, base, imm) ->
        fprintf oc "	ld.a	%a, [%a]%a\n" areg rd index_reg base coqint imm
      | Plda_symb (rd, base, id, ofs) ->
        fprintf oc "	ld.a	%a, [%a]%a\n"
          areg rd areg base low_half (id, ofs)
      | Plda_rr (rd, base) ->
        fprintf oc "	ld.a	%a, [%a]\n" areg rd areg base
      | Pstb (src, base, imm) ->
        fprintf oc "	st.b	[%a]%a, %a\n" index_reg base coqint imm dreg src
      | Pstb_symb (src, base, id, ofs) ->
        fprintf oc "	st.b	[%a]%a, %a\n"
          areg base low_half (id, ofs) dreg src
      | Pstb_rr (src, base) ->
        fprintf oc "	st.b	[%a], %a\n" areg base dreg src
      | Psth (src, base, imm) ->
        fprintf oc "	st.h	[%a]%a, %a\n" index_reg base coqint imm dreg src
      | Psth_symb (src, base, id, ofs) ->
        fprintf oc "	st.h	[%a]%a, %a\n"
          areg base low_half (id, ofs) dreg src
      | Psth_rr (src, base) ->
        fprintf oc "	st.h	[%a], %a\n" areg base dreg src
      ...
      | Pstw_symb  (src, base, id, ofs)
      | Pfstw_symb  (src, base, id, ofs)
      | Pstw_a_symb  (src, base, id, ofs) ->
        fprintf oc "	st.w	[%a]%a, %a\n"
          areg base low_half (id, ofs) dreg src
      | Pstw_rr (src, base)
      | Pfstw_rr (src, base)
      | Pstw_a_rr (src, base) ->
        fprintf oc "	st.w	[%a], %a\n" areg base dreg src
      | Psta (src, base, imm) ->
        fprintf oc "	st.a	[%a]%a, %a\n" index_reg base coqint imm areg src
      | Psta_symb (src, base, id, ofs) ->
        fprintf oc "	st.a	[%a]%a, %a\n"
          areg base low_half (id, ofs) areg src
      | Psta_rr (src, base) ->
        fprintf oc "	st.a	[%a], %a\n" areg base areg src
      | Ploadsi _
      | Pallocframe _
      ...
      | Pbtbl (r, tbl) ->
        ...
        fprintf oc "	movh.a	%%a3, hi:%a\n" label lbl;
        fprintf oc "	lea	%%a3, [%%a3], lo:%a\n" label lbl;
        fprintf oc "	addsc.a	%%a3, %%a3, %a, 2\n" dreg r;
        fprintf oc "	ji	%%a3\n";
         ...
         begin match ef with
           | EF_annot(kind,txt, targs) ->
             begin match (P.to_int kind) with
               | 1 -> let annot = annot_text preg_annot "a10" txt args  in
                 fprintf oc "%s annotation: %S\n" comment annot
               | 2 -> let lbl = new_label () in
                 fprintf oc "%a:\n" label lbl;
                 add_ais_annot lbl preg_annot "a10" txt args
               | _ -> assert false
             end
